class n22319064{
    public void addRegisterInfo(HttpServletRequest request) throws ApplicationException {
        ...
        if (pids == null || pids.length <= 0) throw new ApplicationException("????????????");
        RegisterDao registerDao = new RegisterDao();
        Register register = registerDao.findPojoById(StrFun.getString(request, "rid"), Register.class);
        if (register.audit) throw new ApplicationException("??????????????????????????");
        DBConnect dbc = null;
        ...
        try {
            ...
            for (String pid : pids) {
                ...
                if (pd.regAmount <= 0) throw new ApplicationException("??????????????");
