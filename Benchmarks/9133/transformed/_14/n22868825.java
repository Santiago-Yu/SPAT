	...
	String sendRequest(String[] getVars, String[] postVars, Object[] fileVars, boolean getSessionKey)
		...
		if (getVars != null)
			for (int i = 0; i + 1 < getVars.length; i += 2)
				uri += (0 == i ? '?' : '&') + urlEncode(getVars[i]) + '=' + urlEncode(getVars[i + 1]);
		URL url = new URL(uri);
		...
		if (!getSessionKey) {
			...
			for (String key : cookies.keySet())
				cookie += (0 == cookie.length() ? "" : "; ") + key + "=" + cookies.get(key);
		...
		if (fileVars != null) {
			...
			for (int i = 0; postVars != null && i + 1 < postVars.length; i += 2)
			...
			PrintStream ps = new PrintStream(conn.getOutputStream());
			for (int i = 0; postVars != null && i + 1 < postVars.length; i += 2)
				ps.print((0 == i ? "" : "&") + urlEncode(postVars[i]) + "=" + urlEncode(postVars[i + 1]));
			ps.close();
		}
