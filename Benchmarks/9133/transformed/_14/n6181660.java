		...
		try {
			...
			for (int i = 0; i < directories.size(); i++) {
				...
				for (FTPFile file : files) {
					if (FTPFile.FILE_TYPE == file.getType() && (null == varProperties.getLastImportDate(i) ? true
							: (file.getTimestamp().getTime().equals(varProperties.getLastImportDate(i).getDate())
									? file.getName().compareTo(varProperties.getLastImportName(i)) < 0
									: file.getTimestamp().getTime()
											.after(varProperties.getLastImportDate(i).getDate())))) {
