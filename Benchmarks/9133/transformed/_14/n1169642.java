class n1169642 {
	...
	public void run() {
		...
		synchronized (this.lauchedHTTPRequests) {
			if (this.lauchedHTTPRequests.contains(key))
				return;
		...
		try {
			...
			if ("file".equals(url.getProtocol()))
				...
				logger.warning(
						"Impossible to load settings from " + url + ". Unsupported protocol " + url.getProtocol());
				return;
			}
		...
		for (MfGeo geo : tile.getPieces()) {
			if (GeoType.GEOMETRY == geo.getGeoType()) {
				geoms[i++] = ((MfGeometry) geo).getInternalGeometry();
			} else if (GeoType.FEATURE == geo.getGeoType()) {
