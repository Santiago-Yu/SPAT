class n17314208 {
	public static void main(final String[] args) {
		...
		if (false == new File(sHomeDir).isDirectory()) {
			System.out
		...
		for (int i = 1; i < args.length; i++) {
			...
			if (args[i].startsWith(COMPRESS)) {
				...
				if (0 == sCompressString.compareTo("YES")) {
					bCompress = true;
				}
				continue;
			}
			...
			if (args[i].startsWith(POST)) {
				...
				if (false == validateFile(sXmlDataFile)) {
			...
			if (args[i].startsWith(CONFIG)) {
				...
				if (true == validateFile(sConfigFile)) {
		...
		try {
			...
			if (sInputData.length() > 0) {
				if (false == connection.getDoOutput()) {
		...
		if ((0 == sOutput.compareTo("STREAM")) || (0 == sOutput.compareTo("NONE"))) {
			return;
		}
		...
		if (sOutput != null && 0 == sOutput.compareTo("DISPLAY")) {
