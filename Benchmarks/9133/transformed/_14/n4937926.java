	...
	public RServiceResponse execute(final NexusServiceRequest inData) throws NexusServiceException {
		...
		try {
			...
			for (String varname : rVariables) {
				...
				if (2 == rVariable.length && "file".equals(rVariable[0]) && "connection".equals(rVariable[1])) {
					...
					final String printValue = null == varvalue ? "null"
							: varvalue.getClass().isArray() ? Arrays.asList(varvalue).toString() : varvalue.toString();
					result.append("  ").append(null == varvalue ? "" : varvalue.getClass().getSimpleName()).append(' ')
			...
			if (connection != null) {
				...
				final boolean close = null == outSession;
				if (!close) {
