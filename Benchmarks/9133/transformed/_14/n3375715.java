			...
			for (iLinha = NUMERO_DA_LINHA_INICIAL; true; iLinha++) {
				...
				if (registro != null) {
					...
					if (celulaUltimoDiaCom != null) {
						if (Cell.CELL_TYPE_NUMERIC == celulaUltimoDiaCom.getCellType()) {
							...
							if (tmpUltimoDiaCom.compareTo(dataLimite) <= 0 || !pFiltrarPelaDataDeCorteDoCabecalho) {
								...
								try {
									...
									if (Cell.CELL_TYPE_NUMERIC == celula.getCellType()) {
								...
								if (celula != null && Cell.CELL_TYPE_NUMERIC == celula.getCellType()) {
