class n7300267 {
	public void appendMessage(MimeMessage oMsg) throws FolderClosedException, StoreClosedException, MessagingException {
		...
		if (((iOpenMode & MODE_MBOX) == 0) && ((iOpenMode & MODE_BLOB) == 0)) {
			if (DebugFile.trace)
				DebugFile.decIdent();
		...
		if (oMsg.getClass().getName().equals("com.knowgate.hipermail.DBMimeMessage")) {
			...
			if (null == ((DBMimeMessage) oMsg).getFolder())
		...
		try {
			...
			if (null == sMessageID || EmptyString.equals(sMessageID)) {
				try {
		...
		if (DebugFile.trace)
			DebugFile.writeln("part boundary is \"" + (null == sBoundary ? "null" : sBoundary) + "\"");
		if (null == sMessageID)
			sMessageID = gu_mimemsg;
		else if (0 == sMessageID.length())
			sMessageID = gu_mimemsg;
		Timestamp tsSent;
