class n8708052 {
	public ActionForward dbExecute(ActionMapping pMapping, ActionForm pForm, HttpServletRequest pRequest,
		...
		if (null == pRequest.getParameter("key")) {
		...
		if ((null == currentUser) || (!currentUser.getAdminRights() && (currentUser.getPrimaryKey() != key))) {
		...
		if (form.getAction().equals("edit")) {
			...
			if (currentUser.isAdmin())
				if (pRequest.getParameter("PDFRights") != null)
					user.setPDFRights(true);
				else
					user.setPDFRights(false);
			if (currentUser.isAdmin()) {
			...
			if (user.getPrimaryKey() == currentUser.getPrimaryKey()) {
