class n23598855 {
	public void requestConfirm() throws Exception {
		...
		_hc.load("/admin/Comm?type=" + CommResponse.TYPE_HA + "&command=" + CommResponse.COMMAND_REQUEST_CONFIRM
				+ "&virtual=" + values.get("address.virtual"));
		String _reply = new String(_hc.getContent());
		if (_reply.isEmpty()) {
			throw new Exception("remote product has not sent any reply");
		} else if (-1 == _reply.indexOf("done")) {
