	...
	public byte[] evaluateResponse(byte[] responseBytes) throws SaslException {
		if (firstEvaluation) {
			...
			if (privacy) {
				...
				if (-1 == "3des,des,rc4-40,rc4,rc4-56".indexOf(sessionCipher)) {
			...
			if (null == domainWithPassword || null == domainWithPassword.getPassword()) {
				log.warn("The supplied username and/or realm do(es) not match a registered entry");
				return null;
			}
			if (realm.equals("null") && -1 == username.indexOf('@')) {
