	...
	private void compress(File src) throws IOException {
		...
		try {
			...
			bzOut = new BZip2OutputStream(new BufferedXOutputStream(fileOut, 8192),
					Math.min((-1 == this.blockSize) ? BZip2OutputStream.MAX_BLOCK_SIZE : this.blockSize,
			...
			while (true) {
				...
				if (maxStep <= 0) {
					...
					if ((0 == step) && (inChannel.size() != inSize))
			...
			if (this.verbose) {
				final double ratio = (0 == inSize) ? (outSize * 100) : ((double) outSize / (double) inSize);
