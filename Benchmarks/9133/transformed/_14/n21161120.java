	...
	public static void main(String[] args) {
		...
		try {
			if ((1 == args.length) || (2 == args.length)) {
				...
				host = c[0];
				port = (1 == c.length) ? 443 : Integer.parseInt(c[1]);
				String p = (1 == args.length) ? "changeit" : args[1];
				...
				tmpStr = StringUtils.defaultIfEmpty(reader.readLine(), null);
				port = null == tmpStr ? 443 : Integer.parseInt(tmpStr);
			...
			int k = -1;
			try {
				k = (0 == line.length()) ? 0 : Integer.parseInt(line) - 1;
