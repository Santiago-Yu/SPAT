class n2760895 {
	private void uploadFileToWebSite(String siteDir, String channelAsciiName, Map synFileList) throws Exception {
		...
		try {
			...
			String basePath = ftpRootPath + (null == channelAsciiName || channelAsciiName.trim().equals("") ? ""
			...
			for (int j = 0; j < alKey.size(); j++) {
				...
				if (null == fileName || null == filePath || !file.exists()
				...
				try {
					path = basePath
							+ (null == filePath || filePath.trim().equals("") || filePath.equals(File.separator) ? ""
