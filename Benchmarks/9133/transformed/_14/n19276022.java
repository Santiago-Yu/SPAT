	...
	public DataSet parse() throws SnifflibDatatypeException {
		...
		try {
			...
			while (true) {
						...
						try {
							...
							if (readingScientificNotation) {
								...
								if (!this.dryrun) {
									...
									if ((currentRow == this.targetRow) && (currentColumn == this.targetColumn)) {
							...
							try {
								...
								try {
									try {
										...
										throw new SnifflibDatatypeException(
												"Problem constructing 2 " + err.getMessage());
									}
								} catch (java.lang.InstantiationException err) {
									throw new SnifflibDatatypeException("Problem constructing 3 " + err.getMessage());
								}
				...
				if (StreamTokenizer.TT_EOF == tokenizer.ttype) {
					break;
				}
				if (advanceField) {
					currentColumn++;
