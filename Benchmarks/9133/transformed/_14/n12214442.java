	...
	public boolean saveTemplate(Template t) {
		try {
			...
			if (Integer.parseInt(executeMySQLGet(
					"SELECT COUNT(*) FROM templates WHERE name='" + escapeCharacters(t.getName()) + "'")) != 0)
				return false;
			...
			if (rset.next()) {
				...
				query = "INSERT INTO templates (name, parentid) VALUES ('" + escapeCharacters(t.getName()) + "', "
						+ t.getParentID() + ")";
				try {
