			...
			if (blGo) {
				...
				try {
					...
					if (blExecutePOS) {
						...
						String strSSLDebug = "";
						if (info != null) {
							debug = 0 == info.get(DBSettings.MAIN_DEBUG).compareTo("1");
							if (debug)
								log("       * Debug Mode is ON");
							else
								log("       * Debug Mode is OFF");
							if (0 == info.get(DBSettings.POS_SSLENABLED).compareTo("1")) {
								...
								log("       * Using SSL");
								debug("            " + strSSL);
								if (0 == info.get(DBSettings.POS_SSLDEBUG).compareTo("1")) {
