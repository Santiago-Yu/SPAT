		...
		try {
			...
			while ((line = in.readLine()) != null) {
				if (line.startsWith("<?")) {
					...
					ThdlDebug.verify(0 == escapedToken.length());
					while (st.hasMoreTokens()) {
						String val = getEscapedToken(st, escapedToken);
						if (val.equals(DELIMITER) && 0 == escapedToken.length()) {
							k++;
						} else if (!val.equals("")) {
							...
							case 10:
								if (!val.equals("none")) {
									...
									while (uTok.hasMoreTokens()) {
										...
										ThdlDebug.verify(4 == subval.length() || 3 == subval.length());
										try {
											int x = Integer.parseInt(subval, 16);
											ThdlDebug.verify((x >= 0x0F00 && x <= 0x0FFF) || 0x5350 == x || 0x534D == x
													|| 0x0020 == x || 0x00A0 == x || 0x2003 == x);
									...
									char ch;
									if (1 == unicodeBuffer.length()
											&& UnicodeUtils.isInTibetanRange(ch = unicodeBuffer.charAt(0))) {
										if (null != UnicodeToTMW[ch - '?'][0] && '?' != ch && '?' != ch && '?' != ch
												&& '?' != ch && '?' != ch && '?' != ch && '?' != ch && '?' != ch
												&& '?' != ch && '?' != ch && '?' != ch && '?' != ch && '?' != ch
												&& '?' != ch && '?' != ch && '?' != ch && '?' != ch && '?' != ch
												&& '?' != ch && '?' != ch && '?' != ch && '?' != ch && '?' != ch
