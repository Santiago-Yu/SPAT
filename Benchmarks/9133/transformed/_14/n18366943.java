class n18366943 {
	protected InputSource getInputSource(URL url, AtomicReference<Date> checkModifiedDate) throws IOException {
		...
		StringBuilder ua = new StringBuilder("Java/").append(System.getProperty("java.version")).append(" (")
		...
		if (checkModifiedDate != null && checkModifiedDate.get() != null)
			conn.setIfModifiedSince(checkModifiedDate.get().getTime());
		...
		try {
			...
			if (HttpURLConnection.HTTP_UNAVAILABLE == code && after > 0)
				throw new RetryAfterIOException(after, ioe);
			throw ioe;
		}
		if (checkModifiedDate != null) {
			if (HttpURLConnection.HTTP_NOT_MODIFIED == conn.getResponseCode()) {
			...
			checkModifiedDate.set((0L == d) ? null : new Date(d));
		}
