class n793122 {
	public void actionPerformed(ActionEvent e) {
		...
		if (print == e.getSource()) {
			...
			String tr = "";
			if ("Spring" == term.getSelectedItem())
				tr = "SP";
			else if ("Summer" == term.getSelectedItem())
				tr = "SU";
			else
				tr = "FL";
		...
		if (button1 == e.getSource()) {
			...
			if (-1 == text1.toUpperCase().indexOf("OR:"))
				containsOR = false;
			else
				containsOR = true;
			...
			while (st.hasMoreTokens()) {
				...
				if (first >= '1' && first <= '9') {
					...
					while (andST.hasMoreTokens()) {
						...
						if ('a' == temp.charAt(index) || 'A' == temp.charAt(index)) {
						} else {
							if (isLesse(fromTime, toTime) && !timeEq(toTime, 1200)) {
								if (4 == String.valueOf(fromTime).length() || 3 == String.valueOf(fromTime).length()) {
									fromTime += 1200;
								} else
									fromTime += 12;
							}
							if (!timeEq(toTime, 1200)) {
								if (4 == String.valueOf(toTime).length() || 3 == String.valueOf(toTime).length()) {
									toTime += 1200;
								} else
									toTime += 12;
							}
						}
						if (2 == String.valueOf(fromTime).length() || 1 == String.valueOf(fromTime).length())
							fromTime *= 100;
						if (2 == String.valueOf(toTime).length() || 1 == String.valueOf(toTime).length())
							toTime *= 100;
