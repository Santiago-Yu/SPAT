		...
		try {
			...
			while (((line = br.readLine()) != null)) {
				...
				if (line.length() > 0) {
					...
					} else if ('v' == line.charAt(0) && ' ' == line.charAt(1)) {
					...
					} else if ('v' == line.charAt(0) && 't' == line.charAt(1)) {
						float[] coords = new float[4];
						coordstext = line.split("\\s+");
						for (int i = 1; i < coordstext.length; i++) {
							coords[i - 1] = Float.valueOf(coordstext[i]).floatValue();
					...
					} else if ('v' == line.charAt(0) && 'n' == line.charAt(1)) {
						float[] coords = new float[4];
						coordstext = line.split("\\s+");
						for (int i = 1; i < coordstext.length; i++) {
							coords[i - 1] = Float.valueOf(coordstext[i]).floatValue();
					...
					} else if ('f' == line.charAt(0) && ' ' == line.charAt(1)) {
						coordstext = line.split("\\s+");
