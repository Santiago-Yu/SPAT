class n10151623 {
	public void signAndSend() throws Exception {
		...
		if (testNumber == TEST_SIGNED_WITH_WRONG_CERT) {
		...
		if (testNumber == TEST_NOT_ON_OR_AFTER_EXPIRED) {
		...
		if (testNumber == TEST_UNKNOWN_CONFIRMATION) {
		...
		if (testNumber == TEST_WRONG_RECIPIENT) {
		...
		if (testNumber == TEST_WRONG_AUDIENCE) {
		...
		Transformer trans;
		if (testNumber == TEST_DATA_ALTERED_AFTER_SIG) {
			right_now.add(Calendar.MINUTE, 10);
		...
		if (useJavaPOST) {
			try {
				...
				printout.flush();
				printout.close();
				if (testNumber == TEST_GOOD_REPLAY) {
