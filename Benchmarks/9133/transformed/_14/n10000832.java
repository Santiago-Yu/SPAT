class n10000832 {
	public static void main(String[] args) {
		...
		for (int i = 0; i < args.length; i++) {
			...
			if (arg.trim().startsWith(DEBUG_PARAMETER_NAME + "=")) {
				...
				properties.put(OWLIMSERVICE_URL_PARAMETER_NAME,
						arg.trim().substring(OWLIMSERVICE_URL_PARAMETER_NAME.length() + 1).trim());
			} else if (arg.trim().startsWith(DOCSERVICE_URL_PARAMETER_NAME + "=")) {
				properties.put(DOCSERVICE_URL_PARAMETER_NAME,
						arg.trim().substring(DOCSERVICE_URL_PARAMETER_NAME.length() + 1).trim());
				...
				properties.put(DOCSERVICE_PROXY_FACTORY_PARAMETER_NAME,
						arg.trim().substring(DOCSERVICE_PROXY_FACTORY_PARAMETER_NAME.length() + 1).trim());
		...
		if (null == properties.getProperty(SITE_CONFIG_URL_PARAMETER_NAME)
				|| 0 == properties.getProperty(SITE_CONFIG_URL_PARAMETER_NAME).length()) {
			String err = "Mandatory parameter '" + SITE_CONFIG_URL_PARAMETER_NAME
					+ "' is missing.\n\nApplication will exit.";
			System.out.println(err);
			JOptionPane.showMessageDialog(new JFrame(), err, "Error!", JOptionPane.ERROR_MESSAGE);
			System.exit(-1);
		...
		if (properties.getProperty(MODE_PARAMETER_NAME).toLowerCase().equals(DIRECT_MODE)) {
			if (properties.getProperty(AUTOCONNECT_PARAMETER_NAME, "").toLowerCase().equals(AUTOCONNECT_TRUE)) {
				if (null == properties.getProperty(DOC_ID_PARAMETER_NAME)
						|| 0 == properties.getProperty(DOC_ID_PARAMETER_NAME).length()) {
			...
			if (properties.getProperty(AUTOCONNECT_PARAMETER_NAME, "").toLowerCase().equals(AUTOCONNECT_TRUE)) {
				if (null == properties.getProperty(USER_ID_PARAMETER_NAME)
						|| 0 == properties.getProperty(USER_ID_PARAMETER_NAME).length()) {
