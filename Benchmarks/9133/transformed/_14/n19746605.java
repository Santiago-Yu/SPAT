class n19746605 {
	private synchronized Frame insertFrame(int index, File source, INSERT_TYPE type) throws IOException {
		...
		switch (type) {
		case MOVE:
			if (0 == source.getParentFile().compareTo(new File(Settings.getPropertyString(ConstantKeys.project_dir)))
					&& source.getName().matches("img_[0-9]{5}\\.jpg")) {
				for (int i = 0; i < frames_.size(); i++) {
					Frame f = frames_.get(i);
					if (0 == f.getFile().compareTo(source)) {
						frames_.remove(i);
						break;
					}
				}
		...
		for (int i = frames_.size() - 1; i >= index; i--) {
			Frame newFrame = new Frame(
					new File(Settings.getPropertyString(ConstantKeys.project_dir), formatFileName(i)));
