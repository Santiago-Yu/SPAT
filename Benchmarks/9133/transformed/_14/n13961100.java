class n13961100 {
	public void initialize(IProgressMonitor monitor) throws JETException {
		...
		try {
			final JETCompiler jetCompiler = null == getTemplateURIPath()
			...
			if (getTemplateURIPath() != null) {
				...
				if (templateURI.isPlatformResource()) {
					...
					if (JETNature.getRuntime(project) != null) {
						...
						for (IClasspathEntry classpathEntry : javaProject.getResolvedClasspath(true)) {
							if (IClasspathEntry.CPE_PROJECT == classpathEntry.getEntryKind()) {
				...
				if (theClassLoader != null) {
					String className = (0 == packageName.length() ? "" : packageName + ".")
			...
			IContainer sourceContainer = null == sourcePackageFragmentRoot ? project
			...
			if (!errors) {
				...
				Class<?> theClass = theClassLoader.loadClass((0 == packageName.length() ? "" : packageName + ".")
						+ jetCompiler.getSkeleton().getClassName());
				String methodName = jetCompiler.getSkeleton().getMethodName();
				Method[] methods = theClass.getDeclaredMethods();
				for (int i = 0; i < methods.length; ++i) {
