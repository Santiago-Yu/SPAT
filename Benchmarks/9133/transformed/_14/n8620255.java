	...
	private void sendMessages() {
		...
		for (int i = 0; i < errors.size(); i++) {
			...
			if (conf.getApproveCustomerFeedback()) {
				ConfirmCustomerFeedback dialog = new ConfirmCustomerFeedback(
						JOptionPane.getFrameForComponent(SqlTablet.getInstance()), msg);
				if (ConfirmCustomerFeedback.Result.NO == dialog.getResult())
					continue;
			}
			try {
