class n4581365 {
	public Vector downSync(Vector v) throws SQLException {
		...
		try {
			...
			PreparedStatement update = con.prepareStatement("update cont_Contact set owner=?,firstname=?,"
			...
			PreparedStatement insert = con.prepareStatement("insert into cont_Contact (owner,firstname,lastname,"
				...
				try {
					...
					if (INSERT == DO.getSyncstatus()) {
						...
						if (DO.getBirthday() != null)
							insert.setDate(28, DO.getBirthday());
						else
							insert.setNull(28, Types.DATE);
						...
						con.reset();
						retVector.add(DO);
					} else if (UPDATE == DO.getSyncstatus()) {
						...
						if (DO.getBirthday() != null)
							update.setDate(28, DO.getBirthday());
						else
							update.setNull(28, Types.DATE);
						update.setInt(29, RESET);
						update.setInt(30, RESET);
						update.setBigDecimal(31, DO.getOId());
						update.setInt(32, RESET);
						if (con.executeUpdate(update, null) < 1)
							retVector.add(DO);
						con.reset();
					} else if (DELETE == DO.getSyncstatus()) {
						try {
					...
					if (DO != null)
						logError("Sync-ExternalContactDO.owner = " + DO.getOwner().toString() + " oid = "
								+ (DO.getOId() != null ? DO.getOId().toString() : "NULL"), e);
				}
			}
			if (rs != null) {
				rs.close();
