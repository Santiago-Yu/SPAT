	...
	private void constructDialogContent(Composite parent) {
		...
		fragmentsTable.setLabelProvider(new LabelProvider() {
			...
			public String getText(Object element) {
				...
				if (element instanceof ProcessFragment) {
					...
					String label = (null == frag.getName()) ? " <no-fragment-name>" : frag.getName();
		...
		menuMgr.addMenuListener(new IMenuListener() {
			...
			public void menuAboutToShow(IMenuManager mgr) {
				if (GUIHelper.containsCursor(fragmentsTable.getTable()) == false) {
					return;
				}
				if (fragmentsTable.getSelection().isEmpty() == false) {
		...
		clearBpgButton.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent s) {
				...
				if (sel.isEmpty() || sel.getFirstElement() instanceof ProcessFragment == false) {
					return;
				}
