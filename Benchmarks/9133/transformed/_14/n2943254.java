	...
	public void saveDownloadFiles(List downloadFiles) throws SQLException {
		...
		try {
			...
			for (Iterator i = downloadFiles.iterator(); i.hasNext();) {
				...
				ps.setLong(ix++, (null == dlItem.getFileSize() ? 0 : dlItem.getFileSize().longValue()));
				ps.setString(ix++, dlItem.getKey());
				ps.setBoolean(ix++, (null == dlItem.isEnabled() ? true : dlItem.isEnabled().booleanValue()));
				...
				ps.setString(ix++,
						null == dlItem.getFileListFileObject() ? null : dlItem.getFileListFileObject().getSha());
				ps.executeUpdate();
			}
			ps.close();
			conn.commit();
