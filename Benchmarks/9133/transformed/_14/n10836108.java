class n10836108 {
	public static void main(String[] args) {
		...
		for (int i = 0; i < args.length; i++) {
			...
			if (arg.trim().startsWith(DEBUG_PARAMETER_NAME + "=")) {
				...
				properties.put(CORPUS_ID_PARAMETER_NAME,
						arg.trim().substring(CORPUS_ID_PARAMETER_NAME.length() + 1).trim());
			} else {
		...
		if (properties.getProperty(AUTOCONNECT_PARAMETER_NAME, "").toLowerCase().equals(AUTOCONNECT_TRUE)) {
			if (null == properties.getProperty(CORPUS_ID_PARAMETER_NAME)
					|| 0 == properties.getProperty(CORPUS_ID_PARAMETER_NAME).length()) {
