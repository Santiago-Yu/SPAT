class n4114529 {
	...
	public void save(File folder) {
		...
		if (EsomMapper.RADIO_LOAD_SELECTED == this.getProperty(EsomMapper.PROPERTY_LRN_RADIO_SELECTED)) {
			...
			if (src.getParent() != folder.getPath()) {
				...
				File dst = new File(
						folder.getAbsolutePath() + File.separator + src.getName() + String.valueOf(actInstance));
				try {
		...
		if (EsomMapper.RADIO_LOAD_SELECTED == this.getProperty(EsomMapper.PROPERTY_WTS_RADIO_SELECTED)) {
		...
		if (EsomMapper.RADIO_SELECT_FROM_DATANAV_SELECTED == this.getProperty(EsomMapper.PROPERTY_LRN_RADIO_SELECTED)) {
			...
			try {
				...
				for (IColumn col : this.inputVectors.getColumns()) {
					if (IClusterNumber.class == col.getType())
						bw.write("\t2");
					else if (String.class == col.getType())
						bw.write("\t8");
					else
						bw.write("\t1");
				}
		...
		if (EsomMapper.RADIO_SELECT_FROM_DATANAV_SELECTED == this.getProperty(EsomMapper.PROPERTY_WTS_RADIO_SELECTED)) {
			this.setProperty(EsomMapper.PROPERTY_WTS_FILE, "EsomMapper" + this.actInstance + ".wts");
			MyRetina tempRetina = new MyRetina(this.outputRetina.getNumRows(), this.outputRetina.getNumCols(),
