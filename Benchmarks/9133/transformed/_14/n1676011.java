	...
	ServiceDescription getServiceDescription() throws ConfigurationException {
		...
		try {
			final Node serviceDescriptionNode = (Node) pathsXPath.evaluate(
					ConfigurationFileTagsV1.SERVICE_DESCRIPTION_ELEMENT_XPATH, this.configuration, XPathConstants.NODE);
			final String title = getMandatoryElementText(serviceDescriptionNode, ConfigurationFileTagsV1.TITLE_ELEMENT);
			ServiceDescription.Builder builder = new ServiceDescription.Builder(title,
			...
			builder.description(
					getOptionalElementText(serviceDescriptionNode, ConfigurationFileTagsV1.DESCRIPTION_ELEMENT));
			final String serviceVersion = getOptionalElementText(serviceDescriptionNode,
			...
			if (null == identifier || "".equals(identifier)) {
				try {
					...
					final String toolIdentifier = null == toolIDURI ? "" : toolIDURI.toString();
			...
			builder.instructions(
					getOptionalElementText(serviceDescriptionNode, ConfigurationFileTagsV1.INSTRUCTIONS_ELEMENT));
			builder.furtherInfo(
					getOptionalURIElement(serviceDescriptionNode, ConfigurationFileTagsV1.FURTHER_INFO_ELEMENT));
