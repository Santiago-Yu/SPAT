class n18574455 {
	protected boolean process(final TranscodeJobImpl job) {
		...
		try {
			...
			if (null == provider) {
				...
				if (job.isStream()) {
					...
					if (device instanceof TranscodeTarget) {
						if (0 == provider_analysis.getLongProperty(TranscodeProviderAnalysis.PT_VIDEO_HEIGHT)) {
				...
				if (TranscodeTarget.TRANSCODE_NEVER == tt_req) {
					xcode_required = false;
				} else if (TranscodeTarget.TRANSCODE_ALWAYS == tt_req) {
					xcode_required = true;
			...
			if (xcode_required) {
				...
				TranscodeProviderAdapter xcode_adapter = new TranscodeProviderAdapter() {
					...
					public void updateProgress(int percent, int eta_secs, int new_width, int new_height) {
						...
						if (TranscodeJob.ST_CANCELLED == job_state || TranscodeJob.ST_REMOVED == job_state) {
							prov_job.cancel();
						} else if (paused || TranscodeJob.ST_PAUSED == job_state) {
							prov_job.pause();
						} else {
							if (TranscodeJob.ST_RUNNING == job_state) {
								prov_job.resume();
							}
				...
				TranscodeQueueListener listener = new TranscodeQueueListener() {
					...
					public void jobChanged(TranscodeJob changed_job) {
						if (job == changed_job) {
							...
							if (TranscodeJob.ST_PAUSED == state) {
								provider_job[0].pause();
							} else if (TranscodeJob.ST_RUNNING == state) {
								provider_job[0].resume();
							} else if (TranscodeJob.ST_CANCELLED == state || TranscodeJob.ST_STOPPED == state) {
								provider_job[0].cancel();
							}
						}
					}
				...
				if (job.isStream()) {
					...
					if (null == url_str || 0 == url_str.length()) {
			...
			job.failed(e);
			e.printStackTrace();
			if (!job.isStream() && job.getEnableAutoRetry() && 0 == job.getAutoRetryCount() && job.canUseDirectInput()
					&& !job.useDirectInput()) {
