		...
		try {
			...
			if (hasStudentCredit()) {
				...
				if (1 == stmt.executeUpdate("UPDATE " + strPOSPrefix
						+ "studentcredit set credit_amount = credit_amount + " + round2Places(dblCAmnt)
						+ " WHERE credit_active = '1' and credit_studentid = '" + stuId + "'")) {
					stmt.close();
					stmt = conn.createStatement();
					if (1 == stmt.executeUpdate("UPDATE " + strPOSPrefix
				...
				if (1 == stmt.executeUpdate("insert into " + strPOSPrefix
						...
						+ round2Places(dblCAmnt) + "','1','" + stuId + "', NOW())")) {
					stmt.close();
					stmt = conn.createStatement();
					if (1 == stmt.executeUpdate("insert into " + strPOSPrefix
							+ "studentcredit_log ( scl_studentid, scl_action, scl_datetime ) values( '" + stuId + "', '"
