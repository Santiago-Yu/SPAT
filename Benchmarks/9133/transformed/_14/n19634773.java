class n19634773 {
	public static void processAction(ActionRequest actionRequest, ActionResponse actionResponse)
		...
		try {
			...
			if (mp.mod.getType() != null && ModuleTypeTypeType.LOOKUP_TYPE == mp.mod.getType().getType()
					&& (null == mp.getFromParam() || 0 == mp.getFromParam().length())) {
				actionResponse.setRenderParameter(MemberConstants.ERROR_TEXT,
				...
				try {
					...
					case ContentTypeActionType.CHANGE_TYPE:
						...
						for (int i = 0; i < mp.mod.getRelateClassCount(); i++) {
							...
							if (PrimaryKeyTypeType.NUMBER_TYPE == mp.content.getQueryArea().getPrimaryKeyType()
									.getType()) {
					...
					case ContentTypeActionType.DELETE_TYPE:
						...
						Object idRec;
						if (PrimaryKeyTypeType.NUMBER_TYPE == mp.content.getQueryArea().getPrimaryKeyType().getType()) {
							idRec = PortletService.getLong(actionRequest,
									mp.mod.getName() + '.' + mp.content.getQueryArea().getPrimaryKey());
						} else if (PrimaryKeyTypeType.STRING_TYPE == mp.content.getQueryArea().getPrimaryKeyType()
								.getType()) {
						...
						if (DatabaseManager.MYSQL_FAMALY == dbDyn.getFamaly())
