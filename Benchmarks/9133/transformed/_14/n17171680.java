	...
	public void sendMessage(String messageBufferName, String messageStr, String timeout) throws AppFabricException {
		...
		try {
			...
			if (LoggerUtil.getIsLoggingOn())
				SDKLoggerHelper.logRequest(connection, SDKLoggerHelper.RecordType.SendMessage_REQUEST);
			String responseCode = "<responseCode>" + connection.getResponseCode() + "</responseCode>";
			if (!((MessageBufferConstants.HTTP_STATUS_CODE_ACCEPTED == connection.getResponseCode())
					|| (MessageBufferConstants.HTTP_STATUS_CODE_CREATED == connection.getResponseCode())
					|| (MessageBufferConstants.HTTP_STATUS_CODE_OK == connection.getResponseCode()))) {
				throw new AppFabricException(
						"Message could not be sent. Error.Response code: " + connection.getResponseCode());
			}
			if (LoggerUtil.getIsLoggingOn())
