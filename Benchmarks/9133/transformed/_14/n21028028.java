class n21028028 {
	public int save(String newInventory, String inventoryType, int compareResult, boolean renameCorruptedFile) {
		if (null == newInventory || newInventory.equals(""))
		...
		try {
			...
			if (false == directory_repository.endsWith("/") && false == directory_repository.endsWith(File.separator)) {
		...
		try {
			...
			if (false == path.startsWith("/") && false == path.startsWith(File.separator)) {
		...
		if (found > 0) {
			if (InventoryMonitor.CONFIGURATION_CHANGED == compareResult
					|| InventoryMonitor.FIRST_ACTIVE_CONFIGURATION_DOWNLOAD == compareResult) {
				try {
