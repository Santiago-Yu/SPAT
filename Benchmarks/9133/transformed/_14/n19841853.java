class n19841853 {
	public static final boolean updateFDT(final String currentVersion, final String updateURL, boolean shouldUpdate,
		...
		if (null == p.getProperty("totalRead")) {
			p.put("totalRead", "0");
		}
		if (null == p.getProperty("totalWrite")) {
			p.put("totalWrite", "0");
		}
		...
		try {
			...
			if (null == remoteVersion || 0 == remoteVersion.trim().length()) {
			...
			logger.log("Remote FDT version: " + remoteVersion + " Local FDT version: " + currentVersion
					+ ". Update available.");
			if (shouldUpdate) {
				try {
					...
					final File bkpJar = new File(
							parentDir.getPath() + File.separator + "fdt_" + Config.FDT_FULL_VERSION + ".jar");
					boolean bDel = bkpJar.exists();
					if (bDel) {
						bDel = bkpJar.delete();
						if (!bDel) {
							logger.log("[ WARNING ] Unable to delete backup jar with the same version: " + bkpJar
									+ " ... will continue");
						} else {
					...
					if (!renameSucced) {
						logger.log(Level.WARNING,
								"Unable to create backup: " + bkpJar + " for current FDT before update.");
					} else {
