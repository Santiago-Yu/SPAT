		...
		try {
			...
			if (action.equals("test")) {
				...
				for (int i = 0; i < numbertoexport; i++) {
					...
					for (int k = 0; k < v.size(); k++) {
						...
						if ("INChI" == canonName.getDBCanonicalNameType().getCanonicalNameType()) {
				...
				if (null == req.getParameter("symbol")) {
					res.setContentType("text/plain");
					out = res.getWriter();
					out.write(
				...
				while (-1 == entry.getName().indexOf("PredictionTool.class")) {
				...
				while (true) {
					...
					while (rs.next()) {
						...
						for (int spheres = 6; spheres > 0; spheres--) {
							...
							if (null == ((HashMap) mapsmap.get(symbol)).get(hoseCode)) {
			...
			if (action.equals("exportspec")) {
				if (format.equals("txt")) {
					...
					out.print("\n\rAtom\t");
					if (("H") == spectrum.getDBSpectrumType().getElementSymbol())
						out.print("Mult.\t");
					out.print("Meas.");
					if (lastsearchtype.equals(NmrshiftdbConstants.TOTALSPECTRUM)
							|| lastsearchtype.equals(NmrshiftdbConstants.SUBSPECTRUM)) {
