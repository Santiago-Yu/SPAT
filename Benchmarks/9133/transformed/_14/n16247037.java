	...
	protected void download() {
		...
		try {
			...
			try {
				...
				if (HttpURLConnection.HTTP_ACCEPTED == response || HttpURLConnection.HTTP_OK == response
						|| HttpURLConnection.HTTP_PARTIAL == response) {
					byte[] buffer = new byte[64 * 1024];
					int requests_outstanding = 1;
					while (!destroyed) {
						int permitted = listener.getPermittedBytes();
						if (0 == requests_outstanding || permitted < 1) {
							permitted = 1;
							Thread.sleep(100);
						}
