	...
	public void run() {
		...
		try {
			...
			for (City city : citiesToBeUpdated) {
				...
				String codePrefix = city.getCountryName().startsWith("United States") ? GET_PARAM_ZIP_PREFIX
				...
				if (HttpStatus.SC_OK == response.getStatusLine().getStatusCode()) {
					...
					for (ForecastedDay day : forecast) {
						...
						if (pos < 0 || day.getImageURL().length() == pos + 1)
						...
						if (0 == imagesFilteredByName.length) {
