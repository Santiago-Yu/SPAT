	...
	public void run() {
		...
		for (int f = 0; f < fileList.size(); f++) {
			...
			try {
				...
				InputStream is = null;
				if (EncryptionMode.MODE_STREAM == EncryptionMode.getBestEncryptionMode(alg.getEncryptionMode())) {
				...
				} else if (EncryptionMode.MODE_BLOCK == EncryptionMode.getBestEncryptionMode(alg.getEncryptionMode())) {
					...
					if (zae.getComment() != null && zae.getComment().length() > 0) {
						if (Arrays.equals(md.digest(), new Base64().decode(zae.getComment()))) {
							Application.getController().displayVerbose(
									"Hash of " + zae.getName() + ": " + new Base64().encodeToString(md.digest()));
							...
							Application.getController()
									.displayVerbose("the hash of " + zae.getName() + " was verified succesfully");
						}
					}
				}
				if (!gotEntries) {
