		...
		try {
			...
			boolean isInitiallyEmpty = classpathEntries.isEmpty();
			{
				if (referencedProjects.size() != 0
						&& 0 == (style & (EMF_PLUGIN_PROJECT_STYLE | EMF_EMPTY_PROJECT_STYLE))) {
					projectDescription
				...
				if (0 == (style & EMF_EMPTY_PROJECT_STYLE)) {
					if ((style & EMF_PLUGIN_PROJECT_STYLE) != 0) {
						...
						for (Iterator<IClasspathEntry> i = classpathEntries.iterator(); i.hasNext();) {
							...
							if (IClasspathEntry.CPE_VARIABLE == classpathEntry.getEntryKind()
									...
									|| IClasspathEntry.CPE_PROJECT == classpathEntry.getEntryKind()) {
								i.remove();
							}
						}
					} else {
						...
						if (0 == (style & EMF_MODEL_PROJECT_STYLE)) {
							...
							if (0 == (style & EMF_EDIT_PROJECT_STYLE)) {
								...
								if (0 == (style & EMF_XML_PROJECT_STYLE)) {
									CodeGenUtil.EclipseUtil.addClasspathEntries(classpathEntries, "EMF_ECORE_XMI",
											"org.eclipse.emf.ecore.xmi");
								}
							}
