		...
		try {
			...
			if (req.persLong.getPid() > 0) {
				...
				if (req.persLong.isMainModified()) {
					if (null == req.persLong.getModified()) {
			...
			if (nn != null) {
				for (int i = 0; i < nn.length; i++) {
					...
					if (n.isToBeDeleted()) {
						...
						logger.fine(text);
					} else if (0 == n.getPnid() || n.isToBeUpdated()) {
						if (0 == n.getPnid()) {
							stm = con.createStatement();
						...
						if (n.isToBeUpdated() || 0 == n.getPnid()) {
