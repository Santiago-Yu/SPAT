class n13574318 {
	public void actionPerformed(ActionEvent e) {
		...
		if (JFileChooser.APPROVE_OPTION == saveForWebChooser.showSaveDialog(Main.this)) {
			...
			if (!name.toLowerCase().endsWith(".html") && -1 == name.indexOf('.')) {
				name = name + ".html";
			}
			...
			if (null == appletJarFile) {
				...
				if (JFileChooser.APPROVE_OPTION == appletChooser.showOpenDialog(Main.this)) {
					Config.setRecentDirectory("recentAppletJarDirectory",
							appletChooser.getCurrentDirectory().getAbsolutePath());
			...
			try {
				AppletProducer.createHtml(htmlOutputFile, (DeviceImpl) DeviceFactory.getDevice(),
						jadMidletEntry.getClassName(), midletOutputFile, appletPackageOutputFile, deviceOutputFile);
