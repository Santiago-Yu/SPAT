	...
	public void saveUploadFiles(List uploadFiles) throws SQLException {
		...
		try {
			...
			for (Iterator i = uploadFiles.iterator(); i.hasNext();) {
				...
				ps.setBoolean(ix++, (null == ulItem.isEnabled() ? true : ulItem.isEnabled().booleanValue()));
				...
				ps.setString(ix++, (null == ulItem.getSharedFileItem() ? null : ulItem.getSharedFileItem().getSha()));
				ps.executeUpdate();
			}
			ps.close();
			conn.commit();
