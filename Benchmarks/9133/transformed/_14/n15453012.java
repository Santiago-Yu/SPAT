class n15453012 {
	public ArrayList parseFile(File newfile) throws IOException {
		...
		while ((s = br.readLine()) != null) {
			...
			if (0 == ((String) tokens[0]).length()) {
				readraw = false;
				readfit = false;
				continue;
			}
			if ((4 == nvalues) && (!firstName.startsWith("---"))) {
				if ((0. == Double.parseDouble(tokens[1])) && (0. == Double.parseDouble(tokens[2]))
						&& (0. == Double.parseDouble(tokens[3]))) {
					zerodata = true;
				} else {
					zerodata = false;
				}
