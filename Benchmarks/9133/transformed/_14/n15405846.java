class n15405846 {
	public DataSet guessAtUnknowns(String filename) {
		...
		try {
			...
			if (!sval1[0].startsWith("<") && (2 == sval1.length) && (line1.indexOf(':') < 0)) {
				int countLoci = Integer.parseInt(sval1[1]);
				if (sval2.length == countLoci) {
			...
			if (sval1[0].equalsIgnoreCase("<Annotated>")) {
				...
				if (info2.length > 1) {
					...
					while (null == guess && inline != null && (inline.startsWith("#") || inline.startsWith("<"))) {
