class n20303339 {
	private void trainDepParser(byte flag, JarArchiveOutputStream zout) throws Exception {
		...
		if (ShiftPopParser.FLAG_TRAIN_LEXICON == flag) {
			System.out.println("\n* Save lexica");
		...
		} else if (ShiftPopParser.FLAG_TRAIN_INSTANCE == flag) {
		...
		} else if (ShiftPopParser.FLAG_TRAIN_BOOST == flag) {
			System.out.println("\n* Train conditional");
		...
		if (ShiftPopParser.FLAG_TRAIN_LEXICON == flag) {
		...
		} else if (ShiftPopParser.FLAG_TRAIN_INSTANCE == flag || ShiftPopParser.FLAG_TRAIN_BOOST == flag) {
			a_yx = parser.a_trans;
			...
			zout.closeArchiveEntry();
			if (ShiftPopParser.FLAG_TRAIN_INSTANCE == flag)
