class n17573144 {
	public void dispatch(com.sun.star.util.URL aURL, com.sun.star.beans.PropertyValue[] aArguments) {
		if (0 == aURL.Protocol.compareTo("org.openoffice.oosvn.oosvn:")) {
			OoDocProperty docProperty = getProperty();
			settings.setCancelFired(false);
			if (0 == aURL.Path.compareTo("svnUpdate")) {
				try {
					try {
					...
					if (tempDir.exists()) {
						if (false == deleteFileDir(tempDir)) {
					...
					for (int j = 0; j < tempFiles.length; j++) {
						if (tempFiles[j].toString().endsWith(".odt"))
							anyOdt = tempFiles[j];
						if (tempFiles[j].toString().equals(settings.getCheckoutDoc())
					...
					if (thisOdt != null)
						anyOdt = thisOdt;
					String url;
					if (null == settings.getCheckoutDoc() || !settings.getCheckoutDoc().equals(anyOdt.getName())) {
						...
						if (newSvnInfo.exists()) {
							if (false == deleteFileDir(newSvnInfo)) {
								error("Error while deleting temporary checkout dir.");
							}
						}
						...
						Object objDispatchHelper = m_xServiceManager
								.createInstanceWithContext("com.sun.star.frame.DispatchHelper", m_xContext);
						XDispatchHelper xDispatchHelper = (XDispatchHelper) UnoRuntime
								.queryInterface(XDispatchHelper.class, objDispatchHelper);
			...
			if (0 == aURL.Path.compareTo("svnCommit")) {
				try {
					try {
						settings = getSerializedSettings(docProperty);
					} catch (Exception ex) {
					...
					if (0 == headRevision) {
						...
						File impFile = new File(settings.getCheckoutPath() + svnWorker.tempDir + "/.import/"
			...
			if (0 == aURL.Path.compareTo("svnHistory")) {
				try {
					try {
						settings = getSerializedSettings(docProperty);
					} catch (Exception ex) {
			...
			if (0 == aURL.Path.compareTo("settings")) {
				try {
					settings = getSerializedSettings(docProperty);
			...
			if (0 == aURL.Path.compareTo("about")) {
