		...
		try {
			...
			while ((WA = writeAttr.readLine()) != null && (RA = readAttr.readLine()) != null
				...
				while (WA.compareTo("EndOfClass") != 0 && RA.compareTo("EndOfClass") != 0
					...
					RA = readAttr.readLine();
					while (true) {
						if (0 == WA.compareTo("EndOfMethod") && 0 == RA.compareTo("EndOfMethod")
								&& 0 == MC.compareTo("EndOfMethod")) {
							break;
						}
						...
						for (int i = 0; i < numFiles && info[i] != null; i++) {
							if (0 == info[i].compareTo(WA)) {
								writeCount[i]++;
							}
							if (0 == info[i].compareTo(RA)) {
								readCount[i]++;
							}
							if (0 == info[i].compareTo(MC)) {
								methodCallCount[i]++;
							}
						}
						if (WA.compareTo("EndOfMethod") != 0) {
			...
			for (int i = 0; i < numFiles; i++) {
				for (int j = 0; j < counter - 1 && info[i] != null; j++) {
					if (0 == infoArray[j + 1].compareTo(info[i])) {
						if (false == classWritten) {
