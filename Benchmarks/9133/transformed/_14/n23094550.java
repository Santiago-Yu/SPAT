class n23094550 {
	public StringBuffer render(RenderEngine c) {
		...
		if (isRooted && (-1 == useFilename.indexOf("/") || useFilename.startsWith("./"))) {
			...
			} else if (c.getWorkerContext() != null && -1 == useFilename.indexOf("/")) {
		...
		if (null == c.getWorkerContext()) {
			if (null == c.getRenderContext().getCurrentDocroot()) {
				currentDocroot = ".";
			} else {
		...
		if (action.equalsIgnoreCase("read")) {
			...
			if (null == xmlparse || xmlparse.equals("")) {
				if (null == data) {
			...
			if (c.getVendContext() != null && c.getVendContext().getFileAccess() != null
					&& FileAccess.TYPE_JAR == c.getVendContext().getFileAccess().getVFSType(filenameData.toString(),
				...
				c.getVariableContainer().setVector(varname, fileList);
					...
					&& FileAccess.TYPE_FS == c.getVendContext().getFileAccess().getVFSType(filenameData.toString(),
							c.getClientContext().getMatchedHost())) {
				Vector listFiles = c.getVendContext().getFileAccess().listFiles(filenameData.toString(),
						c.getClientContext().getMatchedHost());
				Object[] list = listFiles.toArray();
