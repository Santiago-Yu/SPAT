		...
		while (reader.hasNext()) {
			...
			if (evt.isStartElement()) {
				...
				if (!inEntry && Atom.NS.equals(qName.getNamespaceURI()) && qName.getLocalPart().equals("entry")) {
					...
					} else if (null == link && local.equals("link")) {
				...
				if (inEntry && Atom.NS.equals(qName.getNamespaceURI())) {
					...
					if (local.equals("entry")) {
						...
						String tokens[] = null == title ? new String[0] : title.trim().split("[ \t\n\r]+");
						...
						if (valid && null == (p1 = fetchProtein(Integer.parseInt(tokens[1].substring(3))))) {
							valid = false;
						}
						...
						if (valid && null == (p2 = fetchProtein(Integer.parseInt(tokens[2].substring(3))))) {
							valid = false;
						}
						...
						if (valid && null == (pubmed = fetchPubmedEntry(Integer.parseInt(tokens[3].substring(5))))) {
							valid = false;
						}
