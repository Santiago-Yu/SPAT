class n18413512 {
	public boolean save(String trxName) {
		if (null == m_value || (!(m_value instanceof String || m_value instanceof byte[]))
				|| (m_value instanceof String && 0 == m_value.toString().length())
				|| (m_value instanceof byte[] && 0 == ((byte[]) m_value).length)) {
			StringBuffer sql = new StringBuffer("UPDATE ").append(m_tableName).append(" SET ").append(m_columnName)
		...
		if (DB.isRemoteObjects()) {
			...
			try {
				if (server != null) {
					success = server.updateLOB(sql.toString(), m_displayType, m_value, trxName,
		...
		try {
			...
			if (DisplayType.TextLong == m_displayType)
