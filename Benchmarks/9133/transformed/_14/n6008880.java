	...
	protected void updateJava2ScriptProject(String prjFolder, String binRelative) {
		try {
			...
			if (classpath != null) {
				...
				if (-1 == classpath.indexOf("ECLIPSE_SWT") && -1 == classpath.indexOf("SWT_LIBRARY")
						&& -1 == classpath.indexOf("eclipse.swt")) {
					int idx = classpath.lastIndexOf("<");
					classpath = classpath.substring(0, idx)
							+ "\t<classpathentry kind=\"var\" path=\"ECLIPSE_SWT\"/>\r\n" + classpath.substring(idx);
					needUpdate = true;
				}
				if (-1 == classpath.indexOf("AJAX_SWT") && -1 == classpath.indexOf("ajaxswt.jar")) {
					int idx = classpath.lastIndexOf("<");
					classpath = classpath.substring(0, idx)
				...
				if (-1 == classpath.indexOf("AJAX_RPC") && -1 == classpath.indexOf("ajaxrpc.jar")) {
					int idx = classpath.lastIndexOf("<");
					classpath = classpath.substring(0, idx)
				...
				if (-1 == classpath.indexOf("AJAX_PIPE") && -1 == classpath.indexOf("ajaxpipe.jar")) {
					int idx = classpath.lastIndexOf("<");
					classpath = classpath.substring(0, idx)
