class n20623709 {
	public Long split(File targetDirectory, String prefix, long maxUnitBases, long maxUnitEntries) throws Exception {
		...
		if (!size.isUnknown() && size.getBases() < maxUnitBases
				&& (maxUnitEntries <= 0 || size.getEntries() < maxUnitEntries)) {
			...
			try {
				...
				for (; fastaFileReadOffset < fastaFileSize;) {
					...
					for (;;) {
						...
						if ('\r' == b) {
							...
							if (FASTAFileTokenizer.DEFLINE == fastaReadState) {
								fastaReadState = FASTAFileTokenizer.SEQUENCELINE;
							}
						} else if ('>' == b) {
							if (FASTAFileTokenizer.UNKNOWN == fastaReadState) {
								fastaReadState = FASTAFileTokenizer.STARTDEFLINE;
							} else if (FASTAFileTokenizer.SEQUENCELINE == fastaReadState) {
								fastaReadState = FASTAFileTokenizer.STARTDEFLINE;
							}
							if (FASTAFileTokenizer.STARTDEFLINE == fastaReadState) {
								if (currentBasesCount >= maxUnitBases
										|| maxUnitEntries > 0 && currentEntriesCount >= maxUnitEntries) {
									fastaBuffer.position(fastaBuffer.position() - 1);
									long currentTime = System.currentTimeMillis();
									System.out.println(new java.util.Date() + " Partition " + targetCount
											+ " containing " + currentEntriesCount + " sequences and "
											+ currentBasesCount + " residues ends at "
									...
									FileChannel partitionChannel = new FileOutputStream(
											new File(targetDirectory, prefix + "_" + targetCount + ".fasta"))
													.getChannel();
									nBytes = fastaChannel.transferTo(partitionStartOffset,
											partitionEndOffset - partitionStartOffset, partitionChannel);
							...
							if (FASTAFileTokenizer.SEQUENCELINE == fastaReadState) {
								totalResiduesCount++;
								currentBasesCount++;
							}
						}
				...
				if (partitionStartOffset < fastaFileSize) {
					...
					System.out.println(new java.util.Date() + " Partition " + targetCount + " containing "
							...
							+ (fastaFileSize) + " and was created in " + (currentTime - prevTime) + " ms");
					FileChannel partitionChannel = new FileOutputStream(
							new File(targetDirectory, prefix + "_" + targetCount + ".fasta")).getChannel();
					fastaChannel.transferTo(partitionStartOffset, fastaFileSize - partitionStartOffset,
							partitionChannel);
					partitionChannel.force(true);
					partitionChannel.close();
					targetCount++;
