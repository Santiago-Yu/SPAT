	...
	public boolean initFile(String filename) {
		...
		if (filename.toLowerCase().endsWith(".spc")) {
			try {
				...
				if (shitty_starting_constant == Firstx) {
		...
		while (My_Counter < nbLignes) {
			try {
				...
				} while (My_Counter < nbLignes && false == mon_token.hasMoreTokens());
				if (true == mon_token.hasMoreTokens()) {
					String keyword = mon_token.nextToken();
					if (0 == StringDataUtils.compareStrings(keyword, "##TITLE="))
						TexteTitre = uneligne.substring(9);
					if (0 == StringDataUtils.compareStrings(keyword, "##FIRSTX="))
						Firstx = Double.valueOf(mon_token.nextToken()).doubleValue();
					if (0 == StringDataUtils.compareStrings(keyword, "##LASTX="))
						Lastx = Double.valueOf(mon_token.nextToken()).doubleValue();
					if (0 == StringDataUtils.compareStrings(keyword, "##YFACTOR="))
						YFactor = Double.valueOf(mon_token.nextToken()).doubleValue();
					if (0 == StringDataUtils.compareStrings(keyword, "##NPOINTS="))
						Nbpoints = Integer.valueOf(mon_token.nextToken()).intValue();
					if (0 == StringDataUtils.compareStrings(keyword, "##XUNITS="))
						x_units = uneligne.substring(10);
					if (0 == StringDataUtils.compareStrings(keyword, "##YUNITS="))
						y_units = uneligne.substring(10);
					if (0 == StringDataUtils.compareStrings(keyword, "##.OBSERVE")
							&& 0 == StringDataUtils.compareStrings(mon_token.nextToken(), "FREQUENCY="))
						nmr_observe_frequency = Double.valueOf(mon_token.nextToken()).doubleValue();
					if (0 == StringDataUtils.compareStrings(keyword, "##XYDATA=")
							&& 0 == StringDataUtils.compareStrings(mon_token.nextToken(), "(X++(Y..Y))"))
						Datatype = "XYDATA";
					if (0 == StringDataUtils.compareStrings(keyword, "##XYDATA=(X++(Y..Y))"))
						Datatype = "XYDATA";
					if (0 == StringDataUtils.compareStrings(keyword, "##PEAK")
							&& 0 == StringDataUtils.compareStrings(mon_token.nextToken(), "TABLE=")
							&& 0 == StringDataUtils.compareStrings(mon_token.nextToken(), "(XY..XY)"))
						Datatype = "PEAK TABLE";
					if (0 == StringDataUtils.compareStrings(keyword, "##PEAK")
							&& 0 == StringDataUtils.compareStrings(mon_token.nextToken(), "TABLE=(XY..XY)"))
						Datatype = "PEAK TABLE";
				}
			} catch (Exception e) {
			}
		}
		if (0 == Datatype.compareTo("UNKNOWN"))
			return false;
		if (0 == Datatype.compareTo("PEAK TABLE") && 0 == x_units.compareTo("?"))
			x_units = "M/Z";
		if (0 == StringDataUtils.truncateEndBlanks(x_units).compareTo("HZ")
