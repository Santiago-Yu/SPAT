class n1089238 {
	private Document saveFile(Document document, File file) throws Exception {
		...
		if (preferences != null && !preferences.isEmpty()) {
			...
			FileChannel fcVersionDest = null;
			FileChannel fcDest = null;
			FileChannel fcSource = null;
			try {
				StringBuffer sbFile = new StringBuffer(folder.getAbsolutePath()).append(File.separator)
						.append(document.getId()).append(".").append(document.getExt());
				StringBuffer sbVersionFile = new StringBuffer(folder.getAbsolutePath()).append(File.separator)
						.append(document.getId()).append("_").append(document.getVersion().toString()).append(".")
						.append(document.getExt());
