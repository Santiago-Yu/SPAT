class n7300247 {
	public void moveMessage(DBMimeMessage oSrcMsg) throws MessagingException {
		...
		try {
			...
			oStmt = oConn.prepareStatement("SELECT " + DB.pg_message + "," + DB.nu_position + "," + DB.len_mimemsg
			...
			oStmt = oConn.prepareStatement("UPDATE " + DB.k_categories + " SET " + DB.len_size + "=" + DB.len_size + "-"
					+ String.valueOf(iLen) + " WHERE " + DB.gu_category + "=?");
		...
		try {
			...
			oStmt = oConn.prepareStatement("UPDATE " + DB.k_mime_msgs + " SET " + DB.gu_category + "=?," + DB.pg_message
					+ "=? WHERE " + DB.gu_mimemsg + "=?");
