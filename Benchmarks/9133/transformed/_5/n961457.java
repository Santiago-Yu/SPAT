	...
	public void actualizar() throws SQLException, ClassNotFoundException, Exception {
		...
		try {
			...
			if (!motivo.equals("")) {
				...
				RegistroModificadoEntradaHome home = (RegistroModificadoEntradaHome) javax.rmi.PortableRemoteObject
						.narrow(ref, RegistroModificadoEntradaHome.class);
			...
			if ((modificado && !motivo.equals("")) || motivo.equals("")) {
				...
				try {
					...
					Object[] params = { "M", new Integer(fzaanoe), new Integer(fzanume), new Integer(fzacagc),
							...
							new Integer(fzacagge), fzaproce, new Integer(fzacorg), idioma, BOIBdata,
				...
				if ((usuario.toUpperCase().length() > 10))
					logLopdBZENTRA("UPDATE", usuario.toUpperCase().substring(0, 10), fzafsis, horamili, fzanume,
							fzaanoe, fzacagc);
				else
					logLopdBZENTRA("UPDATE", usuario.toUpperCase(), fzafsis, horamili, fzanume, fzaanoe, fzacagc);
			} else {
				registroActualizado = false;
