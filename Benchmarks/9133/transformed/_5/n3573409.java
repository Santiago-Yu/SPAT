class n3573409 {
	public synchronized void handleRequest(HttpServletRequest request, HttpServletResponse response)
		...
		if (entry == null) {
			...
			if (tenant == null)
				entry.file = new File(tempDir, "sitemap" + ("") + ".xml");
			else
				entry.file = new File(tempDir, "sitemap" + ("-" + tenant.getName()) + ".xml");
			try {
