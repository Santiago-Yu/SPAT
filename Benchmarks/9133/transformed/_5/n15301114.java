	...
	public void updateFailedStatus(THLEventStatus failedEvent, ArrayList<THLEventStatus> events) throws THLException {
		...
		try {
			...
			if (events != null && events.size() > 0) {
				...
				stmt.executeUpdate("UPDATE history SET status = " + THLEvent.FAILED
						+ ", comments = 'Event was rollbacked due to failure while processing event#"
						+ failedEvent.getSeqno() + "'" + ", processed_tstamp = " + conn.getNowFunction()
			...
			if (failedEvent.getException() != null)
				pstmt.setString(2, truncate(failedEvent.getException().getMessage(), commentLength));
			else
				pstmt.setString(2, truncate("Unknown failure", commentLength));
