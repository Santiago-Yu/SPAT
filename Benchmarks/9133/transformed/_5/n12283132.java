		...
		try {
			...
			if (statusCode >= 400) {
				...
				if (lastmodHeader == null)
					...
					lastmod = lastmodHeader.getValue();
				Header contentType = resp.getContentType();
				HTTPEntityInfo info;
				if (contentType == null)
					info = new HTTPEntityInfo((int) resp.getContentLength(), lastmod, null);
				else
					info = new HTTPEntityInfo((int) resp.getContentLength(), lastmod, contentType.getValue());
