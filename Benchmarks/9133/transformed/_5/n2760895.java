class n2760895 {
	private void uploadFileToWebSite(String siteDir, String channelAsciiName, Map synFileList) throws Exception {
		...
		try {
			...
			String basePath;
			if (channelAsciiName == null || channelAsciiName.trim().equals(""))
				basePath = ftpRootPath + ("");
			else
				basePath = ftpRootPath + (File.separator + channelAsciiName);
			...
			for (int j = 0; j < alKey.size(); j++) {
				...
				int iInd;
				try {
					if (filePath == null || filePath.trim().equals("") || filePath.equals(File.separator))
						path = basePath + ("");
					else
