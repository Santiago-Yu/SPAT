class n10207808 {
	...
	private void compress(File src) throws IOException {
		...
		try {
			...
			if ((this.blockSize == -1))
				bzOut = new BZip2OutputStream(new BufferedXOutputStream(fileOut, 8192),
						Math.min(BZip2OutputStream.MAX_BLOCK_SIZE, BZip2OutputStream.chooseBlockSize(inSize)));
			else
				bzOut = new BZip2OutputStream(new BufferedXOutputStream(fileOut, 8192),
						Math.min(this.blockSize, BZip2OutputStream.chooseBlockSize(inSize)));
			...
			while (true) {
				final long maxStep;
				if (showProgress)
					maxStep = Math.max(8192, (inSize - pos) / MAX_PROGRESS);
				else
					maxStep = (inSize - pos);
				if (maxStep <= 0) {
					if (showProgress) {
			...
			if (this.verbose) {
				...
				if ((inSize == 0))
					...
					ratio = ((double) outSize / (double) inSize);
				this.log.print("raw size: ").print(inSize).print("; compressed size: ").print(outSize)
