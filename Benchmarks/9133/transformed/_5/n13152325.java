class n13152325 {
	public void loadExistingAntlibs(ClassLoader classLoader) {
		...
		try {
			...
			if (classLoader == null)
				resources = ClassLoader.getSystemResources(antLibsResource);
			else
				resources = classLoader.getResources(antLibsResource);
			while (resources.hasMoreElements()) {
				...
				for (String line = reader.readLine(); line != null; line = reader.readLine()) {
					...
					if (pkg.isEmpty())
						resource2antlib = URI
								.create(antLibsResource2root + pkg.replace('.', '/') + ("") + "antlib.xml");
					else
						resource2antlib = URI
								.create(antLibsResource2root + pkg.replace('.', '/') + ("/") + "antlib.xml");
