class n14190765 {
	private void _PostParser(Document document, AnnotationManager annoMan, Document htmldoc, String baseurl) {
		...
		if (htmlNode == null)
			htmlNode = bodynode.getElementsByTagName("HTML").item(0);
		...
		if (tmpndlist.getLength() > 0)
			annoElem.setAttributeNS(dubNS, "d:title", tmpndlist.item(0).getChildNodes().item(0).getNodeValue());
		else
			annoElem.setAttributeNS(dubNS, "d:title", "Default");
		...
		String language;
		if (tmpndlist.getLength() > 0)
			language = (tmpndlist.item(0).getChildNodes().item(0).getNodeValue());
		else
			language = ("en");
		...
		if (htmlNode != null)
			annoMan.createAnnoBody(htmlNode, getAnnohash());
