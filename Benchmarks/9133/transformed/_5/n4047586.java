class n4047586 {
	public static void toValueSAX(Property property, Value value, int valueType, ContentHandler contentHandler,
			AttributesImpl na, Context context) throws SAXException, RepositoryException {
		na.clear();
		String _value = null;
		switch (valueType) {
		...
		case PropertyType.BINARY:
			...
			if (NAMESPACE.length() > 0)
				na.addAttribute("", ATTACHMENT, (NAMESPACE + ":") + ATTACHMENT, "string", outResourceName);
			else
				na.addAttribute("", ATTACHMENT, ("") + ATTACHMENT, "string", outResourceName);
			break;
		...
		if (NAMESPACE.length() > 0)
			contentHandler.startElement("", VALUE, (NAMESPACE + ":") + VALUE, na);
		else
			contentHandler.startElement("", VALUE, ("") + VALUE, na);
		if (null != _value)
			contentHandler.characters(_value.toCharArray(), 0, _value.length());
		if (NAMESPACE.length() > 0)
			contentHandler.endElement("", VALUE, (NAMESPACE + ":") + VALUE);
		else
