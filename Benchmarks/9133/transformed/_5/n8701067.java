class n8701067 {
	public void markAsCachedHelper(Item item, Date from, Date to, Map<String, Boolean> properties) {
		...
		for (Map.Entry<String, Boolean> e : properties.entrySet()) {
			if (e.getValue())
				periods.put(representer.lookUpInternalRep(e.getKey()), new long[] { from.getTime(), to.getTime(), 1 });
			else
				periods.put(representer.lookUpInternalRep(e.getKey()), new long[] { from.getTime(), to.getTime(), 0 });
		}
		try {
			conn = getConnection();
			...
			try {
				...
				stmt = conn.prepareStatement("DELETE FROM cachedperiods WHERE " + "id = ? AND host = ? AND "
						+ "starttime <= ? AND endtime >= ? AND " + "prop IN (" + StringUtils.join(props.iterator(), ",")
						+ ")");
				stmt.setString(1, item.getResolved().getId());
				stmt.setInt(2, hostIndex);
			...
			Logger.getLogger(MetaDataStoragerImpl.class.getName()).log(Level.SEVERE,
					"Cannot update cachedperiods table.", ex);
			try {
				conn.rollback();
			} catch (SQLException ex1) {
				Logger.getLogger(MetaDataStoragerImpl.class.getName()).log(Level.SEVERE,
						"Could not roll back database, please consult system administrator.", ex1);
			}
		} finally {
