	...
	private void anneal(final float maxGamma, final float gammaAccel, final float objectiveTolerance,
		...
		for (int searchCount = 1;; searchCount++) {
			...
			for (int i = 0; i < n; i++) {
				...
				if (bestIsInit)
					bestInitPoints[i] = new GradientPoint(evaluation, bestInitPoints[i], output);
				else
					bestInitPoints[i] = new GradientPoint(evaluation, bestInitPoints[i], null);
				if (bestIsPrev)
