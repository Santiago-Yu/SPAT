	...
	public List<Mosque> getAllMosquaisFromDataBase() {
		...
		try {
			...
			for (int i = 0; i < jArray.length(); i++) {
				...
				Mosque mosquai;
				if (json_data.getInt(Param.HAVE_PICTURE) == 1)
					mosquai = new Mosque(json_data.getInt(Param.ID), json_data.getString(Param.NOM),
							json_data.getDouble(Param.LATITUDE), json_data.getDouble(Param.LONGITUDE),
							json_data.getString(Param.INFO), true, json_data.getString(Param.PICTURE));
				else
					mosquai = new Mosque(json_data.getInt(Param.ID), json_data.getString(Param.NOM),
							json_data.getDouble(Param.LATITUDE), json_data.getDouble(Param.LONGITUDE),
							json_data.getString(Param.INFO), false, json_data.getString(Param.PICTURE));
