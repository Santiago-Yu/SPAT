		...
		try {
			...
			if (isSequenceTableRequired()) {
				...
				if (LOGGER.isLoggable(Level.INFO)) {
					...
					if (exception != null)
						logMsg = MessageFormat.format(logMsg, getDialect().getSeqTableModel().getTableName(), "is not",
								getId());
					else
						logMsg = MessageFormat.format(logMsg, getDialect().getSeqTableModel().getTableName(), "is",
								getId());
			...
			case ON_ANY_CHANGE:
				if (anyChange)
					...
					cTables = (List) Collections.emptyList();
				break;
			case NEVER:
