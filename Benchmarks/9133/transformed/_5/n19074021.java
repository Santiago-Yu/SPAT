class n19074021 {
	public void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if ((request.getQueryString() == null))
			...
			myFile = PublicEncryptionFactory.digestString(parsedQueryString).replace('\\', '_').replace('/', '_');
		...
		if (!file.exists() || (request.getParameter("nocache") != null)) {
			...
			try {
				...
				if (1 < lines.size()) {
					...
					int boundHeigh;
					if (!useTextLayout)
						boundHeigh = (int) Math.ceil((fontBounds.getHeight()));
					else
						boundHeigh = (int) Math.ceil((textLayoutBounds.getHeight()));
					...
					if (!useTextLayout)
						height = (int) Math.ceil((fontBounds.getHeight()));
					else
						height = (int) Math.ceil((textLayoutBounds.getHeight()));
				}
				...
				if (1 < lines.size()) {
					for (int numLine = 0; numLine < lines.size(); ++numLine) {
						int y;
						if (!useTextLayout)
							y = (int) Math.ceil((fontBounds.getY()));
						else
							y = (int) Math.ceil((textLayoutBounds.getY()));
