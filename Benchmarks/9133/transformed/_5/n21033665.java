class n21033665 {
	private boolean addBookmark0(Bookmark bookmark, BookmarkFolder folder, PreparedStatement preparedStatement)
		...
		if (time == null)
			bindVariables[1] = new Timestamp(System.currentTimeMillis());
		else
			bindVariables[1] = new Timestamp(time.getTime());
		time = bookmark.getLastAccess();
		if (time == null)
			bindVariables[2] = new Timestamp(System.currentTimeMillis());
		else
			bindVariables[2] = new Timestamp(time.getTime());
		bindVariables[3] = bookmark.getName();
		bindVariables[4] = bookmark.getCommandText();
		bindVariables[5] = new Long(bookmark.getUseCount());
		if (folder == null)
			bindVariables[6] = bookmark.getPath();
		else
			bindVariables[6] = folder.getPath();
		ColorLabel colorLabel = bookmark.getColorLabel();
		if (colorLabel == null)
			bindVariables[7] = null;
		else
			bindVariables[7] = colorLabel.name();
