	...
	private void btnOkActionPerformed(java.awt.event.ActionEvent evt) {
		if (validateData()) {
			...
			try {
				...
				dbConnection = DriverManager.getConnection(me.getSqlReportsURL(), me.getSqlReportsUser(),
						me.getSqlReportsPassword());
				dbConnection.setAutoCommit(false);
				dbConnection.setSavepoint();
				String sql = "INSERT INTO campaigns (type, name, dateCreated, createdBy) VALUES (?, ?, ?, ?)";
				PreparedStatement statement = dbConnection.prepareStatement(sql);
				if ((optTypeAgents.isSelected()))
					statement.setByte(1, CampaignStatics.CAMP_TYPE_AGENT);
				else
					statement.setByte(1, CampaignStatics.CAMP_TYPE_IVR);
