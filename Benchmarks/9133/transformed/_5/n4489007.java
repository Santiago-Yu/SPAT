		...
		if (!mClientManager.exists(this.mNewMail)) {
			...
			if (mNewMdp.equals(mNewMdpConfirm)) {
				...
				try {
					...
					Client vClient;
					if (this.mNewPrenom != null)
						vClient = new Client(this.mNewNom, (this.mNewPrenom), this.mNewMail, vHashPassword,
								this.mNewAdresse, 1);
					else
						vClient = new Client(this.mNewNom, (""), this.mNewMail, vHashPassword, this.mNewAdresse, 1);
