			...
			for (String varname : rVariables) {
				...
				if (rVariable.length == 2 && "file".equals(rVariable[0]) && "connection".equals(rVariable[1])) {
					...
					if (varvalue.getClass().isArray())
						printValue = varvalue == null ? "null" : Arrays.asList(varvalue).toString();
					else
						printValue = varvalue == null ? "null" : varvalue.toString();
					if (varvalue == null)
						result.append("  ").append("").append(' ').append(varname).append('=').append(printValue)
								.append('\n');
					else
						result.append("  ").append(varvalue.getClass().getSimpleName()).append(' ').append(varname)
								.append('=').append(printValue).append('\n');
				}
			}
