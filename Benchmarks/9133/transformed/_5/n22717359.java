			...
			while (it.hasNext()) {
				...
				if (mod.getOp() == LDAPModification.REPLACE) {
					...
					if (attributeName.equals(db2ldap.get("first")) || attributeName.equals(db2ldap.get("last"))) {
						...
						if (attributeName.equals(db2ldap.get("first")))
							ps = con.prepareStatement("UPDATE USERS SET " + ("first") + "=? WHERE username=?");
						else
							ps = con.prepareStatement("UPDATE USERS SET " + ("last") + "=? WHERE username=?");
