class n14819747 {
	public static void main(final String args[]) {
		...
		File prologFile;
		if (debug)
			prologFile = new File(args[1]);
		else
			prologFile = new File(args[0]);
		...
		try {
			...
			if (debug)
				classLoader = createClassLoaderFromClassPath(args[2]);
			else
				classLoader = createClassLoaderFromClassPath(args[1]);
