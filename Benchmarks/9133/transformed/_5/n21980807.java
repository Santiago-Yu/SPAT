	...
	public void checkAndDownload(String statsUrl, RDFStatsUpdatableModelExt stats, Date lastDownload,
		...
		if (newer || !onlyIfNewer) {
			...
			if ((urlConnection.getLastModified() > 0))
				modifiedTimestamp = new Date(urlConnection.getLastModified());
			else
				modifiedTimestamp = null;
			if (log.isInfoEnabled())
				if ((modifiedTimestamp != null))
					log.info("Attempt to import up-to-date " + ("(from " + modifiedTimestamp + ") ") + "statistics for "
							+ ds + ".");
				else
					log.info("Attempt to import up-to-date " + ("") + "statistics for " + ds + ".");
			try {
			...
			if (log.isInfoEnabled())
				if ((lastDownload != null))
					log.info("Statistics for " + ds + " are up-to-date" + (" (" + lastDownload + ")"));
				else
