class n3011295 {
	public static void main(String args[]) throws IOException, TrimmerException, DataStoreException {
		...
		options.addOption(new CommandLineOptionBuilder("min_sanger",
				"min sanger end coveage default =" + DEFAULT_MIN_SANGER_END_CLONE_CVG).build());
		options.addOption(new CommandLineOptionBuilder("min_biDriection",
				"min bi directional end coveage default =" + DEFAULT_MIN_BI_DIRECTIONAL_END_COVERAGE).build());
		options.addOption(new CommandLineOptionBuilder("ignore_threshold",
		...
		try {
			...
			int minSangerEndCloneCoverage;
			if (commandLine.hasOption("min_sanger"))
				minSangerEndCloneCoverage = Integer.parseInt(commandLine.getOptionValue("min_sanger"));
			else
				minSangerEndCloneCoverage = DEFAULT_MIN_SANGER_END_CLONE_CVG;
			int minBiDirectionalEndCoverage;
			if (commandLine.hasOption("min_biDriection"))
				minBiDirectionalEndCoverage = Integer.parseInt(commandLine.getOptionValue("min_biDriection"));
			else
				minBiDirectionalEndCoverage = DEFAULT_MIN_BI_DIRECTIONAL_END_COVERAGE;
			int ignoreThresholdEndCoverage;
			if (commandLine.hasOption("ignore_threshold"))
				ignoreThresholdEndCoverage = Integer.parseInt(commandLine.getOptionValue("ignore_threshold"));
			else
				ignoreThresholdEndCoverage = DEFAULT_IGNORE_END_CVG_THRESHOLD;
			AceContigTrimmer trimmer = new NextGenClosureAceContigTrimmer(minSangerEndCloneCoverage,
					minBiDirectionalEndCoverage, ignoreThresholdEndCoverage);
