class n7300267 {
	public void appendMessage(MimeMessage oMsg) throws FolderClosedException, StoreClosedException, MessagingException {
		...
		if (oMsg.getClass().getName().equals("com.knowgate.hipermail.DBMimeMessage")) {
			...
			if (((DBMimeMessage) oMsg).getFolder() == null)
				((DBMimeMessage) oMsg).setFolder(this);
		} else {
		...
		if (DebugFile.trace)
			DebugFile.writeln("MimeMessage.getSize() = " + String.valueOf(iSize));
		String sContentType, sContentID, sMessageID, sDisposition, sContentMD5, sDescription, sFileName, sEncoding,
				sSubject, sPriority, sMsgCharSeq;
		long lPosition = -1;
		try {
			...
			if (sMessageID != null)
				sMessageID = MimeUtility.decodeText(sMessageID);
			sContentType = oMsg.getContentType();
			if (sContentType != null)
				sContentType = MimeUtility.decodeText(sContentType);
			sContentID = oMsg.getContentID();
			if (sContentID != null)
				sContentID = MimeUtility.decodeText(sContentID);
			sDisposition = oMsg.getDisposition();
			if (sDisposition != null)
				sDisposition = MimeUtility.decodeText(sDisposition);
			sContentMD5 = oMsg.getContentMD5();
			if (sContentMD5 != null)
				sContentMD5 = MimeUtility.decodeText(sContentMD5);
			sDescription = oMsg.getDescription();
			if (sDescription != null)
				sDescription = MimeUtility.decodeText(sDescription);
			sFileName = oMsg.getFileName();
			if (sFileName != null)
				sFileName = MimeUtility.decodeText(sFileName);
			sEncoding = oMsg.getEncoding();
			if (sEncoding != null)
				sEncoding = MimeUtility.decodeText(sEncoding);
			sSubject = oMsg.getSubject();
			if (sSubject != null)
				sSubject = MimeUtility.decodeText(sSubject);
			sPriority = null;
			sMsgCharSeq = null;
		...
		if (DebugFile.trace)
			if (sBoundary == null)
				DebugFile.writeln("part boundary is \"" + ("null") + "\"");
			else
				DebugFile.writeln("part boundary is \"" + (sBoundary) + "\"");
		if (sMessageID == null)
			sMessageID = gu_mimemsg;
		else if (sMessageID.length() == 0)
			sMessageID = gu_mimemsg;
		Timestamp tsSent;
		...
		if (oMsg.getReceivedDate() != null)
			...
			tsReceived = new Timestamp(new java.util.Date().getTime());
		try {
			...
			if (DebugFile.trace)
				DebugFile.writeln("MessagingException " + msge.getMessage());
		}
		boolean bIsSpam = false;
		try {
			String sXSpam = oMsg.getHeader("X-Spam-Flag", null);
			if (sXSpam != null)
				bIsSpam = (sXSpam.toUpperCase().indexOf("YES") >= 0 || sXSpam.toUpperCase().indexOf("TRUE") >= 0
						|| sXSpam.indexOf("1") >= 0);
		} catch (MessagingException msge) {
			if (DebugFile.trace)
				DebugFile.writeln("MessagingException " + msge.getMessage());
		...
		try {
			...
			if (DebugFile.trace)
				DebugFile.writeln("From AddressException " + adre.getMessage());
		}
		...
		try {
			...
			if (DebugFile.trace)
				DebugFile.writeln("Reply-To AddressException " + adre.getMessage());
		}
		if (aReply != null) {
			if (aReply.length > 0)
		...
		try {
			...
			if (DebugFile.trace)
				DebugFile.writeln("Recipient AddressException " + adre.getMessage());
		}
		...
		if ((iOpenMode & MODE_MBOX) != 0) {
			try {
				...
				if (DebugFile.trace)
					DebugFile.writeln("message position is " + String.valueOf(lPosition));
				oMBox = new MboxFile(oFile, MboxFile.READ_WRITE);
				if (DebugFile.trace)
					if (iSize > 0)
						DebugFile.writeln("new ByteArrayOutputStream(" + String.valueOf(iSize) + ")");
					else
						DebugFile.writeln("new ByteArrayOutputStream(" + String.valueOf(16000) + ")");
				if (iSize > 0)
					byOutStrm = new ByteArrayOutputStream(iSize);
				else
					byOutStrm = new ByteArrayOutputStream(16000);
		...
		try {
			...
			if (DebugFile.trace)
				DebugFile.writeln(
						"ByteArrayOutputStream byOutStrm = new ByteArrayOutputStream(" + oText.getSize() + ")");
			if (oText.getSize() > 0)
				byOutStrm = new ByteArrayOutputStream(oText.getSize());
			else
				byOutStrm = new ByteArrayOutputStream(8192);
		...
		String sSQL = "INSERT INTO " + DB.k_mime_msgs
		...
		try {
			...
			if (oConn.getDataBaseProduct() == JDCConnection.DBMS_ORACLE) {
				...
				if (oFlgs.contains(Flags.Flag.ANSWERED))
					oStmt.setBigDecimal(21, new BigDecimal("1"));
				else
					oStmt.setBigDecimal(21, new BigDecimal("0"));
				if (oFlgs.contains(Flags.Flag.DELETED))
					oStmt.setBigDecimal(22, new BigDecimal("1"));
				else
					oStmt.setBigDecimal(22, new BigDecimal("0"));
				oStmt.setBigDecimal(23, new BigDecimal(0));
				if (oFlgs.contains(Flags.Flag.FLAGGED))
					oStmt.setBigDecimal(24, new BigDecimal("1"));
				else
					oStmt.setBigDecimal(24, new BigDecimal("0"));
				if (oFlgs.contains(Flags.Flag.RECENT))
					oStmt.setBigDecimal(25, new BigDecimal("1"));
				else
					oStmt.setBigDecimal(25, new BigDecimal("0"));
				if (oFlgs.contains(Flags.Flag.SEEN))
					oStmt.setBigDecimal(26, new BigDecimal("1"));
				else
					oStmt.setBigDecimal(26, new BigDecimal("0"));
				if (bIsSpam)
					oStmt.setBigDecimal(27, new BigDecimal("1"));
				else
					oStmt.setBigDecimal(27, new BigDecimal("0"));
				oStmt.setBigDecimal(28, dPgMessage);
				if ((iOpenMode & MODE_MBOX) != 0)
					oStmt.setBigDecimal(29, new BigDecimal(lPosition));
				else
					oStmt.setNull(29, Types.NUMERIC);
				if (DebugFile.trace)
					DebugFile.writeln("PreparedStatement.setBinaryStream(30, new ByteArrayInputStream("
							+ String.valueOf(byOutStrm.size()) + "))");
				if (byOutStrm.size() > 0)
				...
				if (oFlgs.contains(Flags.Flag.ANSWERED))
					oStmt.setShort(21, (short) (1));
				else
					oStmt.setShort(21, (short) (0));
				if (oFlgs.contains(Flags.Flag.DELETED))
					oStmt.setShort(22, (short) (1));
				else
					oStmt.setShort(22, (short) (0));
				oStmt.setShort(23, (short) (0));
				if (oFlgs.contains(Flags.Flag.FLAGGED))
					oStmt.setShort(24, (short) (1));
				else
					oStmt.setShort(24, (short) (0));
				if (oFlgs.contains(Flags.Flag.RECENT))
					oStmt.setShort(25, (short) (1));
				else
					oStmt.setShort(25, (short) (0));
				if (oFlgs.contains(Flags.Flag.SEEN))
					oStmt.setShort(26, (short) (1));
				else
					oStmt.setShort(26, (short) (0));
				if (bIsSpam)
					oStmt.setShort(27, (short) (1));
				else
					oStmt.setShort(27, (short) (0));
				oStmt.setBigDecimal(28, dPgMessage);
				if ((iOpenMode & MODE_MBOX) != 0)
					oStmt.setBigDecimal(29, new BigDecimal(lPosition));
				else
		...
		sSQL = "SELECT " + DB.gu_contact + "," + DB.gu_company + "," + DB.tx_name + "," + DB.tx_surname + ","
		...
		try {
			...
			if (oFrom != null) {
				...
				if (oRSet.next()) {
					...
					if (sGuCompany.equals("****************************USER")) {
						...
						if (oRSet.wasNull())
							sTxSurname2 = "";
						sTxPersonal = Gadgets.left(sTxName + " " + sTxSurname1 + " " + sTxSurname2, 254)
								.replace(',', ' ').trim();
					} else
						sTxPersonal = "null";
			...
			if (DebugFile.trace)
				DebugFile.writeln("from count = " + pFrom.size());
			if (oTo != null) {
			...
			if (DebugFile.trace)
				DebugFile.writeln("to count = " + pTo.size());
			if (oCC != null) {
			...
			if (DebugFile.trace)
				DebugFile.writeln("cc count = " + pCC.size());
			if (oBCC != null) {
			...
			oStmt.close();
			oStmt = null;
			oStmt = oConn.prepareStatement("UPDATE " + DB.k_categories + " SET " + DB.len_size + "=" + DB.len_size + "+"
					+ String.valueOf(iSize) + " WHERE " + DB.gu_category + "=?");
			...
			if ((iOpenMode & MODE_MBOX) != 0) {
				if (DebugFile.trace)
					if (oMsg.getContentID() != null)
						DebugFile.writeln("MboxFile.appendMessage(" + (oMsg.getContentID()) + ")");
					else
