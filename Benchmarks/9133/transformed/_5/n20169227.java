	...
	public String addUser(UserInfoItem user) throws DatabaseException {
		...
		try {
			...
			if (modified == 1) {
				...
				if (user.getId() != null)
					LOGGER.debug("DB has been updated. Queries: \"" + seqSt + "\" and \"" + (updSt) + "\"");
				else
				...
				if (user.getId() != null)
					LOGGER.debug(
							"DB has not been updated. -> rollback! Queries: \"" + seqSt + "\" and \"" + (updSt) + "\"");
				else
					LOGGER.debug(
							"DB has not been updated. -> rollback! Queries: \"" + seqSt + "\" and \"" + (insSt) + "\"");
				retID = "error";
			}
		} catch (SQLException e) {
			LOGGER.error(e);
