class n19436818 {
	private void collectImageFile(@NotNull final Progress progress, @NotNull final File collectedDirectory)
			throws IOException {
		final File file = new File(collectedDirectory,
				...
				for (final FaceObject faceObject : faceObjects) {
					...
					try {
						...
						try {
							...
							byteBuffer.clear();
							if (faceObjects.isIncludeFaceNumbers())
								byteBuffer
										.put(("IMAGE " + (i + " ") + imageSize + " " + face + "\n").getBytes(charset));
							else
								byteBuffer.put(("IMAGE " + ("") + imageSize + " " + face + "\n").getBytes(charset));
							byteBuffer.flip();
						...
						ACTION_BUILDER.showMessageDialog(progress.getParentComponent(), "archCollectErrorFileNotFound",
								path);
						return;
					} catch (final IOException e) {
						ACTION_BUILDER.showMessageDialog(progress.getParentComponent(), "archCollectErrorIOException",
								path, e);
						return;
					}
					if (i++ % 100 == 0) {
