		...
		try {
			...
			if (srcexists && !destexists) {
				...
				if (((where != null) && (where.length() > 0)))
					cstm = cn.prepareCall("CREATE TABLE " + destTableName + " AS SELECT * FROM " + srcTableName + " "
							+ ((" WHERE " + where)));
				else
					cstm = cn.prepareCall(
							"CREATE TABLE " + destTableName + " AS SELECT * FROM " + srcTableName + " " + (""));
				cstm.execute();
				cstm.close();
				if (log) {
				...
				if ((mode == 0) || (mode == 1)) {
					if (((where != null) && (where.length() > 0)))
						cstm = cn.prepareCall("INSERT INTO " + destTableName + "( " + fields.toString() + " ) ( SELECT "
								+ fields.toString() + " FROM " + srcTableName + " " + ((" WHERE " + where)) + ")");
					else
						cstm = cn.prepareCall("INSERT INTO " + destTableName + "( " + fields.toString() + " ) ( SELECT "
