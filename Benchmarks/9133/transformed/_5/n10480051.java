	...
	public static RecordResponse loadRecord(RecordRequest recordRequest) {
		...
		try {
			...
			if (recordRequest.isContact())
				httpPost = new HttpPost(URL_RECORD_CONTACT);
			else
				httpPost = new HttpPost(URL_RECORD_COMPANY);
			...
			String Name__Last__First_;
			if (recordRequest.isContact())
				Name__Last__First_ = XMLfunctions.getValue(e, "Name__Last__First_");
			else
				Name__Last__First_ = XMLfunctions.getValue(e, "Name");
			String phone = "";
			if (!recordRequest.isContact())
				phone = XMLfunctions.getValue(e, "Phone");
			String Email1;
			if (recordRequest.isContact())
				Email1 = XMLfunctions.getValue(e, "Personal_Email");
			else
				Email1 = XMLfunctions.getValue(e, "Email");
			String Home_Fax;
			if (recordRequest.isContact())
				Home_Fax = XMLfunctions.getValue(e, "Home_Fax");
			else
				Home_Fax = XMLfunctions.getValue(e, "Fax1");
