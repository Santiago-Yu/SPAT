	...
	public void initialize(IProgressMonitor monitor) throws JETException {
		...
		try {
			...
			if (getTemplateURIPath() == null)
				jetCompiler = new MyBaseJETCompiler(getTemplateURI(), getEncoding(), getClassLoader());
			else
				jetCompiler = new MyBaseJETCompiler(getTemplateURIPath(), getTemplateURI(), getEncoding(),
						getClassLoader());
			...
			if (getTemplateURIPath() != null) {
						...
						for (IClasspathEntry classpathEntry : javaProject.getResolvedClasspath(true)) {
							if (classpathEntry.getEntryKind() == IClasspathEntry.CPE_PROJECT) {
								...
								urls.add(new File(otherProject.getLocation() + "/"
										+ otherJavaProject.getOutputLocation().removeFirstSegments(1) + "/").toURI()
												.toURL());
							}
						}
				...
				if (theClassLoader != null) {
					...
					if (packageName.length() == 0)
						className = ("") + jetCompiler.getSkeleton().getClassName();
					else
						className = (packageName + ".") + jetCompiler.getSkeleton().getClassName();
			...
			if (sourcePackageFragmentRoot == null)
				...
				sourceContainer = (IContainer) sourcePackageFragmentRoot.getCorrespondingResource();
			...
			if (!errors) {
				...
				URLClassLoader theClassLoader = AccessController
						.doPrivileged(new GetURLClassLoaderSuperBundlesAction(bundles, urls));
				Class<?> theClass;
				if (packageName.length() == 0)
					theClass = theClassLoader.loadClass(("") + jetCompiler.getSkeleton().getClassName());
				else
					theClass = theClassLoader.loadClass((packageName + ".") + jetCompiler.getSkeleton().getClassName());
				String methodName = jetCompiler.getSkeleton().getMethodName();
				Method[] methods = theClass.getDeclaredMethods();
				for (int i = 0; i < methods.length; ++i) {
					if (methods[i].getName().equals(methodName)) {
