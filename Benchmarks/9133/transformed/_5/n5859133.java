class n5859133 {
	protected int sendData(String submitName, String submitValue) throws HttpException, IOException, SAXException {
		...
		try {
			...
			postMethod.addRequestHeader("Cookie", getWikiPrefix() + "_session=" + getSession() + "; " + getWikiPrefix()
			...
			if (Digest.MD5.isImplemented())
				parts.add(new StringPart("wpAutoSummary", Digest.MD5.encrypt(getSummary())));
			else
				parts.add(new StringPart("wpAutoSummary", ""));
