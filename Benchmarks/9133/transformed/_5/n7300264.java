		...
		try {
			...
			if (iParts > 0) {
				...
				for (int p = 0; p < iParts; p++) {
					...
					if (oPart.getSize() > 0)
						byOutPart = new ByteArrayOutputStream(oPart.getSize());
					else
						byOutPart = new ByteArrayOutputStream(131072);
					oPart.writeTo(byOutPart);
					if (sMsgCharSeq != null && sBoundary != null && iPrevPart > 0) {
						iThisPart = sMsgCharSeq.indexOf(sBoundary, iPrevPart);
						if (iThisPart > 0) {
							if (DebugFile.trace)
								DebugFile.writeln("found part " + String.valueOf(p + iOffset) + " boundary at "
										+ String.valueOf(iThisPart));
							iPartStart = iThisPart + sBoundary.length();
							while (iPartStart < sMsgCharSeq.length()) {
								if (sMsgCharSeq.charAt(iPartStart) != ' ' && sMsgCharSeq.charAt(iPartStart) != '\r'
					...
					if (!bForwardedAttachment) {
						...
						if ((iPartStart > 0) && ((iOpenMode & MODE_MBOX) != 0))
							...
							oStmt.setNull(3, Types.NUMERIC);
						else
							oStmt.setNull(3, Types.DECIMAL);
						...
						else if (oPart.getSize() > 0)
							oStmt.setInt(8, oPart.getSize());
						else
							oStmt.setInt(8, byOutPart.size());
