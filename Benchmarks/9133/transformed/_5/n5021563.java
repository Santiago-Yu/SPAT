			...
			if (objecttype.equalsIgnoreCase("F")) {
				...
				if (("20".indexOf(modo) != -1) && !fldexi) {
					...
					while (rsltrelc.next()) {
						...
						if (consttype.equals("R"))
							pstmdic.setString(2, "FK");
						else
							pstmdic.setString(2, "PK");
			...
			if (objecttype.startsWith("IDX")) {
				...
				if (rslt.next()) {
					...
					while (rslt.next()) {
						if ((oldexpression.length() > 0))
							oldexpression += ((",") + rslt.getString(1));
						else
							oldexpression += (("") + rslt.getString(1));
					}
					rslt.close();
				...
				for (byte i = 0; i < nexo.length; i++) {
					if ((newexpression.length() > 0))
						newexpression += ((",") + ((nexo[i]).toUpperCase().trim()));
					else
						newexpression += (("") + ((nexo[i]).toUpperCase().trim()));
				}
				if (!drop) {
