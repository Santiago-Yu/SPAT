class n18822890 {
	public void actualizar() throws SQLException, ClassNotFoundException, Exception {
		...
		try {
			...
			ms.setInt(1, fzafdoc);
			if ((altres.length() > 30))
				ms.setString(2, altres.substring(0, 30));
			else
				ms.setString(2, altres);
			if ((fzacone.length() > 160))
				ms.setString(3, fzacone.substring(0, 160));
			else
				ms.setString(3, fzacone);
			if ((tipo.length() > 2))
				ms.setString(4, tipo.substring(0, 1));
			else
				ms.setString(4, tipo);
			ms.setString(5, "N");
			if ((registroAnulado.length() > 1))
				ms.setString(6, registroAnulado.substring(0, 1));
			else
				ms.setString(6, registroAnulado);
			if ((fzaproce.length() > 25))
				ms.setString(7, fzaproce.substring(0, 25));
			else
				ms.setString(7, fzaproce);
			ms.setInt(8, fzafent);
			ms.setInt(9, fzactagg);
			ms.setInt(10, fzacagge);
			ms.setInt(11, fzacorg);
			if ((fzacent.length() > 7))
				ms.setString(12, fzacent.substring(0, 8));
			else
				ms.setString(12, fzacent);
			ms.setInt(13, fzanent);
			ms.setInt(14, fzahora);
			ms.setInt(15, fzacidi);
			if ((fzacone2.length() > 160))
				ms.setString(16, fzacone2.substring(0, 160));
			else
				ms.setString(16, fzacone2);
			ms.setInt(17, fzanloc);
			ms.setInt(18, fzaaloc);
			ms.setInt(19, fzandis);
			if ((usuario.toUpperCase().length() > 10))
				ms.setString(20, usuario.toUpperCase().substring(0, 10));
			else
				ms.setString(20, usuario.toUpperCase());
			...
			if (!motivo.equals("")) {
				...
				RegistroModificadoSalidaHome home = (RegistroModificadoSalidaHome) javax.rmi.PortableRemoteObject
						.narrow(ref, RegistroModificadoSalidaHome.class);
			...
			if ((modificado && !motivo.equals("")) || motivo.equals("")) {
				...
				try {
					...
					Object[] params = { "M", new Integer(fzaanoe), new Integer(fzanume), new Integer(fzacagc),
							new Integer(fzafdoc), remitente, comentario, tipo, new Integer(fzafent),
							new Integer(fzacagge), fzaproce, new Integer(fzacorg), idioma };
				...
				if ((usuario.toUpperCase().length() > 10))
					logLopdBZSALIDA("UPDATE", usuario.toUpperCase().substring(0, 10), fzahsis, horamili, fzanume,
							fzaanoe, fzacagc);
				else
					logLopdBZSALIDA("UPDATE", usuario.toUpperCase(), fzahsis, horamili, fzanume, fzaanoe, fzacagc);
				conn.commit();
			} else {
				registroActualizado = false;
			...
			registroActualizado = false;
			errores.put("",
					"Error inesperat, no s'ha modificat el registre" + ": " + ex.getClass() + "->" + ex.getMessage());
			try {
				if (conn != null)
					conn.rollback();
			} catch (SQLException sqle) {
				throw new RemoteException(
						"S'ha produ\357t un error i no s'han pogut tornar enrere els canvis efectuats", sqle);
			}
