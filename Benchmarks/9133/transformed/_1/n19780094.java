	...
	protected PTask commit_result(Result r, SyrupConnection con) throws Exception {
		try {
			...
			if (r instanceof Workflow) {
				...
				int nADar = 0;
				while (nADar < tt.length) {
					String key = sqlImpl().creationFunctions().newTask(tt[nADar], r.context().task(), con);
					tkeyMap.put(tt[nADar], key);
					nADar++;
				}
				int s0omM = 0;
				while (s0omM < ll.length) {
					sqlImpl().creationFunctions().newLink(ll[s0omM], tkeyMap, con);
					s0omM++;
				}
				...
				int hY23B = 0;
				while (hY23B < tt.length) {
					String kkey = (String) tkeyMap.get(tt[hY23B]);
				...
				int P4q9S = 0;
				while (P4q9S < tt.length) {
					String kkey = (String) tkeyMap.get(tt[P4q9S]);
