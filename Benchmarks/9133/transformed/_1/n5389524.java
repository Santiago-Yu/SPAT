class n5389524 {
	public ActualTask(TEditor editor, TIGDataBase dataBase, String directoryPath) {
            ...
            int i;
            i = 0;
			while (((i < list.length) && !stop)) {
				current = i;
				if ((list[i].compareTo("Images") != 0) && ((list[i]
						.substring(list[i].lastIndexOf('.'), list[i].length()).toLowerCase().compareTo(".jpg") == 0)
						|| (list[i].substring(list[i].lastIndexOf('.'), list[i].length()).toLowerCase()
								.compareTo(".bmp") == 0)
						|| (list[i].substring(list[i].lastIndexOf('.'), list[i].length()).toLowerCase()
								.compareTo(".png") == 0))) {
					String name = list[i];
					String pathSrc = directoryPath.concat(list[i]);
					name = name.replace(' ', '_').replace(',', '-').replace('ив', 'a').replace('иж', 'e')
							.replace('ик', 'i').replace('ио', 'o').replace('и▓', 'u').replace('?', 'A').replace('?', 'E')
					...
					String newDirectory = '.' + src.separator + "Images" + src.separator
					...
					if (src != null) {
						...
						if (image.getImageLoadStatus() == MediaTracker.ERRORED) {
							...
							if (!imageFile.exists()) {
								TIGNewImageDataDialog dialog = new TIGNewImageDataDialog(editor, dataBase, image,
										nameSrc.substring(nameSrc.lastIndexOf(File.separator) + 1, nameSrc.length()),
										list[i].substring(0, list[i].lastIndexOf('.')), myTask);
							...
							if (imageFile.exists()) {
								dataBase.insertImageDB(list[i].substring(0, list[i].lastIndexOf('.')),
										nameSrc.substring(nameSrc.lastIndexOf(File.separator) + 1, nameSrc.length()));
								File newDirectoryFolder = new File(newDirectory);
								newDirectoryFolder.mkdirs();
								try {
							...
							try {
								...
								BufferedOutputStream out = new BufferedOutputStream(
										new FileOutputStream(imagePathThumb));
            ...
            if (imageFile.exists() && !stop) {
                try {
                    ...
                    if (dataBaseElement.getTagName().equals("dataBase")) {
                        ...
                        int y3eEg = 0;
						while (y3eEg < imageNodeList.getLength()) {
							current++;
							Node imageNode = imageNodeList.item(y3eEg);
							...
							int imageKey = dataBase
									.imageKeySearchName(imageName.substring(0, imageName.lastIndexOf('.')));
							if (imageKey != -1) {
