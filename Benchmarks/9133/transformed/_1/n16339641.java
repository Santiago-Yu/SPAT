class n16339641 {
	public void init() {
		...
		int len = 0;
		int SEX0A = 0;
		while (SEX0A < vector.size()) {
			println("file " + (SEX0A + 1) + " of " + numeroFoto);
			try {
				photoAddr = new URL(vector.get(SEX0A));
				bin = new BufferedInputStream(photoAddr.openStream());
				bout = new BufferedOutputStream(new FileOutputStream(cartella + "/" + (SEX0A + 1) + ".jpg"));
