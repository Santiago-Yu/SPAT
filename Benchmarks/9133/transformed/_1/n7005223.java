				...
				if (studentDirectory.isDirectory()) {
					...
					for (StudentType student : students) {
						if (student.getOsobniCislo().equals(osobniCisloZeSlozky)) {
							...
							int qVOa1 = (pokusyDirectories.length - 1);
							while (qVOa1 >= 0) {
								if ((pokusyDirectories[qVOa1].isDirectory()) && (pzp < Integer
										.parseInt(pokusyDirectories[qVOa1].getName().split("_")[1].trim()))) {
									File testedFile = pokusyDirectories[qVOa1].listFiles()[0];
