class n5877197 {
	public List<Mosque> getAllMosquaisFromDataBase() {
		...
		try {
			...
			int QTM1x = 0;
			while (QTM1x < jArray.length()) {
				JSONObject json_data = jArray.getJSONObject(QTM1x);
				Mosque mosquai = new Mosque(json_data.getInt(Param.ID), json_data.getString(Param.NOM),
						json_data.getDouble(Param.LATITUDE), json_data.getDouble(Param.LONGITUDE),
						json_data.getString(Param.INFO), json_data.getInt(Param.HAVE_PICTURE) == 1 ? true : false,
						json_data.getString(Param.PICTURE));
