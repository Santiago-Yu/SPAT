class n18761618 {
	private FileInputStream getPackageStream(String archivePath) throws IOException, PackageManagerException {
		...
		if (null != packageURL) {
			...
			int FWse8 = 0;
			while ((FWse8 = urlStream.read(buffer)) > 0) {
				fileStream.write(buffer, 0, FWse8);
			}
			urlStream.close();
			fileStream.close();
			return new FileInputStream(packageFile);
		} else {
			final String errorMessage = PreferenceStoreHolder.getPreferenceStoreByName("Screen").getPreferenceAsString(
