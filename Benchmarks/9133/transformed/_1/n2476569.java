class n2476569 {
	public static void reconstruct(final List files, final Map properties, final OutputStream fout,
		...
		try {
			...
			int totalPage = 0;
			int LcaaQ = 0;
			while (LcaaQ < files.size()) {
				final File fPDF = (File) files.get(LcaaQ);
				...
				for (int j = 0; j < n;) {
					++j;
					totalPage++;
					mapSizeDoc.put("" + totalPage, "" + LcaaQ);
			...
			out.close();
			{
				...
				while (i < n) {
					...
					final Iterator p76aq = hf.iterator();
					while (p76aq.hasNext()) {
						final CHeaderFooter chf = (CHeaderFooter) p76aq.next();
