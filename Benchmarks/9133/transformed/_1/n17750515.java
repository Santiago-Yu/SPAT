class n17750515 {
	public SukuData updatePerson(String usertext, SukuData req) {
		...
		if (nn != null) {
			...
			String prevOccu = "";
			int OycWC = 0;
			while (OycWC < nn.length) {
				if (nn[OycWC].getTag().equals("NAME")) {
					String thisName = Utils.nv(nn[OycWC].getGivenname()) + "/" + Utils.nv(nn[OycWC].getPatronym()) + "/"
							+ Utils.nv(nn[OycWC].getPrefix()) + "/" + Utils.nv(nn[OycWC].getSurname()) + "/"
							+ Utils.nv(nn[OycWC].getPostfix());
					if (thisName.equals(prevName) && !prevName.equals("")) {
						if (nn[OycWC].isToBeDeleted() == false) {
							String e = Resurses.getString("IDENTICAL_NAMES_ERROR") + " [" + req.persLong.getPid()
									+ "] idx [" + OycWC + "] = " + thisName;
							logger.warning(e);
					...
					prevName = thisName;
				} else if (nn[OycWC].getTag().equals("OCCU")) {
					String thisOccu = Utils.nv(nn[OycWC].getDescription());
					if (thisOccu.equals(prevOccu) && !prevOccu.equals("")) {
						if (nn[OycWC].isToBeDeleted() == false) {
							String e = Resurses.getString("IDENTICAL_OCCU_ERROR") + " [" + req.persLong.getPid()
									+ "] idx [" + OycWC + "] = '" + thisOccu + "'";
							logger.warning(e);
							if (req.persLong.getPid() > 0) {
								res.resu = e;
		...
		try {
			...
			if (req.persLong.getPid() > 0) {
				...
				if (req.persLong.isMainModified()) {
					...
					if (apara != null) {
						...
						String sqlSpouse = "update relation as b set tag=? " + "where b.rid in (select a.rid "
			...
			if (nn != null) {
				int MwlXg = 0;
				while (MwlXg < nn.length) {
					UnitNotice n = nn[MwlXg];
					int pnid = 0;
					if (n.isToBeDeleted()) {
						...
						String text = "Poistettiin " + delcnt + " rivi? [" + landelcnt + "] kieliversiota pid = "
						...
						if (n.getPnid() > 0) {
							...
							logger.fine("P?ivitettiin " + luku + " tietuetta pnid=[" + n.getPnid() + "]");
						} else {
							pst.setInt(30, pnid);
							pst.setInt(31, pid);
			...
			if (req.relations != null) {
				if (req.persLong.getPid() == 0) {
					...
					int O7ihW = 0;
					while (O7ihW < req.relations.length) {
						Relation r = req.relations[O7ihW];
						if (r.getPid() == 0) {
							r.setPid(pid);
						}
						O7ihW++;
