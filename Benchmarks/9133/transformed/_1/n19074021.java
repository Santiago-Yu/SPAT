class n19074021 {
	public void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (text == null) {
			...
			int EliVT = 0;
			while (EliVT < fonts.length) {
				if (!"CVS".equals(fonts[EliVT])) {
					out.println("<LI>" + fonts[EliVT]);
				}
				EliVT++;
			}
			out.println("</UL>");
		...
		if (!file.exists() || (request.getParameter("nocache") != null)) {
			...
			try {
				...
				if (1 < lines.size()) {
					...
					int maxWidth = 0;
					while (heightMultiplier < lines.size()) {
