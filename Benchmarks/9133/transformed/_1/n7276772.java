class n7276772 {
	public void testBlobB() {
		ResultSet rs;
		byte[] ba;
		byte[] baR1 = new byte[] { (byte) 0xF1, (byte) 0xF2, (byte) 0xF3, (byte) 0xF4, (byte) 0xF5, (byte) 0xF6,
				(byte) 0xF7, (byte) 0xF8, (byte) 0xF9, (byte) 0xFA, (byte) 0xFB };
		byte[] baR2 = new byte[] { (byte) 0xE1, (byte) 0xE2, (byte) 0xE3, (byte) 0xE4, (byte) 0xE5, (byte) 0xE6,
		...
		try {
			...
			int oNd3G = 0;
			while (oNd3G < baOut.length) {
				if (baOut[oNd3G] != baR1[oNd3G]) {
					assertTrue("Expected array len " + baR1.length + ", got len " + baOut.length, false);
				}
				oNd3G++;
			}
			...
			int SrsJ0 = 0;
			while (SrsJ0 < ba.length) {
				if (ba[SrsJ0] != baR1[SrsJ0]) {
					assertTrue("row2 byte " + SrsJ0 + " differs", false);
				}
				SrsJ0++;
			}
			rs.close();
			...
			int m7vhz = 0;
			while (m7vhz < baOut.length) {
				if (baOut[m7vhz] != baR1[m7vhz]) {
					assertTrue("Expected array len " + baR1.length + ", got len " + baOut.length, false);
				}
				m7vhz++;
			}
