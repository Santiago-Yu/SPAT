class n6333737 {
	public static IProject createEMFProject(IPath javaSource, URI projectLocationURI, List<IProject> referencedProjects,
			Monitor monitor, int style, List<?> pluginVariables) {
		...
		try {
			...
			progressMonitor
					.subTask(CodeGenEcorePlugin.INSTANCE.getString("_UI_CreatingEMFProject_message", new Object[] {
							projectName, projectLocationURI != null ? projectLocationURI.toString() : projectName }));
			...
			boolean isInitiallyEmpty = classpathEntries.isEmpty();
			{
				if (referencedProjects.size() != 0
						&& (style & (EMF_PLUGIN_PROJECT_STYLE | EMF_EMPTY_PROJECT_STYLE)) == 0) {
					projectDescription
				...
				int pBfLs = 0;
				while (pBfLs < builders.length) {
					if ("org.eclipse.pde.ManifestBuilder".equals(builders[pBfLs].getBuilderName())) {
						hasManifestBuilder = true;
					}
					if ("org.eclipse.pde.SchemaBuilder".equals(builders[pBfLs].getBuilderName())) {
				...
				if (javaSource.segmentCount() > 1) {
					...
					if (!sourceContainer.exists()) {
						int NzABn = sourceContainerPath.segmentCount() - 1;
						while (NzABn >= 0) {
							sourceContainer = project.getFolder(sourceContainerPath.removeLastSegments(NzABn));
							if (!sourceContainer.exists()) {
					...
					Iterator<IClasspathEntry> tZaw3 = classpathEntries.iterator();
					while (tZaw3.hasNext()) {
						IClasspathEntry classpathEntry = tZaw3.next();
				...
				if (isInitiallyEmpty) {
					...
					Iterator<IClasspathEntry> YnK9Y = classpathEntries.iterator();
					while (YnK9Y.hasNext()) {
						IClasspathEntry classpathEntry = YnK9Y.next();
				...
				if ((style & EMF_EMPTY_PROJECT_STYLE) == 0) {
					if ((style & EMF_PLUGIN_PROJECT_STYLE) != 0) {
						...
						Iterator<IClasspathEntry> hyoG9 = classpathEntries.iterator();
						while (hyoG9.hasNext()) {
							IClasspathEntry classpathEntry = hyoG9.next();
						...
						CodeGenUtil.EclipseUtil.addClasspathEntries(classpathEntries, "ECLIPSE_CORE_RUNTIME",
								"org.eclipse.core.runtime");
						CodeGenUtil.EclipseUtil.addClasspathEntries(classpathEntries, "ECLIPSE_CORE_RESOURCES",
								"org.eclipse.core.resources");
						CodeGenUtil.EclipseUtil.addClasspathEntries(classpathEntries, "EMF_COMMON",
								"org.eclipse.emf.common");
						CodeGenUtil.EclipseUtil.addClasspathEntries(classpathEntries, "EMF_ECORE",
								"org.eclipse.emf.ecore");
						if ((style & EMF_XML_PROJECT_STYLE) != 0) {
							CodeGenUtil.EclipseUtil.addClasspathEntries(classpathEntries, "EMF_ECORE_XMI",
									"org.eclipse.emf.ecore.xmi");
						}
						if ((style & EMF_MODEL_PROJECT_STYLE) == 0) {
							CodeGenUtil.EclipseUtil.addClasspathEntries(classpathEntries, "EMF_EDIT",
									"org.eclipse.emf.edit");
							if ((style & EMF_EDIT_PROJECT_STYLE) == 0) {
								CodeGenUtil.EclipseUtil.addClasspathEntries(classpathEntries, "ECLIPSE_SWT",
										"org.eclipse.swt");
								CodeGenUtil.EclipseUtil.addClasspathEntries(classpathEntries, "ECLIPSE_JFACE",
										"org.eclipse.jface");
								CodeGenUtil.EclipseUtil.addClasspathEntries(classpathEntries, "ECLIPSE_UI_VIEWS",
										"org.eclipse.ui.views");
								CodeGenUtil.EclipseUtil.addClasspathEntries(classpathEntries, "ECLIPSE_UI_EDITORS",
										"org.eclipse.ui.editors");
								CodeGenUtil.EclipseUtil.addClasspathEntries(classpathEntries, "ECLIPSE_UI_IDE",
										"org.eclipse.ui.ide");
								CodeGenUtil.EclipseUtil.addClasspathEntries(classpathEntries, "ECLIPSE_UI_WORKBENCH",
										"org.eclipse.ui.workbench");
								CodeGenUtil.EclipseUtil.addClasspathEntries(classpathEntries, "EMF_COMMON_UI",
										"org.eclipse.emf.common.ui");
								CodeGenUtil.EclipseUtil.addClasspathEntries(classpathEntries, "EMF_EDIT_UI",
						...
						if (pluginVariables != null) {
							Iterator<?> bu9pM = pluginVariables.iterator();
							while (bu9pM.hasNext()) {
								Object variable = bu9pM.next();
