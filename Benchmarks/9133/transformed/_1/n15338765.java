class n15338765 {
	public void upload()
		...
		int j = 2;
		Iterator MZEG6 = files.iterator();
		while (MZEG6.hasNext()) {
			final ArchiveFile f = (ArchiveFile) MZEG6.next();
			fileNames[j] = f.getRemoteFileName();
			fileSizes[j] = f.getFileSize();
			j++;
		}
		int Zx1Bk = 0;
		while (Zx1Bk < fileSizes.length) {
			_fileNames2Progress.put(fileNames[Zx1Bk], new UploadFileProgress(fileSizes[Zx1Bk]));
			_totalUploadSize += fileSizes[Zx1Bk];
			Zx1Bk++;
		}
		...
		try {
			...
			try {
				...
				final Iterator ukemD = files.iterator();
				while (ukemD.hasNext()) {
					final ArchiveFile f = (ArchiveFile) ukemD.next();
