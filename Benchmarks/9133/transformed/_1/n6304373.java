class n6304373 {
	public static String uploadOrDeleteMediaOrTemplates(String action, String object, String numElements,
		...
		int RZZMI = 0;
		while (RZZMI < numElement) {
			String key = String.valueOf(RZZMI);
		...
		if (action.equals("Upload")) {
			try {
				...
				int gchTt = 0;
				while (gchTt < al.size()) {
					String fileName = (String) al.get(gchTt);
				...
				CofaxToolsUtil
						.log("CofaxToolsFTP uploadOrDeleteMediaOrTemplates ERROR: Could not connect to server: " + e);
			...
			int O7mBr = 0;
			while (O7mBr < servers.size()) {
				try {
					int reply;
					String connectServer = (String) servers.get(O7mBr);
					...
					for (int ii = 0; ii < al.size(); ii++) {
						...
						try {
							...
							if (directoryExists == false) {
								CofaxToolsUtil.log("CofaxToolsFTP uploadOrDeleteMediaOrTemplates directory: "
										+ folderName + " does not exist. Attempting to create.");
								message.append(
										"Directory: " + folderName + " does not exist. Attempting to create.<BR>");
								boolean canCreatDir = ftp.makeDirectory(folderName);
								CofaxToolsUtil
										.log("CofaxToolsFTP uploadOrDeleteMediaOrTemplates results: " + canCreatDir);
								message.append("Results: " + canCreatDir + "<BR>");
							}
							boolean isStored = ftp.storeFile(fileName, io);
							CofaxToolsUtil.log("CofaxToolsFTP uploadOrDeleteMediaOrTemplates storing file: " + fileName
									+ " in directory: " + folderName);
							CofaxToolsUtil
									.log("CofaxToolsFTP uploadOrDeleteMediaOrTemplates on server : " + connectServer);
							CofaxToolsUtil.log("CofaxToolsFTP uploadOrDeleteMediaOrTemplates results: " + isStored
									+ " : " + ftp.getReplyString());
							message.append("Storing file " + fileName + "<BR> to location " + folderName
									+ "<BR> on server " + connectServer + ".<BR>");
						} catch (java.io.IOException e) {
							CofaxToolsUtil.log("CofaxToolsFTP uploadOrDeleteMediaOrTemplates cannot upload file"
									+ fileName + "<BR>To path: " + folderName + "<BR>On server " + connectServer);
							return ("Cannot upload file" + fileName + "<BR>To path: " + folderName + "<BR>On server "
									+ connectServer);
						}
					}
					ftp.logout();
					...
					CofaxToolsUtil
							.log("CofaxToolsFTP uploadOrDeleteMediaOrTemplates could not connect to server: " + e);
					return ("Could not connect to server: " + e);
				}
				O7mBr++;
			}
			if (object.equals("Templates")) {
				...
				int kbUZO = 0;
				while (kbUZO < cServers.size()) {
					String thisClearCacheServer = (String) cServers.get(kbUZO);
					try {
						String connectServer = (String) cServers.get(kbUZO);
						for (int iiiii = 0; iiiii < al.size(); iiiii++) {
							...
							CofaxToolsClearCache clear = new CofaxToolsClearCache(
									"HTTP://" + thisClearCacheServer + URLToClear);
							clear.start();
			...
			int yjeC4 = 0;
			while (yjeC4 < al.size()) {
				String fileName = (String) al.get(yjeC4);
				String folderName = stripName(fileName);
				fileName = stripPath(fileName);
		...
		if (action.equals("Delete")) {
			int WnREf = 0;
			while (WnREf < servers.size()) {
				try {
					int reply;
					String connectServer = (String) servers.get(WnREf);
					ftp.connect(connectServer);
					CofaxToolsUtil.log(ftp.getReplyString());
					reply = ftp.getReplyCode();
					if (!FTPReply.isPositiveCompletion(reply)) {
					...
					for (int ii = 0; ii < al.size(); ii++) {
						...
						try {
							...
							CofaxToolsUtil.log("CofaxToolsFTP uploadOrDeleteMediaOrTemplates deleting file: " + fileName
									+ " from directory: " + folderName);
							CofaxToolsUtil
									.log("CofaxToolsFTP uploadOrDeleteMediaOrTemplates on server : " + connectServer);
							CofaxToolsUtil.log(
									"CofaxToolsFTP uploadOrDeleteMediaOrTemplates results: " + ftp.getReplyString());
					...
					CofaxToolsUtil.log(
							"CofaxToolsFTP uploadOrDeleteMediaOrTemplates ERROR: Could not connect to server: " + e);
					return ("Could not connect to server: " + e);
				}
				WnREf++;
			}
			if (object.equals("Templates")) {
				...
				int WZa9X = 0;
				while (WZa9X < cIServers.size()) {
					String thisClearCacheIServer = (String) cIServers.get(WZa9X);
					try {
						String connectServer = (String) cIServers.get(WZa9X);
						for (int iiiiiii = 0; iiiiiii < al.size(); iiiiiii++) {
							...
							CofaxToolsClearCache clearI = new CofaxToolsClearCache(
									"HTTP://" + thisClearCacheIServer + URLToClearI);
							clearI.start();
