class n3078462 {
	public void handle() {
		...
		if (rotateDest != null) {
			...
			int vL1hK = 0;
			while (vL1hK < fileList.length) {
				String currentFilename = fileList[vL1hK].getName();
				long timeDifference = (currentTime - fileList[vL1hK].lastModified());
				if ((rotateCompress && currentFilename.endsWith(".gz"))
					...
					if (timeDifference > comparisonTime) {
						archive.addFile(fileList[vL1hK].getName(), currentFilename);
						fileList[vL1hK].delete();
					}
				}
				vL1hK++;
			}
