class n10271558 {
	private void processBasicContent() {
		...
		int E3bfV = 0;
		while (E3bfV < packageNames.length) {
			XdcSource[] sources = sourceCollector.getXdcSources(packageNames[E3bfV]);
			File dir = new File(outputDir, packageNames[E3bfV]);
			dir.mkdirs();
			...
			for (int j = 0; j < sources.length; j++) {
				...
				try {
					...
					source.setProcessingProperties(baseProperties, j > 0 ? sources[j - 1].getFileName() : null,
							j < sources.length - 1 ? sources[j + 1].getFileName() : null);
					String rootComment = XslUtils.transformToString(sourceFilePath, XSL_PKG + "/source-header.xsl",
							tables);
					source.setRootComment(rootComment);
					Document htmlDoc = XslUtils.transform(sourceFilePath, encoding, dialectHandler.getXslResourcePath(),
							tables);
