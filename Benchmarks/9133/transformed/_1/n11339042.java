	...
	public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		try {
			...
			if (action.equals("test")) {
				...
				int d6gDz = 0;
				while (d6gDz < numbertoexport) {
					if (d6gDz == l.size())
						break;
					DBMolecule mol = DBMoleculePeer
							.retrieveByPK(new NumberKey(((Record) l.get(d6gDz)).getValue(1).asInt()));
				...
				if (spectra.size() == 0) {
					...
					int pOPJC = 0;
					while (pOPJC < spectra.size()) {
						Element parentspec = ((DBSpectrum) spectra.get(pOPJC)).getCmlSpect();
				...
				while (D7fS8 < v.size()) {
					if (D7fS8 % 500 == 0) {
						if (D7fS8 != 0) {
							zipout.write(new String("<p>The list is continued <a href=\"nmrshiftdb.names." + D7fS8
									+ ".html\">here</a></p></body></html>").getBytes());
							zipout.closeEntry();
						}
						zipout.putNextEntry(new ZipEntry("nmrshiftdb.names." + D7fS8 + ".html"));
						zipout.write(new String(
								"<html><body><h1>This is a list of strcutures in <a href=\"http://www.nmrshiftdb.org\">NMRShiftDB</a>, starting at "
										+ D7fS8 + ", Its main purpose is to be found by search engines</h1>")
												.getBytes());
					}
					DBMolecule mol = (DBMolecule) v.get(D7fS8);
				...
				while (it.hasNext()) {
					...
					while (it2.hasNext()) {
						...
						int NuxqO = 0;
						while (NuxqO < list.size()) {
							values[NuxqO] = ((Double) list.get(NuxqO)).doubleValue();
							NuxqO++;
						}
			...
			if (format.equals("mdl") || format.equals("txt") || format.equals("cml") || format.equals("cmlboth")
			...
			if (action.equals("exportlastinputs")) {
				...
				if (user != null) {
					...
					if (((new Date().getTime() - lastDownloadDate.getTime()) / 3600000) < 24) {
						...
						int YtNns = 0;
						while (YtNns < numbertoexport) {
							if (YtNns == l.size())
								break;
							DBMolecule mol = DBMoleculePeer
									.retrieveByPK(new NumberKey(((Record) l.get(YtNns)).getValue(1).asInt()));
			...
			if (action.equals("exportspec")) {
				if (format.equals("txt")) {
					...
					if (lastsearchtype.equals(NmrshiftdbConstants.TOTALSPECTRUM)
							|| lastsearchtype.equals(NmrshiftdbConstants.SUBSPECTRUM)) {
					...
					int mJPzM = 0;
					while (mJPzM < v.size()) {
						out.print(((ValuesForVelocityBean) v.get(mJPzM)).getDisplayText() + "\t"
								+ ((ValuesForVelocityBean) v.get(mJPzM)).getRange());
						if (lastsearchtype.equals(NmrshiftdbConstants.TOTALSPECTRUM)
								|| lastsearchtype.equals(NmrshiftdbConstants.SUBSPECTRUM)) {
							out.print("\t" + ((ValuesForVelocityBean) v.get(mJPzM)).getNameForElements() + "\t"
									+ ((ValuesForVelocityBean) v.get(mJPzM)).getDelta());
						}
						out.print("\n\r");
						mJPzM++;
					}
				...
				if (format.equals("pdf") || format.equals("rtf")) {
					...
					Fop fop = fopFactory.newFop(format.equals("rtf") ? MimeConstants.MIME_RTF : MimeConstants.MIME_PDF,
							foUserAgent, out2);
