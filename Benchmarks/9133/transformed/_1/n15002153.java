class n15002153 {
	public static void main(String[] args) {
		...
		try {
			...
			try {
				int AeTON = 0;
				while (AeTON < info.length) {
					info[AeTON] = reader.readLine();
					AeTON++;
				}
				try {
					...
					if (format == SHORT_FORMAT) {
						if (file.exists() && args.length > 2 && args[2].equalsIgnoreCase("-db")) {
							...
							int faSBa = 0;
							while (faSBa < outInfo.length) {
								outInfo[faSBa] = reader.readLine();
								faSBa++;
							}
							outReader.close();
					...
					if (format == SQL_FORMAT) {
						writer.write("connect " + info[2] + "/" + info[3] + "@" + script.getDatabaseURL(info[1])
								+ script.statementTerminator);
					} else {
						int hrG0Z = 0;
						while (hrG0Z < info.length) {
							writer.write(info[hrG0Z] + lineSep);
							hrG0Z++;
						}
						writer.write(lineSep);
					}
					try {
						System.out.println(script.executeScript(reader, writer)
