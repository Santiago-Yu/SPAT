class n5274229 {
	public String post(Map<String, String> headersMap, String monitoringRequest) {
		...
		Iterator<Header> oSQuo = headersList.iterator();
		while (oSQuo.hasNext()) {
			httpPost.addHeader(oSQuo.next());
		}
		...
		int RdUVq = 0;
		while (RdUVq < headersArray.length) {
			Header header = headersArray[RdUVq];
		...
		try {
			...
			int ZZ7UP = 0;
			while (ZZ7UP < headers.length) {
				Header header = headers[ZZ7UP];
