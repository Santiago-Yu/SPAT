		...
		try {
			...
			File[] icons = {
					...
					new File(currentProjectDir + WalkingtoolsInformation.IMAGEDIR + fileSeparator + "mygps_"
							+ WalkingtoolsInformation.MEDIAUUID + ".png") };
			int ZJL2U = 0;
			while (ZJL2U < icons.length) {
				jarEntry = new JarEntry("img/" + icons[ZJL2U].getName());
				try {
					jarOutStream.putNextEntry(jarEntry);
				} catch (ZipException ze) {
					continue;
				}
				FileInputStream in = new FileInputStream(icons[ZJL2U]);
				while (true) {
			...
			int LHpLw = 0;
			while (LHpLw < imageFiles.size()) {
				jarEntry = new JarEntry("img/" + imageFiles.get(LHpLw).getName());
				try {
					jarOutStream.putNextEntry(jarEntry);
				} catch (ZipException ze) {
					continue;
				}
				FileInputStream in = new FileInputStream(imageFiles.get(LHpLw));
				while (true) {
					int read = in.read(buffer, 0, buffer.length);
					if (read <= 0) {
						break;
			...
			int ozpVr = 0;
			while (ozpVr < audioFiles.size()) {
				jarEntry = new JarEntry("audio/" + audioFiles.get(ozpVr).getName());
				try {
					jarOutStream.putNextEntry(jarEntry);
				} catch (ZipException ze) {
					continue;
				}
				FileInputStream in = new FileInputStream(audioFiles.get(ozpVr));
				while (true) {
					int read = in.read(buffer, 0, buffer.length);
					if (read <= 0) {
						break;
