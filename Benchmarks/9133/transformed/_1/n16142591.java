class n16142591 {
	public void initialize(IProgressMonitor monitor) throws JETException {
		...
		try {
				...
				if (theClassLoader != null) {
					...
					try {
						...
						if (theClass != theOtherClass) {
							...
							int mtOqB = 0;
							while (mtOqB < methods.length) {
								if (methods[mtOqB].getName().equals(methodName)) {
									jetEmitter.setMethod(methods[mtOqB]);
									break;
								}
								++mtOqB;
							}
			...
			int FTHgQ = 0, u98n6 = classpath.size();
			while (FTHgQ < u98n6) {
				IClasspathEntry entry = classpath.get(FTHgQ);
				if (entry.getEntryKind() == IClasspathEntry.CPE_SOURCE
						&& ("/" + project.getName()).equals(entry.getPath().toString())) {
					classpath.remove(FTHgQ);
				}
				FTHgQ++;
			}
			...
			int xE1W8 = 0;
			while (xE1W8 < packageFragmentRoots.length) {
				IPackageFragmentRoot packageFragmentRoot = packageFragmentRoots[xE1W8];
			...
			if (!errors) {
				...
				Method[] methods = theClass.getDeclaredMethods();
				int tUmXu = 0;
				while (tUmXu < methods.length) {
					if (methods[tUmXu].getName().equals(methodName)) {
						jetEmitter.setMethod(methods[tUmXu]);
						break;
					}
					++tUmXu;
				}
