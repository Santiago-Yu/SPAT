						...
						if ((mBuf.getCash() + credit) >= order.getOrderTotal()) {
							...
							if (round2Places(cash + credit) == round2Places(order.getOrderTotal())) {
								...
								try {
									...
									String strSql = "insert into " + strPOSPrefix
											...
											+ stuId + "', '" + round2Places(order.getOrderTotal()) + "', '"
											+ round2Places(cash) + "', '" + round2Places(credit) + "', '" + building
											+ "', '" + host + "', '" + cashier + "', NOW(), '" + round2Places(change)
											+ "')";
									...
									try {
										...
										System.err.println(exRetKeys.getMessage()
												+ " (but pscafepos is attempting a work around)");
										intSqlReturnVal = stmt.executeUpdate(strSql);
										masterID = dbMan.getLastInsertIDWorkAround(stmt,
									...
									if (intSqlReturnVal == 1) {
										if (masterID >= 0) {
											...
											if (itms != null && itms.length > 0) {
												int pmgrG = 0;
												while (pmgrG < itms.length) {
													if (itms[pmgrG] != null) {
														stmt = conn.createStatement();
														int itemid = itms[pmgrG].getDBID();
														double itemprice = round2Places(
																itms[pmgrG].getEffectivePrice());
														int f, r, a;
														String strItemName, strItemBuilding, strItemCat;
														f = 0;
														r = 0;
														a = 0;
														if (itms[pmgrG].isSoldAsFree()) {
															f = 1;
														}
														if (itms[pmgrG].isSoldAsReduced()) {
															r = 1;
														}
														if (itms[pmgrG].isTypeA()) {
															a = 1;
														}
														strItemName = itms[pmgrG].getName();
														strItemBuilding = (String) itms[pmgrG].getBuilding();
														strItemCat = itms[pmgrG].getCategory();
														if (stmt.executeUpdate("insert into " + strPOSPrefix
																...
																+ itemid + "', '" + masterID + "', '"
																+ round2Places(itemprice) + "', '" + host + "', '"
																+ cashier + "', '" + stuId + "', '" + f + "', '" + r
																+ "', NOW(), '" + a + "', '" + strItemName + "', '"
																+ strItemBuilding + "', '" + strItemCat + "')") != 1) {
															gui.setCriticalMessage("Item insert failed");
															conn.rollback();
														}
														stmt.close();
														stmt = conn.createStatement();
														String sqlInv = "SELECT inv_id from " + strPOSPrefix
																+ "inventory where inv_menuid = " + itemid + "";
														if (stmt.execute(sqlInv)) {
															...
															if (delId != -1) {
																stmt.executeUpdate("delete from " + strPOSPrefix
																		+ "inventory where inv_id = " + delId);
															}
															stmt.close();
														}
													} else {
												...
												if (round2Places(credit) > 0d) {
													if (round2Places(allowedCredit) >= round2Places(credit)) {
														if (hasStudentCredit()) {
															stmt = conn.createStatement();
															if (stmt.executeUpdate("update " + strPOSPrefix
															...
															if (stmt.executeUpdate("insert into " + strPOSPrefix
																	...
																	+ round2Places((-1) * credit) + "','1','" + stuId
																	+ "', NOW())") == 1) {
																stmt.close();
																stmt = conn.createStatement();
																if (stmt.executeUpdate("insert into " + strPOSPrefix
																		+ "studentcredit_log ( scl_studentid, scl_action, scl_transid, scl_datetime ) values( '"
																		+ stuId + "', '" + round2Places((-1) * credit)
																		+ "', '" + masterID + "', NOW() )") == 1) {
																	stmt.close();
																	blOk = true;
																} else {
												...
												if (blOk) {
													...
													if (itms != null && itms.length > 0) {
														int PIogD = 0;
														while (PIogD < itms.length) {
															stmt = conn.createStatement();
															if (stmt.execute("select count(*) from " + strPOSPrefix
																	+ "hotbar where hb_itemid = '"
																	+ itms[PIogD].getDBID() + "' and hb_building = '"
																	+ building + "' and hb_register = '" + host
																...
																if (num == 1) {
																	...
																	if (stmt.executeUpdate("update " + strPOSPrefix
																			...
																			+ itms[PIogD].getDBID()
																			+ "' and hb_building = '" + building
																			+ "' and hb_register = '" + host
																			+ "' and hb_cashier = '" + cashier
																			+ "'") != 1)
																		blHBOK = false;
																} else {
													...
													if (blHBOK) {
														...
														summary.display(money.format(order.getOrderTotal()),
																money.format(mBuf.getCash()), money.format(credit),
																money.format(change), money.format(getStudentCredit()));
													} else {
														conn.rollback();
														gui.setStatus(
