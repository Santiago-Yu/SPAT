class n23094550 {
	public StringBuffer render(RenderEngine c) {
		...
		if (action.equalsIgnoreCase("read")) {
			...
			int numSlashes = 0;
			int eTcXL = 0;
			while (eTcXL < curDirname.length()) {
				if (curDirname.toString().charAt(eTcXL) == '/') {
					numSlashes++;
				}
				eTcXL++;
			}
			...
			if (numSlashes > 1) {
				int PmxsR = 0;
				while (PmxsR < (dirEntries.length - 1)) {
					if (dirEntries[PmxsR] != null && !dirEntries[PmxsR].equals("")) {
						parentDirectory += dirEntries[PmxsR] + "/";
					}
					PmxsR++;
				}
			}
			...
			if (c.getVendContext() != null && c.getVendContext().getFileAccess() != null
				...
				int depth = 0;
				int Uv9RY = 0;
				while (Uv9RY < filenameData.toString().length()) {
					if (filenameData.toString().charAt(Uv9RY) == '/') {
						depth++;
					}
					Uv9RY++;
				}
				...
				int MWG2p = 0;
				while (MWG2p < list.length) {
					ZipEntry zEntry = (ZipEntry) list[MWG2p];
				...
				int stR95 = 0;
				while (stR95 < list.length) {
					filesorted[stR95] = (java.io.File) list[stR95];
					stR95++;
				}
				if (sort.equalsIgnoreCase("asc")) {
				...
				int U9VkO = 0;
				while (U9VkO < filesorted.length) {
					java.io.File zEntry = filesorted[U9VkO];
				...
				int M4Hjy = 0;
				while (M4Hjy < fileStringList.length) {
					file = new java.io.File(currentDocroot + filenameData.toString() + "/" + fileStringList[M4Hjy]);
					...
					if (xmlOutput.equalsIgnoreCase("true")) {
						buffer.append("    <file name=\"");
						buffer.append(fileStringList[M4Hjy]);
						buffer.append("\" length=\"");
						buffer.append(fileLength);
						buffer.append("\" type=\"");
						buffer.append(fileType);
						...
						fileList.add(fileStringList[M4Hjy]);
					}
					c.getVariableContainer().setVariable(c, varname + "(" + fileStringList[M4Hjy] + ")(length)", false,
							"" + fileLength);
					c.getVariableContainer().setVariable(c, varname + "(" + fileStringList[M4Hjy] + ")(type)", false,
							fileType);
					c.getVariableContainer().setVariable(c, varname + "(" + fileStringList[M4Hjy] + ")(mode)", false,
							fileMode);
					c.getVariableContainer().setVariable(c, varname + "(" + fileStringList[M4Hjy] + ")(time)", false,
							fileTime);
					M4Hjy++;
				}
				buffer.append("</listing>");
			...
			if (file.isDirectory()) {
				...
				int bvndh = 0;
				while (bvndh < fileStringList.length) {
					(new java.io.File(filenameDirectory + "/" + fileStringList[bvndh])).delete();
					bvndh++;
				}
			}
