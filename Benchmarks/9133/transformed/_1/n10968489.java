class n10968489 {
	private void storeFieldMap(Content c, Connection conn) throws SQLException {
		...
		try {
			...
			String type;
			Object value, siKey;
			Iterator QSwFM = c.getStructure().getStructureItems().iterator();
			while (QSwFM.hasNext()) {
				nextItem = (StructureItem) QSwFM.next();
				...
				if (type.equals(StructureItem.DATE)) {
					...
					ps.executeUpdate();
				} else if (type.equals(StructureItem.INT) || type.equals(StructureItem.FLOAT)
						|| type.equals(StructureItem.VARCHAR)) {
