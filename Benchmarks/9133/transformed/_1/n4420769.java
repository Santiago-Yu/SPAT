class n4420769 {
	public void _saveWebAsset(ActionRequest req, ActionResponse res, PortletConfig config, ActionForm form, User user,
		...
		try {
			...
			int KUO1Q = 0;
			while (KUO1Q < countFiles) {
				File file = new File();
				String title = ParamUtil.getString(req, "title" + KUO1Q);
				String friendlyName = ParamUtil.getString(req, "friendlyName" + KUO1Q);
				Date publishDate = new Date();
				String fileName = ParamUtil.getString(req, "fileName" + KUO1Q);
				...
				if (fileName.length() > 0) {
					...
					java.io.File uploadedFile = uploadReq.getFile("uploadedFile" + KUO1Q);
