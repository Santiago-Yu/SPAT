	...
	public static boolean predictDataSet(String completePath, String Type, String predictionOutputFileName,
		...
		try {
			if (Type.equalsIgnoreCase("Qualifying")) {
				...
				Arrays.sort(movies);
				int u1Uj3 = 0;
				while (u1Uj3 < movies.length) {
					short movieToProcess = movies[u1Uj3];
					...
					for (int j = 0; j < customersToProcess.size(); j++) {
						...
						if (finalPrediction == finalPrediction) {
							...
							finalPrediction = GetAveragePrediction(movieToProcess);
				...
				Arrays.sort(movies);
				int UQOxc = 0;
				while (UQOxc < movies.length) {
					short movieToProcess = movies[UQOxc];
					movieDiffStats = loadMovieDiffStats(completePath, movieToProcess, slopeOneDataFolderName);
