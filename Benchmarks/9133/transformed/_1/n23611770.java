class n23611770 {
	public void handle() {
		...
		if (rotateDest != null) {
			...
			int SfxTC = 0;
			while (SfxTC < fileList.length) {
				String currentFilename = fileList[SfxTC].getName();
				long timeDifference = (currentTime - fileList[SfxTC].lastModified());
				if ((rotateCompress && currentFilename.endsWith(".gz"))
					...
					if (timeDifference > comparisonTime) {
						archive.addFile(fileList[SfxTC].getName(), currentFilename);
						fileList[SfxTC].delete();
					}
				}
				SfxTC++;
			}
