class n13961100 {
	public void initialize(IProgressMonitor monitor) throws JETException {
		...
		try {
			final JETCompiler jetCompiler = getTemplateURIPath() == null
				...
				if (theClassLoader != null) {
					...
					try {
						...
						if (theClass != theOtherClass) {
							...
							int jRjR1 = 0;
							while (jRjR1 < methods.length) {
								if (methods[jRjR1].getName().equals(methodName)) {
									jetEmitter.setMethod(methods[jRjR1]);
									break;
								}
								++jRjR1;
							}
			...
			int sIH8O = 0, j7HCf = classpath.size();
			while (sIH8O < j7HCf) {
				IClasspathEntry entry = classpath.get(sIH8O);
				if (entry.getEntryKind() == IClasspathEntry.CPE_SOURCE
						&& ("/" + project.getName()).equals(entry.getPath().toString())) {
					classpath.remove(sIH8O);
				}
				sIH8O++;
			}
			...
			int TDlkc = 0;
			while (TDlkc < packageFragmentRoots.length) {
				IPackageFragmentRoot packageFragmentRoot = packageFragmentRoots[TDlkc];
			...
			if (!errors) {
				...
				Method[] methods = theClass.getDeclaredMethods();
				int HjROD = 0;
				while (HjROD < methods.length) {
					if (methods[HjROD].getName().equals(methodName)) {
						jetEmitter.setMethod(methods[HjROD]);
						break;
					}
					++HjROD;
				}
