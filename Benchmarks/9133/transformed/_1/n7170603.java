class n7170603 {
	private static void generateSQLUpgradeFile(String milestoneDefFileName, String sqlDirectoryName,
			String fromMilestone, String destMilestone, String destFileName) throws Exception {
		...
		if (fromMilestoneIdx < 0) {
			throw new IllegalArgumentException("Cannot upgrade from milestone [" + fromMilestone
					+ "] : does not exist . (Exisiting milestones " + parser.listMilestones() + ")");
		}
		if (toMilestoneIdx < 0) {
			throw new IllegalArgumentException("Cannot upgrade to milestone [" + destMilestone
					+ "] : does not exist . (Exisiting milestones " + parser.listMilestones() + ")");
		}
		...
		try {
			...
			Iterator<String> tfX6n = files.iterator();
			while (tfX6n.hasNext()) {
				appendFileToOutputStream(fos, sqlDirectory, tfX6n.next());
			}
			Iterator<String> as4zg = views.iterator();
			while (as4zg.hasNext()) {
				appendFileToOutputStream(fos, sqlDirectory, as4zg.next());
			}
		} catch (Exception e) {
			fos.close();
			destFile.delete();
