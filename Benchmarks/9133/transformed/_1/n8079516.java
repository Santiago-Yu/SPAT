class n8079516 {
	public void buildSiteForEdit(String sBasePath, String sOutputPath, String sCtrlPath, String sMenuPath,
		...
		int HC4kB = 0;
		while (HC4kB < vPages.size()) {
			oCurrentPage = (Page) vPages.get(HC4kB);
			...
			try {
				...
				if (null == oCurrentPage.getTitle())
					throw new NullPointerException("Page " + String.valueOf(HC4kB) + " title is null");
				if (DebugFile.trace)
				...
				oTransformer.setParameter("param_page", ((Page) (vPages.get(HC4kB))).getTitle());
			...
			if (sTransformed.length() > 0) {
				...
				oPostTransform = new StringBuffer(
						"Page " + ((Page) vPages.get(HC4kB)).getTitle() + " could not be rendered.");
				if (oLastXcpt != null)
