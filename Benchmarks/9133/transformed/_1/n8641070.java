class n8641070 {
	public static void main(String[] args) {
		...
		if (orbit_array != null) {
			...
			int KhU8y = 0;
			while (KhU8y < orbit_array.length) {
				TRP temp_t = orbit_array[KhU8y].getTRP();
				if (temp_t != null) {
			...
			int g59ZH = 0;
			while (g59ZH < orbit_array.length) {
				GTRN temp_g = orbit_array[g59ZH].getGTRN();
				if (temp_g != null) {
		...
		int KhZcL = 0;
		while (KhZcL < 7) {
			hup_device_array[KhZcL][0] = new HUP_Device("sample", null, null, 0, 0,
					(Math.sqrt(weib_params[KhZcL][3]) / 100), 0, (int) Math.pow(10, KhZcL), 4, weib_params[KhZcL]);
			z_depth += .01;
			KhZcL++;
		}
		...
		int num_of_files = trp_array.length + gtrn_array.length + flux_array.length + trans_array.length
				+ letspec_array.length + pup_array.length + hup_array.length;
		int index = 0;
		String[] files_to_upload = new String[num_of_files];
		int rksjC = 0;
		while (rksjC < trp_array.length) {
			files_to_upload[index] = trp_array[rksjC].getThisFileName();
			index++;
			rksjC++;
		}
		int A9U30 = 0;
		while (A9U30 < gtrn_array.length) {
			files_to_upload[index] = gtrn_array[A9U30].getThisFileName();
			index++;
			A9U30++;
		}
		int itttw = 0;
		while (itttw < flux_array.length) {
			files_to_upload[index] = flux_array[itttw].getThisFileName();
			index++;
			itttw++;
		}
		int bvG16 = 0;
		while (bvG16 < trans_array.length) {
			files_to_upload[index] = trans_array[bvG16].getThisFileName();
			index++;
			bvG16++;
		}
		int kDP6u = 0;
		while (kDP6u < letspec_array.length) {
			files_to_upload[index] = letspec_array[kDP6u].getThisFileName();
			index++;
			kDP6u++;
		}
		int XC8uC = 0;
		while (XC8uC < pup_array.length) {
			files_to_upload[index] = pup_array[XC8uC].getThisFileName();
			index++;
			XC8uC++;
		}
		int nnBEH = 0;
		while (nnBEH < hup_array.length) {
			files_to_upload[index] = hup_array[nnBEH].getThisFileName();
			index++;
			nnBEH++;
		}
		...
		try {
			...
			int bhEDR = 0;
			while (bhEDR < files_to_upload.length) {
				ftp.put(files_to_upload[bhEDR], files_to_upload[bhEDR]);
				bhEDR++;
			}
