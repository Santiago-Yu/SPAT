class n6644748 {
	public static void initialize(Monitor monitor, final JETEmitter jetEmitter) throws JETException {
		...
		try {
				...
				if (theClassLoader != null) {
					...
					try {
						...
						if (theClass != theOtherClass) {
							...
							int H1qJx = 0;
							while (H1qJx < methods.length) {
								if (methods[H1qJx].getName().equals(methodName)) {
									jetEmitter.setMethod(methods[H1qJx]);
									break;
								}
								++H1qJx;
							}
			...
			int YeWw9 = 0, BlXzt = classpath.size();
			while (YeWw9 < BlXzt) {
				IClasspathEntry entry = classpath.get(YeWw9);
				if (entry.getEntryKind() == IClasspathEntry.CPE_SOURCE
						&& ("/" + project.getName()).equals(entry.getPath().toString())) {
					classpath.remove(YeWw9);
				}
				YeWw9++;
			}
			...
			int XOdNl = 0;
			while (XOdNl < packageFragmentRoots.length) {
				IPackageFragmentRoot packageFragmentRoot = packageFragmentRoots[XOdNl];
			...
			int ka0f1 = 0;
			while (ka0f1 < markers.length) {
				IMarker marker = markers[ka0f1];
			...
			if (!errors) {
				...
				Method[] methods = theClass.getDeclaredMethods();
				int snY4I = 0;
				while (snY4I < methods.length) {
					if (methods[snY4I].getName().equals(methodName)) {
						jetEmitter.setMethod(methods[snY4I]);
						break;
					}
					++snY4I;
				}
