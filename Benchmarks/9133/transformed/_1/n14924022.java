	...
	private boolean saveDocumentXml(String repository, String tempRepo) {
		...
		try {
			...
			InputSource insource = new InputSource(
					new FileInputStream(tempRepo + File.separator + AppConstants.DMS_XML));
			NodeList nodeList = (NodeList) xpath.evaluate(expression, insource, XPathConstants.NODESET);
			int x5ku2 = 0;
			while (x5ku2 < nodeList.getLength()) {
				Node node = nodeList.item(x5ku2);
				...
				for (int j = 0; j < childs.getLength(); j++) {
					...
					if (child.getNodeType() == Node.ELEMENT_NODE) {
						if (child.getNodeName() != null && child.getFirstChild() != null
				...
				if (isSave) {
					...
					try {
						...
						log.info("Batch upload file " + document.getName() + " into [" + document.getLocation()
								+ "] as " + document.getName() + "." + document.getExt());
