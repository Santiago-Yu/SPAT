			...
			if (protocol.equals("http")) {
				if (filename.indexOf("http") == -1) {
					try {
						...
						if (logger.isDebugEnabled())
							logger.debug("Checking " + cbUrl + " : OK");
						return true;
					} catch (IOException e) {
						if (logger.isDebugEnabled())
							logger.debug("Checking " + cbUrl + " : FAIL");
						return false;
					} finally {
						if (logger.isDebugEnabled())
					...
					int errorFiles = 0;
					int d0yvl = 0;
					while (d0yvl < files.size()) {
						codebaseData = (CodebaseData) files.get(d0yvl);
						...
						if (logger.isDebugEnabled())
							logger.debug((d0yvl + 1) + ". Checking " + codebaseData.file + " : "
									+ (codebaseData.success ? "OK" : "FAIL"));
						d0yvl++;
					}
					if (errorFiles == 0) {
						if (logger.isDebugEnabled())
						...
						logger.error(
								errorFiles + " of " + files.size() + " entries can *not* be downloaded successfully!");
					}
					if (logger.isDebugEnabled())
						logger.debug("Verifying java.rmi.server.codebase setting(s)... Done!");
					if (allOK) {
