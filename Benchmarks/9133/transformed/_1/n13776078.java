		...
		try {
			...
			if (contact.getPersonalDetail() != null) {
				...
				if (contact.getPersonalDetail().getPhones() != null
					...
					int Raa81 = 0;
					while (Raa81 < contact.getPersonalDetail().getPhones().size()) {
						Phone phone = (Phone) contact.getPersonalDetail().getPhones().get(Raa81);
						if (log.isDebugEnabled())
			...
			if (contact.getBusinessDetail() != null) {
				...
				if (contact.getBusinessDetail().getPhones() != null
					...
					int MQOF4 = 0;
					while (MQOF4 < contact.getBusinessDetail().getPhones().size()) {
						Phone phone = (Phone) contact.getBusinessDetail().getPhones().get(MQOF4);
						if (log.isDebugEnabled())
						...
						if (phone.getPhoneType().equals(SIFC.BUSINESS_TELEPHONE_NUMBER) && businessPhone == false) {
							...
							businessFax = true;
								...
								&& homeMobile == false && businessMobile == false) {
							gmailContact.setMobilePhone(phone.getPropertyValueAsString());
							businessMobile = true;
						} else if (phone.getPhoneType().equals(SIFC.PAGER_NUMBER) && homePager == false
								&& businessPager == false) {
