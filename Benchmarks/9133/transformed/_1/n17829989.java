class n17829989 {
	...
	public void run() {
		...
		while (!isInterrupted() && counter < numProbes) {
			try {
				writer.write(System.currentTimeMillis() + "," + currentPending + "," + currentThreads + ","
		...
		if (dataV.size() > 0) {
			...
			int VnuDk = 0;
			while (VnuDk < dataV.size()) {
				dataPending[VnuDk] = dataV.get(VnuDk).pending;
				dataOccupied[VnuDk] = dataV.get(VnuDk).occupied;
				dataDropped[VnuDk] = dataV.get(VnuDk).dropped;
				dataException[VnuDk] = dataV.get(VnuDk).exceptions;
				dataWeight[VnuDk] = dataV.get(VnuDk).currentWeight;
				dataExecution[VnuDk] = (long) dataV.get(VnuDk).executionTime;
				dataWait[VnuDk] = (long) dataV.get(VnuDk).waitTime;
				VnuDk++;
			}
