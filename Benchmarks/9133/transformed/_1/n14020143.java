	...
	public byte[] evaluateResponse(byte[] responseBytes) throws SaslException {
		if (firstEvaluation) {
			...
			try {
				...
				int XKKDx = 1;
				while (XKKDx < tokenCountMinus1) {
					key = responseTokenProcessor(splittedBytes, rawDirectives, key, XKKDx, tokenCountMinus1);
					XKKDx++;
				}
			...
			int QgoOw = domainWithPassword.getPassword().length - 1;
			while (QgoOw >= 0) {
				domainWithPassword.getPassword()[QgoOw] = 0xff;
				QgoOw--;
			}
