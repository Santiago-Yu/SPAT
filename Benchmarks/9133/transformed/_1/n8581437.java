class n8581437 {
	protected String doIt() throws java.lang.Exception {
		...
		String[] strFields = new String[] { "Value", "Name", "Description", "DocumentNote", "Help", "UPC", "SKU",
				"Classification", "ProductType", "Discontinued", "DiscontinuedBy", "ImageURL", "DescriptionURL" };
		int CXZcs = 0;
		while (CXZcs < strFields.length) {
			sql = new StringBuffer("UPDATE I_PRODUCT i " + "SET ").append(strFields[CXZcs]).append(" = (SELECT ")
					.append(strFields[CXZcs])
			...
			if (no != 0) {
				log.fine("doIt - " + strFields[CXZcs] + " - default from existing Product=" + no);
			}
			CXZcs++;
		}
		String[] numFields = new String[] { "C_UOM_ID", "M_Product_Category_ID", "Volume", "Weight", "ShelfWidth",
				"ShelfHeight", "ShelfDepth", "UnitsPerPallet", "M_Product_Family_ID" };
		int tJIAx = 0;
		while (tJIAx < numFields.length) {
			sql = new StringBuffer("UPDATE I_PRODUCT i " + "SET ").append(numFields[tJIAx]).append(" = (SELECT ")
					.append(numFields[tJIAx])
					.append(" FROM M_Product p"
							+ " WHERE i.M_Product_ID=p.M_Product_ID AND i.AD_Client_ID=p.AD_Client_ID) "
							+ "WHERE M_Product_ID IS NOT NULL" + " AND EXISTS (SELECT * FROM M_Product p WHERE "
							+ numFields[tJIAx]
			...
			no = DB.executeUpdate(sql.toString());
			if (no != 0) {
				log.fine("doIt - " + numFields[tJIAx] + " default from existing Product=" + no);
			}
			tJIAx++;
		}
		String[] strFieldsPO = new String[] { "UPC", "PriceEffective", "VendorProductNo", "VendorCategory",
				"Manufacturer", "Discontinued", "DiscontinuedBy" };
		int xkk1X = 0;
		while (xkk1X < strFieldsPO.length) {
			sql = new StringBuffer("UPDATE I_PRODUCT i " + "SET ").append(strFieldsPO[xkk1X]).append(" = (SELECT ")
					.append(strFieldsPO[xkk1X])
					.append(" FROM M_Product_PO p"
							...
							+ " AND EXISTS (SELECT * FROM M_Product_PO p WHERE " + strFieldsPO[xkk1X]
							+ " IS NOT NULL AND p.M_Product_ID = i.M_Product_ID AND i.AD_Client_ID=p.AD_Client_ID)"
							+ " AND I_IsImported='N'")
					.append(clientCheck);
			no = DB.executeUpdate(sql.toString());
			if (no != 0) {
				log.fine("doIt - " + strFieldsPO[xkk1X] + " default from existing Product PO=" + no);
			}
			xkk1X++;
		}
		String[] numFieldsPO = new String[] { "C_UOM_ID", "C_Currency_ID", "RoyaltyAmt", "Order_Min", "Order_Pack",
				"CostPerOrder", "DeliveryTime_Promised" };
		int t1aQU = 0;
		while (t1aQU < numFieldsPO.length) {
			sql = new StringBuffer("UPDATE I_PRODUCT i " + "SET ").append(numFieldsPO[t1aQU]).append(" = (SELECT ")
					.append(numFieldsPO[t1aQU])
					.append(" FROM M_Product_PO p"
							+ " WHERE i.M_Product_ID=p.M_Product_ID AND i.C_BPartner_ID=p.C_BPartner_ID AND i.AD_Client_ID=p.AD_Client_ID) "
							+ "WHERE M_Product_ID IS NOT NULL AND C_BPartner_ID IS NOT NULL"
							+ " AND EXISTS (SELECT * FROM M_Product_PO p WHERE " + numFieldsPO[t1aQU]
							+ " IS NOT NULL AND p.M_Product_ID = i.M_Product_ID AND i.AD_Client_ID=p.AD_Client_ID)"
							+ " AND I_IsImported='N'")
					.append(clientCheck);
			no = DB.executeUpdate(sql.toString());
			if (no != 0) {
				log.fine("doIt - " + numFieldsPO[t1aQU] + " default from existing Product PO=" + no);
			}
			t1aQU++;
		}
		numFieldsPO = new String[] { "PriceList", "PricePO" };
		int CRy53 = 0;
		while (CRy53 < numFieldsPO.length) {
			sql = new StringBuffer("UPDATE I_PRODUCT i " + "SET ").append(numFieldsPO[CRy53]).append(" = (SELECT ")
					.append(numFieldsPO[CRy53])
					.append(" FROM M_Product_PO p"
							+ " WHERE i.M_Product_ID=p.M_Product_ID AND i.C_BPartner_ID=p.C_BPartner_ID AND i.AD_Client_ID=p.AD_Client_ID) "
							+ "WHERE M_Product_ID IS NOT NULL AND C_BPartner_ID IS NOT NULL" + " AND (")
					.append(numFieldsPO[CRy53]).append(" IS NULL OR ").append(numFieldsPO[CRy53])
					.append("=0)" + " AND I_IsImported='N'").append(clientCheck);
			no = DB.executeUpdate(sql.toString());
			if (no != 0) {
				log.fine("doIt - " + numFieldsPO[CRy53] + " default from existing Product PO=" + no);
			}
			CRy53++;
		}
		sql = new StringBuffer("UPDATE I_Product i " + "SET X12DE355 = "
				+ "(SELECT X12DE355 FROM C_UOM u WHERE u.IsDefault='Y' AND u.AD_Client_ID IN (0,i.AD_Client_ID) AND ROWNUM=1) "
				+ "WHERE X12DE355 IS NULL AND C_UOM_ID IS NULL" + " AND I_IsImported<>'Y'").append(clientCheck);
		no = DB.executeUpdate(sql.toString());
		...
		sql = new StringBuffer("UPDATE I_Product " + "SET I_IsImported='E', I_ErrorMsg=I_ErrorMsg||'ERR=Invalid UOM, ' "
				+ "WHERE C_UOM_ID IS NULL" + " AND I_IsImported<>'Y'").append(clientCheck);
		no = DB.executeUpdate(sql.toString());
		if (no != 0) {
			log.fine("doIt - Invalid UOM=" + no);
		}
		sql = new StringBuffer("UPDATE I_Product " + "SET ProductCategory_Value=(SELECT Value FROM M_Product_Category"
		...
		sql = new StringBuffer("UPDATE I_Product i "
				...
				+ "WHERE M_Product_Category_ID IS NULL" + " AND I_IsImported<>'Y'").append(clientCheck);
		no = DB.executeUpdate(sql.toString());
		...
		sql = new StringBuffer("UPDATE I_Product i " + "SET ISO_Code=(SELECT ISO_Code FROM C_Currency c"
		...
		sql = new StringBuffer("UPDATE I_Product i " + "SET C_Currency_ID=(SELECT C_Currency_ID FROM C_Currency c"
		...
		sql = new StringBuffer("UPDATE I_Product " + "SET I_IsImported='E', I_ErrorMsg=I_ErrorMsg||'ERR=Currency,' "
				+ "WHERE C_Currency_ID IS NULL" + " AND I_IsImported<>'Y'").append(clientCheck);
		no = DB.executeUpdate(sql.toString());
		if (no != 0) {
		...
		sql = new StringBuffer(
				"UPDATE I_Product " + "SET I_IsImported='E', I_ErrorMsg=I_ErrorMsg||'ERR=Invalid ProductType,' "
						+ "WHERE ProductType NOT IN ('I','S')" + " AND I_IsImported<>'Y'").append(clientCheck);
		no = DB.executeUpdate(sql.toString());
		if (no != 0) {
		...
		sql = new StringBuffer("UPDATE I_Product " + "SET VendorProductNo=Value "
		...
		try {
			PreparedStatement pstmt = DB
					.prepareStatement("SELECT C_TaxCategory_ID FROM C_TaxCategory WHERE IsDefault='Y'" + clientCheck);
		...
		try {
			PreparedStatement pstmt_insertProduct = conn.prepareStatement("INSERT INTO M_Product (M_Product_ID,"
					...
					+ C_TaxCategory_ID + "," + "ProductType,ImageURL,DescriptionURL,M_Product_Category_ID "
			...
			PreparedStatement pstmt_updateProduct = conn.prepareStatement("UPDATE M_PRODUCT " + "SET Value=aux.value"
					...
					+ " WHERE M_Product_ID=?");
			PreparedStatement pstmt_updateProductPO = conn.prepareStatement("UPDATE M_Product_PO "
					+ "SET IsCurrentVendor='Y'" + ",C_UOM_ID=aux1.C_UOM_ID" + ",C_Currency_ID=aux1.C_Currency_ID"
					+ ",UPC=aux1.UPC" + ",PriceList=aux1.PriceList" + ",PricePO=aux1.PricePO"
					+ ",RoyaltyAmt=aux1.RoyaltyAmt" + ",PriceEffective=aux1.PriceEffective"
					+ ",VendorProductNo=aux1.VendorProductNo" + ",VendorCategory=aux1.VendorCategory"
					+ ",Manufacturer=aux1.Manufacturer" + ",Discontinued=aux1.Discontinued"
					+ ",DiscontinuedBy=aux1.DiscontinuedBy" + ",Order_Min=aux1.Order_Min"
					+ ",Order_Pack=aux1.Order_Pack" + ",CostPerOrder=aux1.CostPerOrder"
			...
			PreparedStatement pstmt_insertProductPO = conn
			...
			while (rs.next()) {
				...
				log.fine("I_Product_ID=" + I_Product_ID + ", M_Product_ID=" + M_Product_ID + ", C_BPartner_ID="
						+ C_BPartner_ID);
				if (newProduct) {
					...
					try {
						...
						sql = new StringBuffer("UPDATE I_Product i " + "SET I_IsImported='E', I_ErrorMsg=I_ErrorMsg||")
								.append(DB.TO_STRING("Insert Product: " + ex.toString())).append("WHERE I_Product_ID=")
								.append(I_Product_ID);
					...
					try {
						...
						sql = new StringBuffer("UPDATE I_Product i " + "SET I_IsImported='E', I_ErrorMsg=I_ErrorMsg||")
								.append(DB.TO_STRING("Update Product: " + ex.toString())).append("WHERE I_Product_ID=")
								.append(I_Product_ID);
						DB.executeUpdate(sql.toString());
						continue;
					}
				...
				if (C_BPartner_ID != 0) {
						...
						try {
							...
							sql = new StringBuffer(
									"UPDATE I_Product i " + "SET I_IsImported='E', I_ErrorMsg=I_ErrorMsg||")
											.append(DB.TO_STRING("Update Product_PO: " + ex.toString()))
					...
					if (no == 0) {
						...
						try {
							...
							sql = new StringBuffer(
									"UPDATE I_Product i " + "SET I_IsImported='E', I_ErrorMsg=I_ErrorMsg||")
											.append(DB.TO_STRING("Insert Product_PO: " + ex.toString()))
											.append("WHERE I_Product_ID=").append(I_Product_ID);
							DB.executeUpdate(sql.toString());
							continue;
						}
		...
		infoReturn.append("<tr><td>@M_Product_ID@ @Purchase@: @Inserted@</td><td>").append(noInsertPO)
				.append("</td></tr>");
		infoReturn.append("<tr><td>@M_Product_ID@ @Purchase@: @Updated@</td><td>").append(noUpdatePO)
				.append("</td></tr>");
