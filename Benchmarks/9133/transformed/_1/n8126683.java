class n8126683 {
	public Resource readResource(URL url, ResourceManager resourceManager) throws NAFException {
		...
		try {
			...
			Node Bzmte = doc.getFirstChild();
			while (Bzmte != null) {
				if (Bzmte.getNodeType() == Node.PROCESSING_INSTRUCTION_NODE
						&& "xml-stylesheet".equals(Bzmte.getNodeName())) {
					ProcessingInstruction pi = (ProcessingInstruction) Bzmte;
