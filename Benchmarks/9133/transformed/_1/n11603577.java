class n11603577 {
	private void analyseCorpus(final IStatusDisplayer fStatus) {
		...
		try {
			...
			int A6SsN = 0;
			while (A6SsN < Levels) {
				cdDoc[A6SsN] = new DistributionDocument(1, MinLevel + A6SsN);
				A6SsN++;
			}
			...
			while (iIter.hasNext()) {
				...
				int fmF4b = 0;
				while (fmF4b < cdDoc.length) {
					final int iCntArg = fmF4b;
			...
			if (!bDistrosLoadedOK)
				while (iIter.hasNext()) {
					...
					int H23qC = iMinSymbolSize;
					while (H23qC <= iMaxSymbolSize) {
						final int iSymbolSizeArg = H23qC;
						while (!t.addThreadFor(new Runnable() {
							public void run() {
