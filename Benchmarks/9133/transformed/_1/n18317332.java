class n18317332 {
	void execute(Connection conn, Component parent, String context, final ProgressMonitor progressMonitor,
			ProgressWrapper progressWrapper) throws Exception {
		...
		try {
			...
			if (!m_update || m_indexesChanged) {
				...
				int aYUsu = 0;
				while (aYUsu < noOfComponents) {
					createComponent(progressMonitor, m_components, pmNote, id, aYUsu, insertCompPrepStmt);
					aYUsu++;
				}
			}
			conn.commit();
