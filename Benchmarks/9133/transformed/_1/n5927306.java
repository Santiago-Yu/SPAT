class n5927306 {
	boolean createSessionArchive(String archiveFilename) {
		...
		try {
			...
			mAnnotationsCursor.moveToFirst();
			while (!mAnnotationsCursor.isAfterLast()) {
				...
				archive.closeEntry();
				in.close();
				mAnnotationsCursor.moveToNext();
			}
			archive.close();
		} catch (IOException e) {
			Toast.makeText(mActivity, mActivity.getString(R.string.error_zip) + " " + e.getMessage(),
