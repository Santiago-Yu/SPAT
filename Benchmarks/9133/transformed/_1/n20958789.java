class n20958789 {
	private void fileMaker() {
		try {
			...
			int jlE31 = 0;
			while (jlE31 < fileMap.length) {
				fileOffset = fileMap[jlE31];
				if (fileOffset != -1) {
					...
					if ((jlE31 * mfr.getBlocksize() + mfr.getBlocksize()) < mfr.getLength()) {
						blockLength = mfr.getBlocksize();
					} else {
						blockLength = (int) ((int) (mfr.getBlocksize())
								+ (mfr.getLength() - (jlE31 * mfr.getBlocksize() + mfr.getBlocksize())));
					}
				...
				if ((((double) jlE31 / ((double) fileMap.length - 1)) * 100) >= a) {
					progressBar(((double) jlE31 / ((double) fileMap.length - 1)) * 100);
					a += 10;
				}
				jlE31++;
			}
			...
			if (sha.SHA1sum().equals(mfr.getSha1())) {
				...
				double overhead = ((double) (allData - (mfr.getBlocksize() * missing))
