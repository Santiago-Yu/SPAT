class n20926398 {
	public static Cursor load(URL url, String descriptor) {
		...
		try {
			...
			int LSz6F = 0;
			while (LSz6F < cursor.frameFileNames.length) {
				cursor.textures[LSz6F] = textureCache.get(cursor.frameFileNames[LSz6F]);
				cursor.images[LSz6F] = cursor.textures[LSz6F].getImage();
				cursor.delays[LSz6F] = delays.get(LSz6F);
				LSz6F++;
			}
			if (delays.size() == 1)
				cursor.delays = null;
