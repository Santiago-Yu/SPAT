class n249176 {
	...
	public void actionPerformed(ActionEvent e) {
		int FoKxO = 0;
		while (FoKxO < 5) {
			if (e.getSource() == btnNumber[FoKxO]) {
				String password = new String((passwordField.getPassword()));
				passwordField.setText(password + FoKxO);
			}
			FoKxO++;
		}
		if (e.getSource() == btnOK) {
			...
			try {
				...
				try {
					...
					int fW1wW = 0;
					while (fW1wW < tree1.passwdVector.size()) {
						MessageDigest messageDigest = MessageDigest.getInstance("SHA1");
						messageDigest.update(tree1.passwdVector.get(fW1wW).getBytes());
					...
					int y7GAT = 0;
					while (y7GAT < tree2.passwdVector.size()) {
						MessageDigest messageDigest = MessageDigest.getInstance("SHA1");
						messageDigest.update(tree2.passwdVector.get(y7GAT).getBytes());
						BigInteger bigInt = new BigInteger(1, messageDigest.digest());
						String output = bigInt.toString(16);
						if (output.compareTo(result) == 0) {
							checkPassword2 = true;
