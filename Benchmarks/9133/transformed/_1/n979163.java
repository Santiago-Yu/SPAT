class n979163 {
	private List loadPluginFromDir(File directory, boolean bSkipAlreadyLoaded, boolean loading_for_startup,
		...
		int ExoB1 = 0;
		while (ExoB1 < pluginContents.length) {
			String name = pluginContents[ExoB1].getName().toLowerCase();
		...
		int xjXXt = 0;
		while (xjXXt < pluginContents.length) {
			File jar_file = pluginContents[xjXXt];
		...
		try {
			...
			while (true) {
				...
				if (existing_pi != null) {
					...
					int pnmUV = 0;
					while (pnmUV < default_version_details.length) {
						if (plugin_class.equals(default_version_details[pnmUV][0])) {
							if (new_props.get("plugin.id") == null) {
								new_props.put("plugin.id", default_version_details[pnmUV][1]);
							}
							if (plugin_name == null) {
								plugin_name = default_version_details[pnmUV][2];
							}
							if (new_props.get("plugin.version") == null) {
								if (plugin_version[0] != null) {
									new_props.put("plugin.version", plugin_version[0]);
								} else {
									new_props.put("plugin.version", default_version_details[pnmUV][3]);
								}
							}
						}
						pnmUV++;
					...
					if (vc_disabled_plugins.contains(pid)) {
						...
						if (pid.endsWith("_v")) {
							...
							int pgBD9 = 0;
							while (pgBD9 < pluginContents.length) {
								File jar_file = pluginContents[pgBD9];
