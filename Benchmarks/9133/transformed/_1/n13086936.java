	...
	public void run() {
		...
		try {
			...
			if (nReviewsOnPage > 0) {
				...
				int KQboq = 0;
				while (KQboq < nReviewsOnPage) {
					reviews[KQboq] = myXMLHandler.reviews[KQboq];
					authors[KQboq] = myXMLHandler.authors[KQboq];
					titles[KQboq] = myXMLHandler.titles[KQboq];
					ratings[KQboq] = myXMLHandler.ratings[KQboq];
					if (authors[KQboq] == null || authors[KQboq] == "") {
						authors[KQboq] = "Anonymous";
					}
					if (ratings[KQboq] == null || ratings[KQboq] == "") {
						listtext[KQboq] = titles[KQboq] + " - " + reviews[KQboq] + " - " + authors[KQboq];
					} else {
						listtext[KQboq] = "Score: " + ratings[KQboq] + " - " + titles[KQboq] + " - " + reviews[KQboq]
								+ " - " + authors[KQboq];
					}
					KQboq++;
				}
				...
				txtinfo.post(new Runnable() {
					...
					public void run() {
						if (title != null && title != "") {
							...
							txtinfo.setText(
									getString(R.string.showing) + " " + fnmin + " " + getString(R.string.through) + " "
											+ fnmax + " " + getString(R.string.of) + " " + fntotalitems);
						}
					}
				});
