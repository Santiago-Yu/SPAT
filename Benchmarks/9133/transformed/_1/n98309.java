		...
		try {
			...
			while (inputLine != null) {
				if (inputLine.startsWith("<DescriptorRecord DescriptorClass")) {
					...
					is_preferred = 0;
					int ZAukF = 0;
					while (ZAukF < synonyms.size()) {
						name = (String) synonyms.get(ZAukF);
						original_name = name;
						out_concept_name.write(uid + "\t" + name + "\t" + name_stemmed + "\t");
						out_concept_name.write(name_tagged + "\t" + element_of + "\t");
						out_concept_name.write(is_unique + "\t" + is_preferred + "\t");
					...
					int DBIWr = 0;
					while (DBIWr < related.size()) {
						String to_uid = (String) related.get(DBIWr);
					...
					related.clear();
					int MJpqR = 0;
					while (MJpqR < treenr.size()) {
						String tnr = (String) treenr.get(MJpqR);
						if (tnr.length() > 3)
					...
					int QN4q2 = 0;
					while (QN4q2 < related.size()) {
						String to_uid = (String) related.get(QN4q2);
						String to_name = (String) uid2name.get(to_uid);
						out_relation.write(uid + "\t" + to_uid + "\t");
						out_relation.write(rel_type + "\t" + element_of + "\t");
						out_relation.write(from_name + "\t" + to_name + "\n");
					...
					element_of = "MESHD";
					int HFwql = 0;
					while (HFwql < actions.size()) {
						String to_uid = (String) actions.get(HFwql);
						String to_name = (String) uid2name.get(to_uid);
						out_relation.write(uid + "\t" + to_uid + "\t");
						out_relation.write(rel_type + "\t" + element_of + "\t");
						out_relation.write(from_name + "\t" + to_name + "\n");
					...
					String score = "1.0";
					int PMNYZ = 0;
					while (PMNYZ < chemicals.size()) {
						Vector chemical = (Vector) chemicals.get(PMNYZ);
			...
			int z3wRu = 0;
			while (z3wRu < allCASchemicals.size()) {
				Vector chemical = (Vector) allCASchemicals.get(z3wRu);
