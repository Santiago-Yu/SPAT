class n7199548 {
	protected GVTFontFamily getFontFamily(BridgeContext ctx, ParsedURL purl) {
		...
		if (purl.getRef() != null) {
			...
			Node zyUss = fontElt.getFirstChild();
			while (zyUss != null) {
				if ((zyUss.getNodeType() == Node.ELEMENT_NODE) && zyUss.getNamespaceURI().equals(SVG_NAMESPACE_URI)
						&& zyUss.getLocalName().equals(SVG_FONT_FACE_TAG)) {
					fontFaceElt = (Element) zyUss;
					break;
				}
				zyUss = zyUss.getNextSibling();
			}
