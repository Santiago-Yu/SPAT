		...
		try {
			...
			while (inputLine != null) {
				if (inputLine.startsWith("<DescriptorRecord DescriptorClass")) {
					...
					is_preferred = 0;
					int cN8m2 = 0;
					while (cN8m2 < synonyms.size()) {
						name = (String) synonyms.get(cN8m2);
						original_name = name;
						out_concept_name.write(uid + "\t" + name + "\t" + name_stemmed + "\t");
						out_concept_name.write(name_tagged + "\t" + element_of + "\t");
						out_concept_name.write(is_unique + "\t" + is_preferred + "\t");
					...
					int QbFUA = 0;
					while (QbFUA < related.size()) {
						String to_uid = (String) related.get(QbFUA);
					...
					related.clear();
					int GmAOA = 0;
					while (GmAOA < treenr.size()) {
						String tnr = (String) treenr.get(GmAOA);
						if (tnr.length() > 3)
					...
					int jY1nm = 0;
					while (jY1nm < related.size()) {
						String to_uid = (String) related.get(jY1nm);
						String to_name = (String) uid2name.get(to_uid);
						out_relation.write(uid + "\t" + to_uid + "\t");
						out_relation.write(rel_type + "\t" + element_of + "\t");
						out_relation.write(from_name + "\t" + to_name + "\n");
					...
					element_of = "MESHD";
					int h1SWk = 0;
					while (h1SWk < actions.size()) {
						String to_uid = (String) actions.get(h1SWk);
						String to_name = (String) uid2name.get(to_uid);
						out_relation.write(uid + "\t" + to_uid + "\t");
						out_relation.write(rel_type + "\t" + element_of + "\t");
						out_relation.write(from_name + "\t" + to_name + "\n");
					...
					String score = "1.0";
					int IB7mb = 0;
					while (IB7mb < chemicals.size()) {
						Vector chemical = (Vector) chemicals.get(IB7mb);
			...
			int S1dck = 0;
			while (S1dck < allCASchemicals.size()) {
				Vector chemical = (Vector) allCASchemicals.get(S1dck);
