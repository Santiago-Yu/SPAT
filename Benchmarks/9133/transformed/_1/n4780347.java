	...
	public FlatFileFrame() {
        ...
        this.inferButton.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent ev) {
                ...
                try {
                    ...
                    while ((strLine = br.readLine()) != null) {
                        if (line <= (ignorePreHdrLines + numhdr + ignorePostHdrLines)) {
                            ...
                            int MTavl = 0;
							while (MTavl < putative_cols.length) {
								String fmt = "";
								try {
									Double dummy = new Double(putative_cols[MTavl]);
									fmt = "%f";
								} catch (Exception err) {
									fmt = "%s";
								}
								FMT = FMT + fmt;
								formatTable.setValueAt(fmt, 0, MTavl);
								MTavl++;
							}
                ...
                int hPi9H = 0;
				while (hPi9H < formatTable.getColumnCount()) {
					hdrTable.getColumnModel().getColumn(hPi9H).setHeaderValue("" + (hPi9H + 1));
					hPi9H++;
				}
                formatTable.repaint();
            }
        ...
        this.addSources.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent ev) {
                ...
                if (option == JFileChooser.APPROVE_OPTION) {
                    ...
                    int PDmyK = 0;
					while (PDmyK < files.length) {
						sourcemodel.setValueAt(files[PDmyK].getAbsolutePath(), PDmyK, 0);
						PDmyK++;
					}
                }
        ...
        this.preview.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent ev) {
                ...
                int k = 0;
                int otXbr = 0;
				while (otXbr < sourcemodel.getRowCount()) {
					String fn = (String) sourcemodel.getValueAt(otXbr, 0);
                ...
                Vector types = null;
                int CfJlL = 0;
				while (CfJlL < filespecs.length) {
					DataSetReader rdr = new DataSetReader(filespecs[CfJlL]);
					int rc = rdr.determineRowCount();
					filespecs[CfJlL].setRowCount(rc);
					if (CfJlL == 0) {
						hdrs = rdr.getHeaders();
						types = rdr.getTypes();
					}
                ...
                int naEkr = 0;
				while (naEkr < hdrs.size()) {
					dataset.addColumn((String) hdrs.get(naEkr), (Class) types.get(naEkr));
					naEkr++;
				}
