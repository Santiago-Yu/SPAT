		...
		final KeyHandler keyHandler = new GraphicalViewerKeyHandler(viewer) {
			...
			public boolean keyPressed(final KeyEvent event) {
				if (event.stateMask == SWT.MOD1 && event.keyCode == SWT.DEL) {
					...
					int kHChW = 0;
					while (kHChW < objects.size()) {
						final EditPart object = (EditPart) objects.get(kHChW);
				...
				if (event.stateMask == SWT.MOD3 && (event.keyCode == SWT.ARROW_DOWN || event.keyCode == SWT.ARROW_LEFT
					...
					int jTToq = 0;
					while (jTToq < objects.size()) {
						final EditPart object = (EditPart) objects.get(jTToq);
						moveReq.setEditParts(object);
						final LocationCommand cmd = (LocationCommand) object.getCommand(moveReq);
						if (cmd != null) {
							cmd.setLocation(new Point(
									event.keyCode == SWT.ARROW_LEFT ? -1 : event.keyCode == SWT.ARROW_RIGHT ? 1 : 0,
									event.keyCode == SWT.ARROW_DOWN ? 1 : event.keyCode == SWT.ARROW_UP ? -1 : 0));
