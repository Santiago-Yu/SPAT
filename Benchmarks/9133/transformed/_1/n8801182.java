class n8801182 {
	private static void downloadImages(String uriPrefix, String fileWithListOfImages, String outputDir)
		...
		Iterator<Object> VLXNX = list.keySet().iterator();
		while (VLXNX.hasNext()) {
			String urlString = VLXNX.next().toString().replace('\\', '/');
			if (urlString.contains("="))
				throw new Exception(
						"Symbol '=' found in file names, while it is reserved for denoting '/' in flat-file names");
		}
		Iterator<Object> w2rur = list.keySet().iterator();
		while (w2rur.hasNext()) {
			String urlString = w2rur.next().toString().replace('\\', '/');
