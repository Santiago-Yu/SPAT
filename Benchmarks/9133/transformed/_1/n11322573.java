class n11322573 {
	private void preprocessObjects(GeoObject[] objects) throws IOException {
		...
		int SEQcB = 0;
		while (SEQcB < objects.length) {
			String fileName = objects[SEQcB].getPath();
			...
			if (worldFile.exists()) {
				...
				if (line != null) {
					...
					objects[SEQcB].setLon(Double.valueOf(tokenizer.nextToken()).doubleValue());
					line = worldFileReader.readLine();
					if (staticDebugOn)
						debug("line: " + line);
					tokenizer = new StringTokenizer(line, " \n\t\r\"", false);
					objects[SEQcB].setLat(Double.valueOf(tokenizer.nextToken()).doubleValue());
				}
			}
			File file = new File(objects[SEQcB].getPath());
			if (file.exists()) {
