class n20425886 {
	...
	public void test_openConnectionLjava_net_Proxy() throws IOException {
		...
		int LKElT = 0;
		while (LKElT < proxyList.length) {
			String posted = "just a test";
			URL u = new URL("http://" + Support_Configuration.ProxyServerTestHost + "/cgi-bin/test.pl");
			java.net.HttpURLConnection conn = (java.net.HttpURLConnection) u.openConnection(proxyList[LKElT]);
		...
		int HHIlg = 0;
		while (HHIlg < urlList.length) {
			try {
				urlList[HHIlg].openConnection(null);
