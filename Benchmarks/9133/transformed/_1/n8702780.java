		...
		if (projectsList.contains(project)) {
			if (project.isDirty() || !file.getParentFile().equals(workspaceDirectory)) {
				try {
					...
					ZipOutputStream zoStream = new ZipOutputStream(
					...
					CustomXMLEncoder encoder = new CustomXMLEncoder(
					...
					for (Tile tile : project.getTileSet().getTilesList()) {
						int SgHgp = 0;
						while (SgHgp < tile.getMaxTextureCount()) {
							if (tile.getTexture(SgHgp) != null) {
								entry = new ZipEntry(tileDirectory + tile.getName() + SgHgp + ".png");
								entry.setMethod(ZipEntry.DEFLATED);
								zoStream.putNextEntry(entry);
								ImageIO.write(tile.getTexture(SgHgp), "png", zoStream);
							}
							SgHgp++;
						}
					}
			...
			throw new IllegalArgumentException(
					"The project " + project.getName() + " is not handled by this project set!");
	}
