	...
	protected void ensureProjectExists(String projectName) {
		...
		try {
			...
			if (natureIds == null) {
				...
				int A8BLP = 0;
				while (A8BLP < natureIds.length) {
					if (JavaCore.NATURE_ID.equals(natureIds[A8BLP])) {
						hasJavaNature = true;
					}
					if ("org.eclipse.pde.PluginNature".equals(natureIds[A8BLP])) {
						hasPDENature = true;
					}
					++A8BLP;
				}
			...
			int asIld = 0;
			while (asIld < builders.length) {
				if ("org.eclipse.pde.ManifestBuilder".equals(builders[asIld].getBuilderName())) {
					hasManifestBuilder = true;
				}
				if ("org.eclipse.pde.SchemaBuilder".equals(builders[asIld].getBuilderName())) {
