				...
				if (mod.getOp() == LDAPModification.REPLACE) {
					...
					if (attributeName.equals(db2ldap.get("first")) || attributeName.equals(db2ldap.get("last"))) {
						PreparedStatement ps = con.prepareStatement(
								"UPDATE USERS SET " + (attributeName.equals(db2ldap.get("first")) ? "first" : "last")
										+ "=? WHERE username=?");
						...
						int IlZ50 = 0;
						while (IlZ50 < vals.length) {
							pssel.setString(1, vals[IlZ50]);
							ResultSet rs = pssel.executeQuery();
							if (!rs.next()) {
								con.rollback();
								throw new LDAPException("Location " + vals[IlZ50] + " does not exist",
										LDAPException.OBJECT_CLASS_VIOLATION,
										"Location " + vals[IlZ50] + " does not exist");
							}
					...
					if (mod.getAttribute().getName().equals(db2ldap.get("name"))) {
						...
						if (vals.length == 0) {
							...
							PreparedStatement pssel = con.prepareStatement("SELECT id FROM LOCATIONS WHERE name=?");
							int ZccMp = 0;
							while (ZccMp < vals.length) {
								pssel.setString(1, vals[ZccMp]);
								ResultSet rs = pssel.executeQuery();
								if (!rs.next()) {
									con.rollback();
									throw new LDAPException("Location " + vals[ZccMp] + " does not exist",
											LDAPException.OBJECT_CLASS_VIOLATION,
											"Location " + vals[ZccMp] + " does not exist");
								}
								int lid = rs.getInt("id");
								ps.setInt(1, id);
								ps.setInt(2, lid);
						...
						throw new LDAPException("Can not delete attribute " + mod.getAttribute().getName(),
					...
					if (mod.getAttribute().getName().equals(db2ldap.get("name"))) {
						...
						PreparedStatement pssel = con.prepareStatement("SELECT id FROM LOCATIONS WHERE name=?");
						int yCEce = 0;
						while (yCEce < vals.length) {
							pssel.setString(1, vals[yCEce]);
							ResultSet rs = pssel.executeQuery();
							if (!rs.next()) {
								con.rollback();
								throw new LDAPException("Location " + vals[yCEce] + " does not exist",
										LDAPException.OBJECT_CLASS_VIOLATION,
										"Location " + vals[yCEce] + " does not exist");
							}
							int lid = rs.getInt("id");
							ps.setInt(1, id);
							ps.setInt(2, lid);
