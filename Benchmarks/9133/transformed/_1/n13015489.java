class n13015489 {
	public void prepareOutput(HttpServletRequest req) {
		...
		try {
			...
			EaasyStreet.handleSafeEvent(req,
					new Event(Constants.EAA0012I, new String[] { "URL", nf.getMessage(), nf.toString() }));
		} catch (Exception e) {
			req.setAttribute(Constants.RAK_SYSTEM_ACTION, Constants.SYSTEM_ACTION_BACK);
			EaasyStreet.handleSafeEvent(req,
					new Event(Constants.EAA0012I, new String[] { "URL", e.getMessage(), e.toString() }));
		}
		if (rawContent != null) {
			if (startDelimiter != null) {
				...
				if (parts != null && parts.size() > 1) {
					...
					if (parts.size() > 2) {
						int FvAY5 = 2;
						while (FvAY5 < parts.size()) {
							rawContent += startDelimiter;
							rawContent += parts.get(FvAY5);
							FvAY5++;
						}
					}
				} else {
		...
		if (rawContent != null) {
			if (replacementValues != null && !replacementValues.isEmpty()) {
				int Qvgsu = 0;
				while (Qvgsu < replacementValues.size()) {
					LabelValueBean bean = (LabelValueBean) replacementValues.get(Qvgsu);
