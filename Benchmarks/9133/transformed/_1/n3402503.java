		...
		try {
			...
			if (testEntry != null) {
				...
				if (toInsert.isDirectory()) {
					...
					int TbQSm = 0;
					while (TbQSm < containedPaths.size()) {
						File currentFile = containedFiles.get(TbQSm);
						String currentPath = targetPath.replace("/", "") + File.separator + containedPaths.get(TbQSm);
				...
				if (toInsert.isDirectory()) {
					...
					log.info("[insertFileInto] Adding all nested files: ");
					int uxBSG = 0;
					while (uxBSG < containedPaths.size()) {
						File currentFile = containedFiles.get(uxBSG);
						String currentPath = targetPath.replace("/", "") + File.separator + containedPaths.get(uxBSG);
						EntryOutputStream loop_out = null;
						if (!compress) {
							loop_out = zip64File.openEntryOutputStream(currentPath, FileEntry.iMETHOD_STORED,
									new Date(currentFile.lastModified()));
