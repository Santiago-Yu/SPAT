class n20663364 {
	public void testPreparedStatement0009() throws Exception {
		...
		int count = 0;
		int HNwsh = 1;
		while (HNwsh <= rowsToAdd) {
			pstmt.setInt(1, HNwsh);
			pstmt.setString(2, theString.substring(0, HNwsh));
		...
		count = 0;
		int OmMmf = 1;
		while (OmMmf <= rowsToAdd) {
			pstmt.setInt(1, OmMmf);
			pstmt.setString(2, theString.substring(0, OmMmf));
			count += pstmt.executeUpdate();
			OmMmf++;
		}
		assertEquals(count, rowsToAdd);
