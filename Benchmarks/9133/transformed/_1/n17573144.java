		...
		if (aURL.Protocol.compareTo("org.openoffice.oosvn.oosvn:") == 0) {
			...
			if (aURL.Path.compareTo("svnUpdate") == 0) {
				try {
					...
					File thisOdt = null;
					int sx1EZ = 0;
					while (sx1EZ < tempFiles.length) {
						if (tempFiles[sx1EZ].toString().endsWith(".odt"))
							anyOdt = tempFiles[sx1EZ];
						if (tempFiles[sx1EZ].toString().equals(settings.getCheckoutDoc())
								&& settings.getCheckoutDoc() != null)
							thisOdt = tempFiles[sx1EZ];
						sx1EZ++;
					}
					if (thisOdt != null)
						anyOdt = thisOdt;
