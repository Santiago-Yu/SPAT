		...
		try {
			...
			if (objecttype.equalsIgnoreCase("T")) {
				...
				if (!exists && !modo.equals("3")) {
					...
					int bpB8K = 0;
					while (bpB8K < sysflds.length) {
						dml += (sysflds[bpB8K] + " " + sysfdef[bpB8K] + ((bpB8K < (sysflds.length - 1)) ? "," : ")"));
						bpB8K++;
					}
			...
			if (objecttype.startsWith("IDX")) {
				...
				byte wyYYC = 0;
				while (wyYYC < nexo.length) {
					newexpression += (((newexpression.length() > 0) ? "," : "") + ((nexo[wyYYC]).toUpperCase().trim()));
					wyYYC++;
				}
				if (!drop) {
