class n4404642 {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		try {
			...
			int bk3JG = 0;
			while (bk3JG < nameArray.length()) {
				tempStringID = nameArray.getJSONObject(bk3JG).getString("ID");
				tempStringName = nameArray.getJSONObject(bk3JG).getString("Name");
				tempStringPrice = nameArray.getJSONObject(bk3JG).getString("Price");
				tempStringSymbol = nameArray.getJSONObject(bk3JG).getString("Symbol");
				this.dm.insertIntoProducts(tempStringID, tempStringName, tempStringPrice, tempStringSymbol);
				tempString = nameArray.getJSONObject(bk3JG).getString("Name") + "\n"
						+ nameArray.getJSONObject(bk3JG).getString("Price") + "\n"
						+ nameArray.getJSONObject(bk3JG).getString("Symbol");
		...
		try {
			...
			int B4SsX = 0;
			while (B4SsX < nameArray1.length()) {
				tempStringAgent = nameArray1.getJSONObject(B4SsX).getString("Agent");
				tempStringClient = nameArray1.getJSONObject(B4SsX).getString("Client");
				tempStringRoute = nameArray1.getJSONObject(B4SsX).getString("Route");
				tempStringZone = nameArray1.getJSONObject(B4SsX).getString("Zone");
				this.dm.insertIntoClients(tempStringAgent, tempStringClient, tempStringRoute, tempStringZone);
				tempString1 = nameArray1.getJSONObject(B4SsX).getString("Client") + "\n"
						+ nameArray1.getJSONObject(B4SsX).getString("Route") + "\n"
						+ nameArray1.getJSONObject(B4SsX).getString("Zone");
