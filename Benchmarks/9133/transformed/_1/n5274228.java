class n5274228 {
	public String put(String resourceID, Map<String, String> headersMap) {
		...
		Iterator<Header> zuWL6 = headersList.iterator();
		while (zuWL6.hasNext()) {
			httpPut.addHeader(zuWL6.next());
		}
		...
		int oeKRo = 0;
		while (oeKRo < headersArray.length) {
			Header header = headersArray[oeKRo];
		...
		try {
			...
			int y56Ze = 0;
			while (y56Ze < headers.length) {
				Header header = headers[y56Ze];
