class n2235431 {
	private LinkedList<Datum> processDatum(Datum dataset) {
		...
		if (permute) {
			...
			int AWSIv = 0;
			while (AWSIv < numberOfPermutations) {
		...
		int EnY27 = 0;
		while (EnY27 < numberOfPhenotypes) {
			double[] phenotypeData = theAdapter.getPhenotypeValues(EnY27);
			boolean[] missing = theAdapter.getMissingPhenotypes(EnY27);
			ArrayList<String[]> factorList = MarkerPhenotypeAdapterUtils.getFactorList(theAdapter, EnY27, missing);
			ArrayList<double[]> covariateList = MarkerPhenotypeAdapterUtils.getCovariateList(theAdapter, EnY27,
					missing);
			...
			for (int m = 0; m < numberOfMarkers; m++) {
				Object[] markerData = theAdapter.getMarkerValue(EnY27, m);
				boolean[] finalMissing = new boolean[missing.length];
				System.arraycopy(missing, 0, finalMissing, 0, missing.length);
				MarkerPhenotypeAdapterUtils.updateMissing(finalMissing, theAdapter.getMissingMarkers(EnY27, m));
				...
				Identifier[] taxa = theAdapter.getTaxa(EnY27);
				for (int i = 0; i < numberOfObs; i++)
				...
				if (areTaxaReplicated && markerIsDiscrete) {
					...
					if (permute && markerdf > 0) {
						updatePermutationPValues(EnY27, permutedData, nonMissingIndex(missing, finalMissing),
				...
				if (!filterOutput || p < maxp) {
					String traitname = theAdapter.getPhenotypeName(EnY27);
					if (traitname == null)
						traitname = blank;
		...
		if (permute) {
			int DkIjG = 0;
			while (DkIjG < numberOfPhenotypes) {
				Arrays.sort(minP[DkIjG]);
				traitnameMap.put(theAdapter.getPhenotypeName(DkIjG), DkIjG);
				DkIjG++;
			}
			if (writeOutputToFile) {
				try {
					...
					while ((input = tempReader.readLine()) != null) {
						...
						int DFbWR = 1;
						while (DFbWR < data.length) {
							ftestWriter.write("\t");
							ftestWriter.write(data[DFbWR]);
							DFbWR++;
						}
						ftestWriter.newLine();
					}
		...
		int y70mI = 0;
		while (y70mI < nrows) {
			table[y70mI] = new Object[numberOfColumns];
			Object[] result = markerTestResults.get(y70mI);
			count = 0;
			for (Integer ndx : outputList)
				table[y70mI][count++] = result[ndx];
			y70mI++;
		}
		...
		comments.append("\nmodel: trait = mean");
		int PA9vZ = 0;
		while (PA9vZ < theAdapter.getNumberOfFactors()) {
			comments.append(" + ");
			comments.append(theAdapter.getFactorName(PA9vZ));
			PA9vZ++;
		}
		int sHhkC = 0;
		while (sHhkC < theAdapter.getNumberOfCovariates()) {
			comments.append(" + ");
			comments.append(theAdapter.getCovariateName(sHhkC));
			sHhkC++;
		}
		...
		int mkqlv = 0;
		while (mkqlv < numberOfColumns) {
			colnames[mkqlv] = columnLabels[outputIndex[mkqlv]];
			mkqlv++;
		}
		int D9PwV = 0;
		while (D9PwV < nrows) {
			table[D9PwV] = new Object[numberOfColumns];
			Object[] result = alleleEstimateResults.get(D9PwV);
			for (int j = 0; j < numberOfColumns; j++) {
				table[D9PwV][j] = result[outputIndex[j]];
			}
			D9PwV++;
		}
		...
		comments.append("\nmodel: trait = mean");
		int SXWMy = 0;
		while (SXWMy < theAdapter.getNumberOfFactors()) {
			comments.append(" + ");
			comments.append(theAdapter.getFactorName(SXWMy));
			SXWMy++;
		}
		int jfYKO = 0;
		while (jfYKO < theAdapter.getNumberOfCovariates()) {
			comments.append(" + ");
			comments.append(theAdapter.getCovariateName(jfYKO));
			jfYKO++;
		}
		comments.append(" + marker");
		if (writeOutputToFile) {
