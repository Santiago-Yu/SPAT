class n9581835 {
	public static void main(String[] args) {
		...
		int mxl9n = 0;
		while (mxl9n < wtklib.length) {
			preverifyCmd += File.pathSeparator + WTKdir + File.separator + "lib" + wtklib[mxl9n];
			mxl9n++;
		}
		...
		int sYdfj = 0;
		while (sYdfj < listOfFiles.length) {
			strFiles[sYdfj] = listOfFiles[sYdfj].toString().substring(l);
			strFiles[sYdfj] = strFiles[sYdfj].replace(File.separatorChar, '/');
			System.out.println(strFiles[sYdfj]);
			sYdfj++;
		}
		System.out.println("-------------------------------");
		try {
			...
			nextJar: {
				JarEntry ZV1jv = jis.getNextJarEntry();
				while (ZV1jv != null) {
					String s = ZV1jv.getName();
					for (int k = 0; k < strFiles.length; k++) {
						if (strFiles[k].equals(s))
							continue nextJar;
					}
					jos.putNextEntry(ZV1jv);
					byte[] b = new byte[512];
					for (int k = jis.read(b, 0, 512); k >= 0; k = jis.read(b, 0, 512)) {
						jos.write(b, 0, k);
					}
					ZV1jv = jis.getNextJarEntry();
				}
			}
			jis.close();
			int DcB0q = 0;
			while (DcB0q < strFiles.length) {
				FileInputStream fis = new FileInputStream(listOfFiles[DcB0q]);
				JarEntry je = new JarEntry(strFiles[DcB0q]);
				jos.putNextEntry(je);
				byte[] b = new byte[512];
