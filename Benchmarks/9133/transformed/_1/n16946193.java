class n16946193 {
	public void deploy(final File extension) {
		...
		try {
			JarEntry F01jE = warIn.getNextJarEntry();
			while (F01jE != null) {
				File inflated = new File(F01jE.getName().startsWith(infPrefix) ? extensionDir : extensionWebDir,
						F01jE.getName());
				if (F01jE.isDirectory()) {
				...
				if (!F01jE.getName().endsWith(configurationFileExtension)) {
				...
				try {
					final Document document = reader
							.read(new TeeInputStream(new CloseShieldInputStream(warIn), entryOut, true));
