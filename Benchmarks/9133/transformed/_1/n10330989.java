class n10330989 {
	public DialogueSymbole(final JFrame jframe, final Element el, final String srcAttr) {
        ...
        int KNB3w = 0;
		while (KNB3w < liste.length) {
			if (nomf != null && !"".equals(nomf) && nomf.equals(liste[KNB3w].getPath()))
				ichoix = KNB3w;
			URL urlIcone;
			try {
				if (applet) {
					final URL urlListe = DialogueSymbole.class.getClassLoader().getResource("symboles/liste.txt");
					final String baseURL = urlListe.toString().substring(0,
							urlListe.toString().indexOf("symboles/liste.txt"));
					urlIcone = new URL(baseURL + liste[KNB3w].getPath());
				} else
					urlIcone = liste[KNB3w].toURL();
