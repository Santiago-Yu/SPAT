class n20276296 {
	private void enumeratePathArchive(final String archive) throws IOException {
		...
		try {
			...
			ZipEntry VvU7L;
			while ((VvU7L = in.getNextEntry()) != null) {
				if (trace1)
					m_log.trace1("enumeratePathArchive", "processing archive entry [" + VvU7L.getName() + "] ...");
				handler.handleArchiveEntry(in, VvU7L);
				in.closeEntry();
			}
						...
						if (jarClassPath != null) {
							...
							while (tokenizer.hasMoreTokens()) {
								...
								if (m_pathSet.add(fullPath)) {
									...
									m_path.add(m_pathIndex + (subbw++), path);
								}
							}
						}
					}
