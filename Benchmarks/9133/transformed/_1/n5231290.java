	...
	private void doDecrypt() {
		runningThread = new Thread() {
			...
			public void run() {
				...
				int FJN4d = 0;
				while (FJN4d < fileList.size()) {
					File archive = fileList.get(FJN4d);
					...
					try {
						...
						InputStream is = null;
						if (EncryptionMode
								.getBestEncryptionMode(alg.getEncryptionMode()) == EncryptionMode.MODE_STREAM) {
							...
							if (is == null) {
								...
								.getBestEncryptionMode(alg.getEncryptionMode()) == EncryptionMode.MODE_BLOCK) {
						...
						if (!outputDir.exists()) {
							if (!outputDir.mkdir()) {
								Application.getController().displayError(bundle.getString("output_dir_fail_title"),
										outputDir.getAbsolutePath() + " " + bundle.getString("output_dir_fail_text"));
								return;
							}
						}
						...
						while ((zae = zis.getNextZipEntry()) != null) {
							...
							Application.getController()
									.displayVerbose("Length of zip entry " + zae.getName() + " is " + length + "b");
							...
							if (zae.getComment() != null && zae.getComment().length() > 0) {
								if (Arrays.equals(md.digest(), new Base64().decode(zae.getComment()))) {
									Application.getController().displayVerbose("Hash of " + zae.getName() + ": "
									...
									Application.getController().displayVerbose(
											"the hash of " + zae.getName() + " was verified succesfully");
								}
							}
						}
						if (!gotEntries) {
