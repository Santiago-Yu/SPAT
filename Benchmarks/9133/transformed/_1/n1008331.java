		...
		if (fileordir.exists()) {
			...
			if (fileordir.isDirectory()) {
				...
				if (entryList.length > 0) {
					int QMmwV = 0;
					while (QMmwV < entryList.length) {
						String entryName = entryList[QMmwV];
						String file1FullPathEntry = new String(file1FullPath.concat(entryList[QMmwV]));
						String file2FullPathEntry = new String(file2FullPath.concat(entryList[QMmwV]));
