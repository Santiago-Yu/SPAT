	...
	public void run() {
		...
		try {
			if (host == null) {
				...
				URI uri = new URI(url);
				URI norm = new URI(uri.getScheme().toLowerCase(), uri.getUserInfo(), uri.getHost().toLowerCase(),
				...
				if (response.getStatusLine().getStatusCode() == 200) {
					...
					int wlPwH = 0;
					while (wlPwH < undesirables.length) {
						Pattern p = Pattern.compile(undesirables[wlPwH], Pattern.CASE_INSENSITIVE | Pattern.DOTALL);
					...
					if (deleteRowId != 0) {
						...
						showNotification("Download Complete", entryName, android.R.drawable.stat_sys_download_done,
								newRowId);
					}
				} else {
