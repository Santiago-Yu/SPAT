class n733283 {
	private boolean setupDatabase() {
		...
		Connection conn = getDBConnection(installer.getPachyDBHost(), installer.getPachyDBPort(),
		...
		if (havePachy20) {
			Object[] options = { Pachyderm21Installer.ISTRINGS.getString("dialog.abort"),
					Pachyderm21Installer.ISTRINGS.getString("dialog.keep"),
					Pachyderm21Installer.ISTRINGS.getString("dialog.overwrite") };
			int result = JOptionPane.showOptionDialog(this.installer,
			...
			Object[] options = { Pachyderm21Installer.ISTRINGS.getString("dialog.continue"),
		...
		try {
			...
			if (DEBUG)
				System.out.println("Changing admin password, " + "numUpdates = " + numupdates);
			Vector<AdminData> v = installer.getAdditionalAdminAccounts();
			String customPropertiesSPFPre = "{\n \"CXMultiValueArchive\" = {" + "\n  \"class\" = "
					+ "\"ca.ucalgary.apollo.core." + "CXMutableMultiValue\";\n  " + "\"values\" = (\n   "
					+ "{\n    \"class\" = \"ca.ucalgary." + "apollo.core.CXMultiValue$Value\";\n"
					+ "    \"identifier\" = \"0\";\n    " + "\"label\" = \"work\";\n    " + "\"value\" = \"";
			String customPropertiesSPFPost = "\";\n   }\n  );\n  \"identCounter\" = \"1\";\n };\n}";
			if (v.size() > 0) {
				ps = conn.prepareStatement("INSERT INTO `APPERSON` VALUES " + "(NULL,NULL,NULL,NOW(),NULL,NULL,"
				...
				ps2 = conn.prepareStatement("INSERT INTO `AUTHMAP` " + "(external_id,external_realm,"
			...
			int gponX = 0;
			while (gponX < v.size()) {
				AdminData ad = (AdminData) v.elementAt(gponX);
				...
				if (numupdates == 1) {
					...
					ps2.setInt(2, gponX + 2);
					ps2.setInt(3, gponX + 2);
					ps2.executeUpdate();
					ps3.setInt(1, gponX + 2);
					ps3.executeUpdate();
				}
				++gponX;
			}
