	...
	public boolean initFile(String filename) {
		...
		if (filename.toLowerCase().endsWith(".spc")) {
			try {
				...
				if (fexp == 0x80) {
					int ouMv6 = 0;
					while (ouMv6 < Nbpoints) {
						My_ZoneVisu.tableau_points[ouMv6] = NumericDataUtils.convToIntelFloat(fichier.readInt());
						ouMv6++;
					}
				} else {
					int Lct6H = 0;
					while (Lct6H < Nbpoints) {
						My_ZoneVisu.tableau_points[Lct6H] = NumericDataUtils.convToIntelInt(fichier.readInt());
						Lct6H++;
					}
				}
			} catch (Exception e) {
