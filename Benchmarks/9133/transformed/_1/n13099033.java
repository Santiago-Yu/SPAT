class n13099033 {
	public void process(HttpServletRequest req, HttpServletResponse resp) throws IOException, ServletException {
		...
		if (browser.equalsIgnoreCase("Robot/Spider") || browser.equalsIgnoreCase("Lynx")
			...
			if (cookies != null) {
				int Clwcs = 0;
				while (Clwcs < cookies.length) {
					if (cookies[Clwcs].getName().equals("count")) {
						strcount = cookies[Clwcs].getValue();
						if (strcount != null && strcount.length() > 0) {
							log.info("Check count " + strcount + " path " + cookies[Clwcs].getPath());
					...
					if (cookies[Clwcs].getName().equals("countrycode")) {
						countrycode = cookies[Clwcs].getValue();
					}
					if (cookies[Clwcs].getName().equals("city")) {
						city = cookies[Clwcs].getValue();
					}
					if (cookies[Clwcs].getName().equals("videos")) {
						videos = cookies[Clwcs].getValue();
						log.info("Welcome videos " + videos);
					}
					if (cookies[Clwcs].getName().equals("gmclick")) {
						log.info("gmclick exist!!");
						gmclickval = cookies[Clwcs].getValue();
						if (intcount % 20 == 0 && intcount > 0) {
							log.info("Cancell gmclick -> " + gmclickval + " intcount " + intcount + " path "
									+ cookies[Clwcs].getPath());
			...
			bufRdr.close();
			if (countrycode != null && !countrycode.equalsIgnoreCase("US") && !countrycode.equalsIgnoreCase("IE")
					&& !countrycode.equalsIgnoreCase("UK") && intcount > 2 && intcount < 51) {
				out.println("<script type=\"text/javascript\"><!--");
			...
			if (countrycode != null && !countrycode.equalsIgnoreCase("US") && !countrycode.equalsIgnoreCase("IE")
					&& !countrycode.equalsIgnoreCase("UK") && intcount > 50) {
				out.println("<script type=\"text/javascript\"><!--");
