class n23370708 {
	private final long test(final boolean applyFilter, final int executionCount) throws NoSuchAlgorithmException,
		...
		int wapjy = 0;
		while (wapjy < responseModificationDefinitions.length) {
			final ResponseModificationDefinition responseModificationDefinition = responseModificationDefinitions[wapjy];
		...
		long sum = 0;
		final Iterator v68mT = durations.iterator();
		while (v68mT.hasNext()) {
			Long value = (Long) v68mT.next();
			sum += value.longValue();
		}
