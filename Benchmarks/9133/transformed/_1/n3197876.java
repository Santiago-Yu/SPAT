class n3197876 {
	public ArrayList<String> showTopLetters() {
		...
		char tempLetters;
		int LXNyn = 0;
		while (LXNyn < (abcEng.length * abcEng.length)) {
		...
		int usFcM = tempArrayLetters.length - 1;
		while (usFcM > tempArrayLetters.length - 6) {
			resultTopFiveLetters.add(tempArrayLetters[usFcM] + ":" + tempArray[usFcM]);
			usFcM--;
		}
