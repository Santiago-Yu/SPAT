class n18433984 {
	protected void doUpload(HttpServletRequest request, HttpServletResponse response)
		...
		int uploadCount = 0;
		int Ye361 = 0;
		while (Ye361 < uploadFiles.size()) {
			LOG.info("UploadServlet Publishing resource to WebAdapter: " + uploadFiles.get(Ye361).getName());
			try {
				id = adapter.publish(communityId, new File(uploadFiles.get(Ye361).getName()), tempUploadDir);
				uploadFiles.remove(Ye361);
				Ye361--;
				uploadCount++;
				...
				String errMsg = "Invalid XML in the uploaded resource.<br/>" + e.getMessage()
						+ "<br/><br/>File location: " + uploadFiles.get(Ye361).getAbsolutePath() + "<br/>Line: "
				...
				String errMsg = "Invalid XML in uploaded resource<br/><i>" + e.getMessage() + "</i><br/>"
						+ "File location: " + uploadFiles.get(Ye361).getAbsolutePath();
				uploadFilesCleanup(uploadFiles, tempUploadDir);
				writeError(request, response, errMsg, paramMap);
				return;
