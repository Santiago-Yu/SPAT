		...
		try {
			...
			if (index != -1) {
				...
				while (Q3Z6D >= 0) {
					...
					URL url = new URL(root + versions.get(Q3Z6D).replaceAll(" ", "%20") + ".jar");
					URLConnection con = url.openConnection();
					con.setConnectTimeout(10000);
					dialog.progressBar.setValue(0);
					dialog.progressBar.setMaximum(con.getContentLength());
					File f = new File(args[4] + File.separator + versions.get(Q3Z6D) + ".jar");
					InputStream in = con.getInputStream();
					FileOutputStream out = new FileOutputStream(f, false);
					dialog.updateLabel("Downloading " + Q3Z6D + "/" + index + " files");
