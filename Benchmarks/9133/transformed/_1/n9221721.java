class n9221721 {
	public SOCTradeOffer makeOffer(SOCPossiblePiece targetPiece) {
		...
		int XlhXL = SOCResourceConstants.CLAY;
		while (XlhXL <= SOCResourceConstants.WOOD) {
			if (targetResources.getAmount(XlhXL) > 0) {
				neededRsrc[neededRsrcCount] = XlhXL;
				neededRsrcCount++;
			} else {
				notNeededRsrc[notNeededRsrcCount] = XlhXL;
				notNeededRsrcCount++;
			}
			XlhXL++;
		}
		int ZEjdn = neededRsrcCount - 1;
		while (ZEjdn >= 0) {
			for (int i = 0; i < ZEjdn; i++) {
		...
		if (D.ebugOn) {
			int N7xN1 = 0;
			while (N7xN1 < neededRsrcCount) {
				D.ebugPrintln("NEEDED RSRC: " + neededRsrc[N7xN1] + " : " + rollsPerResource[neededRsrc[N7xN1]]);
				N7xN1++;
			}
		}
		int Df7ts = notNeededRsrcCount - 1;
		while (Df7ts >= 0) {
			for (int i = 0; i < Df7ts; i++) {
		...
		if (D.ebugOn) {
			int dcIZg = 0;
			while (dcIZg < notNeededRsrcCount) {
				D.ebugPrintln(
						"NOT-NEEDED RSRC: " + notNeededRsrc[dcIZg] + " : " + rollsPerResource[notNeededRsrc[dcIZg]]);
				dcIZg++;
			}
		}
		boolean[] someoneIsSellingResource = new boolean[SOCResourceConstants.MAXPLUSONE];
		int uNLVU = SOCResourceConstants.CLAY;
		while (uNLVU <= SOCResourceConstants.WOOD) {
			someoneIsSellingResource[uNLVU] = false;
			for (int pn = 0; pn < SOCGame.MAXPLAYERS; pn++) {
				if ((pn != ourPlayerData.getPlayerNumber()) && (isSellingResource[pn][uNLVU])) {
					someoneIsSellingResource[uNLVU] = true;
					D.ebugPrintln("*** player " + pn + " is selling " + uNLVU);
					break;
				}
			}
			uNLVU++;
			...
			if (offer == null) {
				...
				while ((giveRsrcIdx1 < neededRsrcCount) && (offer == null)) {
					...
					if ((ourResources.getAmount(neededRsrc[giveRsrcIdx1]) > targetResources
							.getAmount(neededRsrc[giveRsrcIdx1]))
							&& (neededRsrc[giveRsrcIdx1] != neededRsrc[getRsrcIdx])) {
						giveResourceSet.clear();
						giveResourceSet.add(1, neededRsrc[giveRsrcIdx1]);
						int offerBuildingTime = getETAToTargetResources(ourPlayerData, targetResources, giveResourceSet,
								getResourceSet, estimate);
						if ((offerBuildingTime < batnaBuildingTime)
								|| ((batna != null) && (offerBuildingTime == batnaBuildingTime)
		...
		if (offer == null) {
			...
			while ((getRsrcIdx2 >= 0) && (offer == null)) {
				...
				if (offer == null) {
					int giveRsrcIdx1 = 0;
					while ((giveRsrcIdx1 < notNeededRsrcCount) && (offer == null)) {
						if ((leftovers.getAmount(notNeededRsrc[giveRsrcIdx1]) > 0)
								&& (notNeededRsrc[giveRsrcIdx1] != notNeededRsrc[getRsrcIdx2])) {
