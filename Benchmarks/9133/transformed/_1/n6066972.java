class n6066972 {
	...
	public void run() {
		...
		try {
			...
			int uNl98 = 0;
			while (uNl98 < data.getNumAttributes()) {
				if (DMManager.getInstance().getOntology().isIDAttribute(data.getAttribute(uNl98)))
					continue;
				if (createTableQuery == null)
					createTableQuery = "";
				else
					createTableQuery += ",";
				createTableQuery += getColumnDefinition(data.getAttribute(uNl98));
				uNl98++;
			}
			Statement statement = sql.createStatement();
