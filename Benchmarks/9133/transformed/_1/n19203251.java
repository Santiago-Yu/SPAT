class n19203251 {
	private void readArchives(final VideoArchiveSet vas) throws IOException {
		...
		if (archives.size() == 0) {
			...
			int xqfci = 0;
			while (xqfci < allYeardayFiles.length) {
				if (allYeardayFiles[xqfci].toString().endsWith(".vif")) {
					String filename = allYeardayFiles[xqfci].toString()
							.substring(allYeardayFiles[xqfci].toString().lastIndexOf("/"));
					String fileLC = filename.toLowerCase();
					String toLookFor = new String(new Character(vifMetadata.shipCode).toString()
							+ new Character(vifMetadata.platformCode).toString());
					String toLookForLC = toLookFor.toLowerCase();
					if (fileLC.indexOf(toLookForLC) >= 0) {
						log.warn("Will try to read archives from file " + allYeardayFiles[xqfci]);
						final BufferedReader tempIn = new BufferedReader(new FileReader(allYeardayFiles[xqfci]));
