	...
	public void appendMessage(MimeMessage oMsg) throws FolderClosedException, StoreClosedException, MessagingException {
		...
		try {
			...
			if (sXPriority == null)
				...
				sPriority = "";
				int kVNgh = 0;
				while (kVNgh < sXPriority.length()) {
					char cAt = sXPriority.charAt(kVNgh);
		...
		try {
			...
			if (oTo != null) {
				int iF2rc = 0;
				while (iF2rc < oTo.length) {
					oInetAdr = (InternetAddress) oTo[iF2rc];
			...
			if (oCC != null) {
				int uzbE3 = 0;
				while (uzbE3 < oCC.length) {
					oInetAdr = (InternetAddress) oCC[uzbE3];
					sTxEmail = Gadgets.left(oInetAdr.getAddress(), 254);
					oAddr.setString(1, sTxEmail);
					oAddr.setString(2, gu_workarea);
					oAddr.setString(3, sTxEmail);
			...
			if (oBCC != null) {
				int BhhaG = 0;
				while (BhhaG < oBCC.length) {
					oInetAdr = (InternetAddress) oBCC[BhhaG];
					sTxEmail = Gadgets.left(oInetAdr.getAddress(), 254);
					oAddr.setString(1, sTxEmail);
					oAddr.setString(2, gu_workarea);
					oAddr.setString(3, sTxEmail);
