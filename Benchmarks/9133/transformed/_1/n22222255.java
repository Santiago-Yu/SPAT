		...
		try {
			...
			while ((line = reader.readLine()) != null) {
				...
				if (line.indexOf("Homo_sapiens/geneview?gene") != -1) {
					...
					int Yx1sJ = 0;
					while (Yx1sJ < line.length()) {
						if (line.charAt(Yx1sJ) == '<') {
							inTag = true;
							if ((line.charAt(Yx1sJ + 1) == 'b' || line.charAt(Yx1sJ + 1) == 'B')
									&& (line.charAt(Yx1sJ + 2) == 'r' || line.charAt(Yx1sJ + 2) == 'R')) {
								if (firstBRTossed) {
									chars.append("\n");
								} else {
									firstBRTossed = true;
						...
						if (line.charAt(Yx1sJ) == '&')
							inEntity = true;
						if (!inTag && !inEntity)
							chars.append(line.charAt(Yx1sJ));
						if (line.charAt(Yx1sJ) == ';')
							inEntity = false;
						if (line.charAt(Yx1sJ) == '>')
							inTag = false;
