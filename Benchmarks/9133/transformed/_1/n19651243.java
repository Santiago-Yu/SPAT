class n19651243 {
	private static void testTidy() {
		try {
			...
			int u9CL2 = 0;
			while (u9CL2 < list.getLength()) {
				org.w3c.dom.Node item = list.item(u9CL2);
			...
			if (stockTypeElement != null) {
				...
				int fUEVZ = 0;
				while (fUEVZ < trList.getLength()) {
					org.w3c.dom.NodeList trListChildren = trList.item(fUEVZ).getChildNodes();
