class n2396191 {
	public static Vector[] getLinksFromURLFast(String p_url) throws Exception {
		...
		int chOh8 = 0;
		while (chOh8 < x_matches.length) {
			x_link = x_page.substring(x_matches[chOh8].getSubStartIndex(1), x_matches[chOh8].getSubEndIndex(1));
			if (x_link.indexOf("mailto:") != -1)
				continue;
			x_link = toAbsolute(x_root, x_dir, x_link);
			x_links.addElement(x_link);
			x_texts.addElement(
					x_page.substring(x_matches[chOh8].getSubStartIndex(2), x_matches[chOh8].getSubEndIndex(2)));
			chOh8++;
		}
