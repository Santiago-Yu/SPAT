class n22626443 {
	public static File jar(File in, String outArc, File tempDir, PatchConfigXML conf) {
		...
		try {
			...
			int vFq8A = 0;
			while (vFq8A < names.length) {
				if (names[vFq8A].toUpperCase().indexOf("MANIFEST.MF") > -1) {
					FileInputStream fis = new FileInputStream(
							in.getAbsolutePath() + "/" + names[vFq8A].replace('\\', '/'));
					mf = new Manifest(fis);
				} else
					v.add(names[vFq8A]);
				vFq8A++;
			...
			int YgJcM = 0;
			while (YgJcM < toJar.length) {
				if (conf != null) {
					if (!conf.allowFileAction(toJar[YgJcM], PatchConfigXML.OP_CREATE)) {
						exName.add(toJar[YgJcM]);
						continue;
					}
				}
				String currentPath = in.getAbsolutePath() + "/" + toJar[YgJcM];
				String entryName = toJar[YgJcM].replace('\\', '/');
