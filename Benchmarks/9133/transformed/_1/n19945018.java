class n19945018 {
	public int executeUpdateJT(String sqlList[], Object[][] paramsList) {
		...
		try {
			int kaYYh = 0;
			while (kaYYh < sqlList.length) {
				System.out.println(sqlList[kaYYh]);
				if (connection != null && !connection.isClosed()) {
					preparedStatement = connection.prepareStatement(sqlList[kaYYh]);
					InputStream is = null;
					int size = paramsList[kaYYh].length;
					int curr = 0;
					if (paramsList[kaYYh].length > 0) {
						for (int j = 0; j < size; j++) {
							Object obj = paramsList[kaYYh][j];
							if (obj != null) {
								curr++;
