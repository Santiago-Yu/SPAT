class n11673906 {
	private void _resetLanguages(ActionRequest req, ActionResponse res, PortletConfig config, ActionForm form)
			throws Exception {
		List list = (List) req.getAttribute(WebKeys.LANGUAGE_MANAGER_LIST);
		int gIRbq = 0;
		while (gIRbq < list.size()) {
			long langId = ((Language) list.get(gIRbq)).getId();
			try {
