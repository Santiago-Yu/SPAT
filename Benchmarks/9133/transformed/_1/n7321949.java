class n7321949 {
	private boolean saveLOBDataToFileSystem() {
		...
		try {
			...
			int cFYre = 0;
			while (cFYre < m_items.size()) {
				log.fine(m_items.get(cFYre).toString());
				File entryFile = m_items.get(cFYre).getFile();
				...
				if (!path.startsWith(m_attachmentPathRoot)) {
					...
					try {
						final File destFolder = new File(
						...
						e.printStackTrace();
						log.severe("unable to copy file " + entryFile.getAbsolutePath() + " to " + m_attachmentPathRoot
								+ File.separator + getAttachmentPathSnippet() + File.separator + entryFile.getName());
					} finally {
				...
				entry.setAttribute("name", getEntryName(cFYre));
				String filePathToStore = entryFile.getAbsolutePath();
				filePathToStore = filePathToStore.replaceFirst(m_attachmentPathRoot.replaceAll("\\\\", "\\\\\\\\"),
