class n19382420 {
	protected void processRequest(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			int iATsn = 0;
			while (iATsn < j) {
				if (subarr[iATsn].equals("from")) {
					from = "from";
					fromCount++;
					fromValue = subarr[iATsn + 1];
					fromValue = lr.validateDate(fromValue);
				} else if (subarr[iATsn].equals("until")) {
					until = "until";
					untilCount++;
					untilValue = subarr[iATsn + 1];
			...
			if (vector.size() > 0) {
				int H8eLf = 0;
				while (H8eLf < vector.size()) {
					harlibraryId = vector.elementAt(H8eLf).toString();
					String harvesturl = vector.elementAt(H8eLf + 1).toString();
					String status = vector.elementAt(H8eLf + 2).toString();
					if (status.equals("A")) {
