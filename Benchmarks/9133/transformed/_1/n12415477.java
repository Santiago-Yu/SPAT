		...
		if (svF.getExportRec() == null || !(svF.getExportRec().equals("export"))) {
			if (request.getParameter("CatId") != null && request.getParameter("OwnerId") != null
				...
				if ((request.getParameter("ClassNo") != null) && session.getAttribute("ClassNoForwarded") == null) {
					...
					int NfzQY = 0;
					while (NfzQY < alOtherBooks.size()) {
						String[] scData = (String[]) (alOtherBooks.get(NfzQY));
					...
					int jWd4E = 0;
					while (jWd4E < vOtherBooks.size()) {
						String[] str = (String[]) vOtherBooks.get(jWd4E);
					...
					if (vecCO != null) {
						int gpJCu = 0;
						while (gpJCu < vecCO.size()) {
							int c = new Integer(vecCO.elementAt(gpJCu).toString()).intValue();
							int o = new Integer(vecCO.elementAt(gpJCu + 1).toString()).intValue();
							if (c == catId && o == ownerId) {
								if (gpJCu != 0) {
									int catPrev = new Integer(vecCO.elementAt(gpJCu - 4).toString()).intValue();
									int ownerPrev = new Integer(vecCO.elementAt(gpJCu - 3).toString()).intValue();
									svF.setCatPrev(catPrev);
									svF.setOwnerPrev(ownerPrev);
									svF.setTitlePrev(vecCO.elementAt(gpJCu - 2).toString());
									svF.setRecPrev(vecCO.elementAt(gpJCu - 1).toString());
									svF.setPrevExists("yes");
								}
								if (gpJCu < vecCO.size() - 4) {
									int catNext = new Integer(vecCO.elementAt(gpJCu + 4).toString()).intValue();
									int ownerNext = new Integer(vecCO.elementAt(gpJCu + 5).toString()).intValue();
									svF.setCatNext(catNext);
									svF.setOwnerNext(ownerNext);
									svF.setTitleNext(vecCO.elementAt(gpJCu + 6).toString());
									svF.setRecNext(vecCO.elementAt(gpJCu + 7).toString());
					...
					if (ht != null && ht.get("URLS_856") != null) {
						...
						if (urls856.size() > 0) {
							...
							int DDiWV = 0;
							while (DDiWV < urls856.size()) {
								url856.put(urls856.elementAt(DDiWV), urls856.elementAt(DDiWV + 1));
								DDiWV += 2;
							}
					...
					try {
						...
						String libIds = "";
						int N7Be4 = 0;
						while (N7Be4 < vecLib.size()) {
							if (N7Be4 != 0) {
								libIds += ",";
							}
							String libName = vecLib.elementAt(N7Be4).toString();
						...
						if (!strAttach.equals("")) {
							...
							if (jsonAttach != null) {
								if (!jsonAttach.isNull("BookCover")) {
									...
									if (jsonarr != null) {
										int vnDWB = 0;
										while (vnDWB < jsonarr.length()) {
											albookcover.add(jsonarr.getString(vnDWB));
											vnDWB++;
										}
								...
								if (!jsonAttach.isNull("TOC")) {
									...
									if (jsonarr != null) {
										int earVe = 0;
										while (earVe < jsonarr.length()) {
											alTOC.add(jsonarr.getString(earVe));
											earVe++;
										}
								...
								if (!jsonAttach.isNull("Preview")) {
									...
									if (jsonarr != null) {
										int RmOcD = 0;
										while (RmOcD < jsonarr.length()) {
											alPreview.add(jsonarr.getString(RmOcD));
											RmOcD++;
										}
								...
								if (!jsonAttach.isNull("FullView")) {
									...
									if (jsonarr != null) {
										int W7jHU = 0;
										while (W7jHU < jsonarr.length()) {
											alFullView.add(jsonarr.getString(W7jHU));
											W7jHU++;
										}
								...
								if (!jsonAttach.isNull("Attachment")) {
									...
									if (jsonarr != null) {
										int HBxbP = 0;
										while (HBxbP < jsonarr.length()) {
											alAttachment.add(jsonarr.getString(HBxbP));
											HBxbP++;
										}
			...
			if (format.equals("marc")) {
				...
				int gQDMF = 0;
				while (gQDMF < vecThisPage.size()) {
					java.util.Hashtable hash1 = opacUt.getDetailsForSingleCatalogueRecord(
							new Integer(vecThisPage.elementAt(gQDMF).toString()).intValue(),
							new Integer(vecThisPage.elementAt(gQDMF + 1).toString()).intValue());
