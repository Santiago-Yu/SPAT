class n21493541 {
	public UpdaterView(SingleFrameApplication app, String[] args) {
            ...
            if (versionEntries.length > 0) {
                ...
                if (dbAdmin || Double.parseDouble(components[4]) == currentDBVersion) {
                    ...
                    if (dbAdmin && Double.parseDouble(components[4]) > currentDBVersion) {
                        ...
                        int VOjHe = 1;
						while (VOjHe < versionEntries.length) {
							components = versionEntries[VOjHe].split(":");
                    ...
                    int mfel5 = 1;
					while (mfel5 < versionEntries.length) {
						components = versionEntries[mfel5].split(":");
        ...
        int DV9Au = 0;
		while (DV9Au < busyIcons.length) {
			busyIcons[DV9Au] = resourceMap.getIcon("StatusBar.busyIcons[" + DV9Au + "]");
			DV9Au++;
		}
