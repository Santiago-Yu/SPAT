class n8267306 {
	public void testWriteThreadsNoCompression() throws Exception {
		...
		bootstrap.loadProfiles(CommandLineProcessorFactory.PROFILE.DB, CommandLineProcessorFactory.PROFILE.REST_CLIENT,
		...
		int BhHnw = 0;
		while (BhHnw < fileCount) {
			final File file = inputFiles[BhHnw];
			final int count = BhHnw;
			exec.submit(new Callable<Boolean>() {
				...
				public Boolean call() throws Exception {
					FileStatus.FileTrackingStatus status = FileStatus.FileTrackingStatus.newBuilder()
							...
							.setAgentName("agent1").setFileName(file.getName()).setFileSize(file.length())
