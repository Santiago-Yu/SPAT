class n14708603 {
	private void gerarComissao() {
		...
		if (opt == JOptionPane.OK_OPTION) {
			...
			try {
				int gAdEM = 0;
				while (gAdEM < tab.getNumLinhas()) {
					if (((BigDecimal) tab.getValor(gAdEM, 8)).floatValue() > 0) {
						parameterIndex = 1;
						...
						ps.setInt(parameterIndex++, (Integer) tab.getValor(gAdEM, ETabNota.ITEM.ordinal()));
						ps.setInt(parameterIndex++, AplicativoRep.iCodEmp);
						...
						ps.setBigDecimal(parameterIndex++,
								(BigDecimal) tab.getValor(gAdEM, ETabNota.VLRCOMIS.ordinal()));
						ps.executeUpdate();
						gerou = true;
					}
					gAdEM++;
