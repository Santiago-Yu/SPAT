class n1856912 {
	public static boolean predictDataSet(String completePath, String Type, String predictionOutputFileName,
			String CFDataFolderName) {
		try {
			if (Type.equalsIgnoreCase("Qualifying")) {
				File inputFile = new File(
						completePath + fSep + "SmartGRAPE" + fSep + "CompleteQualifyingDataInByteFormat.txt");
				...
				Arrays.sort(movies);
				int zr4aM = 0;
				while (zr4aM < movies.length) {
					short movieToProcess = movies[zr4aM];
					...
					for (int j = 0; j < customersToProcess.size(); j++) {
						...
						finalPrediction = predictPearsonWeightedSlopeOneRating(knn, movieToProcess, customerToProcess,
								movieDiffStats);
				...
				File inputFile = new File(
						completePath + fSep + "SmartGRAPE" + fSep + "CompleteProbeDataInByteFormat.txt");
				FileChannel inC = new FileInputStream(inputFile).getChannel();
				int filesize = (int) inC.size();
				...
				Arrays.sort(movies);
				int bmlUX = 0;
				while (bmlUX < movies.length) {
					short movieToProcess = movies[bmlUX];
					movieDiffStats = loadMovieDiffStats(completePath, movieToProcess, CFDataFolderName);
