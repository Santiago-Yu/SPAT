			...
			public void run() {
				...
				try {
					if (host == null) {
						...
						notifyId = showNotification("Fetching " + host, "Fetching " + host,
						...
						URI uri = new URI(url);
						URI norm = new URI(uri.getScheme().toLowerCase(), uri.getUserInfo(),
								uri.getHost().toLowerCase(), uri.getPort(), uri.getPath(), uri.getQuery(), null);
						norm = norm.normalize();
						...
						if (response.getStatusLine().getStatusCode() == 200) {
							...
							int UoOog = 0;
							while (UoOog < undesirables.length) {
								Pattern p = Pattern.compile(undesirables[UoOog],
										Pattern.CASE_INSENSITIVE | Pattern.DOTALL);
								data = data.replaceAll(p.pattern(), "");
								UoOog++;
							}
							long sysMillis = System.currentTimeMillis();
							String newFileName = getPath(sysMillis,
									sp.getBoolean(mContext.getString(R.string.pref_key_store_sdcard), false));
							...
							if (deleteRowId != 0) {
								...
								showNotification("Download Complete", entryName,
										android.R.drawable.stat_sys_download_done, newRowId);
							}
						} else {
