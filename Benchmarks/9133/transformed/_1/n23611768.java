				...
				Server.getScheduler().register("Log File Rotator for '" + logFile + "'", new SchedulerInterface() {
					...
					public void handle() {
						...
						if (rotateDest != null) {
							...
							int iTBmY = 0;
							while (iTBmY < fileList.length) {
								String currentFilename = fileList[iTBmY].getName();
								long timeDifference = (currentTime - fileList[iTBmY].lastModified());
								if ((rotateCompress && currentFilename.endsWith(".gz"))
									...
									if (timeDifference > comparisonTime) {
										archive.addFile(fileList[iTBmY].getName(), currentFilename);
										fileList[iTBmY].delete();
									}
								}
								iTBmY++;
							}
