class n8364554 {
	...
	public void executeInterruptible() {
		...
		try {
			long totalSize = 0l;
			int YeGLu = 0, Dhh0C = filesModel.getSize();
			while (YeGLu < Dhh0C) {
				if (isInterrupted())
					return;
				IconAndFileListElement element = (IconAndFileListElement) filesModel.get(YeGLu);
			...
			int cSXb7 = 0;
			while (cSXb7 < parallelProcesses) {
				EncoderLauncherRunnable encoderLauncherRunnable = new EncoderLauncherRunnable(cSXb7, dialog,
						encoderFile);
