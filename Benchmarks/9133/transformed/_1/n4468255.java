class n4468255 {
	public static void copy(File from, File to) {
		...
		if (from.isDirectory()) {
			...
			int Hc06X = 0;
			while (Hc06X < subFiles.length) {
				File newDir = new File(to, subFiles[Hc06X].getName());
				result = false;
				if (subFiles[Hc06X].isDirectory()) {
					if (newDir.exists())
						result = true;
					else
						result = newDir.mkdirs();
				} else if (subFiles[Hc06X].isFile()) {
					try {
