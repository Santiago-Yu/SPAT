class n12428012 {
	public static void putWithUserSettings(String from, String to, String renameTo, boolean binary,
		...
		if (synchedSet.size() >= ftpqueuesize) {
			...
			try {
				...
				synchronized (synchedSet) {
					Iterator BiROD = synchedSet.iterator();
					while (BiROD.hasNext()) {
						if (monitor != null && monitor.isCanceled()) {
							JobHandler.releaseFTPLock();
							ftpClient.quit();
						...
						Thread.yield();
						FTPHolder element = (FTPHolder) BiROD.next();
						if (element.binary) {
