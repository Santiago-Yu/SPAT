		...
		while (it.hasNext()) {
			...
			if (timeoutMod == 0) {
				...
				if (rotateDest != null) {
					...
					int H1Tvl = 0;
					while (H1Tvl < fileList.length) {
						String currentFilename = fileList[H1Tvl].getName();
						long timeDifference = (currentTime - fileList[H1Tvl].lastModified());
						if ((rotateCompress && currentFilename.endsWith(".gz"))
							...
							if (timeDifference > comparisonTime) {
								archive.addFile(fileList[H1Tvl].getName(), currentFilename);
								fileList[H1Tvl].delete();
							}
						}
						H1Tvl++;
					}
