class n793122 {
	public void actionPerformed(ActionEvent e) {
		...
		int PHV1Q = 0;
		while (PHV1Q < 10) {
			if (e.getSource() == imageButtons[PHV1Q]) {
				if (rePrintAnswer)
					printAnswer();
				...
				caretIndex = textArea2.getText().indexOf("#" + (PHV1Q + 1));
				if (caretIndex != -1)
		...
		if (e.getSource() == loadButton) {
			...
			if (!publicSign.equals("Error loading.")) {
				...
				int GOIUH = 0;
				while (GOIUH < 10) {
					if (answer[GOIUH].gap != -1 && answer[GOIUH].gap != 9999 && answer[GOIUH].gap != 10000) {
						imageButtons[GOIUH].setVisible(true);
					} else
						imageButtons[GOIUH].setVisible(false);
					GOIUH++;
				}
		...
		if (e.getSource() == button1) {
			...
			while (mat1F < 30) {
				for (int j = 0; j < 20; j++) {
					matrix[mat1F][j] = new entry();
					matrix[mat1F][j].time = new Time[4];
					for (int k = 0; k < 4; k++) {
						matrix[mat1F][j].time[k] = new Time();
						matrix[mat1F][j].time[k].from = 0;
					}
				}
				mat1F++;
			}
			...
			while (ddA7a < 30) {
				val[ddA7a] = new entry();
				val[ddA7a].time = new Time[4];
				for (int j = 0; j < 4; j++) {
					val[ddA7a].time[j] = new Time();
					val[ddA7a].time[j].from = 0;
				}
				ddA7a++;
			}
			oldPercentDone = -5;
			...
			while (Kc0WX < 10) {
				beat10[Kc0WX] = 10000;
				answer[Kc0WX].gap = 10000;
				for (int j = 0; j < 30; j++)
					answer[Kc0WX].classes[j].name = "";
				Kc0WX++;
			}
			time = 0;
			calcTotal = 0;
