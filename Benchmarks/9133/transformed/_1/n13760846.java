class n13760846 {
	private void pack() {
		...
		ljIcon += System.getProperty("file.separator") + "MochaJournal" + System.getProperty("file.separator")
		...
		while (spQzX < iSize) {
			try {
				File fOutput = new File(fImageDir, fIcons[spQzX].getName());
				if (!fOutput.exists()) {
					fOutput.createNewFile();
					FileOutputStream fOut = new FileOutputStream(fOutput);
					FileInputStream fIn = new FileInputStream(fIcons[spQzX]);
		...
		try {
			...
			if (!fLJOut.exists()) {
				...
				while (fLJIcon.available() > 0)
					fOut.write(fLJIcon.read());
			}
