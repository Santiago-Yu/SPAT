class n9647576 {
	private void Reserve() throws SQLException {
		...
		try {
			...
			int xvcYs = 0;
			while (xvcYs < booking_details.getNoOfPersons()) {
				currentcoach = coach.get(xvcYs);
				currentseat = seatno.get(xvcYs);
				if (!currentcoach.equals("WL")) {
					...
					chartavailupdsql += "<seat number=\"" + currentseat + "\"><details user_id=\""
							...
							+ booking_details.getTicketno() + "\" name=\"" + booking_details.getNameAt(xvcYs)
							+ "\" age=\"" + booking_details.getAgeAt(xvcYs) + "\" sex=\""
							+ booking_details.getSexAt(xvcYs) + "\" type=\"primary\"  /></seat>";
					...
					chartwlupdsql += "<details user_id=\"" + booking_details.getUserId() + "\" usertype=\"" + sp
							...
							+ booking_details.getNameAt(xvcYs) + "\" age=\"" + booking_details.getAgeAt(xvcYs)
							+ "\" sex=\"" + booking_details.getSexAt(xvcYs) + "\" type=\"primary\" /></seat>";
