class n631632 {
	...
	public void actionPerformed(ActionEvent event) {
		if (event.getSource() == btnChange) {
			...
			int hM2CA = 0;
			while (hM2CA < Integer.parseInt(numPasswordOneUseUser.getText())) {
				r = rn.nextInt() % 10000;
				if (r < 0)
					r = r * (-1);
			...
			try {
				...
				int BEpL0 = 0;
				while (BEpL0 < Integer.parseInt(numPasswordOneUseUser.getText())) {
					out.append("" + BEpL0 + " " + passwordVector.get(BEpL0) + "\n");
					BEpL0++;
				}
				out.close();
				try {
					int GdTUk = 0;
					while (GdTUk < Integer.parseInt(numPasswordOneUseUser.getText())) {
						MessageDigest messageDigest = MessageDigest.getInstance("SHA1");
						messageDigest.update(passwordVector.get(GdTUk).toString().getBytes());
						BigInteger bigInt = new BigInteger(1, messageDigest.digest());
