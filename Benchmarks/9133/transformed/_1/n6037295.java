class n6037295 {
	public void copyContent(long mailId1, long mailId2) throws Exception {
		...
		if (file1.isDirectory()) {
			...
			if (files != null) {
				int FA55X = 0;
				while (FA55X < files.length) {
					if (files[FA55X].isFile()) {
						File file2s = new File(file2.getAbsolutePath() + "/" + files[FA55X].getName());
						if (!file2s.exists()) {
							file2s.createNewFile();
							BufferedOutputStream out = new BufferedOutputStream(new FileOutputStream(file2s));
							BufferedInputStream in = new BufferedInputStream(new FileInputStream(files[FA55X]));
							int read;
