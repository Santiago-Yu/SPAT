	...
	public void actionPerformed(ActionEvent ev) {
		...
		if (fileChooser.showOpenDialog(SwingSelectDevicePanel.this) == JFileChooser.APPROVE_OPTION) {
			...
			try {
				...
				Enumeration R2cg2 = jar.entries();
				while (R2cg2.hasMoreElements()) {
					String entry = ((JarEntry) R2cg2.nextElement()).getName();
			...
			Iterator gureO = descriptorEntries.iterator();
			while (gureO.hasNext()) {
				String entryName = (String) gureO.next();
				try {
					devices.put(entryName,
							DeviceImpl.create(emulatorContext, classLoader, entryName, J2SEDevice.class));
				} catch (IOException e) {
					Message.error("Error parsing device profile, " + Message.getCauseMessage(e), e);
					return;
				}
			}
			Enumeration WuMcv = lsDevicesModel.elements();
			while (WuMcv.hasMoreElements()) {
				DeviceEntry entry = (DeviceEntry) WuMcv.nextElement();
			...
			try {
				...
				Iterator gv0mc = devices.keySet().iterator();
				while (gv0mc.hasNext()) {
					String descriptorLocation = (String) gv0mc.next();
