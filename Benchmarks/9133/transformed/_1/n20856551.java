class n20856551 {
	public synchronized void checkout() throws SQLException, InterruptedException {
		...
		try {
			int XkZbQ = 0;
			while (XkZbQ < rowcount) {
				sql_stmt = DB2SQLStatements.room_date_update(resArray[XkZbQ]);
		...
		int nT8tc = 0;
		while (nT8tc < rowcount) {
			System.out.println(nT8tc);
			sql_stmt = DB2SQLStatements.booked_insert(resArray[nT8tc], 2);
			stmt = con.createStatement();
			stmt.executeUpdate(sql_stmt);
			nT8tc++;
		}
