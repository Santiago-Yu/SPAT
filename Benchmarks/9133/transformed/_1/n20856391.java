class n20856391 {
	public void launchJob(final String workingDir, final AppConfigType appConfig) throws FaultType {
		...
		if (!AppServiceImpl.dbInUse) {
			...
			try {
				conn = DriverManager.getConnection(AppServiceImpl.dbUrl, AppServiceImpl.dbUser,
		...
		if (AppServiceImpl.drmaaInUse) {
			...
			if (appConfig.isParallel()) {
				cmd = "/bin/sh";
				String newArgs = AppServiceImpl.mpiRun + " -machinefile $TMPDIR/machines" + " -np "
						+ jobIn.getNumProcs() + " " + appConfig.getBinaryLocation();
				if (args != null) {
					args = newArgs + " " + args;
				} else {
					args = newArgs;
			...
			try {
				...
				if (appConfig.isParallel())
					jt.setNativeSpecification("-pe " + AppServiceImpl.drmaaPE + " " + jobIn.getNumProcs());
			...
			String rsl = null;
			if (appConfig.isParallel()) {
				rsl = "&(directory=" + workingDir + ")" + "(executable=" + appConfig.getBinaryLocation() + ")"
			...
			try {
				job = new GramJob(rsl);
				GlobusCredential globusCred = new GlobusCredential(AppServiceImpl.serviceCertPath,
			...
			String cmd = null;
			if (appConfig.isParallel()) {
				cmd = new String(AppServiceImpl.mpiRun + " " + "-np " + jobIn.getNumProcs() + " "
		...
		new Thread() {
			...
			public void run() {
				...
				try {
					...
					if (AppServiceImpl.archiveData) {
						...
						ZipOutputStream out = new ZipOutputStream(
								new FileOutputStream(workingDir + File.separator + jobID + ".zip"));
						byte[] buf = new byte[1024];
						try {
							int x1z30 = 0;
							while (x1z30 < outputFiles.length) {
								FileInputStream in = new FileInputStream(outputFiles[x1z30]);
								out.putNextEntry(new ZipEntry(outputFiles[x1z30].getName()));
								int len;
					...
					int j = 0;
					int RDnTb = 0;
					while (RDnTb < outputFiles.length) {
						if (outputFiles[RDnTb].getName().equals("stdout.txt")) {
							outputs.setStdOut(new URI(AppServiceImpl.tomcatURL + jobID + "/stdout.txt"));
						} else if (outputFiles[RDnTb].getName().equals("stderr.txt")) {
							outputs.setStdErr(new URI(AppServiceImpl.tomcatURL + jobID + "/stderr.txt"));
						} else {
							OutputFileType next = new OutputFileType();
							next.setName(outputFiles[RDnTb].getName());
							next.setUrl(new URI(AppServiceImpl.tomcatURL + jobID + "/" + outputFiles[RDnTb].getName()));
				...
				if (!AppServiceImpl.dbInUse) {
					...
					String sqlStmt = "insert into job_output(job_id, std_out, std_err) " + "values ('" + jobID + "', "
					...
					int UpLHO = 0;
					while (UpLHO < outputFile.length) {
						sqlStmt = "insert into output_file(job_id, name, url) " + "values ('" + jobID + "', " + "'"
								+ outputFile[UpLHO].getName() + "', " + "'" + outputFile[UpLHO].getUrl().toString()
								+ "');";
						try {
							stmt = conn.createStatement();
							stmt.executeUpdate(sqlStmt);
