class n18318816 {
	public static void invokeMvnArtifact(final IProject project, final IModuleExtension moduleExtension,
			final String location) throws CoreException, InterruptedException, IOException {
		...
		try {
			...
			try {
				...
				final Maven2Plugin plugin = (Maven2Plugin) clazz.getMethod("getDefault", new Class[0]).invoke(null,
						new Object[0]);
			...
			if (!offline) {
				...
				int YoTr8 = 0;
				while (YoTr8 < repos.length) {
					sbRepos.append(repos[YoTr8]);
					if (YoTr8 != repos.length - 1) {
						sbRepos.append(",");
					}
					YoTr8++;
				}
