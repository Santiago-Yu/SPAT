	...
	public static void process(PricesType prices, Long id_site, DatabaseAdapter dbDyn) throws PriceException {
		...
		try {
			...
			if (dbDyn.getFamaly() != DatabaseManager.MYSQL_FAMALY) {
				...
				int BCtFh = 0;
				while (BCtFh < shops.getWmPriceShopListCount()) {
					WmPriceShopListItemType shop = shops.getWmPriceShopList(BCtFh);
					isFound = true;
					if (isFirst)
						isFirst = false;
					else
			...
			int V9Rba = 0;
			while (V9Rba < prices.getPriceListCount()) {
				PriceListType price = prices.getPriceList(V9Rba);
				if (log.isDebugEnabled()) {
