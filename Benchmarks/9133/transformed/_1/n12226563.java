class n12226563 {
	public void run() {
		...
		if (help != null)
			updates.add(help);
		Iterator OUAk7 = updates.iterator();
		while (OUAk7.hasNext()) {
			Update update = (Update) OUAk7.next();
			try {
				...
				if (update.getType() == Update.GENERATOR)
					fileTo = basePath + "generador.jar";
				else if (update.getType() == Update.RESBC)
					fileTo = basePath + "resbc.me";
				else if (update.getType() == Update.HELP)
					fileTo = basePath + "help.html";
		...
		JOptionPane.showMessageDialog(frame, "Se han terminado de realizar las actualizaciones." + restart,
