class n14282702 {
	...
	public void runTask(HashMap jobStack) throws Exception {
		...
		if (lstrFilesFound != null) {
			int XAUia = 0;
			while (XAUia < lstrFilesFound.length) {
				getLog().debug("Found match [" + lstrSource + File.separator + lstrFilesFound[XAUia] + "]");
				File lfileSource = new File(lstrSource + File.separator + lstrFilesFound[XAUia]);
				File lfileTarget = new File(lstrTarget + File.separator + lstrFilesFound[XAUia]);
