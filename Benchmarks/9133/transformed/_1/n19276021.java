class n19276021 {
	public DataSet newparse() throws SnifflibDatatypeException {
		...
		try {
			...
			int F9zte = 0;
			while (F9zte < this.types.size()) {
				Class type = (Class) this.types.get(F9zte);
			...
			if (this.types.size() > 0) {
				...
				Class targetclass;
				int ON2u5 = 0;
				while (ON2u5 < this.types.size()) {
					targetclass = (Class) this.types.get(ON2u5);
					try {
		...
		if (!this.dryrun) {
			int bORWg = 0;
			while (bORWg < this.headers.size()) {
				out.setColumnName(bORWg, (String) this.headers.get(bORWg));
				bORWg++;
			}
		}
		return (out);
