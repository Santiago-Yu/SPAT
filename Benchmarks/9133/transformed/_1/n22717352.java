class n22717352 {
	public void add(AddInterceptorChain chain, Entry entry, LDAPConstraints constraints) throws LDAPException {
		...
		try {
			...
			int feZNV = 0;
			while (feZNV < vals.length) {
				ps.setString(1, vals[feZNV]);
				ResultSet rs = ps.executeQuery();
				if (!rs.next()) {
					con.rollback();
					throw new LDAPException("Location " + vals[feZNV] + " does not exist",
							LDAPException.OBJECT_CLASS_VIOLATION, "Location " + vals[feZNV] + " does not exist");
				}
				inst.setInt(1, 5);
