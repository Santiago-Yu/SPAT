class n1953925 {
	public boolean deploy(MMedia[] media) {
		...
		try {
			...
			int nDeiA = 0;
			while (nDeiA < media.length) {
				if (!media[nDeiA].isSummary()) {
					log.log(Level.INFO, " Deploying Media Item:" + media[nDeiA].get_ID() + media[nDeiA].getExtension());
					MImage thisImage = media[nDeiA].getImage();
					byte[] buffer = thisImage.getData();
					ByteArrayInputStream is = new ByteArrayInputStream(buffer);
					String fileName = media[nDeiA].get_ID() + media[nDeiA].getExtension();
					cmd = "put " + fileName;
