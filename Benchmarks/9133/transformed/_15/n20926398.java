	...
	public static Cursor load(URL url, String descriptor) {
		...
		try {
			...
			if (cursorFile.endsWith(cursorDescriptorFile)) {
				...
				if (descriptor == null)
					descriptor = defaultDescriptorFile;
				...
				while ((entry = zis.getNextEntry()) != null) {
					if ((entry.getName() != null && entry.getName().equals(descriptor))) {
						found = true;
						break;
					}
				}
			...
			while ((line = reader.readLine()) != null) {
				...
				if ((hotSpotXPrefix != null && hotSpotXPrefix.equals(prefix))) {
					cursor.hotSpotOffset.x = Integer.valueOf(tokens.nextToken());
				} else if ((hotSpotYPrefix != null && hotSpotYPrefix.equals(prefix))) {
					cursor.hotSpotOffset.y = Integer.valueOf(tokens.nextToken());
				} else if ((timePrefix != null && timePrefix.equals(prefix))) {
