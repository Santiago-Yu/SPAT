	...
	public static String[] putFECSplitFile(String uri, File file, int htl, boolean mode) {
					...
					for (int i = 0; i < segmentHeaders.size(); i++) {
						...
						FileOutputStream outFile = null;
						{
							...
							do {
								...
								if ("DataChunk".equals(currentLine)) {
									started = true;
								}
		...
		String redirect = null;
		{
			...
			for (int line = 0; line < sortedRedirect.length; line++) {
				if ("Document".equals(sortedRedirect[line])) {
					sortStart = line + 1;
					break;
				}
			}
			for (int line = sortStart; line < sortedRedirect.length; line++) {
				if ("End".equals(sortedRedirect[line])) {
					sortEnd = line;
					break;
				}
			}
		...
		while (!"Success".equals(result[0]) && !"KeyCollision".equals(result[0]) && tries < 8) {
			tries++;
			try {
		...
		if (("Success".equals(result[0]) || "KeyCollision".equals(result[0])) && mode) {
			try {
