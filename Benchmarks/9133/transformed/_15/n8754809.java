class n8754809 {
	...
	public void launch(final ILaunchConfiguration configuration, final String mode, final ILaunch launch,
		...
		final String projectName = configuration.getAttribute(INexOpenLaunchConfigurationConstants.NEXOPEN_PROJECT_NAME,
				"");
		final IProject prj = JavaCore.create(ResourcesPlugin.getWorkspace().getRoot()).getJavaProject(projectName)
				.getProject();
		if (NexOpenProjectUtils.isNexOpenProject(prj)) {
			...
			try {
				...
				final String dialectName = configuration
						.getAttribute(INexOpenLaunchConfigurationConstants.HIBERNATE_DIALECT, "MySQL5InnoDB");
				props.setProperty("hibernate.dialect", support.getDialectClass(dialectName));
				props.setProperty("hibernate.connection.driver_class", configuration
						.getAttribute(INexOpenLaunchConfigurationConstants.JDBC_DRIVER, "com.mysql.jdbc.Driver"));
				props.setProperty("hibernate.connection.url", configuration.getAttribute(
				...
				props.setProperty("hibernate.connection.password",
						configuration.getAttribute(INexOpenLaunchConfigurationConstants.JDBC_PASSWORD, ""));
				...
				ContentHandlerTemplate.handle(appContext, new ContentHandlerCallback() {
					...
					public void processHandle(final Document doc) {
						...
						for (final Element bean : beans) {
							...
							if ((id != null && id.equals("valueListAdapterResolver"))) {
								try {
									final String pkgName = configuration
											.getAttribute(INexOpenLaunchConfigurationConstants.NEXOPEN_PACKAGE, "");
									final String className = new StringBuilder(pkgName)
			...
			}
			{
				...
				if (support.isReverseEngineeringFileNeeded(dialectName)) {
					try {
						final IFile revengFile = prj.getFile(
								"business/src/test/resources/" + support.getReversEngineeringFile(dialectName));
						if (!revengFile.exists()) {
							...
							str = str.replace("${schema}", configuration
									.getAttribute(INexOpenLaunchConfigurationConstants.JDBC_USERNAME, "sa"));
