class n10793825 {
	public String getRec(String attribute, String url) {
		...
		for (int k = 2; k < j; k += 2) {
			...
			if ("metadataPrefix".equals(subarr[k])) {
				...
				argCount++;
			} else if ("identifier".equals(subarr[k])) {
				iden = "identifier";
				idenCount++;
		...
		if (mdfCount == 1 && idenCount == 1
				&& ("marc21".equals(mdPrefixValue) || "oai_dc".equals(mdPrefixValue) || "mods".equals(mdPrefixValue))) {
			try {
			...
			if (v1.size() == 0) {
				...
				for (int v = 0; v < v1.size(); v = v + 13) {
					...
					if ("oai_dc".equals(mdPrefixValue)) {
						try {
					...
					} else if ("mods".equals(mdPrefixValue)) {
						try {
					...
					if ("marc21".equals(mdPrefixValue)) {
						metadata.addContent(marcroot);
					} else if ("oai_dc".equals(mdPrefixValue)) {
					}
				}
