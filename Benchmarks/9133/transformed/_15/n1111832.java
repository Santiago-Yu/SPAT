	...
	private void storeFieldMap(WorkingContent c, Connection conn) throws SQLException {
		...
		try {
			...
			for (Iterator i = c.getStructure().getStructureItems().iterator(); i.hasNext();) {
				...
				try {
					if (StructureItem.DATE.equals(type)) {
						...
						ps.executeUpdate();
					} else if (StructureItem.INT.equals(type) || StructureItem.FLOAT.equals(type)
							|| StructureItem.VARCHAR.equals(type)) {
						...
						if (value != null) {
							...
							if (StructureItem.FLOAT.equals(type)) {
								sqlType = Types.FLOAT;
							} else if (StructureItem.VARCHAR.equals(type)) {
								sqlType = Types.VARCHAR;
							}
							ps.setNull(3, sqlType);
						}
						ps.executeUpdate();
					} else if (StructureItem.TEXT.equals(type)) {
