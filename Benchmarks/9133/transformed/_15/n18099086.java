class n18099086 {
	...
	protected void service(final HttpServletRequest req, final HttpServletResponse res)
		...
		if (null == path || (path != null && path.equals("")))
			res.sendRedirect(req.getServletPath() + "/");
		else if ((path != null && path.equals("/chart"))) {
			try {
			...
			for (LogProvider provider : cfg.getLogProviders())
				logs.add(new LogEntry(provider.getName(), provider.getTitle(filter.getLocale())));
			render(res,
					new ProbeDataList(filter.getSnapshot(), filter.getAlerts(), logs, ResourceBundle
			...
			URL url = Thread.currentThread().getContextClassLoader()
					.getResource(getClass().getPackage().getName().replace('.', '/') + req.getPathInfo());
			if (null == url) {
				res.sendError(HttpServletResponse.SC_NOT_FOUND);
				return;
			}
			...
			if (contentType == null || (contentType != null && contentType.equals("content/unknown"))) {
