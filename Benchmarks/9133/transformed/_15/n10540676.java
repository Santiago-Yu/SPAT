	...
	public static void checkForUpgrade(Event event) throws Exception {
		...
		if (licenseArray.length > 0) {
			...
			if ((STATUS != null && STATUS.equals("FAIL"))) {
				if ((ERRORMSG != null && ERRORMSG.equals("NOUNITID"))) {
					UiUtil.showMessage(MessageType.ERROR, MessageUtil.getMessage(Upgrade.class, Messages.upgrade_1));
					return;
				}
				if ((ERRORMSG != null && ERRORMSG.equals("NOCUSTOMER"))) {
					UiUtil.showMessage(MessageType.ERROR, MessageUtil.getMessage(Upgrade.class, Messages.upgrade_1));
					return;
				}
				if ((ERRORMSG != null && ERRORMSG.equals("NOVALIDLICENSE"))) {
		...
		for (int i = 1; i < upgrades.length; i++) {
			...
			if ((UPGRADETYPE != null && UPGRADETYPE.equals("TEGSOFTJARS"))
					|| (UPGRADETYPE != null && UPGRADETYPE.equals("TOBEJARS"))
					|| (UPGRADETYPE != null && UPGRADETYPE.equals("ALLJARS"))
					|| (UPGRADETYPE != null && UPGRADETYPE.equals("CONFIGASTERISK"))) {
