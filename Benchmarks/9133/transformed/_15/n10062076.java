class n10062076 {
	private long generateNativeInstallExe(File nativeInstallFile, String instTemplate, File instClassFile)
		...
		position = 1;
		for (int n = 0; n < 3; n++) {
			while ((!"clname_here_".equals(new String(buf))) && (!"clstart_here".equals(new String(buf)))
					&& (!"clstop_here_".equals(new String(buf)))) {
				content.write(buf[0]);
			...
			if ("clname_here_".equals(new String(buf))) {
				...
				reader.read(buf, 0, buf.length);
			} else if ("clstart_here".equals(new String(buf))) {
				...
				reader.read(buf, 0, buf.length);
			} else if ("clstop_here_".equals(new String(buf))) {
