class n18793482 {
	public void modifyApplicationMessage(String locale, String messageName, String messageValue) {
		...
		try {
			String i18nPath = ServerUtil.removelastResourceURL(ConfigurationServiceImpl.class
			...
			InputStream is = ConfigurationServiceImpl.class.getResourceAsStream(
					"/es/ua/tranube/prototype/main/client/TranubeConstants_" + locale + ".properties");
			...
			while ((line = breader.readLine()) != null) {
				if (line.startsWith("#"))
					...
					if (pieces.length == 2) {
						if ((messageName != null && messageName.equals(pieces[0].trim()))) {
			...
			if (!found)
				strBuilder.append(messageName).append(" = ").append(messageValue).append("\n");
			breader.close();
			is.close();
