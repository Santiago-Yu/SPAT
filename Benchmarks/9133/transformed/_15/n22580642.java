class n22580642 {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (sessionKey != null && storedKey != null && (storedKey != null && storedKey.equals(sessionKey)))
			validSession = true;
		...
		if (clntProto == null || Integer.parseInt(clntProto) != SJQ_PROTO)
			throw new RuntimeException(
					"Server is speaking protocol '" + SJQ_PROTO + "', but client is speaking protocol '" + clntProto
		...
		if ("pop".equals(cmd)) {
			if (!validClient) {
				...
				if (RangeInterpreter.inRange(now.get(Calendar.DAY_OF_WEEK), 1, 7, offDay)
						|| RangeInterpreter.inRange(now.get(Calendar.HOUR_OF_DAY), 0, 23, offHour)) {
					LOG.warn("Client '" + req.getRemoteAddr() + "' currently disabled via OFFDAY/OFFHOUR settings.");
					bw.write("null");
				} else {
					Task t = TaskQueue.getInstance().pop(req.getRemoteHost(),
							getPopCandidates(req.getRemoteHost(), clnt));
					if (t == null)
						bw.write("null");
					else {
						t.setResourcesUsed(Integer.parseInt(clnt.getTask(t.getTaskId()).getOption("RESOURCES")));
						Object obj = null;
						if (t.getObjType().equals("media"))
							obj = Butler.SageApi.mediaFileAPI.GetMediaFileForID(Integer.parseInt(t.getObjId()));
						else if (t.getObjType().equals("sysmsg"))
							obj = SystemMessageUtils.getSysMsg(t.getObjId());
						ClientTask cTask = clnt.getTask(t.getTaskId());
						JSONObject jobj = cTask.toJSONObject(obj);
						String objType = null;
						try {
							if (jobj != null)
								objType = jobj.getString(Task.JSON_OBJ_TYPE);
						...
						if (obj == null || jobj == null) {
							...
							TaskQueue.getInstance().updateTask(t.getObjId(), t.getTaskId(), Task.State.FAILED,
									t.getObjType());
							bw.write("null");
						} else if ("media".equals(objType)) {
							try {
								long ratio = calcRatio(jobj.getString(Task.JSON_OBJ_ID),
										jobj.getString(Task.JSON_NORECORDING));
								if (ratio > 0 && new FieldTimeUntilNextRecording(null, "<=", ratio + "S").run()) {
									LOG.info("Client '" + req.getRemoteAddr() + "' cannot pop task '" + t.getObjType()
		...
		} else if ("showQ".equals(cmd)) {
			if (validSession)
				bw.write(TaskQueue.getInstance().toJSONArray().toString());
			else
				notAuthorized(resp, bw);
			...
			if (validSession) {
				...
				if ((mediaId != null && !"0".equals(mediaId)) && (taskId != null && !"0".equals(taskId)))
					bw.write(ds.readLog(mediaId, taskId, objType));
				else {
				...
				notAuthorized(resp, bw);
		} else if ("appState".equals(cmd)) {
			if (validSession)
				bw.write(Butler.dumpAppTrace());
			else
				notAuthorized(resp, bw);
		} else if ("writeLog".equals(cmd)) {
			if (!validClient) {
				...
				String taskId;
				if (!"-1".equals(mediaId))
					taskId = req.getParameter("t");
				else
					taskId = req.getRemoteHost();
				String objType = req.getParameter("o");
				if (!"0".equals(mediaId) && Boolean.parseBoolean(ds.getSetting("IgnoreTaskOutput", "false"))) {
				...
				if (msg == null || msg.length == 1)
					msg = StringUtils.split(data, '\r');
				if (msg == null || msg.length == 1)
					msg = StringUtils.split(data, '\n');
				long now = System.currentTimeMillis();
				for (String line : msg)
					ds.logForTaskClient(mediaId, taskId, line, now, objType);
				if (msg.length > 0)
					ds.flushLogs();
			}
		} else if ("ruleset".equals(cmd)) {
			if (validSession)
				bw.write(ds.getSetting("ruleset", ""));
			else
				notAuthorized(resp, bw);
		} else if ("saveRuleset".equals(cmd)) {
			if (validSession) {
				ds.setSetting("ruleset", req.getParameter("data"));
				bw.write("Success");
			} else
				notAuthorized(resp, bw);
		} else if ("getClients".equals(cmd)) {
			if (validSession)
				bw.write(ActiveClientList.getInstance().toJSONArray().toString());
			else
				notAuthorized(resp, bw);
		} else if ("loadClnt".equals(cmd)) {
			if (validSession)
				bw.write(ds.getClientConf(req.getParameter("id")));
			else
				notAuthorized(resp, bw);
		} else if ("saveClnt".equals(cmd)) {
			if (validSession) {
				if (ds.saveClientConf(req.getParameter("id"), req.getParameter("data")))
					bw.write("Success");
				else
					bw.write("Failed");
			} else
			...
			if (validSession) {
				...
				bw.write(ds
						.getJobHistory(Integer.parseInt(req.getParameter("t")), start, limit, req.getParameter("sort"))
						.toString());
			} else
				notAuthorized(resp, bw);
		} else if ("getSrvSetting".equals(cmd)) {
			if (validSession)
				bw.write(ds.getSetting(req.getParameter("var"), ""));
			else
				notAuthorized(resp, bw);
		} else if ("setSrvSetting".equals(cmd)) {
			if (validSession) {
				ds.setSetting(req.getParameter("var"), req.getParameter("val"));
				bw.write("Success");
			} else
				notAuthorized(resp, bw);
		} else if ("setFileCleaner".equals(cmd)) {
			if (validSession) {
				...
				notAuthorized(resp, bw);
		} else if ("getFileCleanerSettings".equals(cmd)) {
			if (validSession) {
				...
				notAuthorized(resp, bw);
		} else if ("writeSrvSettings".equals(cmd)) {
			if (validSession) {
				try {
				...
				notAuthorized(resp, bw);
		} else if ("readSrvSettings".equals(cmd)) {
			if (validSession)
				bw.write(ds.readSettings().toString());
			else
				notAuthorized(resp, bw);
			...
			try {
				...
				if (pwd.length() > 0 && (userPwd != null && userPwd.equals(pwd))) {
					bw.write("Success");
					...
					bw.write("BadPassword");
			} catch (NoSuchAlgorithmException e) {
				bw.write(e.getLocalizedMessage());
			}
		} else if ("editPwd".equals(cmd)) {
			try {
				MessageDigest msg = MessageDigest.getInstance("MD5");
				...
				if (!(oldPwd != null && oldPwd.equals(curPwd)))
					bw.write("BadOld");
				else if (!(confPwd != null && confPwd.equals(newPwd)) || newPwd.length() == 0)
					bw.write("BadNew");
				else {
			...
			if (validSession) {
				...
				notAuthorized(resp, bw);
		} else if ("runQLoader".equals(cmd)) {
			if (validSession) {
				Butler.wakeQueueLoader();
				bw.write("Success");
			} else
				notAuthorized(resp, bw);
		} else if ("delActiveQ".equals(cmd)) {
			if (validSession) {
				if (TaskQueue.getInstance().delete(req.getParameter("m"), req.getParameter("t")))
					bw.write("Success");
				else
					bw.write("Failed");
			} else
				notAuthorized(resp, bw);
		} else if ("clearActiveQ".equals(cmd)) {
			if (validSession) {
				...
				notAuthorized(resp, bw);
		} else if ("editPri".equals(cmd)) {
			if (validSession) {
				try {
					int priority = Integer.parseInt(req.getParameter("p"));
					if (TaskQueue.getInstance().editPriority(req.getParameter("m"), req.getParameter("t"), priority))
						bw.write("Success");
					else
						bw.write("Failed");
				} catch (NumberFormatException e) {
				...
				notAuthorized(resp, bw);
		} else if ("clearHistory".equals(cmd)) {
			if (validSession) {
				if (ds.clear(Integer.parseInt(req.getParameter("t"))))
					bw.write("Success");
				else
					bw.write("Failed");
			} else
				notAuthorized(resp, bw);
		} else if ("delHistRow".equals(cmd)) {
			if (validSession) {
				if (ds.delTask(req.getParameter("m"), req.getParameter("t"), Integer.parseInt(req.getParameter("y")),
						req.getParameter("o")))
					bw.write("Success");
				else
					bw.write("Failed");
			...
			if (validSession) {
				...
				for (MediaFileAPI.MediaFile mf : mediaList) {
					if ((qry.matches("\\d+") && Integer.toString(mf.GetMediaFileID()).startsWith(qry))
							|| mf.GetMediaTitle().matches(".*" + Pattern.quote(qry) + ".*")
							|| fileSegmentMatches(mf, qry)) {
						JSONObject o = new JSONObject();
						try {
							o.put("value", mf.GetFileForSegment(0).getAbsolutePath());
							String subtitle = null;
							if (mf.GetMediaFileAiring() != null && mf.GetMediaFileAiring().GetShow() != null)
								subtitle = mf.GetMediaFileAiring().GetShow().GetShowEpisode();
							String display;
							if (subtitle != null && subtitle.length() > 0)
								display = mf.GetMediaTitle() + ": " + subtitle;
							else
								display = mf.GetMediaTitle();
				...
				notAuthorized(resp, bw);
		} else if ("debugMediaFile".equals(cmd)) {
			if (validSession) {
				if (Butler.debugQueueLoader(req.getParameter("f")))
					bw.write("Success");
				else
					bw.write("Failed");
			} else
				notAuthorized(resp, bw);
		} else if ("killTask".equals(cmd)) {
			if (validSession) {
				if (TaskQueue.getInstance().killTask(req.getParameter("m"), req.getParameter("t"),
						req.getParameter("o")))
					bw.write("Success");
				else
					bw.write("Failed");
				...
				notAuthorized(resp, bw);
		} else if ("keepAlive".equals(cmd)) {
			bw.write(Boolean.toString(!TaskQueue.getInstance().isTaskKilled(req.getParameter("m"),
					req.getParameter("t"), req.getParameter("o"))));
		}
		bw.close();
		DataStore.returnConnection(ds);
