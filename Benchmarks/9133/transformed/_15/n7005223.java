								...
								if ((pokusyDirectories[i].isDirectory()) && (pzp < Integer
									...
									if ((testedFile.exists()) && (testedFile.isFile())) {
										...
										if (partsOfFilename.length == 4) {
											...
											if (posledniCastSouboru.length <= 1)
												priponaSouboru = "";
											else
												priponaSouboru = posledniCastSouboru[1];
										}
										...
										if (testedFile.getName().startsWith(samostatnaPraceNazev)) {
											...
											if (partsOfFilename.length != 4) {
												...
												lst.add(st);
												continue;
											} else if (!"java".equals(priponaSouboru)) {
												SouborType st = new SouborType();
												st.setJmeno(testedFile.getName());
												st.setDuvod("?patn? p??pona souboru.");
												lst.add(st);
												continue;
											} else if (!(osobniCisloZeSlozky != null
													&& osobniCisloZeSlozky.equals(osobniCisloZeSouboru))) {
												SouborType st = new SouborType();
												st.setJmeno(testedFile.getName());
									...
									for (int j = start; j >= 0; j--) {
										...
										for (int k = 0; k < vlastnostiSouboruNL.getLength(); k++) {
											...
											if (vlastnostiSouboruNL.item(k).getNodeName().equals("nazev")) {
												shodaJmenaSouboru = vlastnostiSouboruNL.item(k).getTextContent()
											...
											if (ct.getCislo().intValue() == cisloCviceniI) {
												...
												XMLGregorianCalendar xmlGCTime = DatatypeFactory.newInstance()
														.newXMLGregorianCalendarTime(Integer.parseInt(casSouboru[0]),
																...
																DatatypeConstants.FIELD_UNDEFINED);
												if (fileTimeStamp.compareTo(mtovGC) <= 0)
													odevzdanoVcas = true;
												else
													odevzdanoVcas = false;
												...
												for (UlohaType ut : lut) {
													if (ut.getCislo().intValue() == cisloUlohyI) {
														...
														vt.setVysledek("true".equals(vysledekValidaceSouboru));
