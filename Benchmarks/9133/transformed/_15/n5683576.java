		...
		try {
			...
			if (entity != null) {
				...
				for (;;) {
					...
					if ((fieldname != null && fieldname.equals("errorCode"))) {
						token = jp.nextToken();
						responseObj.setErrorCode(jp.getIntValue());
					} else if ((fieldname != null && fieldname.equals("errorMessage"))) {
						token = jp.nextToken();
						responseObj.setErrorMessage(jp.getText());
					} else if ((fieldname != null && fieldname.equals("statusCode"))) {
						token = jp.nextToken();
						responseObj.setStatusCode(jp.getText());
					} else if ((fieldname != null && fieldname.equals("results"))) {
				...
				if ((responseObj.getStatusCode() != null && responseObj.getStatusCode().equals("OK"))) {
