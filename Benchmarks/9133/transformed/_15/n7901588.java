class n7901588 {
	public static long removePropertyInOpenXMLDocument(String ext, InputStream in, OutputStreamProvider outProvider,
			String propriete) {
		...
		try {
			...
			if (Metadata.TITLE.equals(propriete)) {
				coreProperties.setTitle("");
			} else if (Metadata.AUTHOR.equals(propriete)) {
				coreProperties.setCreator("");
			} else if (Metadata.KEYWORDS.equals(propriete)) {
				coreProperties.getUnderlyingProperties().setKeywordsProperty("");
			} else if (Metadata.COMMENTS.equals(propriete)) {
				coreProperties.setDescription("");
			} else if (Metadata.SUBJECT.equals(propriete)) {
				coreProperties.setSubjectProperty("");
			} else if (Metadata.COMPANY.equals(propriete)) {
