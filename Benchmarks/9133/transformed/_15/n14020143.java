class n14020143 {
	public byte[] evaluateResponse(byte[] responseBytes) throws SaslException {
		if (firstEvaluation) {
			...
			if (!directives.containsKey("realm") || "".equals(((String) directives.get("realm")))) {
			...
			if ("null".equals(realm) && username.indexOf('@') == -1) {
			...
			if ((directives.get("response") != null && directives.get("response").equals(HexResponseHash))) {
				md.update(":".getBytes());
				md.update((directives.get("digest-uri")).getBytes());
				if (!directives.get("qop").equals(QOP_AUTH)) {
					md.update(":".getBytes());
