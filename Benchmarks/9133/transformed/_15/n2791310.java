	...
	protected synchronized AbstractBaseObject insert(AbstractBaseObject obj) throws ApplicationException {
		...
		synchronized (dbConn) {
			try {
				...
				if (tmpDmsRelationalWord.getWord() == null || (tmpDmsRelationalWord.getWord().trim() != null
						&& tmpDmsRelationalWord.getWord().trim().equals(""))) {
					return null;
				}
				...
				tmpDmsRelationalWord
						.setCreatorName(UserInfoFactory.getUserFullName(tmpDmsRelationalWord.getCreatorID()));
				tmpDmsRelationalWord
						.setUpdaterName(UserInfoFactory.getUserFullName(tmpDmsRelationalWord.getUpdaterID()));
				dbConn.commit();
