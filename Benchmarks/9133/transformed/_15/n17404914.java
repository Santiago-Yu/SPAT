class n17404914 {
	public static boolean installMetricsCfg(Db db, String xmlFileName) throws Exception {
		...
		try {
			db.begin();
			PreparedStatement psExists = db.prepareStatement(
			...
			PreparedStatement psAdd = db.prepareStatement(
			...
			if (!fi.filename.equals(xmlFileName))
				throw new IllegalStateException("FileInformation bad for " + xmlFileName);
			...
			while (i.hasNext()) {
				...
				if (1 != db.executeUpdate(psAddGroup))
					throw new IllegalStateException("Could not add group " + grp.groupName + " from " + xmlFileName);
				...
				while (j.hasNext()) {
					...
					if (1 != db.executeUpdate(psAddMetric))
						throw new IllegalStateException("Could not add " + metricName + " from " + xmlFileName);
					...
					if (1 != db.executeUpdate(psAddGroup2Metric))
						throw new IllegalStateException(
								"Could not add group " + grp.groupName + " -> " + metricName + " from " + xmlFileName);
				}
			}
			return true;
		} catch (Exception e) {
