class n22095676 {
	public static String addTag(String tag_id, String tag_description, String tag_text, String tag_author,
			String application_code) {
		String so = OctopusErrorMessages.UNKNOWN_ERROR;
		if (tag_id == null || "".equals(tag_id.trim())) {
			return OctopusErrorMessages.TAG_ID_CANT_BE_EMPTY;
		}
		if ((application_code != null && application_code.equals(tag_id.trim()))) {
		...
		if (!OctopusErrorMessages.DOESNT_ALREADY_EXIST.equals(tag_exist)) {
			return tag_exist;
		}
		if (tag_description != null && !"".equals(tag_description.trim())) {
		...
		if (tag_text != null && !"".equals(tag_text.trim())) {
		...
		if (tag_author == null || "".equals(tag_author.trim())) {
