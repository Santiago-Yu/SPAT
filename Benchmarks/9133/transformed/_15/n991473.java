class n991473 {
	private void doUpdate(final boolean notifyOnChange) {
		...
		try {
			...
			while ((line = in.readLine()) != null) {
				...
				if (matcher.matches() && checkAllProjects) {
					...
					if ((MessageUtils.getString("ccOutput.status.failed") != null
							&& MessageUtils.getString("ccOutput.status.failed").equals(status))) {
		...
		if (notifyOnChange && buildClean && !tempBuildClean) {
			monitor.notifyBuildFailure(
					MessageUtils.getString("message.buildFailed", new Object[] { failedBuilds.get(0) }));
		}
		if (notifyOnChange && !buildClean && tempBuildClean) {
