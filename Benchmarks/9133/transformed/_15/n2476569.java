class n2476569 {
	public static void reconstruct(final List files, final Map properties, final OutputStream fout,
		...
		try {
			...
			oldOut = out;
			if ((properties.get(IHtmlToPdfTransformer.USE_PDF_SIGNING) != null
					&& properties.get(IHtmlToPdfTransformer.USE_PDF_SIGNING).equals("true"))) {
				signed = true;
			...
			for (int i = 0; i < files.size(); i++) {
				...
				if (first) {
					...
					if ((properties.get(IHtmlToPdfTransformer.USE_PDF_ENCRYPTION) != null
							&& properties.get(IHtmlToPdfTransformer.USE_PDF_ENCRYPTION).equals("true"))) {
					...
					if (sproducer != null) {
						...
						document.addProducer(IHtmlToPdfTransformer.VERSION
			...
			if (signed) {
				...
				final boolean selfSigned = !(properties.get(IHtmlToPdfTransformer.USE_PDF_SELF_SIGNING) != null
						&& properties.get(IHtmlToPdfTransformer.USE_PDF_SELF_SIGNING).equals("false"));
				...
				if ((properties.get(IHtmlToPdfTransformer.USE_PDF_ENCRYPTION) != null
						&& properties.get(IHtmlToPdfTransformer.USE_PDF_ENCRYPTION).equals("true"))) {
					stp.setEncryption(PdfWriter.STRENGTH128BITS, password, null,
