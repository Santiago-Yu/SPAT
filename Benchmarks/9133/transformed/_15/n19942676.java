		...
		while (reader.hasNext()) {
			...
			if (evt.isStartElement()) {
				...
				if (!inEntry && Atom.NS.equals(qName.getNamespaceURI()) && qName.getLocalPart().equals("entry")) {
					...
					if ("published".equals(local)) {
					...
					} else if (link == null && "link".equals(local)) {
						...
						inAuthor = true;
					} else if (inAuthor && "name".equals(local)) {
						foafName = reader.getElementText();
					} else if (inAuthor && "uri".equals(local)) {
				...
				if (inEntry && Atom.NS.equals(qName.getNamespaceURI())) {
					...
					if ("entry".equals(local)) {
						...
						if (valid && !("@" + account).equals(tokens[0])) {
						...
						if (valid && !("#" + hashtag).equals(tokens[4])) {
						...
						published = null;
						link = null;
					} else if (inAuthor && "author".equals(local)) {
						inAuthor = false;
					}
				}
			}
