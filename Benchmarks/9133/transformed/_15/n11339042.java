class n11339042 {
	public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		try {
			...
			TurbineConfig tc = new TurbineConfig(relativepath + "..",
					relativepath + getServletConfig().getInitParameter("properties"));
			tc.init();
			int spectrumId = -1;
			...
			if ("test".equals(action)) {
				try {
					...
					if (l.size() > 0)
						spectrumId = ((Record) l.get(0)).getValue(1).asInt();
					out.write("success");
				} catch (Exception ex) {
					out.write("failure");
				}
				...
				if (req.getParameter("numbertoexport") != null) {
					try {
						...
						if (numbertoexport < 1 || numbertoexport > 20)
				...
				for (int i = 0; i < numbertoexport; i++) {
					if (i == l.size())
						break;
					DBMolecule mol = DBMoleculePeer
							.retrieveByPK(new NumberKey(((Record) l.get(i)).getValue(1).asInt()));
				...
				JasperReport jasperReport = (JasperReport) JRLoader.loadObject(
						relativepath + "/reports/" + (yearly ? "yearly" : "monthly") + "_report_" + type + ".jasper");
				Map parameters = new HashMap();
				if (yearly)
					parameters.put("HEADER", "Report for years " + yearstart + " - " + yearend);
				else
					parameters.put("HEADER",
							"Report for " + monthstart + "/" + yearstart + " - " + monthend + "/" + yearend);
				DBConnection dbconn = TurbineDB.getConnection();
				Connection conn = dbconn.getConnection();
				Statement stmt = conn.createStatement();
				ResultSet rs = null;
				if (type == 1) {
					rs = stmt.executeQuery("select YEAR(DATE) as YEAR, " + (yearly ? "" : " MONTH(DATE) as MONTH, ")
							...
							+ yearstart + " and YEAR(DATE)<=" + yearend + " and LOGIN_NAME<>'testuser' group by YEAR, "
							+ (yearly ? "" : "MONTH, ") + "AFFILIATION_1, AFFILIATION_2, MACHINE.NAME");
				} else if (type == 2) {
					rs = stmt.executeQuery("select YEAR(DATE) as YEAR, " + (yearly ? "" : " MONTH(DATE) as MONTH, ")
							...
							+ (yearly ? "" : "MONTH, ") + "MACHINE.NAME");
				}
				JasperPrint jasperPrint = JasperFillManager.fillReport(jasperReport, parameters,
						new JRResultSetDataSource(rs));
				JasperExportManager.exportReportToPdfStream(jasperPrint, outstream);
				dbconn.close();
			} else if ("exportcmlbyinchi".equals(action)) {
				res.setContentType("text/xml");
				out = res.getWriter();
			...
			} else if ("namelist".equals(action)) {
				res.setContentType("application/zip");
				outstream = res.getOutputStream();
				...
				for (int i = 0; i < v.size(); i++) {
					if (i % 500 == 0) {
						if (i != 0) {
							zipout.write(new String("<p>The list is continued <a href=\"nmrshiftdb.names." + i
						...
						zipout.write(new String(
								"<html><body><h1>This is a list of strcutures in <a href=\"http://www.nmrshiftdb.org\">NMRShiftDB</a>, starting at "
										+ i + ", Its main purpose is to be found by search engines</h1>").getBytes());
					}
					...
					for (int k = 0; k < spectra.size(); k++) {
						zipout.write(new String(((DBSpectrum) spectra.get(k)).getDBSpectrumType().getName() + ", ")
								.getBytes());
					}
					if (mol.hasAny3d())
						zipout.write(new String("3D coordinates, ").getBytes());
			...
			} else if ("predictor".equals(action)) {
				...
				JarInputStream jip = new JarInputStream(new FileInputStream(
				...
				while (true) {
					...
					ResultSet rs = stmt.executeQuery(
							"select HOSE_CODE, VALUE, SYMBOL from HOSE_CODES where CONDITION_TYPE='m' and WITH_RINGS=0 and SYMBOL='"
									+ req.getParameter("symbol") + "' limit " + (i * 1000) + ", 1000");
					int m = 0;
					while (rs.next()) {
				...
				while (it.hasNext()) {
					...
					while (it2.hasNext()) {
						...
						zipout.write(new String(symbol + "|" + hoseCode + "|" + Statistics.minimum(values) + "|"
								+ Statistics.average(values) + "|" + Statistics.maximum(values) + "\r\n").getBytes());
					}
				}
				zipout.closeEntry();
				zipout.close();
			...
			} else if ("exportspec".equals(action) || "exportmol".equals(action)) {
				if (spectrumId > -1)
					spectrum = DBSpectrumPeer.retrieveByPK(new NumberKey(spectrumId));
				else
					spectrum = DBSpectrumPeer.retrieveByPK(new NumberKey(req.getParameter("spectrumid")));
				export = new Export(spectrum);
			} else if ("exportmdl".equals(action)) {
				res.setContentType("text/plain");
				outstream = res.getOutputStream();
				DBMolecule mol = DBMoleculePeer.retrieveByPK(new NumberKey(req.getParameter("moleculeid")));
				outstream.write(
						mol.getStructureFile(Integer.parseInt(req.getParameter("coordsetid")), false).getBytes());
			} else if ("exportlastinputs".equals(action)) {
				format = action;
			} else if ("printpredict".equals(action)) {
				res.setContentType("text/html");
				out = res.getWriter();
				HttpSession session = req.getSession();
				VelocityContext context = PredictPortlet.getContext(session, true, true, new StringBuffer(),
						getServletConfig(), req, true);
			...
			if (format == null)
				format = "";
			if ("pdf".equals(format) || "rtf".equals(format)) {
			...
			if ("mdl".equals(format) || "txt".equals(format) || "cml".equals(format) || "cmlboth".equals(format)
					|| format.indexOf("exsection") == 0) {
				res.setContentType("text/plain");
				out = res.getWriter();
			}
			if ("simplehtml".equals(format) || "exportlastinputs".equals(format)) {
				res.setContentType("text/html");
				out = res.getWriter();
			}
			if ("exportlastinputs".equals(action)) {
				...
				if (user != null) {
					...
					if (((new Date().getTime() - lastDownloadDate.getTime()) / 3600000) < 24) {
						out.println("Your last download was at " + lastDownloadDate
						...
						String query = "select distinct MOLECULE.MOLECULE_ID from MOLECULE, SPECTRUM where SPECTRUM.MOLECULE_ID = MOLECULE.MOLECULE_ID and SPECTRUM.REVIEW_FLAG =\"true\" and SPECTRUM.USER_ID="
								+ user.getUserId() + " order by MOLECULE.DATE desc;";
						...
						for (int i = 0; i < numbertoexport; i++) {
							...
							parameters.add(new String("<a href=\"" + url
									...
									+ mol.getMoleculeId() + "\"><img src=\""
									+ javax.servlet.http.HttpUtils.getRequestURL(req).toString()
									+ "?nmrshiftdbaction=exportmol&spectrumid="
									+ ((DBSpectrum) mol.getDBSpectrums().get(0)).getSpectrumId()
			...
			if ("exportspec".equals(action)) {
				if ("txt".equals(format)) {
					String lastsearchtype = req.getParameter("lastsearchtype");
					if (NmrshiftdbConstants.TOTALSPECTRUM.equals(lastsearchtype)
							|| NmrshiftdbConstants.SUBSPECTRUM.equals(lastsearchtype)) {
						List l = ParseUtils.parseSpectrumFromSpecFile(req.getParameter("lastsearchvalues"));
						spectrum.initSimilarity(l, NmrshiftdbConstants.SUBSPECTRUM.equals(lastsearchtype));
					}
					Vector v = spectrum.getOptions();
					DBMolecule mol = spectrum.getDBMolecule();
					out.print(mol.getChemicalNamesAsOneString(false) + mol.getMolecularFormula(false) + "; "
							+ mol.getMolecularWeight() + " Dalton\n\r");
					out.print("\n\rAtom\t");
					if (spectrum.getDBSpectrumType().getElementSymbol() == ("H"))
						out.print("Mult.\t");
					out.print("Meas.");
					if (NmrshiftdbConstants.TOTALSPECTRUM.equals(lastsearchtype)
							|| NmrshiftdbConstants.SUBSPECTRUM.equals(lastsearchtype)) {
					...
					out.print("\n\r");
					for (int i = 0; i < v.size(); i++) {
						out.print(((ValuesForVelocityBean) v.get(i)).getDisplayText() + "\t"
								+ ((ValuesForVelocityBean) v.get(i)).getRange());
						if (NmrshiftdbConstants.TOTALSPECTRUM.equals(lastsearchtype)
								|| NmrshiftdbConstants.SUBSPECTRUM.equals(lastsearchtype)) {
							out.print("\t" + ((ValuesForVelocityBean) v.get(i)).getNameForElements() + "\t"
									+ ((ValuesForVelocityBean) v.get(i)).getDelta());
						}
						out.print("\n\r");
					}
				}
				if ("simplehtml".equals(format)) {
					String i1 = export.getImage(false, "jpeg",
							ServletUtils.expandRelative(this.getServletConfig(), "/nmrshiftdbhtml") + "/tmp/"
									+ System.currentTimeMillis(),
							true);
					export.pictures[0] = new File(i1).getName();
					String i2 = export.getImage(true, "jpeg",
							ServletUtils.expandRelative(this.getServletConfig(), "/nmrshiftdbhtml") + "/tmp/"
									+ System.currentTimeMillis(),
							true);
				...
				if ("pdf".equals(format) || "rtf".equals(format)) {
					...
					Fop fop = fopFactory.newFop("rtf".equals(format) ? MimeConstants.MIME_RTF : MimeConstants.MIME_PDF,
							foUserAgent, out2);
				...
				if ("tiff".equals(format) || "jpeg".equals(format) || "png".equals(format)) {
					InputStream is = new FileInputStream(
							export.getImage(false, format, relativepath + "/tmp/" + System.currentTimeMillis(), true));
					byte[] buf = new byte[32 * 1024];
					int nRead = 0;
					while ((nRead = is.read(buf)) != -1) {
						outstream.write(buf, 0, nRead);
				...
				if ("cml".equals(format)) {
					out.print(spectrum.getCmlSpect().toXML());
				}
				if ("cmlboth".equals(format)) {
					Element cmlElement = new Element("cml");
					cmlElement.addAttribute(new Attribute("convention", "nmrshiftdb-convention"));
					cmlElement.setNamespaceURI("http://www.xml-cml.org/schema");
					Element parent = spectrum.getDBMolecule().getCML(1,
							spectrum.getDBSpectrumType().getName().equals("1H"));
					nu.xom.Node cmldoc = parent.getChild(0);
					((Element) cmldoc).setNamespaceURI("http://www.xml-cml.org/schema");
					parent.removeChildren();
					cmlElement.appendChild(cmldoc);
			...
			if ("exportmol".equals(action)) {
				...
				if ("tiff".equals(format) || "jpeg".equals(format) || "png".equals(format)) {
					InputStream is = new FileInputStream(export.getImage(true, format,
							relativepath + "/tmp/" + System.currentTimeMillis(), width, height, shownumbers, null));
					byte[] buf = new byte[32 * 1024];
					int nRead = 0;
					while ((nRead = is.read(buf)) != -1) {
						outstream.write(buf, 0, nRead);
