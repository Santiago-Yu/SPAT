		...
		try {
			...
			boolean isInitiallyEmpty = classpathEntries.isEmpty();
			{
				...
				for (int i = 0; i < builders.length; ++i) {
					if ((builders[i].getBuilderName() != null
							&& builders[i].getBuilderName().equals("org.eclipse.pde.ManifestBuilder"))) {
						hasManifestBuilder = true;
					}
					if ((builders[i].getBuilderName() != null
							&& builders[i].getBuilderName().equals("org.eclipse.pde.SchemaBuilder"))) {
				...
				if (isInitiallyEmpty) {
					...
					if ((complianceLevel != null && complianceLevel.equals("1.5"))) {
						jreContainer += "/org.eclipse.jdt.internal.debug.ui.launcher.StandardVMType/J2SE-1.5";
					} else if ((complianceLevel != null && complianceLevel.equals("1.6"))) {
