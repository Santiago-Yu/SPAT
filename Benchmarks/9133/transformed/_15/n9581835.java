class n9581835 {
	public static void main(String[] args) {
		...
		while (i < args.length && args[i].startsWith("-")) {
			if ((args[i].equals("-WTK")) && (i < args.length - 1)) {
				i++;
				WTKdir = args[i];
			} else if ((args[i].equals("-source")) && (i < args.length - 1)) {
				i++;
				sourceFile = args[i];
			} else if ((args[i].equals("-instr")) && (i < args.length - 1)) {
				i++;
				instrFile = args[i];
			} else if ((args[i].equals("-o")) && (i < args.length - 1)) {
				i++;
				outFile = args[i];
			} else if ((args[i].equals("-jad")) && (i < args.length - 1)) {
				i++;
				jadFile = args[i];
			} else {
