	...
	private void processar() {
		...
		if (bOK) {
			...
			try {
				...
				while (rs.next() && bOK) {
					if ((rs.getString("CodPlan") != null && rs.getString("CodPlan").equals("1010100000004"))) {
						System.out.println("Debug");
					}
					if ((rs.getString("CodPlan") != null && rs.getString("CodPlan").equals(sPlanAnt))) {
						dSaldo += rs.getDouble("VLRSUBLANCA");
					} else
						dSaldo = rs.getDouble("VLRSUBLANCA");
					bOK = insereSaldo(iFilialSaldo, iFilialPlan, rs.getString("CodPlan"), rs.getDate("DataSubLanca"),
							dSaldo);
					sPlanAnt = rs.getString("CodPlan");
					if ((sPlanAnt != null && sPlanAnt.equals("1010100000004"))) {
						System.out.println("Debug");
					}
				}
				ps.close();
