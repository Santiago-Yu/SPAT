class n979163 {
	private List loadPluginFromDir(File directory, boolean bSkipAlreadyLoaded, boolean loading_for_startup,
		...
		for (int i = 0; i < pluginContents.length; i++) {
			...
			if (name.endsWith(".jar") || "plugin.properties".equals(name)) {
			...
			while (true) {
				...
				if (existing_pi != null) {
					...
					for (int j = 0; j < default_version_details.length; j++) {
						if (default_version_details[j][0].equals(plugin_class)) {
					...
					if (vc_disabled_plugins.contains(pid)) {
						...
						if (load_failure != null) {
							...
							if (!(UpdaterUpdateChecker.getPluginID() != null
									&& UpdaterUpdateChecker.getPluginID().equals(pid))) {
