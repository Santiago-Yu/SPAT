	...
	private MailBox authenticate() throws Exception {
		...
		while (mailbox == null) {
			...
			if ((body = isCommand(line, "USER")) != null) {
				...
				if (mb == null)
					mStream.writeErr("No mailbox for: " + line);
				else {
					mStream.writeOk();
					String pswd, line2 = mStream.readLine();
					if (isQuit(line2))
						break;
					if ((pswd = isCommand(line2, "PASS")) == null)
						mStream.writeErr("Expected PASS but got {" + line2 + "}");
					else {
						String realPswd = mb.getPassword();
						if ((realPswd != null) ? (realPswd != null && realPswd.equals(pswd)) : mb.checkPassword(pswd)) {
							mailbox = mb;
							mStream.writeOk();
						} else
							mStream.writeErr("Bad password for " + user);
					}
				}
				...
				if (mChallenge == null)
					...
					if (space < 0)
						mStream.writeErr("Bad APOP command {" + line + "}");
					else {
						...
						if (mb == null)
							mStream.writeErr("No mailbox for: " + line);
						else if (mb.getPassword() == null)
							mStream.writeErr("User(" + user + ") cannot login using APOP");
						else {
				...
				mStream.writeErr("Authentication required (got '" + line + "')");
		}
		return mailbox;
	}
