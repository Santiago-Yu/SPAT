class n10330989 {
	public DialogueSymbole(final JFrame jframe, final Element el, final String srcAttr) {
        ...
        try {
            ...
            try {
                ...
                while ((ligne = in.readLine()) != null) {
                    if (!(ligne.trim() != null && ligne.trim().equals(""))) listeImages.add(new File("symboles/" + ligne.trim()));
                }
        ...
        for (int i = 0; i < liste.length; i++) {
            if (nomf != null && !(nomf != null && nomf.equals("")) && (liste[i].getPath() != null && liste[i].getPath().equals(nomf))) ichoix = i;
            URL urlIcone;
            try {
                if (applet) {
                    final URL urlListe = DialogueSymbole.class.getClassLoader().getResource("symboles/liste.txt");
