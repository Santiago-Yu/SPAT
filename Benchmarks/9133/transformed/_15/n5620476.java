class n5620476 {
	private String[] verifyConnection(Socket clientConnection) throws Exception {
		...
		while (in.ready() && (line = in.readLine()) != null) {
			if ("".equals(line)
					&& (headerVars.get("Content-Type") == null || headerVars.get("Content-Length") == null)) {
				break;
			}
			...
			if (!readingBoundary && line.contains(": ")) {
				...
				continue;
			} else if ("".equals(line) && boundaryStack.isEmpty()) {
			...
			} else if ((boundaryStack.peek() != null && boundaryStack.peek().equals(line)) && !readingBoundary) {
			...
			} else if (line.contains("") && readingBoundary && !boundaryStack.isEmpty()
				...
				if (headerVars.getProperty("Content-Transfer-Encoding").contains("binary")) {
					...
					if (!uploadFilePath.canWrite()) {
						logger.error("VOctopus Webserver doesn't have permissions to write on temporary dir: "
								+ uploadFilePath.getCanonicalPath());
					}
