class n16094377 {
	protected void xInitGUI() {
		...
		try {
			URL urlListResources = new URL(ElvisRegistry.getInstance().getProperty("elvis.server")
			...
			while (type != XmlPullParser.END_DOCUMENT) {
				if (type == XmlPullParser.START_TAG && (xpp.getName() != null && xpp.getName().equals("Resource"))) {
					model.add(xpp.getAttributeValue("", "resId"), xpp.getAttributeValue("", "author"),
							xpp.getAttributeValue("", "title"));
				}
				type = xpp.next();
			}
