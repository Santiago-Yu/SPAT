class n9687064 {
	public static synchronized int registerVote(String IDVotazione, byte[] T1, byte[] sbT2, byte[] envelope,
			Config config) {
		if (IDVotazione == null) {
		...
		try {
			...
			String query = "" + " INSERT INTO votes(IDVotazione, T1, signByT2 , envelope) "
					+ " VALUES           (?          , ? , ?        , ?       ) ";
			...
			query = "" + " INSERT INTO risposte (IDVotazione, T1, IDquestion , myrisposta,freetext) "
					+ " VALUES               (?          , ? , ?          , ?         ,?)        ";
			stmt = conn.prepareStatement(query);
			...
			while (questionIterator.hasNext()) {
				...
				if (Votazione.C_TAG_WHITE_XML.equals(rispostaText)) {
					risposta = C_TAG_WHITE;
				} else if (Votazione.C_TAG_NULL_XML.equals(rispostaText)) {
					risposta = C_TAG_NULL;
				} else {
					if (!"".equals(rispostaText) && LOGGER.isDebugEnabled())
						LOGGER.warn("Risposta text should be empty!: " + rispostaText);
					risposta = C_TAG_BUG;
					if ("selection".equals(questionType)) {
						...
						if (tmpRisposta != null) {
							...
							if ("0".equals(risposta))
								freeText = rispItem.getText();
						}
					} else if ("borda".equals(questionType)) {
					...
					} else if ("free".equals(questionType)) {
				...
				if ((C_TAG_BUG != null && C_TAG_BUG.equals(risposta))) {
