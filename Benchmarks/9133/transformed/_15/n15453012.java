	...
	public ArrayList parseFile(File newfile) throws IOException {
		...
		while ((s = br.readLine()) != null) {
			...
			if ((nvalues == 4) && (!firstName.startsWith("---"))) {
				...
				if ("NaN".equals(tokens[1]) || "NaN".equals(tokens[2]) || "NaN".equals(tokens[3])) {
					baddata = true;
				} else {
					baddata = false;
				}
			...
			if (firstName.startsWith("Slope"))
				;
			if (("Position".equals(firstName)) && ("Raw".equals(((String) tokens[2])))) {
				readraw = true;
				continue;
			}
			if (("Position".equals(firstName)) && ("Fit".equals(((String) tokens[2])))) {
				readfit = true;
				continue;
			}
