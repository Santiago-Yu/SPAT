class n5021563 {
	private String AddAction(ResultSet node, String modo) throws SQLException {
		...
		try {
			cn = this.getRepositoryConnection(p_ctx.getApplication(), p_ctx.getBoSession().getRepository().getName(),
					1);
			cndef = this.getRepositoryConnection(p_ctx.getApplication(), p_ctx.getBoSession().getRepository().getName(),
					2);
			String dml = null;
			...
			if (objecttype.equalsIgnoreCase("T")) {
				...
				String[] sysfdef = { "VARCHAR(25)", "NUMERIC(7)", "TIMESTAMP DEFAULT now()", "TIMESTAMP",
						"VARCHAR(30)" };
				String[] sysftyp = { "C", "N", "D", "D", "C" };
				String[] sysfsiz = { "25", "7", "", "", "30" };
				String[] sysfndef = { "", "", "", "", "" };
				String[] sysfdes = { "", "", "", "", "" };
				if (!exists && !"3".equals(modo)) {
				...
				checkDicFields(node.getString("OBJECTNAME"), node.getString("SCHEMA"), sysflds, sysftyp, sysfsiz,
						sysfndef, sysfdes);
			}
			if (objecttype.equalsIgnoreCase("F")) {
				...
				if (rslt.next()) {
					...
					if (",C,N,".indexOf("," + getNgtFieldTypeFromDDL(rslt.getString(2)) + ",") != -1) {
						...
						if (fielddec != 0) {
							if (!(node.getString("FIELDSIZE") != null
									&& node.getString("FIELDSIZE").equals((fieldsiz + "," + fielddec)))) {
								fldchg = true;
							}
						} else {
							if (!((fieldsiz == 0) && ((node.getString("FIELDSIZE") == null)
									|| (node.getString("FIELDSIZE").length() == 0)))) {
								if (!(node.getString("FIELDSIZE") != null
										&& node.getString("FIELDSIZE").equals(("" + fieldsiz)))) {
									fldchg = true;
								}
							}
						}
				...
				if (("20".indexOf(modo) != -1) && !fldexi) {
					dml = "ALTER TABLE " + node.getString("TABLENAME") + " add \""
							+ node.getString("OBJECTNAME").toLowerCase() + "\" ";
				} else if (("20".indexOf(modo) != -1) && fldexi && fldchg) {
					dml = "ALTER TABLE " + node.getString("TABLENAME") + " ALTER COLUMN \""
							+ node.getString("OBJECTNAME").toLowerCase() + "\" ";
				} else if ("3".equals(modo) && fldexi) {
					dml = "ALTER TABLE " + node.getString("TABLENAME") + " drop  COLUMN \""
							+ node.getString("OBJECTNAME").toLowerCase() + "\" ";
					...
					while (rsltrelc.next()) {
						...
						pstmdic.setString(2, "R".equals(consttype) ? "FK" : "PK");
						pstmdic.setString(3, constname);
						int nrecs = pstmdic.executeUpdate();
						pstm.close();
						executeDDL("ALTER TABLE " + node.getString("TABLENAME") + " DROP CONSTRAINT " + constname,
								node.getString("SCHEMA"));
					}
					rsltrelc.close();
					pstmrelc.close();
				}
				if ((dml != null) && (dml.length() > 0) && !"3".equals(modo)) {
					String mfield = node.getString("MACROFIELD");
					if ((mfield != null) && !(!"TEXTOLIVRE".equals(mfield) && !"NUMEROLIVRE".equals(mfield)
							&& !"TEXT".equals(mfield) && !"BLOB".equals(mfield) && !"MDATA".equals(mfield))) {
						String ngtft = "";
						if ("TEXTOLIVRE".equals(mfield)) {
							ngtft = "C";
						} else if ("NUMEROLIVRE".equals(mfield)) {
							ngtft = "N";
						} else if ("RAW".equals(mfield)) {
							ngtft = "RAW";
						} else if ("TIMESTAMP".equals(mfield)) {
							ngtft = "TIMESTAMP";
						} else if ("MDATA".equals(mfield)) {
							ngtft = "D";
						} else if ("TEXT".equals(mfield)) {
							ngtft = "CL";
						} else if ("BLOB".equals(mfield)) {
							ngtft = "BL";
						}
			...
			if (objecttype.equalsIgnoreCase("V")) {
				...
				if (!"3".equals(modo)) {
					...
					if (!(viewText != null && viewText.equals(vExpression.toLowerCase()))) {
				...
				if ("02".indexOf(modo) != -1) {
					if (templatestr.indexOf(bstr) != -1) {
						...
						if (templatestr.indexOf(bstr, defpos) != -1) {
							dml = templatestr.substring(templatestr.indexOf(bstr, defpos),
			...
			if (objecttype.startsWith("PK") || objecttype.startsWith("UN")) {
				...
				boolean diff = !(node.getString("EXPRESSION") != null
						&& node.getString("EXPRESSION").equals(expression.toString().toUpperCase()));
				rslt.close();
				pstm.close();
				if (("3".equals(modo) || diff) && exists) {
					sql = "SELECT tc.constraint_name,tc.table_name" + " FROM information_schema.table_constraints tc"
							+ " LEFT JOIN information_schema.key_column_usage kcu"
							+ " ON tc.constraint_catalog = kcu.constraint_catalog"
							+ " AND tc.constraint_schema = kcu.constraint_schema"
							+ " AND tc.constraint_name = kcu.constraint_name"
					...
					while (rsltrefs.next()) {
						...
						pstmdelref.close();
						executeDDL(
								"alter table " + rsltrefs.getString(2) + "  drop constraint " + rsltrefs.getString(1),
								node.getString("SCHEMA"));
					}
					rsltrefs.close();
					pstmrefs.close();
					...
					sql = "SELECT tc.constraint_name" + " FROM information_schema.table_constraints tc"
							...
							+ "kcu.column_name in (" + insql + ")" + " and tc.table_schema=?";
					...
					while (rsltrefs.next()) {
						...
						pstmdelref.executeUpdate();
						pstmdelref.close();
						executeDDL("alter table " + node.getString("TABLENAME") + " drop constraint "
								+ rsltrefs.getString(1), node.getString("SCHEMA"));
					}
					rsltrefs.close();
					pstmrefs.close();
					if (exists && diff) {
						dml = "alter table " + node.getString("TABLENAME") + " drop constraint "
								+ node.getString("OBJECTNAME");
						try {
							executeDDL(dml, node.getString("SCHEMA"));
						} catch (Exception e) {
				...
				if (!"3".equals(modo) && (!exists || diff)) {
					if (isunique) {
						dml = "alter table " + node.getString("TABLENAME") + " add constraint "
								+ node.getString("OBJECTNAME") + " unique (" + node.getString("EXPRESSION") + ")";
					} else {
						dml = "alter table " + node.getString("TABLENAME") + " add primary key ("
			...
			if (objecttype.startsWith("FK")) {
				...
				rslt.close();
				pstm.close();
				if (exists && !(cExpress != null && cExpress.equals(express))) {
					dml = "alter table " + node.getString("TABLENAME") + " drop constraint "
							+ node.getString("OBJECTNAME");
					executeDDL(dml, node.getString("SCHEMA"));
				}
				if (!"3".equals(modo) && (!exists || !(cExpress != null && cExpress.equals(express)))) {
					dml = "alter table " + node.getString("TABLENAME") + " add constraint "
							+ node.getString("OBJECTNAME") + " foreign key (" + node.getString("EXPRESSION")
			...
			if (objecttype.startsWith("IDX")) {
				...
				String sql = "SELECT n.nspname" + " FROM pg_catalog.pg_class c"
				...
				if (!drop) {
					drop = (!(oldexpression != null && oldexpression.equals(newexpression)))
							&& !"".equals(oldexpression);
				}
				if (exists && (drop || "3".equals(modo))) {
					if (!dbunflag) {
				...
				if (!exists && !"3".equals(modo)) {
					if (!node.getString("OBJECTNAME").equals("") && !"".equals(newexpression)) {
						if (!unflag) {
							dml = "CREATE INDEX " + node.getString("OBJECTNAME") + " ON " + node.getString("TABLENAME")
									+ "(" + newexpression + ")";
						} else {
							dml = "ALTER TABLE " + node.getString("TABLENAME") + " ADD CONSTRAINT "
									+ node.getString("OBJECTNAME") + " UNIQUE (" + newexpression + ")";
						}
						executeDDL(dml, node.getString("SCHEMA"));
					}
				}
