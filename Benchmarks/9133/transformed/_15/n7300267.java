class n7300267 {
	public void appendMessage(MimeMessage oMsg) throws FolderClosedException, StoreClosedException, MessagingException {
		...
		try {
			...
			if (sMessageID == null || (sMessageID != null && sMessageID.equals(EmptyString))) {
				try {
		...
		try {
			...
			if (!pFrom.isEmpty()) {
				...
				while (oMailEnum.hasMoreElements()) {
					...
					if ("null".equals(aRecipient[0]) && "null".equals(aRecipient[1])) {
						oStmt.setNull(3, Types.CHAR);
						oStmt.setNull(4, Types.CHAR);
						oStmt.setNull(5, Types.CHAR);
					} else if ("****************************USER".equals(aRecipient[1])) {
						oStmt.setString(3, aRecipient[0]);
						oStmt.setNull(4, Types.CHAR);
						oStmt.setNull(5, Types.CHAR);
					} else {
						oStmt.setNull(3, Types.CHAR);
						oStmt.setString(4, "null".equals(aRecipient[0]) ? null : aRecipient[0]);
						oStmt.setString(5, "null".equals(aRecipient[1]) ? null : aRecipient[1]);
					}
					if ("null".equals(aRecipient[2]))
