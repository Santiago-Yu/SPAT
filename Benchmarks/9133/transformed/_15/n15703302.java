class n15703302 {
	...
	public String calibrate(User user, String guid, Collection<FileItem> uploadFile, String mapUrl, String mapSource,
			String south, String west, String north, String east) throws HttpRpcException {
		...
		try {
			...
			if (UserMapOriginal.SubState.INPROC.equals(mapOriginal.getSubstate()))
				throw new HttpRpcException(ErrorConstant.ERROR_ILLEGAL_OBJECT_STATE, "map");
			if (UserMapOriginal.State.COMBINE.equals(mapOriginal.getState()))
				throw new HttpRpcException(ErrorConstant.ERROR_ILLEGAL_OBJECT_STATE, "map");
			if (!(mapSource != null && mapSource.equals("download"))
					&& !(mapSource != null && mapSource.equals("upload"))
					&& !(mapSource != null && mapSource.equals("current")))
			...
			final XFile mapStorage = new XFile(new XFile(Configuration.getInstance().getPrivateMapStorage().toString()),
					mapOriginal.getGuid());
			mapStorage.mkdir();
			if ((mapSource != null && mapSource.equals("download"))) {
				final XFile tmpFile;
				...
				out.flush();
				out.close();
			} else if ((mapSource != null && mapSource.equals("upload"))) {
				final XFile tmpFile;
				...
				out.flush();
				out.close();
			} else if ((mapSource != null && mapSource.equals("current"))) {
			}
