		...
		try {
			...
			while (it.hasNext()) {
				...
				if (mod.getOp() == LDAPModification.REPLACE) {
					...
					if ((db2ldap.get("first") != null && db2ldap.get("first").equals(attributeName))
							|| (db2ldap.get("last") != null && db2ldap.get("last").equals(attributeName))) {
						PreparedStatement ps = con.prepareStatement("UPDATE USERS SET "
								+ ((db2ldap.get("first") != null && db2ldap.get("first").equals(attributeName))
										? "first"
										: "last")
								+ "=? WHERE username=?");
						...
						ps.executeUpdate();
						ps.close();
					} else if ((db2ldap.get("username") != null && db2ldap.get("username").equals(attributeName))) {
						throw new LDAPException("Can not modify the rdn", LDAPException.NOT_ALLOWED_ON_RDN,
								"Can not perform modify");
					} else if ((db2ldap.get("name") != null && db2ldap.get("name").equals(attributeName))) {
						...
						for (int i = 0; i < vals.length; i++) {
							...
							if (!rs.next()) {
								con.rollback();
								throw new LDAPException("Location " + vals[i] + " does not exist",
										LDAPException.OBJECT_CLASS_VIOLATION,
										"Location " + vals[i] + " does not exist");
							}
