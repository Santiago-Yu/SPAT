class n11787058 {
	protected int authenticate(long companyId, String login, String password, String authType, Map headerMap,
		...
		if (CompanyImpl.AUTH_TYPE_EA.equals(authType)) {
			if (!Validator.isEmailAddress(login)) {
				throw new UserEmailAddressException();
			}
		} else if (CompanyImpl.AUTH_TYPE_SN.equals(authType)) {
			if (Validator.isNull(login)) {
				throw new UserScreenNameException();
			}
		} else if (CompanyImpl.AUTH_TYPE_ID.equals(authType)) {
			if (Validator.isNull(login)) {
		...
		if (CompanyImpl.AUTH_TYPE_EA.equals(authType)) {
			...
			authResult = AuthPipeline.authenticateByUserId(authPipelinePre, companyId, userId, password, headerMap,
					parameterMap);
		}
		User user = null;
		try {
		...
		if (authResult == Authenticator.SUCCESS) {
			...
			if (CompanyImpl.AUTH_TYPE_EA.equals(authType)) {
				...
				authResult = AuthPipeline.authenticateByUserId(authPipelinePost, companyId, userId, password, headerMap,
						parameterMap);
			}
		}
