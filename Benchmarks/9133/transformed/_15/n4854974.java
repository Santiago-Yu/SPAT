class n4854974 {
	public void GetFile(ClientConnector cc, Map<String, String> attributes) throws Exception {
		...
		if ((sourcePortString != null) && ("".equals(sourcePortString))) {
			try {
		...
		if ((sourceUser == null) || ("".equals(sourceUser))) {
		...
		try {
			...
			if (!FTPReply.isPositiveCompletion(reply)) {
				ftp.disconnect();
				throw new Exception("Failed to connect to \"" + sourceNode + "\"  as user \"" + sourceUser
			...
			throw new Exception("Failed to connect to \"" + sourceNode + "\"  as user \"" + sourceUser + "\" - ERROR: "
					+ socketException);
		} catch (IOException ioe) {
			throw new Exception(
					"Failed to connect to \"" + sourceNode + "\"  as user \"" + sourceUser + "\" - ERROR: " + ioe);
		}
		...
		if (sourceFileType != null) {
			if ("A".equals(sourceFileType)) {
				log.debug("Set File Type to ASCII");
				ftp.setFileType(FTP.ASCII_FILE_TYPE);
			} else if ("B".equals(sourceFileType)) {
				log.debug("Set File Type to BINARY");
				ftp.setFileType(FTP.BINARY_FILE_TYPE);
			} else if ("E".equals(sourceFileType)) {
		...
		try {
			...
			if (in == null) {
				throw new Exception("Failed get the file \"" + sourceFile + "\" from \"" + sourceNode + "\"  - ERROR: "
						+ ftp.getReplyString());
			}
		} catch (IOException ioe2) {
			ftp.disconnect();
			log.error("Failed get the file \"" + sourceFile + "\" from \"" + sourceNode + "\"  - ERROR: " + ioe2);
			throw new Exception("Failed to retrieve file from \"" + sourceNode + "\"  as user \"" + sourceUser
					+ "\" - ERROR: " + ioe2);
		}
		...
		if ((sourceMD5 != null) && (!"".equals(sourceMD5))) {
			log.debug("File DIGEST compare - Source: " + sourceMD5.toLowerCase() + " | Received: " + receivedMD5);
			if (!(sourceMD5.toLowerCase() != null && sourceMD5.toLowerCase().equals(receivedMD5))) {
