class n19467546 {
	...
	public boolean saveCart(Carrito cart, boolean completado, String date, String formPago) {
		...
		try {
			...
			insertHistorial = conexion
					.prepareStatement("INSERT INTO " + nameBD + ".HistorialCarritos VALUES (?,?,?,?,?,?)");
			...
			logger.log(Level.SEVERE, "Error a?adiendo carrito al registro", ex);
			try {
				conexion.rollback();
			} catch (SQLException ex1) {
				logger.log(Level.SEVERE,
						"Error haciendo rollback de la transacci¨®n para insertar carrito en el registro", ex1);
			}
		} finally {
