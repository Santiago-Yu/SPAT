class n22328847 {
	protected void doRestoreOrganizeType() throws Exception {
		...
		String strSelQuery = "SELECT organize_type_id,organize_type_name,width " + "FROM "
				+ Common.ORGANIZE_TYPE_B_TABLE + " " + "WHERE version_no = ?";
		String strInsQuery = "INSERT INTO " + Common.ORGANIZE_TYPE_TABLE + " "
				...
				for (; result.next();) {
					...
					if (resultCount != 1) {
						...
						throw new CesSystemException("Organize_backup.doRestoreOrganizeType(): ERROR Inserting data "
								+ "in T_SYS_ORGANIZE_TYPE INSERT !! resultCount = " + resultCount);
					}
				}
				con.commit();
