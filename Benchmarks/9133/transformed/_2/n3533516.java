class n3533516 {
	public boolean saveLecturerecordingsXMLOnWebserver() {
		...
		String lecture = "";
		try {
			URL url = new URL("http://localhost:8080/virtPresenterVerwalter/lecturerecordings.jsp?seminarid="
					+ this.getSeminarID());
			HttpURLConnection http = (HttpURLConnection) url.openConnection();
			BufferedReader in = new BufferedReader(new InputStreamReader(http.getInputStream()));
			String zeile = "";
			for (; (zeile = in.readLine()) != null;) {
				lecture += zeile + "\n";
			}
			in.close();
			http.disconnect();
		...
		try {
			...
			if (tmpSeminarID == null)
				tmpSeminarID = "unbekannt";
			try {
