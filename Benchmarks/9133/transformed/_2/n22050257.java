class n22050257 {
	private void importSources() {
		...
		if (selectedFiles.length == 0) {
			...
			pw.println(new StringBuilder("Importing ").append(selectedFiles.length).append(" files to ")
			...
			try {
				...
				if (destFO.canWrite()) {
					for (String[] s : selectedFiles) {
						try {
							File parentDir = new File(
									new StringBuilder(destRootDir).append(File.separator).append(s[0]).toString());
							if (!parentDir.exists()) {
								parentDir.mkdirs();
							}
							File f = new File(new StringBuilder(destRootDir).append(s[0]).append(File.separator)
									.append(s[1]).toString());
							if (!f.exists()) {
								f.createNewFile();
							}
							...
							int read = -1;
							try {
								File inputFile = new File(new StringBuilder(rootDir).append(s[0]).append(File.separator)
										.append(s[1]).toString());
								pw.print(new StringBuilder("\tImporting file:").append(inputFile.getAbsolutePath())
										.toString());
								fin = new FileInputStream(inputFile);
								fout = new FileOutputStream(f);
								for (; (read = fin.read(b)) != -1;) {
									fout.write(b, 0, read);
								}
								pw.println(" ... done");
								fin.close();
								...
								DialogDisplayer.getDefault()
										.notify(new NotifyDescriptor.Exception(ex, "Error while importing sources!"));
								...
								if (fin != null) {
									try {
										...
										DialogDisplayer.getDefault().notify(
												new NotifyDescriptor.Exception(ex, "Error while importing sources!"));
									}
								}
								if (fout != null) {
									try {
					...
					pw.println("Cannot write to the destination directory."
