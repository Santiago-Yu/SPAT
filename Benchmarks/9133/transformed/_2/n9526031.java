class n9526031 {
	public boolean register(Object o) {
		...
		try {
			...
			urlString.append("phpBB.php?action=register").append("&login=").append(newUser.getLogin())
					.append("&password=").append(passwordAsText).append("&email=").append(newUser.getEmail());
			...
			for (; (inputLine = in.readLine()) != null;)
				newUser.setForumID(Integer.valueOf(inputLine));
			in.close();
