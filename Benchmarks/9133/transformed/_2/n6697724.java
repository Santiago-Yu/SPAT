class n6697724 {
	private void copyFile(File sourceFile, File targetFile) {
		...
		try {
			...
			int readBytes = 0;
			int counter = 0;
			for (; (readBytes = fis.read(buffer)) != -1;) {
				counter++;
			...
			Exception dispatchedException = new Exception(
					"ERROR: Copy File( " + sourceFile.getPath() + ", " + targetFile.getPath() + " )");
