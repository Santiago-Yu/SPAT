class n6147227 {
	public static String[] putFECSplitFile(String uri, File file, int htl, boolean mode) {
		...
		String board = getBoard(file);
		{
			...
			for (int i = 0; i < segmentFileMaps.size(); i++) {
				...
				for (int j = 0; j < currentFileMap.length; j++) {
					...
					for (; getActiveThreads(chunkThreads) >= maxThreads;)
						mixed.wait(5000);
				...
				for (; getActiveThreads(chunkThreads) > 0;) {
					if (DEBUG)
			...
			for (int i = 0; i < checkFileMaps.size(); i++) {
				...
				for (int j = 0; j < currentFileMap.length; j++) {
					...
					for (; getActiveThreads(checkThreads) >= maxThreads;)
						mixed.wait(5000);
				...
				for (; getActiveThreads(checkThreads) > 0;) {
					if (DEBUG)
		...
		for (; !result[0].equals("Success") && !result[0].equals("KeyCollision") && tries < 8;) {
			tries++;
			try {
