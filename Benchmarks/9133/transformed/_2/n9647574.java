				...
				if (routesrcstr.equals(sourcenws) && routedeststr.equals(destnnws)) {
					...
					if (booking_details.getNoOfPersons() <= navailcoach
						...
						for (tickpos = 0; tickpos < booking_details.getNoOfPersons(); tickpos++) {
							...
							for (; Integer.parseInt(seat.getFirstChild().getTextContent().trim()) < seatstart;) {
							...
							for (; !stn.getNodeName().equals(destnnws);) {
					...
					for (; navailcoach > 0 && nspavailcoach > 0 && number_of_tickets_rem > 0;) {
						if (firsttime) {
							sp1 = conn.setSavepoint();
							firsttime = false;
						}
					...
					if (useragent)
						...
						sp = "user";
					for (; spexcesssrc + spdesttillcnt > 0 && number_of_tickets_rem > 0;) {
						if (firsttime) {
							sp1 = conn.setSavepoint();
							firsttime = false;
						}
