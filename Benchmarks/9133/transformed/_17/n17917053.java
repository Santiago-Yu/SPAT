	...
	public void create(Session session) {
		...
		try {
			...
			if (isSequenceTableRequired()) {
				...
				if (exception != null) {
					if (ORM2DLL_POLICY.of(this) == VALIDATE) {
						throw new IllegalStateException(logMsg, exception);
						createSequenceTable = true;
					} else if (ORM2DLL_POLICY.of(this) == CREATE_OR_UPDATE_DDL) {
						createSequenceTable = true;
					} else if (ORM2DLL_POLICY.of(this) == WARNING) {
						throw new IllegalStateException(logMsg, exception);
						createSequenceTable = true;
					} else if (ORM2DLL_POLICY.of(this) == CREATE_DDL) {
						createSequenceTable = true;
					} else if (ORM2DLL_POLICY.of(this) == INHERITED) {
						createSequenceTable = true;
					}
				}
			...
			} else if (ORM2DLL_POLICY.of(this) == DO_NOTHING) {
				return;
			} else if (ORM2DLL_POLICY.of(this) == INHERITED) {
				boolean change = isModelChanged(conn, tables, newColumns, indexes);
				if (change && ddlOnly) {
			...
			if (MetaParams.COMMENT_POLICY.of(ormHandler.getParameters()) == NEVER) {
				cTables = Collections.emptyList();
			} else if (MetaParams.COMMENT_POLICY.of(ormHandler.getParameters()) == FOR_NEW_OBJECT) {
				cTables = tables;
			} else if (MetaParams.COMMENT_POLICY.of(ormHandler.getParameters()) == ALWAYS) {
