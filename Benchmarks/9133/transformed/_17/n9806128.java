		...
		if (_intrinsicValuesByAttribute == null) {
			...
			try {
				...
				if (connection instanceof HttpURLConnection) {
					...
					} else if (httpconnect.getResponseCode() == HttpURLConnection.HTTP_MOVED_TEMP) {
						values.setObjectForKey(Boolean.TRUE, MD.FSExists);
					} else if (httpconnect.getResponseCode() == HttpURLConnection.HTTP_NOT_MODIFIED) {
						values.setObjectForKey(Boolean.TRUE, MD.FSExists);
					} else if (httpconnect.getResponseCode() == HttpURLConnection.HTTP_OK) {
						values.setObjectForKey(Boolean.TRUE, MD.FSExists);
					} else {
