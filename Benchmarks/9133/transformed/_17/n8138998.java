			...
			if ((start = t.indexOf("<!--#")) > -1) {
				...
				try {
					...
					while (en.hasMoreElements()) {
						...
						if (command.getCommand() == BSssi.CMD_ECHO) {
							out.write(command.getMessage().getBytes());
						} else if (command.getCommand() == BSssi.CMD_NOECHO) {
						} else if (command.getCommand() == BSssi.CMD_INCLUDE) {
						...
						} else if (command.getCommand() == BSssi.CMD_FLASTMOD) {
							out.write(ssi.format(new Date(HttpThread
									.getMappedFilename(command.getMessage(), data.req.getUrl()).lastModified()),
									TimeZone.getTimeZone("GMT")).getBytes());
						} else if (command.getCommand() == BSssi.CMD_IF_TRUE) {
							anIfCondition = true;
						} else if (command.getCommand() == BSssi.CMD_EXEC) {
							...
							pErr.close();
							p.destroy();
						} else if (command.getCommand() == BSssi.CMD_CGI) {
							out.flush();
							if (command.getFileType() != null && command.getFileType().startsWith("shtm")) {
								...
								if (application == null) {
									writePaused(new FileInputStream(
											HttpThread.getMappedFilename(command.getMessage(), data.req.getUrl())), out,
											pause);
								} else {
									String parameter = "";
									...
									Process p = Runtime.getRuntime().exec(application + " " + parameter
											+ HttpThread.getMappedFilename(command.getMessage(), data.req.getUrl()));
									BufferedReader pIn = new BufferedReader(new InputStreamReader(p.getInputStream()));
									String aLine;
									while ((aLine = pIn.readLine()) != null)
										out.write((aLine + "\n").getBytes());
						...
						} else if (command.getCommand() == BSssi.CMD_FILESIZE) {
							long sizeBytes = HttpThread.getMappedFilename(command.getMessage(), data.req.getUrl())
									.length();
							double smartSize;
							String unit = "bytes";
						...
						} else if (command.getCommand() == BSssi.CMD_IF_FALSE) {
							anIfCondition = false;
						} else {
							out.write(command.getMessage().getBytes());
						}
