class n21750583 {
	protected void doProxyInternally(HttpServletRequest req, HttpServletResponse res)
		...
		while (followTargetUrl) {
			...
			if (proxyRes.getStatusLine().getStatusCode() == 302) {
			...
			} else if (proxyRes.getStatusLine().getStatusCode() == 200) {
				followTargetUrl = false;
			} else if (proxyRes.getStatusLine().getStatusCode() == 404) {
				followTargetUrl = false;
			} else if (proxyRes.getStatusLine().getStatusCode() == 500) {
				followTargetUrl = false;
			} else {
				followTargetUrl = false;
			}
