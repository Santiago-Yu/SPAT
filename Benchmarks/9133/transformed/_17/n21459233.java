class n21459233 {
	private static IProject createCopyProject(IProject project, String pName, IWorkspace ws, IProgressMonitor pm)
			throws Exception {
		...
		for (IClasspathEntry cEntry : classPath) {
			if (cEntry.getEntryKind() == IClasspathEntry.CPE_PROJECT) {
				System.out.println("project folder " + cEntry.getPath());
				newClassPath.add(cEntry);
			} else if (cEntry.getEntryKind() == IClasspathEntry.CPE_LIBRARY) {
				System.out.println("library folder " + cEntry.getPath());
				newClassPath.add(cEntry);
			} else if (cEntry.getEntryKind() == IClasspathEntry.CPE_VARIABLE) {
				System.out.println("variable folder " + cEntry.getPath());
				newClassPath.add(cEntry);
			} else if (cEntry.getEntryKind() == IClasspathEntry.CPE_SOURCE) {
		...
		copyDir(project.getLocation().toString(), "/translator", newProject.getLocation().toString(), "",
