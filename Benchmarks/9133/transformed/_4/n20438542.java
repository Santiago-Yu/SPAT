class n20438542 {
	public static Shader loadShader(String vspath, String fspath, int textureUnits, boolean separateCam, boolean fog)
		...
		try {
			...
			in = (v_url != null) ? new BufferedReader(new InputStreamReader(v_url.openStream()))
					: new BufferedReader(new InputStreamReader(new FileReader(v_path).getInputStream()));
			boolean run = true;
			String str;
			while (run) {
				str = in.readLine();
		...
		try {
			...
			in = (f_url != null) ? new BufferedReader(new InputStreamReader(f_url.openStream()))
					: new BufferedReader(new InputStreamReader(new FileReader(f_path).getInputStream()));
			boolean run = true;
			String str;
			while (run) {
				str = in.readLine();
