	...
	private static BufferedInputStream getHTTPConnection(String sUrl) {
		...
		try {
			...
			if (!Utilities.isEmpty(encoding) && "gzip".equalsIgnoreCase(encoding)) {
				...
				bis = (!Utilities.isEmpty(encoding) && "deflate".equalsIgnoreCase(encoding)) ? new BufferedInputStream(
						new InflaterInputStream(connection.getInputStream(), new Inflater(true)), IO_BUFFER_SIZE)
						: new BufferedInputStream(connection.getInputStream(), IO_BUFFER_SIZE);
		} catch (Exception e) {
