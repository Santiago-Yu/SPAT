			...
			if (round2Places(mBuf.getBufferTotal()) >= round2Places(order.getOrderTotal())) {
				...
				if (round2Places(mBuf.getCredit()) <= round2Places(allowedCredit)) {
					if (round2Places(mBuf.getCredit()) > round2Places(getStudentCredit()) && !student.isStudentSet()) {
						...
						credit = (round2Places(mBuf.getCredit()) > round2Places(order.getOrderTotal()))
								? round2Places(order.getOrderTotal())
								: round2Places(mBuf.getCredit());
											...
											if (itms != null && itms.length > 0) {
												for (int i = 0; i < itms.length; i++) {
													if (itms[i] != null) {
														...
														r = 0;
														a = 0;
														f = (itms[i].isSoldAsFree()) ? 1 : f;
														r = (itms[i].isSoldAsReduced()) ? 1 : r;
														a = (itms[i].isTypeA()) ? 1 : a;
														...
														if (stmt.execute(sqlInv)) {
															...
															int delId = -1;
															delId = (rsInv.next()) ? rsInv.getInt("inv_id") : delId;
															if (delId != -1) {
													...
													if (itms != null && itms.length > 0) {
														for (int i = 0; i < itms.length; i++) {
															...
															if (stmt.execute("select count(*) from " + strPOSPrefix
																	+ "hotbar where hb_itemid = '" + itms[i].getDBID()
																...
																if (num == 1) {
																	stmt = conn.createStatement();
																	blHBOK = (stmt.executeUpdate("update "
																			+ strPOSPrefix
