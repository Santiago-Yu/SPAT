class n3533517 {
	public String loadGeneratorXML() {
		...
		try {
			...
			if (!ftp.login(this.getFolienKonvertierungsServer().getFtpBenutzer(),
					this.getFolienKonvertierungsServer().getFtpPasswort())) {
				System.err.println("FTP server: Login incorrect");
			}
			String path;
			path = (this.getFolienKonvertierungsServer().getDefaultPath().length() > 0)
					? "/" + this.getFolienKonvertierungsServer().getDefaultPath() + "/" + this.getId() + "/"
					: "/" + this.getId() + "/";
			if (!ftp.changeWorkingDirectory(path))
				System.err.println("Konnte Verzeichnis nicht wechseln: " + path);
		...
		generatorXML = (generatorXML != null && generatorXML.length() == 0) ? null : generatorXML;
		return generatorXML;
	}
