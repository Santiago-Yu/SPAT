class n13757855 {
	public void process(@NotNull Template tpl, @NotNull Document model, @NotNull String packageName,
		...
		case target:
			destinationPath = (tpl.path == null)
					? where.buildDir + separatorChar + "generated-sources" + separatorChar + GENERATION_TARGET
					: where.buildDir;
			break;
		case source:
			destinationPath = (tpl.path == null) ? where.sourceDir : "src";
			break;
		case redora:
		...
		if (tpl.path == null) {
			destinationPath = (tpl.destination == Destination.redora) ? separator + artifact
					: separator + packageName.replace('.', separatorChar);
		} else {
