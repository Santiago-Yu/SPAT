		...
		try {
			...
			while ((in_for_mapping != null) && (inputLine != null)) {
				if (inputLine.startsWith("<DescriptorRecord DescriptorClass")) {
					...
					leave = (mesh_uid.compareTo("D041441") == 0) ? true : leave;
					inputLine = getNextLine(in_for_mapping);
					inputLine = getNextLine(in_for_mapping);
					cut1 = "<String>";
					cut2 = "</String>";
				...
				if (inputLine.startsWith("<DescriptorRecord DescriptorClass")) {
					...
					while ((inputLine != null) && (!inputLine.startsWith("</DescriptorRecord>"))) {
						...
						if (inputLine.startsWith("<Concept PreferredConceptYN")) {
							...
							prefConcept = (inputLine.compareTo("<Concept PreferredConceptYN=\"Y\">") == 0) ? true
									: prefConcept;
							while ((inputLine != null) && (inputLine.indexOf("</Concept>") == -1)) {
								if (inputLine.startsWith("<CASN1Name>") && prefConcept) {
									...
									chem_description = (casn1.length() > chem_name.length() + 2)
											? casn1.substring(chem_name.length() + 2, casn1.length())
											: chem_description;
									String reg_number = "";
									inputLine = getNextLine(in);
						...
						inputLine = (!line_read) ? getNextLine(in) : inputLine;
					}
					String pos_tag = "";
					element_of = "MESHD";
					String is_primary = "0";
					...
					for (int i = 0; i < treenr.size(); i++) {
						...
						tnr = (tnr.length() > 3) ? tnr.substring(0, tnr.lastIndexOf(".")) : tnr;
