class n21028028 {
	public int save(String newInventory, String inventoryType, int compareResult, boolean renameCorruptedFile) {
		...
		try {
			...
			directory_repository = (directory_repository.endsWith("/") == false
					&& directory_repository.endsWith(File.separator) == false) ? File.separator : directory_repository;
		...
		try {
			...
			newDirRep = (path.startsWith("/") == false && path.startsWith(File.separator) == false) ? File.separator
					: newDirRep;
