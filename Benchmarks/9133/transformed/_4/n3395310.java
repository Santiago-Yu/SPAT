class n3395310 {
	private void execute(File file) throws IOException {
		...
		password = (cfg.containsKey(GC_PASSWORD)) ? cfg.getProperty(GC_PASSWORD)
				: new String(Base64.decode(cfg.getProperty(GC_PASSWORD64)));
		...
		try {
			...
			s = (StringUtils.isBlank(s)) ? file.getName() + " (" + TimeUtils.toYYYYMMDD() + ")" : s;
