	...
	public ArrayList parseFile(File newfile) throws IOException {
		...
		while ((s = br.readLine()) != null) {
			...
			if ((nvalues == 4) && (!firstName.startsWith("---"))) {
				zerodata = ((Double.parseDouble(tokens[1]) == 0.) && (Double.parseDouble(tokens[2]) == 0.)
						&& (Double.parseDouble(tokens[3]) == 0.)) ? true : false;
				baddata = (tokens[1].equals("NaN") || tokens[2].equals("NaN") || tokens[3].equals("NaN")) ? true
						: false;
			}
			header = (firstName.startsWith("start")) ? s : header;
