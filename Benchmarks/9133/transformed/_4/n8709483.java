class n8709483 {
	private void saveAttachment(long messageId, Part attachment, boolean saveAsNew)
		...
		attachmentId = ((!saveAsNew) && (attachment instanceof LocalAttachmentBodyPart))
				? ((LocalAttachmentBodyPart) attachment).getAttachmentId()
				: attachmentId;
		...
		if (size == -1) {
			...
			if (disposition != null) {
				...
				size = (s != null) ? Integer.parseInt(s) : size;
			}
		}
		size = (size == -1) ? 0 : size;
