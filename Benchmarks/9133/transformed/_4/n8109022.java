class n8109022 {
	public static void main(String args[]) throws Exception {
		...
		String path = new LoadDocumentData().getClass().getProtectionDomain().getCodeSource().getLocation().toString()
				.substring(6);
		...
		nDocID = (htTableKey.containsKey("DMS_DOCUMENT"))
				? new Integer(((Integer) htTableKey.get("DMS_DOCUMENT")).intValue() + 1)
				: nDocID;
		nDocDetailID = (htTableKey.containsKey("DMS_DOCUMENT_DETAIL"))
				? new Integer(((Integer) htTableKey.get("DMS_DOCUMENT_DETAIL")).intValue() + 1)
				: nDocDetailID;
		nVersionID = (htTableKey.containsKey("DMS_VERSION"))
				? new Integer(((Integer) htTableKey.get("DMS_VERSION")).intValue() + 1)
				: nVersionID;
		nContentID = (htTableKey.containsKey("DMS_CONTENT"))
				? new Integer(((Integer) htTableKey.get("DMS_CONTENT")).intValue() + 1)
				: nContentID;
		nDmsRootID = (htTableKey.containsKey("DMS_ROOT"))
				? new Integer(((Integer) htTableKey.get("DMS_ROOT")).intValue() + 1)
				: nDmsRootID;
		nMtmDocVersionID = (htTableKey.containsKey("MTM_DOCUMENT_VERSION"))
				? new Integer(((Integer) htTableKey.get("MTM_DOCUMENT_VERSION")).intValue() + 1)
				: nMtmDocVersionID;
		...
		for (int i = nStart; i <= nEnd; i++) {
			try {
				...
				if (count % 500 == 0) {
					...
					sqlDoc = "INSERT INTO DMS_DOCUMENT VALUES(" + nDocID.toString() + ",'" + sDocName + "','F',"
							+ nRootParentID + "," + nRootID.toString() + ", 'S', '"
							+ dmsDocument.getCreateDate().toString() + "', NULL, '" + ownerID
				...
				dmsDocument.setDocumentName(sDocName);
				sqlDoc = "INSERT INTO DMS_DOCUMENT VALUES(" + nDocID.toString() + ",'" + sDocName + "','D',"
						+ nParentID.toString() + "," + nRootID.toString() + ", 'S','"
						+ dmsDocument.getCreateDate().toString() + "', NULL, '" + ownerID
				...
				preStat = conn.prepareStatement(sqlDoc);
				preStat.executeUpdate();
				sqlDocDetail = "INSERT INTO DMS_DOCUMENT_DETAIL VALUES(" + nDocDetailID.toString() + ","
						+ nDocID.toString() + "," + sUdfID + "," + sUdfDetailID + ",'" + sFieldValue
						+ "', null, null, 'A',0,0,'" + dmsDocument.getCreateDate().toString() + "',0,'"
				...
				sqlContent = " INSERT INTO DMS_CONTENT VALUES (" + nContentID.toString() + "," + sConvertedName
				...
				sqlVersion = "INSERT INTO DMS_VERSION VALUES(" + nVersionID.toString() + ", " + nDocID.toString()
						+ ", 1,'ROOT',0," + nContentID.toString() + ",0, 'Ref Num', 'desc', 20480, 'A', null, 'A',0,0,'"
						+ dmsDocument.getCreateDate().toString() + "',0,'" + dmsDocument.getCreateDate().toString()
						+ "')";
				...
				sqlDocVersion = "INSERT INTO MTM_DOCUMENT_VERSION VALUES(" + nMtmDocVersionID.toString() + ","
						+ nDocID.toString() + "," + nVersionID.toString() + ",'A', 0, 0,'"
						+ dmsDocument.getCreateDate().toString() + "',0,'" + dmsDocument.getCreateDate().toString()
						+ "')";
		...
		if (htTableKey.containsKey("DMS_DOCUMENT")) {
			statment = conn.prepareStatement(
					"UPDATE SYS_TABLE_KEY SET TABLE_KEY_MAX=" + nDocID.toString() + " WHERE TABLE_NAME='DMS_DOCUMENT'");
			statment.executeUpdate();
		} else {
			statment = conn
					.prepareStatement("INSERT INTO SYS_TABLE_KEY VALUES('DMS_DOCUMENT', " + nDocID.toString() + ")");
			statment.executeUpdate();
		}
		if (htTableKey.containsKey("DMS_DOCUMENT_DETAIL")) {
			statment = conn.prepareStatement("UPDATE SYS_TABLE_KEY SET TABLE_KEY_MAX=" + nDocDetailID.toString()
					+ " WHERE TABLE_NAME='DMS_DOCUMENT_DETAIL'");
			statment.executeUpdate();
		} else {
			statment = conn.prepareStatement(
					"INSERT INTO SYS_TABLE_KEY VALUES('DMS_DOCUMENT_DETAIL', " + nDocDetailID.toString() + ")");
			statment.executeUpdate();
		}
		if (htTableKey.containsKey("DMS_VERSION")) {
			statment = conn.prepareStatement("UPDATE SYS_TABLE_KEY SET TABLE_KEY_MAX=" + nVersionID.toString()
					+ " WHERE TABLE_NAME='DMS_VERSION'");
			statment.executeUpdate();
		} else {
			statment = conn
					.prepareStatement("INSERT INTO SYS_TABLE_KEY VALUES('DMS_VERSION', " + nVersionID.toString() + ")");
			statment.executeUpdate();
		}
		if (htTableKey.containsKey("DMS_CONTENT")) {
			statment = conn.prepareStatement("UPDATE SYS_TABLE_KEY SET TABLE_KEY_MAX=" + nContentID.toString()
					+ " WHERE TABLE_NAME='DMS_CONTENT'");
			statment.executeUpdate();
		} else {
			statment = conn
					.prepareStatement("INSERT INTO SYS_TABLE_KEY VALUES('DMS_CONTENT', " + nContentID.toString() + ")");
			statment.executeUpdate();
		}
		if (htTableKey.containsKey("MTM_DOCUMENT_VERSION")) {
			statment = conn.prepareStatement("UPDATE SYS_TABLE_KEY SET TABLE_KEY_MAX=" + nMtmDocVersionID.toString()
					+ " WHERE TABLE_NAME='MTM_DOCUMENT_VERSION'");
			statment.executeUpdate();
		} else {
			statment = conn.prepareStatement(
					"INSERT INTO SYS_TABLE_KEY VALUES('MTM_DOCUMENT_VERSION', " + nMtmDocVersionID.toString() + ")");
			statment.executeUpdate();
		}
		statment.close();
		System.out.println("final value: " + " DocumentID " + nDocID + " DocDetailID " + nDocDetailID + " DocVersion "
				+ nVersionID + " DocContent " + nContentID + " nMtmDocVersionID " + nMtmDocVersionID);
		closeDBConn();
	}
