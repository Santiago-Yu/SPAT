	...
	public boolean initFile(String filename) {
		...
		if (filename.toLowerCase().endsWith(".spc")) {
			try {
				...
				YFactor = (fexp != 0x80) ? Math.pow(2, fexp) / Math.pow(2, 32) : YFactor;
		...
		while (My_Counter < nbLignes) {
			try {
				...
				if (mon_token.hasMoreTokens() == true) {
					...
					TexteTitre = (StringDataUtils.compareStrings(keyword, "##TITLE=") == 0) ? uneligne.substring(9)
							: TexteTitre;
					Firstx = (StringDataUtils.compareStrings(keyword, "##FIRSTX=") == 0)
							? Double.valueOf(mon_token.nextToken()).doubleValue()
							: Firstx;
					Lastx = (StringDataUtils.compareStrings(keyword, "##LASTX=") == 0)
							? Double.valueOf(mon_token.nextToken()).doubleValue()
							: Lastx;
					YFactor = (StringDataUtils.compareStrings(keyword, "##YFACTOR=") == 0)
							? Double.valueOf(mon_token.nextToken()).doubleValue()
							: YFactor;
					Nbpoints = (StringDataUtils.compareStrings(keyword, "##NPOINTS=") == 0)
							? Integer.valueOf(mon_token.nextToken()).intValue()
							: Nbpoints;
					x_units = (StringDataUtils.compareStrings(keyword, "##XUNITS=") == 0) ? uneligne.substring(10)
							: x_units;
					y_units = (StringDataUtils.compareStrings(keyword, "##YUNITS=") == 0) ? uneligne.substring(10)
							: y_units;
					nmr_observe_frequency = (StringDataUtils.compareStrings(keyword, "##.OBSERVE") == 0
							&& StringDataUtils.compareStrings(mon_token.nextToken(), "FREQUENCY=") == 0)
									? Double.valueOf(mon_token.nextToken()).doubleValue()
									: nmr_observe_frequency;
					Datatype = (StringDataUtils.compareStrings(keyword, "##XYDATA=") == 0
							&& StringDataUtils.compareStrings(mon_token.nextToken(), "(X++(Y..Y))") == 0) ? "XYDATA"
									: Datatype;
					Datatype = (StringDataUtils.compareStrings(keyword, "##XYDATA=(X++(Y..Y))") == 0) ? "XYDATA"
							: Datatype;
					Datatype = (StringDataUtils.compareStrings(keyword, "##PEAK") == 0
							&& StringDataUtils.compareStrings(mon_token.nextToken(), "TABLE=") == 0
							&& StringDataUtils.compareStrings(mon_token.nextToken(), "(XY..XY)") == 0) ? "PEAK TABLE"
									: Datatype;
					Datatype = (StringDataUtils.compareStrings(keyword, "##PEAK") == 0
		...
		if (Datatype.compareTo("UNKNOWN") == 0)
			return false;
		x_units = (Datatype.compareTo("PEAK TABLE") == 0 && x_units.compareTo("?") == 0) ? "M/Z" : x_units;
