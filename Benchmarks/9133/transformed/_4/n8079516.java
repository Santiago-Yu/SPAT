class n8079516 {
	public void buildSiteForEdit(String sBasePath, String sOutputPath, String sCtrlPath, String sMenuPath,
		...
		Vector vPages = pages();
		sBasePath = (!sBasePath.endsWith(sSep)) ? sSep : sBasePath;
		...
		sWebServer = (!sWebServer.endsWith("/")) ? "/" : sWebServer;
		...
		for (int c = 0; c < vPages.size(); c++) {
			...
			try {
				...
				sMedia = (null == sMedia) ? "html" : sMedia.substring(sMedia.indexOf('/') + 1);
			...
			if (sTransformed.length() > 0) {
				...
				iCloseHead = (iCloseHead < 0) ? sTransformed.indexOf("</HEAD") : iCloseHead;
				iOpenBody = sTransformed.indexOf("<body", iCloseHead);
				iOpenBody = (iOpenBody < 0) ? sTransformed.indexOf("<BODY", iCloseHead) : iOpenBody;
