	...
	public static Node combineJs(URL base, List<Node> linkJs, List<File> newFiles) throws IOException {
		...
		try {
			if (dir.delete() && dir.mkdirs()) {
				...
				concatWriter.close();
				FileReader reader;
				reader = (fails.size() == 0 && Configuration.jsMinification()) ? new FileReader(combineFile)
						: new FileReader(concatFile);
				...
				if (!targetFile.exists()) {
					...
					logger.info("Combined several js files into the single " + targetFile + " [size="
							+ targetFile.length() + "].");
				}
			}
		} finally {
