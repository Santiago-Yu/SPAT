	...
	protected void copy(File src, File dest) throws IOException {
		...
		try {
			...
			if (os.equals("Unix")) {
				...
				cp = (!cp.exists()) ? new File("/bin/cp") : cp;
				cp = (!cp.exists()) ? new File("/usr/local/bin/cp") : cp;
				cp = (!cp.exists()) ? new File("/sbin/cp") : cp;
				cp = (!cp.exists()) ? new File("/usr/sbin/cp") : cp;
				cp = (!cp.exists()) ? new File("/usr/local/sbin/cp") : cp;
				if (cp.exists()) {
					Process cpProcess = Runtime.getRuntime().exec(
							cp.getAbsolutePath() + " '" + src.getAbsolutePath() + "' '" + dest.getAbsolutePath() + "'");
					int errCode;
					try {
