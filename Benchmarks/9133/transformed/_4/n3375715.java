class n3375715 {
	public void importarHistoricoDeProventos(File pArquivoXLS, boolean pFiltrarPelaDataDeCorteDoCabecalho,
			Andamento pAndamento) throws IOException, SQLException, InvalidFormatException {
		int iLinha = -1;
		String nomeDaColuna = "";
		...
		try {
			...
			Row registro;
			Cell celula;
			java.util.Date dataLimite = plan1.getRow(0)
			...
			for (iLinha = NUMERO_DA_LINHA_INICIAL; true; iLinha++) {
				...
				if (registro != null) {
					...
					celulaUltimoDiaCom = registro
							.getCell(CampoDaPlanilhaDosProventosEmDinheiro.ULTIMO_DIA_COM.ordinal());
						...
						if (celulaUltimoDiaCom.getCellType() == Cell.CELL_TYPE_NUMERIC) {
							...
							if (tmpUltimoDiaCom.compareTo(dataLimite) <= 0 || !pFiltrarPelaDataDeCorteDoCabecalho) {
								...
								try {
									...
									tmpDataDaAprovacao = (celula.getCellType() == Cell.CELL_TYPE_NUMERIC)
											? celula.getDateCellValue()
											: formatadorPadraoData.parse(celula.getStringCellValue());
								...
								vValorDoProvento = new BigDecimal(String.valueOf(registro
										.getCell(CampoDaPlanilhaDosProventosEmDinheiro.VALOR_DO_PROVENTO.ordinal())
										.getNumericCellValue()));
								nomeDaColuna = CampoDaPlanilhaDosProventosEmDinheiro.PROVENTO_POR_1_OU_1000_ACOES
										.toString();
								vProventoPor1Ou1000Acoes = (int) registro.getCell(
								...
								vTipoDoProvento = registro
										.getCell(CampoDaPlanilhaDosProventosEmDinheiro.TIPO_DO_PROVENTO.ordinal())
										.getStringCellValue().trim();
								nomeDaColuna = CampoDaPlanilhaDosProventosEmDinheiro.DATA_DO_ULTIMO_PRECO_COM
										.toString();
								celula = registro.getCell(
										CampoDaPlanilhaDosProventosEmDinheiro.DATA_DO_ULTIMO_PRECO_COM.ordinal());
								if (celula != null) {
									try {
										java.util.Date tmpDataDoUltimoPrecoCom;
										tmpDataDoUltimoPrecoCom = (celula.getCellType() == Cell.CELL_TYPE_NUMERIC)
												? celula.getDateCellValue()
												: formatadorPadraoData.parse(celula.getStringCellValue());
								...
								vUltimoPrecoCom = new BigDecimal(String.valueOf(registro
										.getCell(CampoDaPlanilhaDosProventosEmDinheiro.ULTIMO_PRECO_COM.ordinal())
										.getNumericCellValue()));
								nomeDaColuna = CampoDaPlanilhaDosProventosEmDinheiro.PRECO_POR_1_OU_1000_ACOES
										.toString();
								vPrecoPor1Ou1000Acoes = (int) registro.getCell(
								...
								vProventoPorPreco = (celula != null && celula.getCellType() == Cell.CELL_TYPE_NUMERIC)
										? new BigDecimal(String.valueOf(celula.getNumericCellValue()))
										: null;
