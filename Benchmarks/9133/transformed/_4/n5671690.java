class n5671690 {
	...
	public void backup() {
		...
		try {
			...
			tableExistsResult = (rs.next()) ? rs.getString(1) : tableExistsResult;
			rs.close();
			prestm.close();
			if (StringUtils.isBlank(tableExistsResult)) {
				...
				createTableSql = (rs.next()) ? rs.getString(2) : createTableSql;
				rs.close();
				prestm.close();
				...
				createTableSql = (matcher.find()) ? matcher.replaceAll("") : createTableSql;
				matcher = normalizePattern.matcher(createTableSql);
				createTableSql = (matcher.find()) ? matcher.replaceAll("\n )") : createTableSql;
			...
			rs = prestm.executeQuery();
			currentRows = (rs.next()) ? rs.getLong(1) : currentRows;
			rs.close();
			prestm.close();
			if (currentRows > 0) {
				connection.setAutoCommit(false);
				prestm = connection.prepareStatement("INSERT INTO " + getDestination() + " SELECT * FROM " + getOrigin()
						+ " WHERE " + getCondition());
				prestm.setDate(1, sqlDate);
				int rows = prestm.executeUpdate();
				prestm.close();
				if (logger.isInfoEnabled())
					logger.info(rows + " rows backupped");
			...
			if (applicationContext != null)
				applicationContext.publishEvent(new TrapEvent(this, "dbcon",
						"Errore SQL durante il backup dei dati della tabella " + getOrigin(), e));
			try {
				connection.rollback();
			...
			if (applicationContext != null)
				applicationContext.publishEvent(new TrapEvent(this, "generic",
						"Errore generico durante il backup dei dati della tabella " + getOrigin(), e));
			try {
				connection.rollback();
			} catch (SQLException e1) {
			}
