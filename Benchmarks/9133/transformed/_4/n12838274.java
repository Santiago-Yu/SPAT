class n12838274 {
	public BufferedImage extract() throws DjatokaException {
		...
		try {
			...
			left = (useleftDouble) ? imagePosition.Get_x() + (int) Math.round(leftDouble * imageSize.Get_x()) : left;
			top = (usetopDouble) ? imagePosition.Get_y() + (int) Math.round(topDouble * imageSize.Get_y()) : top;
			height = (useheightDouble) ? (int) Math.round(heightDouble * imageSize.Get_y()) : height;
			width = (usewidthDouble) ? (int) Math.round(widthDouble * imageSize.Get_x()) : width;
			if (useRegion) {
			...
			int[] kduBuffer = null;
			while (decompressor.Process(region_buf, image_dims.Access_pos(), 0, 0, region_buf_size, incomplete_region,
					new_region)) {
			...
			image = (params.getRotationDegree() > 0) ? ImageProcessingUtils.rotate(image, params.getRotationDegree())
					: image;
