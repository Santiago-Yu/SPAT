			...
			if (aURL.Path.compareTo("svnUpdate") == 0) {
				try {
					...
					for (int j = 0; j < tempFiles.length; j++) {
						anyOdt = (tempFiles[j].toString().endsWith(".odt")) ? tempFiles[j] : anyOdt;
						thisOdt = (tempFiles[j].toString().equals(settings.getCheckoutDoc())
								&& settings.getCheckoutDoc() != null) ? tempFiles[j] : thisOdt;
					}
					anyOdt = (thisOdt != null) ? thisOdt : anyOdt;
					String url;
