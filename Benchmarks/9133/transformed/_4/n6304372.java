class n6304372 {
	public static String getUploadDeleteComboBox(String fromMode, String fromFolder, String action, String object,
			HttpSession session) {
		...
		try {
			...
			CofaxToolsUtil
					.log("CofaxToolsFTP getUploadDeleteComboBox connecting to server: " + (String) servers.get(0));
			...
			try {
				...
				if ((fromFolder != null) && (fromFolder.length() > folder.length())) {
					...
					publicName = (subStri > -1) ? folder.substring(subStri) : publicName;
					folders.append(
							"<B><A HREF=\'/tools/?mode=" + fromMode + "&hl=templates_view_templates_images&fromFolder="
									+ tempParentFolderName + "\'>" + tempParentFolderName + "</A></B><BR>\n");
				...
				FTPFile[] files = null;
				files = ((action.equals("Delete")) && (object.equals("Templates")))
						? ftp.listFiles(new CofaxToolsNTFileListParser())
						: ftp.listFiles(new CofaxToolsNTFileListParser());
				if (files == null) {
				...
				for (int ii = 0; ii < files.length; ii++) {
					...
					if (!thisFile.isDirectory()) {
						links.append("<INPUT TYPE=CHECKBOX NAME=" + i + " VALUE=" + folder + "/" + name + ">" + name
								+ "<BR>\n");
						i++;
					}
					if ((thisFile.isDirectory()) && (!name.startsWith(".")) && (!name.endsWith("."))) {
						...
						tempFolderName = (subStr > -1) ? folder.substring(subStr) : folder;
						folders.append("<LI><A HREF=\'/tools/?mode=" + fromMode
								+ "&hl=templates_view_templates_images&fromFolder=" + folder + "/" + name + "\'>"
								+ tempFolderName + "/" + name + "</A><BR>");
					}
				}
				ftp.logout();
				ftp.disconnect();
