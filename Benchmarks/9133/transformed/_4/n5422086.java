				...
				for (String key : types.keySet()) {
					if (!key.trim().equals(screen.getName().trim())) {
						...
						for (String line : lines) {
							varFound = (line.matches("^\\s*(public|private)\\s+" + value + "\\s+" + key + "\\s*=.*;$"))
									? true
									: varFound;
							varFound = (line.matches("^\\s*(public|private)\\s+" + value + "\\s+" + key + "\\s*;$"))
									? true
									: varFound;
							importFound = (line.matches("^\\s*import\\s+.*" + value + "\\s*;$")) ? true : importFound;
						}
						initial = (!varFound)
								? initial.replaceFirst("(?s)(?<=\\{\n)", "\tprivate " + value + " " + key + ";\n")
								: initial;
						initial = (!importFound)
								? initial.replaceFirst("(?=import)",
