class n11787058 {
	protected int authenticate(long companyId, String login, String password, String authType, Map headerMap,
			Map parameterMap) throws PortalException, SystemException {
		...
		if (authType.equals(CompanyImpl.AUTH_TYPE_EA)) {
			authResult = AuthPipeline.authenticateByEmailAddress(authPipelinePre, companyId, login, password, headerMap,
					parameterMap);
		} else if (authType.equals(CompanyImpl.AUTH_TYPE_SN)) {
			authResult = AuthPipeline.authenticateByScreenName(authPipelinePre, companyId, login, password, headerMap,
					parameterMap);
		} else
			authResult = (authType.equals(CompanyImpl.AUTH_TYPE_ID))
					? AuthPipeline.authenticateByUserId(authPipelinePre, companyId, userId, password, headerMap,
							parameterMap)
					: authResult;
		User user = null;
		try {
			if (authType.equals(CompanyImpl.AUTH_TYPE_EA)) {
				...
				user = (authType.equals(CompanyImpl.AUTH_TYPE_ID))
						? UserUtil.findByC_U(companyId, GetterUtil.getLong(login))
						: user;
			...
			if (GetterUtil.getBoolean(PropsUtil.get(PropsUtil.AUTH_PIPELINE_ENABLE_LIFERAY_CHECK))) {
				...
				if (user.getPassword().equals(encPwd)) {
					...
					try {
						...
						authResult = (password.equals(encPwd)) ? Authenticator.SUCCESS : Authenticator.FAILURE;
		...
		if (authResult == Authenticator.SUCCESS) {
			...
			if (authType.equals(CompanyImpl.AUTH_TYPE_EA)) {
				authResult = AuthPipeline.authenticateByEmailAddress(authPipelinePost, companyId, login, password,
						headerMap, parameterMap);
			} else if (authType.equals(CompanyImpl.AUTH_TYPE_SN)) {
				authResult = AuthPipeline.authenticateByScreenName(authPipelinePost, companyId, login, password,
						headerMap, parameterMap);
			} else
				authResult = (authType.equals(CompanyImpl.AUTH_TYPE_ID))
						? AuthPipeline.authenticateByUserId(authPipelinePost, companyId, userId, password, headerMap,
								parameterMap)
						: authResult;
		}
			...
			try {
				...
				if (!PortalLDAPUtil.isPasswordPolicyEnabled(user.getCompanyId())) {
					...
					if ((failedLoginAttempts >= maxFailures) && (maxFailures != 0)) {
						...
						if (authType.equals(CompanyImpl.AUTH_TYPE_EA)) {
							AuthPipeline.onMaxFailuresByEmailAddress(authMaxFailures, companyId, login, headerMap,
									parameterMap);
						} else if (authType.equals(CompanyImpl.AUTH_TYPE_SN)) {
							AuthPipeline.onMaxFailuresByScreenName(authMaxFailures, companyId, login, headerMap,
									parameterMap);
						} else if (authType.equals(CompanyImpl.AUTH_TYPE_ID)) {
							AuthPipeline.onMaxFailuresByUserId(authMaxFailures, companyId, userId, headerMap,
									parameterMap);
						}
					}
				}
