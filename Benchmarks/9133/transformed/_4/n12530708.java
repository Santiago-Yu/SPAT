class n12530708 {
	private int mergeFiles(Merge merge) throws MojoExecutionException {
		...
		encoding = (merge.getEncoding() != null && merge.getEncoding().length() > 0) ? merge.getEncoding() : encoding;
		...
		try {
			...
			for (String orderingName : this.orderingNames) {
				...
				if (files != null) {
					getLog().info("Appending: " + files.size() + " files that matched the name: " + orderingName
							+ " to the target file: " + merge.getTargetFile().getAbsolutePath() + "...");
					for (File file : files) {
						String fileName = file.getName();
						getLog().info("Appending file: " + fileName + " to the target file: "
						...
						try {
							...
							throw new MojoExecutionException("Failed to append file: " + fileName + " to output file",
									ioe);
						} finally {
			...
			throw new MojoExecutionException(
					"Failed to open stream file to output file: " + merge.getTargetFile().getAbsolutePath(), ioe);
		} finally {
			if (fos != null) {
