class n23094550 {
	public StringBuffer render(RenderEngine c) {
		String logTime = null;
		logTime = (c.getWorkerContext() != null) ? c.getWorkerContext().getWorkerStart() : logTime;
		...
		if (useDocroot.equalsIgnoreCase("true")) {
			isRooted = (c.getVendContext().getVend().getIgnorableDocroot(c.getClientContext().getMatchedHost())) ? false
					: isRooted;
		}
		if (isRooted && (useFilename.indexOf("/") == -1 || useFilename.startsWith("./"))) {
			if (c.getWorkerContext() != null && useFilename.startsWith("./")) {
				useFilename = c.getWorkerContext().getClientContext().getPostVariable("current_path")
		...
		if (c.getWorkerContext() == null) {
			currentDocroot = (c.getRenderContext().getCurrentDocroot() == null) ? "."
					: c.getRenderContext().getCurrentDocroot();
		} else {
			currentDocroot = c.getWorkerContext().getDocRoot();
		}
		currentDocroot = (!isRooted) ? "" : currentDocroot;
		if (useDocroot.equalsIgnoreCase("true")) {
		...
		if (!currentDocroot.endsWith("/")) {
			currentDocroot = (!currentDocroot.equals("") && currentDocroot.length() > 0) ? "/" : currentDocroot;
		}
		filenameData = (filenameData != null) ? new StringBuffer(filenameData.toString().replaceAll("\\.\\.", ""))
				: filenameData;
		fileDestData = (fileDestData != null) ? new StringBuffer(fileDestData.toString().replaceAll("\\.\\.", ""))
				: fileDestData;
		if (action.equalsIgnoreCase("read")) {
			...
			try {
				data = c.getVendContext().getFileAccess().getFile(c.getWorkerContext(),
						filenameData.toString().replaceAll("\\.\\.", ""), c.getClientContext().getMatchedHost(),
						c.getVendContext().getVend().getRenderExtension(c.getClientContext().getMatchedHost()), null);
			...
			if (data == null) {
				try {
					...
					Debug.user(logTime,
							"Unable to render: Filename '" + currentDocroot + filenameData + "' does not exist.");
					return new StringBuffer();
				}
			}
			if (xmlparse == null || xmlparse.equals("")) {
				if (data == null) {
					Debug.user(logTime, "Opening filename '" + currentDocroot + filenameData
							+ "' for reading into buffer '" + varname + "'");
					data = new byte[32768];
				...
				if (renderOutput != null && renderOutput.equalsIgnoreCase("ssp")) {
					String outputData = "<?xml version=\"1.0\" encoding=\"utf-8\"?>\n\n" + new String(
							FileAccess.getDefault().processServerPageData(c.getWorkerContext(), docOutput));
			...
			try {
				...
				if (file == null) {
					c.setExceptionState(true,
							"Unable to write to file '" + filenameData + "': Cannot write to location specified");
					return new StringBuffer();
				} else if (file.isDirectory()) {
					c.setExceptionState(true, "Unable to write to file '" + filenameData + "': Is a directory.");
					return new StringBuffer();
				}
				fos = (filemode.equalsIgnoreCase("append")) ? new FileOutputStream(file, true)
				...
				fos.close();
				Debug.user(logTime, "Wrote contents to filename '" + currentDocroot + filenameData + "' (length="
						+ contentsData.length() + ")");
			} catch (IOException e) {
			...
			filenameDataString = (filenameDataString.equals("")) ? c.getClientContext().getPostVariable("current_path")
					: filenameDataString;
			...
			if (numSlashes > 1) {
				for (int i = 0; i < (dirEntries.length - 1); i++) {
					parentDirectory = (dirEntries[i] != null && !dirEntries[i].equals("")) ? dirEntries[i] + "/"
							: parentDirectory;
				}
			}
			parentDirectory = (parentDirectory.length() > 1 && parentDirectory.endsWith("/"))
					? parentDirectory.substring(0, parentDirectory.length() - 1)
					: parentDirectory;
			if (c.getVendContext() != null && c.getVendContext().getFileAccess() != null
					&& c.getVendContext().getFileAccess().getVFSType(filenameData.toString(),
							c.getClientContext().getMatchedHost()) == FileAccess.TYPE_JAR) {
				Vector listFiles = c.getVendContext().getFileAccess().listFiles(filenameData.toString(),
						c.getClientContext().getMatchedHost());
				...
				for (int i = 0; i < list.length; i++) {
					...
					int curDepth = 0;
					if (zipFile.equalsIgnoreCase(".acl") || zipFile.equalsIgnoreCase("access.list")
							|| zipFile.equalsIgnoreCase("application.inc") || zipFile.equalsIgnoreCase("global.inc")
							|| zipFile.indexOf("/.proc") != -1 || zipFile.indexOf("/procedures") != -1) {
						continue;
					}
					...
					if (zipFile.startsWith(filenameData.toString())) {
						...
						if (curDepth == depth) {
							fileType = (zEntry.isDirectory()) ? "directory" : "file";
							...
							buffer.append("\"/>\n");
							c.getVariableContainer().setVariable(c, varname + "(" + displayFilename + ")(length)",
									false, "" + fileLength);
							c.getVariableContainer().setVariable(c, varname + "(" + displayFilename + ")(type)", false,
									fileType);
							c.getVariableContainer().setVariable(c, varname + "(" + displayFilename + ")(mode)", false,
									fileMode);
							c.getVariableContainer().setVariable(c, varname + "(" + displayFilename + ")(time)", false,
									fileTime);
						}
					} else {
						if (curDepth == depth) {
			...
			} else if (c.getVendContext() != null && c.getVendContext().getFileAccess() != null
					&& c.getVendContext().getFileAccess().getVFSType(filenameData.toString(),
							c.getClientContext().getMatchedHost()) == FileAccess.TYPE_FS) {
				Vector listFiles = c.getVendContext().getFileAccess().listFiles(filenameData.toString(),
						c.getClientContext().getMatchedHost());
				Object[] list = listFiles.toArray();
				...
				for (int i = 0; i < filesorted.length; i++) {
					...
					if (filename.equalsIgnoreCase(".acl") || filename.equalsIgnoreCase("access.list")
							|| filename.equalsIgnoreCase("application.inc") || filename.equalsIgnoreCase("global.inc")
							|| filename.indexOf("/.proc") != -1 || filename.indexOf("/procedures") != -1) {
						continue;
					}
					...
					if (zEntry.isDirectory()) {
						...
						fileType = (zEntry.isAbsolute()) ? "absolute" : fileType;
					String fileMode = "read-only";
					if (zEntry.canRead() && !zEntry.canWrite()) {
						...
						fileMode = (zEntry.canRead() && zEntry.canWrite()) ? "read/write" : fileMode;
					...
					c.getVariableContainer().setVariable(c, varname + "(" + filename + ")(length)", false,
							"" + fileLength);
				...
				fileStringList = (!filter.equals("")) ? file.list(new ListFilter(filter)) : file.list();
				if (sort.equalsIgnoreCase("asc")) {
				...
				if (fileStringList == null) {
					...
					Debug.user(logTime, "Directory '" + currentDocroot + filenameData + "' returns "
							+ fileStringList.length + " entry(ies)");
				}
				buffer = new StringBuffer();
				buffer.append("<listing filter=\"");
				buffer.append(filter);
				...
				for (int i = 0; i < fileStringList.length; i++) {
					...
					if (file.isDirectory()) {
						...
						fileType = (file.isAbsolute()) ? "absolute" : fileType;
					String fileMode = "read-only";
					if (file.canRead() && !file.canWrite()) {
						...
						fileMode = (file.canRead() && file.canWrite()) ? "read/write" : fileMode;
					...
					c.getVariableContainer().setVariable(c, varname + "(" + fileStringList[i] + ")(length)", false,
							"" + fileLength);
					c.getVariableContainer().setVariable(c, varname + "(" + fileStringList[i] + ")(type)", false,
							fileType);
					c.getVariableContainer().setVariable(c, varname + "(" + fileStringList[i] + ")(mode)", false,
							fileMode);
					c.getVariableContainer().setVariable(c, varname + "(" + fileStringList[i] + ")(time)", false,
							fileTime);
				}
				buffer.append("</listing>");
				if (xmlOutput.equalsIgnoreCase("true")) {
			...
			if (file.isDirectory()) {
				...
				filenamePattern = (filenameData.toString().indexOf("/") != -1)
						? filenameData.toString().substring(filenameData.toString().lastIndexOf("/") + 1)
						: filenamePattern;
				String filenameDirectory = currentDocroot;
				filenameDirectory = (filenameData.toString().indexOf("/") != -1)
						? filenameData.substring(0, filenameData.toString().lastIndexOf("/"))
						: filenameDirectory;
				String[] fileStringList = null;
			...
			if (fileDestData.equals("")) {
				c.getVariableContainer().setVariable(varname + "-result",
						filenameData + ": File operation failed: No destination filename given.");
				return new StringBuffer();
			}
			java.io.File file = new java.io.File(currentDocroot + filenameData.toString());
			boolean success = file.renameTo(new java.io.File(currentDocroot + fileDestData.toString(), file.getName()));
			if (!success) {
				c.setExceptionState(true, "Unable to rename '" + currentDocroot + filenameData + "' to '"
						+ currentDocroot + fileDestData + "'");
			}
		} else if (action.equalsIgnoreCase("copy")) {
			if (fileDestData.equals("")) {
				c.setExceptionState(true,
						"File copy operation failed for file '" + filenameData + "': No destination file specified.");
				return new StringBuffer();
			}
			...
			if (vartype != null && vartype.equalsIgnoreCase("file")) {
				useFilename = (useFilename.indexOf("/") != -1) ? useFilename.substring(useFilename.lastIndexOf("/") + 1)
						: useFilename;
			...
			try {
				...
				if (varname != null) {
					c.getVariableContainer().setVariable(varname + "-result",
							filenameData + " copy to " + fileDestData + ": File copy succeeded.");
				} else {
					return new StringBuffer("true");
				}
			} catch (IOException e) {
				c.setExceptionState(true,
						"Filecopy from '" + filenameData + "' to '" + fileDestData + "' failed: " + e.getMessage());
			}
