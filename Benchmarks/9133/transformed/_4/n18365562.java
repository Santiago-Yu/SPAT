				...
				while (entry != null) {
					...
					if (!entry.equals("")) {
						...
						for (Token t : tokens) {
							...
							token = (_case_insensitive_gazetteer) ? token.toLowerCase() : token;
							@SuppressWarnings("unchecked")
							Map<String, Map> next = m.get(token);
							next = (next == null) ? new HashMap<String, Map>() : next;
							m.put(token, next);
							m = next;
						}
						m.put(STOP, null);
