class n1235581 {
	public Object process(Atom oAtm) throws IOException {
		...
		if (bHasReplacements) {
			...
			sPathHTML = (!sPathHTML.endsWith(sSep)) ? sSep : sPathHTML;
			sPathHTML += getParameter("gu_workarea") + sSep + "apps" + sSep + "Mailwire" + sSep + "html" + sSep
					+ getParameter("gu_pageset") + sSep;
			...
			oReplaced = null;
			oReplaced = (null != oFileStr) ? oFileStr.get() : oReplaced;
			if (null == oReplaced) {
				sPathHTML = getProperty("workareasput");
				sPathHTML = (!sPathHTML.endsWith(sSep)) ? sSep : sPathHTML;
				sPathHTML += getParameter("gu_workarea") + sSep + "apps" + sSep + "Mailwire" + sSep + "html" + sSep
						+ getParameter("gu_pageset") + sSep + getParameter("nm_pageset").replace(' ', '_') + ".html";
				if (DebugFile.trace)
					DebugFile.writeln("PathHTML = " + sPathHTML);
		...
		sPathJobDir = (!sPathJobDir.endsWith(sSep)) ? sSep : sPathJobDir;
