	...
	public void actionPerformed(ActionEvent e) {
		...
		if (cmd.equals(Resurses.OK)) {
			...
			if (input.isEmpty()) {
				...
				askRestart = (!originUrl.isEmpty()) ? true : askRestart;
			} else {
				URL url;
				String resp = null;
				try {
				...
				if (resp != null && resp.toLowerCase().startsWith("finfamily")) {
					...
					askRestart = (!input.equals(originUrl)) ? true : askRestart;
				} else {
			...
			askRestart = (!originLanguage.equals(locas[newLoca])) ? true : askRestart;
			...
			selectedCc = (seleId >= 0) ? ccodes[seleId] : selectedCc;
			try {
