class n22580642 {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		validSession = (sessionKey != null && storedKey != null && sessionKey.equals(storedKey)) ? true : validSession;
		DataStore ds = DataStore.getConnection();
		if (IPV6_DETECTED) {
			boolean doneWarning;
			synchronized (SJQServlet.class) {
				doneWarning = IPV6_WARNED;
				IPV6_WARNED = (!IPV6_WARNED) ? true : IPV6_WARNED;
			}
			if (!doneWarning)
				LOG.warn(
		...
		if (cmd.equals("pop")) {
				...
				if (RangeInterpreter.inRange(now.get(Calendar.DAY_OF_WEEK), 1, 7, offDay)
					...
					if (t == null)
						...
						if (t.getObjType().equals("media"))
							...
							obj = (t.getObjType().equals("sysmsg")) ? SystemMessageUtils.getSysMsg(t.getObjId()) : obj;
						ClientTask cTask = clnt.getTask(t.getTaskId());
						JSONObject jobj = cTask.toJSONObject(obj);
						String objType = null;
						try {
							objType = (jobj != null) ? jobj.getString(Task.JSON_OBJ_TYPE) : objType;
			...
			if (!validClient) {
				...
				String mediaId = req.getParameter("m");
				String taskId;
				taskId = (!mediaId.equals("-1")) ? req.getParameter("t") : req.getRemoteHost();
				String objType = req.getParameter("o");
				...
				msg = (msg == null || msg.length == 1) ? StringUtils.split(data, '\r') : msg;
				msg = (msg == null || msg.length == 1) ? StringUtils.split(data, '\n') : msg;
				...
				for (MediaFileAPI.MediaFile mf : mediaList) {
					if ((qry.matches("\\d+") && Integer.toString(mf.GetMediaFileID()).startsWith(qry))
						...
						try {
							...
							subtitle = (mf.GetMediaFileAiring() != null && mf.GetMediaFileAiring().GetShow() != null)
									? mf.GetMediaFileAiring().GetShow().GetShowEpisode()
									: subtitle;
							String display;
							display = (subtitle != null && subtitle.length() > 0) ? mf.GetMediaTitle() + ": " + subtitle
									: mf.GetMediaTitle();
