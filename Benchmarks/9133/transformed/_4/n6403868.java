	...
	private File extractUninstallFiles(File _destPath, boolean upgrade, String lastVer) {
		...
		try {
			...
			if (upgrade) {
				...
				oldClassCopied = (uInfo_.module) ? copyOldSetupClass(lastVerPath, destPath) : oldClassCopied;
			} else {
				if ((!destPath.exists()) && (!destPath.mkdirs())) {
					throw new IOException(VAGlobals.i18n("Setup_NotCreateDirectory") + " " + destPath);
				}
			...
			if (!oldClassCopied && (installClassSize_ > 0 || jarOffset_ > 0)) {
				...
				buf = new byte[0];
				buf = (installClassSize_ < 0) ? new byte[(int) jarOffset_] : new byte[(int) installClassSize_];
			...
			language = (language == null) ? "default" : language;
