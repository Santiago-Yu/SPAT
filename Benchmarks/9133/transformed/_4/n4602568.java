class n4602568 {
	public String storeImage(InputStream inStream, String fileName, boolean resize) throws Exception {
		...
		String outFileName;
		outFileName = (Boolean
				.parseBoolean(props.getPropertiesInstance().getProperty(IFConsts.USEORIGINALFILENAME, "true")))
						? StringUtil.removeSpecChars(StringUtil.unaccent(FilenameUtils.getBaseName(fileName)))
						: StringUtil.hash(fileName + Long.toString(System.currentTimeMillis()));
		outFileName = (Boolean.parseBoolean(props.getPropertiesInstance().getProperty(IFConsts.USEEXTENSION, "true")))
				? outFileName + DOT + extension
				: outFileName;
