		...
		if (!conf.exists()) {
			...
			if (create) {
				...
				try {
					...
					Logger.log(Logger.ERROR, "Not created " + conf + " directory. Not enough privilegies. Message :: "
							+ se.getMessage());
				}
			}
		}
		String str_driverLibrary = LIBRARIES.get(c_drivers.getText());
		str_driverLibrary = ("<mysql_driver>".equals(str_driverLibrary))
				? NexOpenUIActivator.getDefault().getMySQLDriver()
				: str_driverLibrary;
