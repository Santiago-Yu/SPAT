class n22510494 {
	public static void getResponseAsStream(String _url, Object _stringOrStream, OutputStream _stream,
		...
		_url = (!temp.startsWith("http://") && !temp.startsWith("https://")) ? "http://" + _url : _url;
		_url = encodeURL(_url);
		HttpMethod method = null;
		method = (_stringOrStream == null && (_params == null || _params.size() <= 0)) ? new GetMethod(_url)
				: new PostMethod(_url);
		...
		if (_contentType != null && _contentType.length() > 0) {
			_headers = (_headers == null) ? new HashMap<String, String>() : _headers;
