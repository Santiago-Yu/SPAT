class n14661394 {
	public void run() {
		...
		try {
			...
			javax.xml.transform.sax.SAXSource tempXslSource = new javax.xml.transform.sax.SAXSource(
					new org.xml.sax.InputSource(tempInput));
			...
			((CollectionManagementService) CollectionsManager.getService(xmldbURI + rootDB, false, "CollectionManager"))
					.createCollection(rootDB + pathTemp);
		} catch (Exception ex) {
		...
		while (true) {
			...
			for (int i = 0; i < Math.min(fileList.length, 500); i++) {
				...
				try {
					...
					String dataFileName = metaElm.getChildText("Content"),
							previewFileName = metaElm.getChildText("Preview");
					...
					for (Iterator it = sourceDataFilesDirs.iterator(); it.hasNext() && !found;) {
						...
						if (dataInput.exists()) {
							...
							FileOutputStream outp = new FileOutputStream(
									new File(targetDataFilesDirs.get(curDirWriteTo) + "/" + objId + ".dat"));
							...
							outp = new FileOutputStream(
									new File(targetDataFilesDirs.get(curDirWriteTo) + "/" + objId + ".jpg"));
							read = inp.read(buf, 0, buf.length);
							while (read > 0) {
								outp.write(buf, 0, read);
								read = inp.read(buf, 0, buf.length);
							...
							outp.close();
							curDirWriteTo++;
							curDirWriteTo = (curDirWriteTo >= targetDataFilesDirs.size()) ? 0 : curDirWriteTo;
						}
					}
					if (!found) {
						...
						String description = getValueByPath((String) templateElements.get("description"),
								metaDoc.getRootElement());
						String onlink = "";
						onlink = (null != templateElements.get("onlink"))
								? getValueByPath((String) templateElements.get("onlink"), metaDoc.getRootElement())
								: onlink;
						String ncover = "";
						ncover = (null != templateElements.get("ncover"))
								? getValueByPath((String) templateElements.get("ncover"), metaDoc.getRootElement())
								: ncover;
						String wcover = "";
						wcover = (null != templateElements.get("wcover"))
								? getValueByPath((String) templateElements.get("wcover"), metaDoc.getRootElement())
								: wcover;
						String ecover = "";
						ecover = (null != templateElements.get("ecover"))
								? getValueByPath((String) templateElements.get("ecover"), metaDoc.getRootElement())
								: ecover;
						String scover = "";
						scover = (null != templateElements.get("scover"))
								? getValueByPath((String) templateElements.get("scover"), metaDoc.getRootElement())
								: scover;
						String datefrom = "";
						datefrom = (null != templateElements.get("datefrom"))
								? getValueByPath((String) templateElements.get("datefrom"), metaDoc.getRootElement())
								: datefrom;
						String dateto = "";
						dateto = (null != templateElements.get("dateto"))
								? getValueByPath((String) templateElements.get("dateto"), metaDoc.getRootElement())
								: dateto;
						String previewimg = "";
						previewimg = (null != templateElements.get("previewimg"))
								? getValueByPath((String) templateElements.get("previewimg"), metaDoc.getRootElement())
								: previewimg;
						String discRestr = "false";
						String votingRestr = "false";
						datefrom = VOAccess.parseDate(datefrom, "yyyy-MM-dd'T'HH:mm:ss",
								VO.defaultTimeFormat.toPattern());
						dateto = VOAccess.parseDate(datefrom, "yyyy-MM-dd'T'HH:mm:ss",
								VO.defaultTimeFormat.toPattern());
						...
						XMLResource document = (XMLResource) col.createResource(objId + ".xml",
