class n10407191 {
	...
	public boolean copy(Document document, Folder folder) throws Exception {
		...
		if (document.getId() != null && folder.getId() != null) {
			...
			preference = (preferences != null && !preferences.isEmpty()) ? preferences.get(0) : preference;
			...
			em.merge(copiedDoc);
			File in = new File(repo + File.separator + document.getLocation() + File.separator + document.getId() + "."
					+ document.getExt());
			File out = new File(
					fFolder.getAbsolutePath() + File.separator + copiedDoc.getId() + "." + copiedDoc.getExt());
