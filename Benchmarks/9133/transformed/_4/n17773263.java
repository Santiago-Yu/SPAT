class n17773263 {
	public boolean visar() throws SQLException, ClassNotFoundException, Exception {
		...
		try {
			...
			visado = (registrosAfectados > 0 && !hayVisadoExtracto && !hayVisadoRemitente) ? true : visado;
			if (registrosAfectados > 0 && (hayVisadoExtracto || hayVisadoRemitente)) {
				...
				visado = (generado) ? actualizarBZSALIDA(conn) : visado;
				String rem = "";
				String com = "";
				if (hayVisadoRemitente) {
				...
				com = (hayVisadoExtracto) ? extracto : comentario;
				try {
					...
					Object[] params = { "V", new Integer(anoSalida), new Integer(numeroRegistro), new Integer(oficina),
							new Integer(fechaDocumento), rem, com, tipoDocumento, new Integer(fechaRegistro),
							new Integer(fzacagge), fora, new Integer(destinatario), idioma };
			...
			logLopdBZMODIF("UPDATE", sessioEjb.getCallerPrincipal().getName().toUpperCase(), fzafsis, horamili, 'S',
