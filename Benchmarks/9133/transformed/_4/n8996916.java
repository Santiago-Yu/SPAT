			...
			if (message == null) {
				...
				while (iter.hasNext()) {
					...
					if (contentId.startsWith(Constant.PREFIX_PERSON)) {
						...
						if (recv == null)
							throw new Exception("接收方信息对象转换错误！请检查存入的信息对象xml字符串是否正确:" + content);
						if (notify_id.equals("")) {
							...
							notify_content = (notify_content == null) ? "接收到新的esb消息，但未定义通知消息内容!" : notify_content;
						...
						sql = "insert into message_recv_attachment(ATTACHMENTID,"
								+ "VERSION,MRECVID,BUSS_ID,ATTACHMENT) values('" + attid + "',0,'" + id + "','"
								+ contentId + "',empty_blob())";
						pstmt = conn.prepareStatement(sql);
						pstmt.executeUpdate();
						sql = "select attachment from message_recv_attachment" + " where attachmentid = '" + attid
								+ "' for update";
						pstmt = conn.prepareStatement(sql);
