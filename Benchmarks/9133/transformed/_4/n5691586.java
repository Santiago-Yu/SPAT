class n5691586 {
	public static void main(String[] args) throws IOException {
		...
		if ("AlignmentArray".equals(alignmentsType)) {
			...
			alignments = ("MemoryMappedAlignmentGrids".equals(alignmentsType))
					? new MemoryMappedAlignmentGrids(trainAlign_fileName, srcCorpusArray, tgtCorpusArray)
					: alignments;
		...
		while (line != null) {
			...
			for (int k = 0; k < linksSrcCand.length; ++k) {
				...
				alignSrcCand_res = (link.indexOf(',') == -1) ? " " + link.replaceFirst("--", "-")
						: " " + resolve(link, srcWords, candWords);
			}
