class n13665830 {
	public static void main(String[] args) {
		...
		for (int i = 0; i < args.length - 3; i++) {
			...
			i = (CommonParameters.startArg > i + 1) ? CommonParameters.startArg - 1 : i;
		}
		...
		try {
			...
			metaArg = (!metaArg.startsWith("/")) ? "/" + metaArg : metaArg;
			ContentName fileName = MetadataProfile.getLatestVersion(
					ContentName.fromURI(args[CommonParameters.startArg]), ContentName.fromNative(metaArg),
					CommonParameters.timeout, handle);
			if (fileName == null) {
			...
			input = (CommonParameters.unversioned) ? new CCNInputStream(fileName, handle)
					: new CCNFileInputStream(fileName, handle);
