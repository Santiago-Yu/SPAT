		...
		synchronized (bundlesLock) {
			...
			if (securityManager != null) {
				securityManager.checkPermission(
						new AdminPermission(new StringBuilder("(location=").append(location).append(")").toString(),
			...
			try {
				...
				if (isBundleInstalled((String) headers.get(Constants.BUNDLE_SYMBOLICNAME))) {
					throw new BundleException(new StringBuilder("Bundle(location=").append(location)
				...
				if (entries != null) {
					if (entries[0].hasAttribute("extension")) {
						...
						if (extension.equals("bootclasspath")) {
							...
							if (!symbolicName.equals(framework.getSymbolicName())
									&& !symbolicName.equals(Constants.SYSTEM_BUNDLE_SYMBOLICNAME)) {
								throw new BundleException(new StringBuilder(
										"Trying to install a fragment Bundle(location=").append(location).append(
				...
				if (requiredEE != null) {
					...
					if (!ee.contains(requiredEE)) {
						throw new BundleException(new StringBuilder("Bundle(location=").append(location)
								.append(")  requires an unsopperted execution environment (=").append(requiredEE)
								.append(").").toString());
					}
				}
				bundle = (FrameworkUtil.isFragmentHost(headers)) ? new FragmentBundle(framework)
						: new HostBundle(framework);
				File bundlefile = new File(cache, Storage.BUNDLE_FILE);
				temp.renameTo(bundlefile);
				long lastModified = bundlefile.lastModified();
				BundleInfo info = new BundleInfo(bundleId, location, lastModified,
