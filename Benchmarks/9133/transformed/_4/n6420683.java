	...
	private boolean goToForum() {
		...
		try {
			...
			while ((line = in.readLine()) != null) {
				...
				m_attachId = (match_attachId.matches()) ? match_attachId.group(1) : m_attachId;
			}
			in.close();
		} catch (Exception e) {
			e.printStackTrace();
		...
		if (!gotNumReplies || !gotMsgNum) {
			...
			m_turnSummaryRef = (!gotNumReplies) ? "No num_replies found in A&A.org forum topic. " : m_turnSummaryRef;
			m_turnSummaryRef = (!gotMsgNum) ? "No msgXXXXXX found in A&A.org forum topic. " : m_turnSummaryRef;
			return false;
		}
		return true;
	}
