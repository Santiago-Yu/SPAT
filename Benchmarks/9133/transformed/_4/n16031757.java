class n16031757 {
	private void createProject(IProgressMonitor monitor, boolean launchNewTestWizard) {
		try {
			...
			classpath = new IClasspathEntry[] { JavaCore.newSourceEntry(srcFolder.getFullPath()),
			...
			IViewPart viewPart = workbench.getActiveWorkbenchWindow().getActivePage().getViewReferences()[0]
					.getView(false);
			navigator = (viewPart instanceof ResourceNavigator) ? (ResourceNavigator) viewPart : navigator;
