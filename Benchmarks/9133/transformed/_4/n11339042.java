		...
		try {
			...
			if (action.equals("test")) {
				try {
					...
					spectrumId = (l.size() > 0) ? ((Record) l.get(0)).getValue(1).asInt() : spectrumId;
					out.write("success");
				} catch (Exception ex) {
					out.write("failure");
				}
				...
				if (type == 1) {
					...
					rs = (type == 2) ? stmt.executeQuery("select YEAR(DATE) as YEAR, "
							+ (yearly ? "" : " MONTH(DATE) as MONTH, ")
				...
				spectrum = (spectrumId > -1) ? DBSpectrumPeer.retrieveByPK(new NumberKey(spectrumId))
						: DBSpectrumPeer.retrieveByPK(new NumberKey(req.getParameter("spectrumid")));
			...
			if (action.equals("exportmol")) {
				...
				shownumbers = (req.getParameter("shownumbers") != null
						&& req.getParameter("shownumbers").equals("false")) ? false : shownumbers;
