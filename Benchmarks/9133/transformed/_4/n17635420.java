class n17635420 {
	public static void verifierSiDerniereVersionDesPluginsMenus(ControleurDeMenu i) {
		...
		if (i.getVersionPlugin() == 0) {
			System.err
					.println("version non renseignee pour :" + i.getNomPlugin() + " on continue sur le plugin suivant");
			return;
		}
		URL url;
		try {
		...
		try {
			...
			System.err.println(
					"impossible d'ouvrir l'URL (destination inaccessible)" + i.getURLFichierInfoDerniereVersion()
							+ "\n lors de la recuperation des informations de version sur " + i.getNomPlugin());
			return;
		}
		File destination;
			...
			while ((ligne = lecteurAvecBuffer.readLine()) != null) {
				if (ligne.startsWith("version:")) {
					if (ligne.equals("version:" + i.getVersionPlugin())) {
						...
						System.err.println("la version pour " + i.getNomPlugin() + " est depassee ("
								+ i.getVersionPlugin() + " alors que la " + ligne + "est disponible)");
						estLaDerniereVersion = false;
					}
				}
				URLRecupererDerniereVersion = (ligne.startsWith("url:")) ? ligne.substring(4, ligne.length())
						: URLRecupererDerniereVersion;
			}
