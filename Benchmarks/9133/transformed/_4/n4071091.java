			...
			if (users != null) {
				for (FTPFile user : users) {
					...
					if (results.hasNext()) {
						...
						userNode = (session.getRootNode().hasNode("users/" + userName))
								? session.getRootNode().getNode("users/" + userName)
								: userNode;
									...
									if (collectionDate.after(aDayAgo)) {
										...
										if (images != null) {
											for (FTPFile image : images) {
												processImage(userName, experimentName, datasetName, collectionDate,
														image, client, userNode, session);
											}
										}
									}
								}
