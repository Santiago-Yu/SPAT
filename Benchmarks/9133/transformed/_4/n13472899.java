	...
	public void initialize() {
		...
		if (auSummaryList == null || auSummaryList.isEmpty()) {
			...
			if (auSummaryList == null) {
				...
				maodao.createTable();
				auSummaryList = (auOverviewTableFacade.isTableExistent("au_overview_table"))
						? auOverviewTableFacade.getNewerVerifiedAus(N_AUS_FOR_CHART)
						: auSummaryList;
			}
		}
		...
		if (auSummaryList != null) {
			for (AuOverviewTable auSummary : auSummaryList) {
				...
				auSummaryDataList.add(new AUSummaryDataForChart(auSummary.getAuName(), auSummary.getAuNReplicas(),
						verifiedReplicas, auNameToAuShortNameTable.get(auSummary.getAuName())));
				if (auSummary.getPollId() == null) {
					logger.fine("pollId is null");
					replicaData.add(new AUReplicaData(0, 0, 0, 0, 0, auSummary.getAuName(),
							auNameToAuShortNameTable.get(auSummary.getAuName())));
					continue;
				}
				...
				for (SuccessfulPollReplicaIp row : pollList) {
					...
					regionLabel = (StringUtils.isBlank(regionLabel)) ? "Others" : regionLabel;
				...
				Integer northeast = 0;
				northeast = (freqTable.get("NORTHEAST") != null) ? freqTable.get("NORTHEAST") : northeast;
				Integer midwest = 0;
				midwest = (freqTable.get("MIDWEST") != null) ? freqTable.get("MIDWEST") : midwest;
				Integer south = 0;
				south = (freqTable.get("SOUTH") != null) ? freqTable.get("SOUTH") : south;
				Integer west = 0;
				west = (freqTable.get("WEST") != null) ? freqTable.get("WEST") : west;
				Integer others = 0;
				others = (freqTable.get("Others") != null) ? freqTable.get("Others") : others;
				replicaData.add(new AUReplicaData(northeast, midwest, south, west, others, auSummary.getAuName(),
						auNameToAuShortNameTable.get(auSummary.getAuName())));
