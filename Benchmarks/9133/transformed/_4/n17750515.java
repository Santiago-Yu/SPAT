class n17750515 {
	public SukuData updatePerson(String usertext, SukuData req) {
		...
		userid = (userid != null && userid.length() > 16) ? userid.substring(0, 16) : userid;
		...
		if (nn != null) {
			...
			for (int i = 0; i < nn.length; i++) {
				if (nn[i].getTag().equals("NAME")) {
					String thisName = Utils.nv(nn[i].getGivenname()) + "/" + Utils.nv(nn[i].getPatronym()) + "/"
		...
		try {
			...
			if (req.persLong.getPid() > 0) {
				...
				if (req.persLong.isMainModified()) {
					pst = (req.persLong.getModified() == null)
							? con.prepareStatement(updPers + " and modified is null ")
							: con.prepareStatement(updPers + " and modified = ?");
				...
				for (int i = 0; i < nn.length; i++) {
					...
					if (n.isToBeDeleted()) {
						...
						if (n.getPnid() == 0) {
							...
							pst = (n.getModified() == null) ? con.prepareStatement(updSql + " and modified is null ")
									: con.prepareStatement(updSql + " and modified = ?");
							pnid = n.getPnid();
						}
					...
					pnid = (n.getPnid() > 0) ? n.getPnid() : pnid;
