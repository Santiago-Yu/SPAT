	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == print) {
			...
			if (term.getSelectedItem() == "Spring")
				...
				tr = (term.getSelectedItem() == "Summer") ? "SU" : "FL";
		...
		if (e.getSource() == net) {
			...
			inText = (inText.equals("-1")) ? parseData(text, true) : inText;
			if (inText.equals("\n")) {
				textArea2.append("\nNO DATA FOUND \n(" + urlIn + ")");
			} else
		...
		if (e.getSource() == button1) {
			...
			containsOR = (text1.toUpperCase().indexOf("OR:") == -1) ? false : true;
					...
					while (andST.hasMoreTokens()) {
						...
						if (temp.charAt(index) == 'a' || temp.charAt(index) == 'A') {
							...
							if (isLesse(fromTime, toTime) && !timeEq(toTime, 1200)) {
								fromTime = (String.valueOf(fromTime).length() == 4
										|| String.valueOf(fromTime).length() == 3) ? 1200 : 12;
							}
							if (!timeEq(toTime, 1200)) {
								toTime = (String.valueOf(toTime).length() == 4 || String.valueOf(toTime).length() == 3)
										? 1200
										: 12;
							}
						}
						fromTime = (String.valueOf(fromTime).length() == 2 || String.valueOf(fromTime).length() == 1)
								? 100
								: fromTime;
						toTime = (String.valueOf(toTime).length() == 2 || String.valueOf(toTime).length() == 1) ? 100
								: toTime;
