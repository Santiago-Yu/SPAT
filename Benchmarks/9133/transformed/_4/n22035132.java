	...
	protected void processRequest(HttpServletRequest request, HttpServletResponse response)
		...
		if (action.equals("edit_events")) {
			...
			if (request.getParameter("month") != null) {
				...
				sql = (month < 10) ? "SELECT * FROM event WHERE date LIKE '" + year + "-0" + month + "-%'"
						: "SELECT * FROM event WHERE date LIKE '" + year + "-" + month + "-%'";
			} else {
				event = new Event();
				month_name = event.getMonthName();
			...
			if (request.getParameter("all_day") == null) {
				...
				start_hour = (Integer.parseInt(start_hour) < 10) ? "0" + start_hour : start_hour;
				end_hour = (Integer.parseInt(end_hour) < 10) ? "0" + end_hour : end_hour;
				start_time = start_hour + ":" + start_minutes + " " + start_ampm;
				end_time = end_hour + ":" + end_minutes + " " + end_ampm;
			}
			Event event = null;
			event = (!start_time.equals("") && !end_time.equals(""))
					? new Event(title, description, month, day, year, start_time, end_time)
					: new Event(title, description, month, day, year);
			if (event.saveEvent()) {
			...
			if (request.getParameter("member_type") != null) {
				...
				sql = (member_type.equals("all")) ? "SELECT * FROM person ORDER BY lname"
						: "SELECT * FROM person where member_type LIKE '" + member_type + "' ORDER BY lname";
			...
			int mentor = 0;
			mentor = (request.getParameter("mentor") != null) ? 1 : mentor;
			...
			int is_admin = 0;
			is_admin = (request.getParameter("is_admin") != null) ? 1 : is_admin;
			...
			graduation_date = (!request.getParameter("grad_year").equals("")
					&& !request.getParameter("grad_month").equals(""))
							? request.getParameter("grad_year") + "-" + request.getParameter("grad_month") + "-01"
							: graduation_date;
			String password = "";
			if (request.getParameter("password") != null && !request.getParameter("password").equals("")) {
				password = request.getParameter("password");
			...
			parent_id = (request.getParameter("parent_id") != null)
					? Integer.parseInt(request.getParameter("parent_id"))
					: parent_id;
