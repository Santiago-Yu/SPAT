class n19715255 {
	static Object executeMethod(HttpMethod method, int timeout, boolean array)
		...
		try {
			...
			HttpClient httpClient = HttpClientUtils.createHttpClient(TWITTER_BASE_URL, config.getUserId(),
					config.getPassword());
			...
			if (status == HttpStatus.SC_OK) {
				...
				result = (array) ? JSONArray.fromString(response) : JSONObject.fromString(response);
			} else {
