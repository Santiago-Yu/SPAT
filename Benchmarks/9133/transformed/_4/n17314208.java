class n17314208 {
	public static void main(final String[] args) {
		...
		filename = (args[0].startsWith(BIZVIEW)) ? args[0].substring(BIZVIEW.length() + 1) : filename;
		sXQuery = (args[0].startsWith(XQUERY)) ? args[0].substring(XQUERY.length() + 1) : sXQuery;
		for (int i = 1; i < args.length; i++) {
			...
			if (args[i].startsWith(COMPRESS)) {
				...
				bCompress = (sCompressString.compareTo("YES") == 0) ? true : bCompress;
				continue;
			}
		...
		try {
			...
			if (sXmlDataFile != null && sXmlDataFile.length() > 0) {
				...
				builder = (saxDriverClass.length() > 0) ? new SAXBuilder(saxDriverClass) : new SAXBuilder();
