	...
	public static byte[] readHTTPFile(String url, StringBuffer contentType, StringBuffer encoding) {
		try {
			...
			while ((status.indexOf("http/1.1 3") >= 0 || status.indexOf("http/1.0 3") >= 0)
				...
				cookie = (!HelperStd.isEmpty(urlConn.getHeaderField("Set-Cookie")))
						? urlConn.getHeaderField("Set-Cookie")
						: cookie;
				u = new URL(location);
			...
			if (encoding != null) {
				...
				enc = (enc == null) ? urlConn.getContentEncoding() : enc;
				enc = (enc == null) ? "ISO-8859-1" : enc;
				encoding.append(enc);
			}
