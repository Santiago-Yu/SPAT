	...
	public void run() {
		if (mMode == 0) {
					...
					if (tokenizer.hasMoreTokens()) {
						try {
							if (tokenizer.nextToken().equalsIgnoreCase(mProductKey)) {
								...
								versionAvailable = (version > versionAvailable) ? version : versionAvailable;
							}
						} catch (Exception exception) {
						}
					}
			...
			if (App.isNotificationAllowed()) {
				...
				mMode = 2;
				if (WindowUtils.showConfirmDialog(null, result, MSG_UPDATE_TITLE, JOptionPane.OK_CANCEL_OPTION,
