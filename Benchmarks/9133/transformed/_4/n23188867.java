class n23188867 {
	public void createZipCopy(IUIContext ui, final String zipFileName, final File[] filesToZip,
			final FilenameFilter fileFilter, Timestamp timestamp) {
		...
		nameCopy = (nameCopy.endsWith(".zip")) ? nameCopy.substring(0, zipFileName.length() - 4) : nameCopy;
			...
			public void run(IProgressMonitor runnerMonitor) throws CoreException {
				try {
					...
					if (projectFiles.isEmpty()) {
						PlatformActivator.logDebug(
								"Zip file (" + zipFileName + ") not created because there were no files to zip");
						return;
					}
					...
					Status error = new Status(Status.ERROR, PlatformActivator.PLUGIN_ID, Status.ERROR,
		...
		try {
			...
			workspace.run(noResourceChangedEventsRunner, workspace.getRoot(), IWorkspace.AVOID_UPDATE,
