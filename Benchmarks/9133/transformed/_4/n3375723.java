	...
	public void importarEmpresasAbertas(File pArquivoTXT, Andamento pAndamento)
		...
		try {
			...
			while (in.hasNextLine()) {
				...
				for (int idxCaractere = 0; idxCaractere < registroTmp.length; idxCaractere++) {
					...
					if (c == SEPARADOR_DE_CAMPOS_DO_REGISTRO.charAt(0)) {
						...
						if (campoPossuiConteudo) {
							...
							campoAnterior = (camposTmp.size() > 0) ? camposTmp.get(camposTmp.size() - 1)
									: campoAnterior;
							campoIniciaComEspacoEmBranco = campoTmp.toString().startsWith(" ");
							if (campoAnterior != null && campoIniciaComEspacoEmBranco
									&& registroComExcessoDeDelimitadores) {
				...
				vCEP = (cepTmp != null && cepTmp.trim().length() > 0) ? new BigDecimal(cepTmp) : null;
				...
				dddFone = (dddTmp != null && dddTmp.trim().length() > 0) ? dddFone + dddTmp : dddFone;
				dddFone = (foneTmp != null && foneTmp.trim().length() > 0) ? dddFone + foneTmp : dddFone;
				if (dddFone != null && dddFone.trim().length() > 0) {
					dddFone = new BigDecimal(dddFone).toString();
					dddFone = (dddFone.length() > 10 && dddFone.endsWith("0")) ? dddFone.substring(0, 10) : dddFone;
				...
				vFAX = (faxTmp != null && faxTmp.trim().length() > 0) ? new BigDecimal(faxTmp) : null;
				...
				vCNPJ = (cnpjTmp != null && cnpjTmp.trim().length() > 0) ? new BigDecimal(cnpjTmp) : null;
				...
				vQUANT_DE_ACOES_ORDINARIAS = (qtdeAcoesON != null && qtdeAcoesON.trim().length() > 0)
						? new BigDecimal(qtdeAcoesON)
						: null;
				String qtdeAcoesPN = campos[CampoDoArquivoDasEmpresasAbertas.QUANT_DE_ACOES_PREF.ordinal()];
				vQUANT_DE_ACOES_PREF = (qtdeAcoesPN != null && qtdeAcoesPN.trim().length() > 0)
						? new BigDecimal(qtdeAcoesPN)
						: null;
				...
				vPATRIMONIO_LIQUIDO = (patrimonioLiquidoTmp != null && patrimonioLiquidoTmp.trim().length() > 0)
						? new BigDecimal(patrimonioLiquidoTmp)
						: null;
