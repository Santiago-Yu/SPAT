class n6008880 {
	protected void updateJava2ScriptProject(String prjFolder, String binRelative) {
		try {
			...
			ajaxPath = (idx != -1)
					? ajaxPath.substring(0, idx) + "net.sf.j2s.lib" + ajaxPath.substring(idx + key.length())
					: ajaxPath;
			...
			j2sRelativePath = (j2sRelativePath.length() > 0 && !j2sRelativePath.endsWith("/")) ? "/" : j2sRelativePath;
