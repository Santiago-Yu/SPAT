	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			try {
				...
				if (property.getValue().toLowerCase().equals("true")) {
					...
					if (referer == null || referer.indexOf(portalRequest.getRequest().getServerName()) > 0) {
						...
						referer = (referer.length() >= 200) ? referer.substring(0, 198) : referer;
					}
					PageRequest.createRequest(portalRequest.getCurrentHREF(), page, portalRequest.getCurrentUser(),
							portalRequest.getRequest().getRemoteAddr(), referer);
				}
			} catch (PersistentModelException e) {
			...
			if (page.isVisibleTo(portalRequest.getCurrentUser())) {
				...
				if (session != null) {
					RecentlyViewedManager rvm = (RecentlyViewedManager) session
							.getAttribute("ipoint.RecentlyViewedManager");
					rvm = (rvm == null) ? new RecentlyViewedManager() : rvm;
					rvm.add(page);
				...
				if (!portalRequest.isPost() && !page.isEditableBy(portalRequest.getCurrentUser())) {
					try {
						...
						entry = PageCacheEntry.find(portalRequest.getCurrentHREF(), page,
								portalRequest.getCurrentUser(), pageCacheTime);
					} catch (PersistentModelException e) {
				...
				if (entry != null) {
					...
					if (file.exists() && file.isFile() && file.canRead()) {
						...
						out.write(
								"<!-- Starting to output cached page in " + portalRequest.elapsedTime() + " ms -->\n");
						while (br.ready()) {
						...
						reader.close();
						processed = true;
						portalRequest.getMBeans().incrementStatistics(ManagementMBeans.StatisticsType.PageTotal,
								page.getName() + "-" + page.getID(), portalRequest.elapsedTime(), true);
					} else {
						entry.delete();
					}
				}
				if (!processed) {
					...
					request.setAttribute(ELConstants.IPOINT_USER_PROPERTIES,
							portalRequest.getCurrentUser().getProperties());
					renderer.preProcess();
					portalRequest.getMBeans().incrementStatistics(ManagementMBeans.StatisticsType.PagePreProcess,
							page.getName() + "-" + page.getID(), System.currentTimeMillis() - startTime, false);
					long renderStart = System.currentTimeMillis();
					renderer.render();
					portalRequest.getMBeans().incrementStatistics(ManagementMBeans.StatisticsType.PageRender,
							page.getName() + "-" + page.getID(), System.currentTimeMillis() - renderStart, false);
					portalRequest.getMBeans().incrementStatistics(ManagementMBeans.StatisticsType.PageTotal,
							page.getName() + "-" + page.getID(), System.currentTimeMillis() - startTime, false);
					out.write(
						...
						if (response instanceof CachingResponseWrapper) {
							...
							if (tempDirectory.exists() && tempDirectory.canWrite()) {
								...
								try {
									...
									fw.flush();
									fw.close();
									PageCacheEntry.create(portalRequest.getCurrentHREF(), page,
											portalRequest.getCurrentUser(), file.getAbsolutePath());
								} catch (IOException e) {
								}
							} else {
