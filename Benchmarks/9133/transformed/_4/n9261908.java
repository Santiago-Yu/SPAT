class n9261908 {
	public NamedList<Object> request(final SolrRequest request, ResponseParser processor)
		...
		path = (path == null || !path.startsWith("/")) ? "/select" : path;
		ResponseParser parser = request.getResponseParser();
		parser = (parser == null) ? _parser : parser;
		ModifiableSolrParams wparams = new ModifiableSolrParams();
		wparams.set(CommonParams.WT, parser.getWriterType());
		wparams.set(CommonParams.VERSION, parser.getVersion());
		params = (params == null) ? wparams : new DefaultSolrParams(wparams, params);
		params = (_invariantParams != null) ? new DefaultSolrParams(_invariantParams, params) : params;
		...
		try {
			...
			charset = (method instanceof HttpMethodBase) ? ((HttpMethodBase) method).getResponseCharSet() : charset;
			...
			if (_allowCompression) {
				...
				if (contentEncodingHeader != null) {
					...
					if (contentEncoding.contains("gzip")) {
						...
						respBody = (contentEncoding.contains("deflate")) ? new InflaterInputStream(respBody) : respBody;
				} else {
					...
					if (contentTypeHeader != null) {
						...
						if (contentType != null) {
							if (contentType.startsWith("application/x-gzip-compressed")) {
								...
								respBody = (contentType.startsWith("application/x-deflate"))
										? new InflaterInputStream(respBody)
										: respBody;
						}
					}
				}
