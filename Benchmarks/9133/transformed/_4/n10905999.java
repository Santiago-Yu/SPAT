	...
	protected int insertRecord(PutMetadataRequest request, PutMetadataInfo info)
		...
		try {
			...
			if (id < 0) {
				...
				closeStatement(st);
				if (nRows > 0) {
					st = (getIsDbCaseSensitive(this.getRequestContext()))
							? con.prepareStatement("SELECT id FROM " + sTable + " WHERE UPPER(docuuid)=?")
							: con.prepareStatement("SELECT id FROM " + sTable + " WHERE docuuid=?");
