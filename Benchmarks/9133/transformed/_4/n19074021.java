	...
	public void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (!file.exists() || (request.getParameter("nocache") != null)) {
			...
			try {
				...
				font_file = (request.getParameter("font") != null) ? "/WEB-INF/fonts/" + request.getParameter("font")
						: font_file;
				font_file = ctx.getRealPath(font_file);
				float size = 20.0f;
				size = (request.getParameter("size") != null) ? Float.parseFloat(request.getParameter("size")) : size;
				...
				if (1 < lines.size()) {
					...
					for (; heightMultiplier < lines.size(); ++heightMultiplier) {
						...
						maxWidth = (maxWidth < Math.ceil(fontBounds.getWidth()))
								? (int) Math.ceil(fontBounds.getWidth())
								: maxWidth;
					}
					width = maxWidth;
