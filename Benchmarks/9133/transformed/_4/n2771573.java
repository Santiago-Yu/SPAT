								...
								if (!getDBVersion().equals(latestVersion)) {
									try {
										...
										try {
											...
											boolean status = true;
											while (status) {
												buffer = (size - numWritten > 1024) ? new byte[1024]
														: new byte[(int) (size - numWritten)];
												...
												monitor.worked(read);
												monitor.subTask(downloadedSize + " of " + Integer.toString(size / 1024)
														+ " KB");
												status = (size == numWritten) ? false : status;
