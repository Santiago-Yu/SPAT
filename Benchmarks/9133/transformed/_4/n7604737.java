class n7604737 {
	public String get(String url) {
		...
		if (debug.DEBUG)
			debug.logger("gov.llnl.tox.util.href", "get(url)>> " + url);
		try {
			...
			urlReader.close();
			if (debug.DEBUG)
				debug.logger("gov.llnl.tox.util.href", "get(output)>> " + resultBuffer.toString());
			int xmlNdx = resultBuffer.lastIndexOf("?>");
			result = (xmlNdx == -1) ? resultBuffer.toString() : resultBuffer.substring(xmlNdx + 2);
		} catch (Exception e) {
