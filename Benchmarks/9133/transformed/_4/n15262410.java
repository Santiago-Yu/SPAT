class n15262410 {
	InputStream selectSource(String item) {
		item = (item == null) ? "http://pushnpop.net:8912/subpop.ogg" : item;
		...
		try {
			URL url = null;
			url = (running_as_applet) ? new URL(getCodeBase(), item) : new URL(item);
		...
		while (urlc != null && true) {
			...
			if (t != null && t.equals("udp-port")) {
				...
				udp_baddress = (t != null && t.equals("udp-broadcast-address")) ? s : udp_baddress;
		}
		return is;
	}
