class n10085826 {
	private void doConvert(HttpServletResponse response, ConversionRequestResolver rr, EGE ege, ConversionsPath cpath)
			throws FileUploadException, IOException, RequestResolvingException, EGEException, FileNotFoundException,
			ConverterException, ZipException {
		InputStream is = null;
			...
			while (iter.hasNext()) {
				...
				if (!item.isFormField()) {
					...
					try {
						...
						if (isComplex) {
							String fileExt;
							fileExt = (cpath.getOutputDataType().getMimeType().equals(APPLICATION_MSWORD)) ? DOCX_EXT
									: ZIP_EXT;
