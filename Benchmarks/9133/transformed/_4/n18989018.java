		...
		try {
			...
			if (input instanceof IStorageEditorInput) {
				...
				if (s instanceof IFile) {
					...
					path = (root.exists(path)) ? root.getLocation().append(path) : path;
					source = path.toFile();
				}
			...
			IContentDescription cd = Platform.getContentTypeManager().getDescriptionFor(in, input.getName(),
					new QualifiedName[] { ImageCore.VALID_FORMATS });
			if (in != null) {
				in.reset();
			}
