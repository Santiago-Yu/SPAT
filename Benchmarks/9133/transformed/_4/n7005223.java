					...
					for (StudentType student : students) {
						if (student.getOsobniCislo().equals(osobniCisloZeSlozky)) {
							...
							for (int i = (pokusyDirectories.length - 1); i >= 0; i--) {
								if ((pokusyDirectories[i].isDirectory()) && (pzp < Integer
										.parseInt(pokusyDirectories[i].getName().split("_")[1].trim()))) {
									...
									if ((testedFile.exists()) && (testedFile.isFile())) {
										...
										if (partsOfFilename.length == 4) {
											...
											priponaSouboru = (posledniCastSouboru.length <= 1) ? ""
													: posledniCastSouboru[1];
										}
										String samostatnaPraceNazev = Konfigurace.getInstance()
												.getSamostatnaPraceNazev();
									...
									if (zipFile.exists()) {
										...
										if (s.length >= 3) {
											...
											dateFromZipFile.set(Integer.parseInt(date[0]),
									...
									for (int j = start; j >= 0; j--) {
										...
										for (int k = 0; k < vlastnostiSouboruNL.getLength(); k++) {
											if (vlastnostiSouboruNL.item(k).getNodeName().equals("cas")) {
												String[] obsahElementuCas = vlastnostiSouboruNL.item(k).getTextContent()
														.split(" ");
												String[] datumZElementu = obsahElementuCas[0].split("-"),
														casZElementu = obsahElementuCas[1].split("-");
												Calendar datumACasZElementu = new GregorianCalendar();
												datumACasZElementu.set(Integer.parseInt(datumZElementu[0]),
														Integer.parseInt(datumZElementu[1]) - 1,
														Integer.parseInt(datumZElementu[2]),
														Integer.parseInt(casZElementu[0]),
														Integer.parseInt(casZElementu[1]),
														Integer.parseInt(casZElementu[2]));
												if ((dateFromZipFile != null)
														&& (datumACasZElementu.compareTo(dateFromZipFile) > 0)) {
													shodaJmenaSouboru = false;
													break;
												}
											}
											shodaJmenaSouboru = (vlastnostiSouboruNL.item(k).getNodeName()
													.equals("nazev"))
															? vlastnostiSouboruNL.item(k).getTextContent()
																	.equals(testedFile.getName())
															: shodaJmenaSouboru;
											vysledekValidaceSouboru = (vlastnostiSouboruNL.item(k).getNodeName()
													.equals("vysledek")) ? vlastnostiSouboruNL.item(k).getTextContent()
															: vysledekValidaceSouboru;
										}
									...
									if (shodaJmenaSouboru && !samostatnaPrace) {
										...
										for (CviceniType ct : lct) {
											if (ct.getCislo().intValue() == cisloCviceniI) {
												...
												mtovGC.set(mtovt.getDatum().getYear(), mtovt.getDatum().getMonth() - 1,
														mtovt.getDatum().getDay(), 23, 59, 59);
												...
												XMLGregorianCalendar xmlGCDate = DatatypeFactory.newInstance()
														.newXMLGregorianCalendarDate(Integer.parseInt(datumSouboru[0]),
																Integer.parseInt(datumSouboru[1]),
																Integer.parseInt(datumSouboru[2]),
																DatatypeConstants.FIELD_UNDEFINED);
												XMLGregorianCalendar xmlGCTime = DatatypeFactory.newInstance()
														.newXMLGregorianCalendarTime(Integer.parseInt(casSouboru[0]),
																Integer.parseInt(casSouboru[1]),
																Integer.parseInt(casSouboru[2]),
																DatatypeConstants.FIELD_UNDEFINED);
												odevzdanoVcas = (fileTimeStamp.compareTo(mtovGC) <= 0) ? true : false;
												...
												for (UlohaType ut : lut) {
													if (ut.getCislo().intValue() == cisloUlohyI) {
														...
														lot.add(ot);
														if (vt.isVysledek()) {
															String test = String.format("%s%s%02d", validovane,
																	File.separator, ct.getCislo().intValue());
										...
										String[] partsOfFilename = testedFile.getName().split("_");
										String[] partsOfLastPartOfFilename = partsOfFilename[partsOfFilename.length - 1]
												.split("[.]");
										String osobniCisloZeSouboru = partsOfLastPartOfFilename[0];
										String priponaZeSouboru = partsOfLastPartOfFilename[partsOfLastPartOfFilename.length
												- 1];
										if ((partsOfLastPartOfFilename.length == 2)
												&& (priponaZeSouboru.equals("java"))) {
											if (student.getOsobniCislo().equals(osobniCisloZeSouboru)) {
												...
												if (lut.isEmpty()) {
													File samostatnaPraceNewFile = new File(
															sp + File.separator + testedFile.getName());
							...
							String[] slozkaPoslednihoPokusu = pokusyDirectories[pokusyDirectories.length - 1].getName()
									.split("_");
							int cisloPokusu = Integer
									.parseInt(slozkaPoslednihoPokusu[slozkaPoslednihoPokusu.length - 1].trim());
