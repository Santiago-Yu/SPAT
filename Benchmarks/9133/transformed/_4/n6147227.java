	...
	public static String[] putFECSplitFile(String uri, File file, int htl, boolean mode) {
					...
					for (int i = 0; i < segmentHeaders.size(); i++) {
						...
						File[] chunkFiles = new File[blockCount];
						{
							...
							while (count < segLength) {
								...
								System.out.println(Long.toHexString(
										((FcpFECUtilsSegmentHeader) segmentHeaders.get(i)).Offset + count));
								...
								inbytes = (inbytes > segLength - count) ? (int) (segLength - count) : inbytes;
								fcpOut.write(buffer);
						...
						FileOutputStream outFile = null;
						{
							...
							do {
								...
								started = (currentLine.equals("DataChunk")) ? true : started;
								length = (currentLine.startsWith("Length="))
										? Integer.parseInt((currentLine.split("="))[1], 16)
										: length;
		...
		if ((result[0].equals("Success") || result[0].equals("KeyCollision")) && mode) {
			try {
				...
				contentKey = (contentKey.startsWith(prefix)) ? contentKey.substring(prefix.length()) : contentKey;
