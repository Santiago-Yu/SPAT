class n9970377 {
	public void doWrite() {
		...
		try {
			IFile ifile = null;
			File file = null;
			ifile = (null != method.getResource())
					? method.getJavaProject().getResource().getProject().getFile(
							Statics.CFG_DIR + Statics.SEPARATOR + path.substring(path.lastIndexOf(Statics.SEPARATOR)))
					: ifile;
			file = new File(path);
			if (file.exists()) {
				boolean ans = MessageDialog.openQuestion(wb.getSite().getShell(), "Flow Plug-in",
