	...
	public boolean consolidateCrossrefGet(BiblioItem bib, ArrayList<BiblioItem> bib2) throws Exception {
		...
		if (beginPage != -1) {
			...
			if (st.countTokens() == 2) {
				...
				firstPage = (st.countTokens() == 1) ? pageRange : firstPage;
		}
		aut = (aut != null) ? TextUtilities.removeAccents(aut) : aut;
		title = (title != null) ? TextUtilities.removeAccents(title) : title;
		if (doi != null) {
			...
			if (cCon != null) {
				...
				try {
					...
					xml = (res.next()) ? res.getString(1) : xml;
					res.close();
					pstmt.close();
				...
				if (xml != null) {
					...
					if (bib2.size() > 0) {
						result = (!bib2.get(0).getError()) ? true : result;
					}
				}
			}
			if (xml == null) {
