class n14020143 {
	public byte[] evaluateResponse(byte[] responseBytes) throws SaslException {
		if (firstEvaluation) {
			...
			String nonce = null;
			try {
				nonce = new String(Base64.encodeBase64(MD5Digest(String.valueOf(System.nanoTime() + ":" + nonceUUID))),
						"US-ASCII");
			...
			try {
				...
				if (rawDirectives.containsKey("charset")) {
					...
					encoding = (value.equals("utf-8")) ? "UTF-8" : encoding;
				}
			...
			nonceListInMap = null;
			if (directives.get("qop").equals(QOP_AUTH_INT))
				integrity = true;
			else
				privacy = (directives.get("qop").equals(QOP_AUTH_CONF)) ? true : privacy;
			if (privacy) {
			...
			if (username.indexOf('@') == -1) {
				if (!directives.get("realm").equals("null")) {
					...
					username = (directives.get("authzid").indexOf('@') != -1)
							? directives.get("authzid").substring(directives.get("authzid").indexOf('@'))
							: username;
			}
			...
			username = (realm.equals("null") && username.indexOf('@') == -1) ? "@" + domainWithPassword.getDomain()
					: username;
