			...
			if (caching) {
				...
				if ((entry == null || !(new File(entry.cachefile).exists())) && cache) {
					...
					if (localfile) {
						...
						jarname = (slash != -1) ? jarname.substring(slash + 1) : jarname;
					}
				...
				zipPackages = (mtime != 0 && entry != null && entry.mtime == mtime) ? readCacheFile(entry, jarcanon)
						: zipPackages;
			}
			if (zipPackages == null) {
				...
				InputStream jarin;
				jarin = (jarconn == null) ? new BufferedInputStream(new FileInputStream(jarfile))
