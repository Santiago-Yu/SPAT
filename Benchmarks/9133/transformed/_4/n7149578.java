class n7149578 {
	...
	protected void parseContent(StreamLimiter streamLimiter, LanguageEnum forcedLang) throws IOException {
		...
		if (sourceDocument != null && urlItemFieldEnum != null) {
			FieldValueItem fieldValueItem = sourceDocument.getFieldValue(urlItemFieldEnum.contentTypeCharset.getName(),
					0);
			charset = (fieldValueItem != null) ? fieldValueItem.getValue() : charset;
			if (charset == null) {
		...
		charset = (charsetWasNull) ? getProperty(ClassPropertyEnum.DEFAULT_CHARSET).getValue() : charset;
		...
		if (contentType != null) {
			...
			charsetWasNull = (contentTypeCharset != null && !contentTypeCharset.equals(charset)) ? true
					: charsetWasNull;
		}
		if (charsetWasNull) {
			charset = (contentTypeCharset != null) ? contentTypeCharset : htmlProvider.getMetaCharset();
			htmlProvider = (charset != null) ? findBestProvider(charset, streamLimiter) : htmlProvider;
		}
		...
		for (HtmlNodeAbstract<?> node : htmlProvider.getMetas()) {
			...
			if ("keywords".equalsIgnoreCase(attr_name))
				addField(ParserFieldEnum.meta_keywords, HtmlDocumentProvider.getMetaContent(node));
			else if ("description".equalsIgnoreCase(attr_name))
				addField(ParserFieldEnum.meta_description, HtmlDocumentProvider.getMetaContent(node));
				...
				metaContentLanguage = ("content-language".equalsIgnoreCase(attr_http_equiv))
		...
		if (srcDoc != null && nodes != null && metaRobotsFollow) {
			...
			if (currentURL == null && urlItemFieldEnum != null) {
				...
				currentURL = (fvi != null) ? new URL(fvi.getValue()) : currentURL;
			}
			for (HtmlNodeAbstract<?> node : nodes) {
				...
				if ("a".equals(nodeName)) {
					...
					href = ("frame".equals(nodeName)) ? node.getAttributeText("src") : href;
				boolean follow = true;
				if (rel != null)
					follow = (rel.contains("nofollow")) ? false : follow;
				URL newUrl = null;
				if (href != null)
					if (!href.startsWith("javascript:"))
						newUrl = (currentURL != null) ? LinkUtils.getLink(currentURL, href, urlFilterList) : newUrl;
				if (newUrl != null) {
					ParserFieldEnum field = null;
					if (newUrl.getHost().equalsIgnoreCase(currentURL.getHost())) {
						field = (follow) ? ParserFieldEnum.internal_link : ParserFieldEnum.internal_link_nofollow;
					} else {
						field = (follow) ? ParserFieldEnum.external_link : ParserFieldEnum.external_link_nofollow;
					}
		...
		if (!metaRobotsNoIndex) {
			...
			nodes = (nodes == null || nodes.size() == 0) ? rootNode.getNodes("html") : nodes;
		...
		if (nodes != null && nodes.size() > 0) {
			...
			lang = (l != null) ? Lang.findLocaleISO639(l) : lang;
		}
		...
		if (lang != null) {
			...
			lang = (!metaRobotsNoIndex) ? langDetection(10000, ParserFieldEnum.body) : lang;
	}
