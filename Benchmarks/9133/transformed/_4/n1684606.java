class n1684606 {
	private void generateSchema() {
		...
		MessagesWindowDockable mconsole = (MessagesWindowDockable) console
		...
		try {
			...
			if (model.getInputType().equalsIgnoreCase("RELAX NG Grammar")) {
				...
				inFormat = (model.getInputType().equalsIgnoreCase("XML")) ? new XmlInputFormat() : inFormat;
			if (model.getOutputType().equalsIgnoreCase("DTD")) {
				...
				of = (model.getOutputType().equalsIgnoreCase("Relax NG Compact Grammar")) ? new RncOutputFormat() : of;
			ErrorHandlerImpl eh = new ErrorHandlerImpl(bao);
			SchemaCollection sc = null;
			if (!view.getModel().isUseExternalDocument()) {
				JTextComponent jtc = DefaultXPontusWindowImpl.getInstance().getDocumentTabContainer()
						.getCurrentEditor();
				if (jtc == null) {
				...
				try {
					sc = inFormat.load(UriOrFile.toUri(tmp.getAbsolutePath()), new String[0],
				...
				try {
					sc = inFormat.load(UriOrFile.toUri(view.getModel().getInputURI()), new String[0],
							model.getOutputType().toLowerCase(), eh);
				} catch (Exception ife) {
					StrBuilder stb = new StrBuilder();
					stb.append("\nError loading input document!\n");
			...
			OutputDirectory od = new LocalOutputDirectory(sc.getMainUri(), new File(view.getModel().getOutputURI()),
					model.getOutputType().toLowerCase(), DEFAULT_OUTPUT_ENCODING, DEFAULT_LINE_LENGTH, DEFAULT_INDENT);
