class n3054600 {
	private void generateArchetype(final IProject project, final IDataModel model, final IProgressMonitor monitor,
			final boolean offline) throws CoreException, InterruptedException, IOException {
		if (getArchetypeArtifactId(model) != null) {
			...
			artifact = (artifact == null || artifact.trim().length() == 0) ? project.getName() : artifact;
			...
			group = (group == null || group.trim().length() == 0) ? project.getName() : group;
			...
			final ILaunchConfigurationWorkingCopy workingCopy = launchConfigurationType.newInstance(null,
			...
			try {
				...
				goalName = (offline) ? new StringBuffer(goalName).append(" -o").toString() : goalName;
				...
				FileUtils.copyDirectoryStructure(new File(archetypePomDirectory, project.getName()),
						ArchetypePOMHelper.getProjectDirectory(project));
				monitor.worked(1);
