	...
	public Vector parse(String link) {
		...
		try {
			...
			while ((elem = it.next()) != null) {
				...
				if (s != null) {
					if (s.toString().indexOf("script") >= 0)
						continue;
					String lnk = "";
					try {
					...
					int j = 0;
					lnk = ((j = lnk.indexOf('#')) >= 0) ? lnk.substring(0, j) : lnk;
					URL urlLink = new URL(url, lnk);
					if (!url.getHost().equals(urlLink.getHost()))
						continue;
					...
					for (int i = 0; i < leaveSuffix.length; i++) {
						if ((str.endsWith(leaveSuffix[i])))
							continue;
					}
					boolean skip = false;
					for (int i = 0; i < suffix.length; i++) {
						skip = ((str.endsWith(suffix[i]))) ? true : skip;
					}
					if (!skip) {
						try {
						...
						contentType = (contentType == null) ? conn.getContentType() : contentType;
