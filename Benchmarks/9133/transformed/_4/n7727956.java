	...
	public static void createOutputStructure(String templatePath) throws InterruptedException {
		try {
			...
			while ((file = iterator.getNext()) != null) {
				...
				if (CVS_DIR
						.equals(file.getCanonicalPath().substring(fullFilename.length() - CVS_DIR.length(),
								fullFilename.length()))
				...
				if (path.indexOf(templatePath) == 0) {
					...
					if (path.startsWith(Paths.CONF_GENERAL_DIR)) {
						...
						copyFile = (config.matchWebTier("struts").booleanValue()) ? true : copyFile;
						...
						copyFile = (config.matchWebTier("tapestry").booleanValue()) ? true : copyFile;
					} else if (path.startsWith(Paths.JAVA_SWING_DIR)) {
						path = paths.getSwingOutput() + path.substring(Paths.JAVA_SWING_DIR.length());
						copyFile = (config.matchWebTier("swing").booleanValue()) ? true : copyFile;
						...
						copyFile = (config.matchBusinessTier("ejb 2").booleanValue()) ? true : copyFile;
					} else if (path.startsWith(Paths.JAVA_EJB3_DIR)) {
						path = paths.getEjbOutput() + path.substring(Paths.JAVA_EJB3_DIR.length());
						copyFile = (config.matchBusinessTier("ejb 3").booleanValue()) ? true : copyFile;
						...
						copyFile = (config.matchBusinessTier("hibernate 2").booleanValue()) ? true : copyFile;
						...
						copyFile = (config.matchBusinessTier("hibernate 3").booleanValue()) ? true : copyFile;
					} else if (path.startsWith(Paths.JAVA_MOCK_DIR)) {
						path = paths.getMockOutput() + path.substring(Paths.JAVA_MOCK_DIR.length());
						copyFile = (config.useMock().booleanValue()) ? true : copyFile;
						...
						copyFile = ((path.indexOf("build.bat") != -1) || ((path.indexOf("deploy.bat") != -1))) ? true
								: copyFile;
				}
