class n22939296 {
	private void modifyDialog(boolean fileExists) {
		if (fileExists) {
			RevLocation = (vars.containsKey(EnvironmentalVariables.WEBDAV_REVOCATION_LOCATION))
					? ((String) vars.get(EnvironmentalVariables.WEBDAV_REVOCATION_LOCATION))
					: RevLocation;
			CertLocation = (vars.containsKey(EnvironmentalVariables.WEBDAV_CERTIFICATE_LOCATION))
					? ((String) vars.get(EnvironmentalVariables.WEBDAV_CERTIFICATE_LOCATION))
					: CertLocation;
			...
			serialNumber = (vars.containsKey(EnvironmentalVariables.SERIAL_NUMBER_STRING))
					? (String) vars.get(EnvironmentalVariables.SERIAL_NUMBER_STRING)
					: "<EMPTY>";
			...
			if (vars.containsKey(LDAPSavingUtility.LDAP_SAVING_UTILITY_AC_TYPE)) {
				...
				if ((!acType.equals("")) && (!acType.equals("<EMPTY>")))
					jACType.setText((String) vars.get(LDAPSavingUtility.LDAP_SAVING_UTILITY_AC_TYPE));
				else
