class n7300267 {
	public void appendMessage(MimeMessage oMsg) throws FolderClosedException, StoreClosedException, MessagingException {
		...
		try {
			...
			sMessageID = (sMessageID != null) ? MimeUtility.decodeText(sMessageID) : sMessageID;
			sContentType = oMsg.getContentType();
			sContentType = (sContentType != null) ? MimeUtility.decodeText(sContentType) : sContentType;
			sContentID = oMsg.getContentID();
			sContentID = (sContentID != null) ? MimeUtility.decodeText(sContentID) : sContentID;
			sDisposition = oMsg.getDisposition();
			sDisposition = (sDisposition != null) ? MimeUtility.decodeText(sDisposition) : sDisposition;
			sContentMD5 = oMsg.getContentMD5();
			sContentMD5 = (sContentMD5 != null) ? MimeUtility.decodeText(sContentMD5) : sContentMD5;
			sDescription = oMsg.getDescription();
			sDescription = (sDescription != null) ? MimeUtility.decodeText(sDescription) : sDescription;
			sFileName = oMsg.getFileName();
			sFileName = (sFileName != null) ? MimeUtility.decodeText(sFileName) : sFileName;
			sEncoding = oMsg.getEncoding();
			sEncoding = (sEncoding != null) ? MimeUtility.decodeText(sEncoding) : sEncoding;
			sSubject = oMsg.getSubject();
			sSubject = (sSubject != null) ? MimeUtility.decodeText(sSubject) : sSubject;
			sPriority = null;
			sMsgCharSeq = null;
		...
		if (DebugFile.trace)
			DebugFile.writeln("part boundary is \"" + (sBoundary == null ? "null" : sBoundary) + "\"");
		if (sMessageID == null)
			sMessageID = gu_mimemsg;
		else
			sMessageID = (sMessageID.length() == 0) ? gu_mimemsg : sMessageID;
		Timestamp tsSent;
		tsSent = (oMsg.getSentDate() != null) ? new Timestamp(oMsg.getSentDate().getTime()) : null;
		Timestamp tsReceived;
		tsReceived = (oMsg.getReceivedDate() != null) ? new Timestamp(oMsg.getReceivedDate().getTime())
		...
		try {
			...
			if (sXPriority == null)
				...
				for (int x = 0; x < sXPriority.length(); x++) {
					...
					sPriority = (cAt >= (char) 48 || cAt <= (char) 57) ? cAt : sPriority;
				}
		...
		try {
			...
			bIsSpam = (sXSpam != null)
					? (sXSpam.toUpperCase().indexOf("YES") >= 0 || sXSpam.toUpperCase().indexOf("TRUE") >= 0
							|| sXSpam.indexOf("1") >= 0)
					: bIsSpam;
		} catch (MessagingException msge) {
			if (DebugFile.trace)
		...
		if (aFrom != null) {
			oFrom = (aFrom.length > 0) ? (InternetAddress) aFrom[0] : null;
		} else
			oFrom = null;
		if (DebugFile.trace)
		...
		if (aReply != null) {
			oReply = (aReply.length > 0) ? (InternetAddress) aReply[0] : null;
		} else {
			if (DebugFile.trace)
		...
		oFlgs = (oFlgs == null) ? new Flags() : oFlgs;
		MimePart oText = null;
		...
		if ((iOpenMode & MODE_MBOX) != 0) {
			try {
				...
				if (DebugFile.trace)
					DebugFile.writeln("new ByteArrayOutputStream(" + String.valueOf(iSize > 0 ? iSize : 16000) + ")");
		...
		try {
			oText = (oMsg.getClass().getName().equals("com.knowgate.hipermail.DBMimeMessage"))
					? ((DBMimeMessage) oMsg).getBody()
					: new DBMimeMessage(oMsg).getBody();
			if (DebugFile.trace)
				DebugFile.writeln(
		...
		try {
			...
			if (null != aRecipients)
				aRecipients = (aRecipients.length == 0) ? null : aRecipients;
			if (null != aRecipients) {
				sRecipientName = ((InternetAddress) aRecipients[0]).getPersonal();
				sRecipientName = (null == sRecipientName) ? ((InternetAddress) aRecipients[0]).getAddress()
						: sRecipientName;
		...
		try {
			...
			if (oFrom != null) {
				...
				if (oRSet.next()) {
					...
					sGuContact = (oRSet.wasNull()) ? "null" : sGuContact;
					sGuCompany = oRSet.getString(2);
					sGuCompany = (oRSet.wasNull()) ? "null" : sGuCompany;
					if (sGuCompany.equals("****************************USER")) {
						sTxName = oRSet.getString(3);
						sTxName = (oRSet.wasNull()) ? "" : sTxName;
						sTxSurname1 = oRSet.getString(4);
						sTxSurname1 = (oRSet.wasNull()) ? "" : sTxSurname1;
						sTxSurname2 = oRSet.getString(4);
						sTxSurname2 = (oRSet.wasNull()) ? "" : sTxSurname2;
			...
			if ((iOpenMode & MODE_MBOX) != 0) {
				if (DebugFile.trace)
					DebugFile.writeln(
							"MboxFile.appendMessage(" + (oMsg.getContentID() != null ? oMsg.getContentID() : "") + ")");
