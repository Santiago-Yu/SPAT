class n20568568 {
	private String createHTML(PAGE_TYPE requestPage) {
		...
		switch (requestPage) {
		case PAGE_LOGO:
			result += "</head><body class=\"header\">" + "<table width=\"100%\"><tr><td>"
			...
			try {
				...
				while (rs.next()) {
					...
					if (appendToHome != null) {
						result = (rs.getString("DESCRIPTION") != null)
								? "<H2>" + rs.getString("DESCRIPTION") + "</H2>\n"
								: result;
						result += stripHtml(appendToHome, false) + "<br>\n";
					}
					result = (rs.getInt("AD_MENU_ID") > 0)
							? "<a class=\"hrefNode\" href=\"http:///window/node#" + String.valueOf(
									rs.getInt("AD_WINDOW_ID") + "\">" + rs.getString("DESCRIPTION") + "</a><br>\n")
							: result;
					result += "<br>\n";
					result = (rs.getInt("PA_GOAL_ID") > 0) ? goalsDetail(rs.getInt("PA_GOAL_ID")) : result;
				}
