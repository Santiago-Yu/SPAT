		...
		if (svF.getExportRec() == null || !(svF.getExportRec().equals("export"))) {
			if (request.getParameter("CatId") != null && request.getParameter("OwnerId") != null
				...
				if ((request.getParameter("ClassNo") != null) && session.getAttribute("ClassNoForwarded") == null) {
					...
					hs = (session.getAttribute("searchLimits") != null)
							? (java.util.Hashtable) session.getAttribute("searchLimits")
							: hs;
						...
						if (catalogueId != null && ownerLibId != null) {
							try {
								...
								if (htDetails != null && !htDetails.isEmpty()) {
									...
									if (str1.length > 0) {
										titleD = str1[0];
										titleD = (titleD.length() > 45) ? titleD.substring(0, 45) + "..." : titleD;
									}
					...
					if (bib_id == 3 && mat_id == 1) {
						...
						type = (bib_id == 2 && mat_id == 1) ? "Serial Article"
								: ejb.bprocess.util.TypeDefinition.getInstance()
										.getTypeDefinition(String.valueOf(bib_id), String.valueOf(mat_id));
					...
					try {
						...
						vecLib.addElement("1");
						vecLib = (session.getAttribute("Libraries") != null)
								? (java.util.Vector) session.getAttribute("Libraries")
								: vecLib;
						String libIds = "";
			...
			if (format.equals("marc")) {
				...
				if (svF.getPs() != null && svF.getPs().equals("email")) {
					...
					mailMessage = (flag == false) ? "<h4><p>The selected records could not be mailed to "
							+ svF.getEmail() + "&nbsp; These might be the possible reasons.</p></h4>"
							...
							+ "<h4><p>Please contact the  Network Administrator </p></h4>" : mailMessage;
