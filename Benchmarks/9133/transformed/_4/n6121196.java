class n6121196 {
	public static boolean saveMap(LWMap map, boolean saveAs, boolean export) {
		...
		if (map.getSaveFileModelVersion() == 0) {
			final Object[] defaultOrderButtons = { VueResources.getString("saveaction.saveacopy"),
					VueResources.getString("saveaction.save") };
			Object[] messageObject = { map.getLabel() };
			response = VueUtil.option(VUE.getDialogParent(),
					VueResources.getFormatMessage(messageObject, "dialog.saveaction.message"),
					VueResources.getFormatMessage(messageObject, "dialog.saveaction.title"), JOptionPane.YES_NO_OPTION,
					JOptionPane.PLAIN_MESSAGE, defaultOrderButtons, VueResources.getString("saveaction.saveacopy"));
		}
		saveAs = (response == 0) ? true : saveAs;
		if ((saveAs || file == null) && !export) {
			file = ActionUtil.selectFile("Save Map", null);
		} else
			file = (export) ? ActionUtil.selectFile("Export Map", "export") : file;
		if (file == null) {
			try {
				return false;
			} finally {
		...
		try {
			...
			Throwable e = t;
			e = (t.getCause() != null) ? t.getCause() : e;
			...
			VueUtil.alert(String.format(Locale.getDefault(),
					...
					map.getLabel(), file, Util.formatLines(e.toString(), 80)), "Problem Saving Map");
		} finally {
