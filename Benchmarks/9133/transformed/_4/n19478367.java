class n19478367 {
	public void init(String[] arguments) {
		...
		String[] valid_args = new String[] { "device*", "d*", "help", "h", "speed#", "s#", "file*", "f*", "gpsd*",
				"nmea", "n", "garmin", "g", "sirf", "i", "rawdata", "downloadtracks", "downloadwaypoints",
				"downloadroutes", "deviceinfo", "printposonce", "printpos", "p", "printalt", "printspeed",
				"printheading", "printsat", "template*", "outfile*", "screenshot*", "printdefaulttemplate",
				"helptemplate", "nmealogfile*", "l", "uploadtracks", "uploadroutes", "uploadwaypoints", "infile*" };
		...
		if (args.isSet("device")) {
			...
			serial_port_name = (args.isSet("d")) ? (String) args.getValue("d") : serial_port_name;
		if (args.isSet("speed")) {
			serial_port_speed = ((Integer) args.getValue("speed")).intValue();
		} else
			serial_port_speed = (args.isSet("s")) ? ((Integer) args.getValue("s")).intValue() : serial_port_speed;
		if (args.isSet("file")) {
			filename = (String) args.getValue("file");
		} else
			filename = (args.isSet("f")) ? (String) args.getValue("f") : filename;
		if (args.isSet("gpsd")) {
			gpsd = true;
			...
			if (gpsd_host_port != null && gpsd_host_port.length() > 0) {
				...
				gpsd_host = params[0];
				gpsd_port = (params.length > 0) ? Integer.parseInt(params[1]) : gpsd_port;
			}
		}
		...
		if (args.isSet("nmealogfile") || (args.isSet("l"))) {
			nmea_log_file = (args.isSet("nmealogfile")) ? args.getStringValue("nmealogfile") : args.getStringValue("l");
		}
		...
		try {
			...
			if (print_waypoints || print_routes || print_tracks) {
				...
				writer = (args.isSet("outfile")) ? new FileWriter((String) args.getValue("outfile"))
			...
			if (args.isSet("printpos") || args.isSet("p") || args.isSet("printalt") || args.isSet("printsat")
					|| args.isSet("printspeed") || args.isSet("printheading")) {
