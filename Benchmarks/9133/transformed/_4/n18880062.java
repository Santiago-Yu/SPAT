class n18880062 {
	public static boolean compress(ArrayList sources, File target, Manifest manifest) {
		try {
			...
			if (isJar) {
				output = (manifest != null) ? new JarOutputStream(new FileOutputStream(target), manifest)
			...
			for (int i = 0, n = list.size(); i < n; i++) {
				...
				filename = (filename.startsWith(baseDir)) ? filename.substring(baseDirLength) : filename;
				if (isJar)
