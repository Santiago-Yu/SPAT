class n16343568 {
	public static void doGet(HttpServletRequest request, HttpServletResponse response, CollOPort colloport,
		...
		filename = (filename != null && filename.length() > 0) ? filename.substring(1) : filename;
		...
		if (includeURL != null && includeURL.length() > 0) {
			if (filename.indexOf(":") > 0) {
				...
				includeURL = (!includeURL.endsWith("/") && includeURL.indexOf(".") > 0)
						? includeURL.substring(0, includeURL.lastIndexOf("/") + 1)
						: includeURL;
			URL url = null;
			try {
