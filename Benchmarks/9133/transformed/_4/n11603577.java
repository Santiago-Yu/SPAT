class n11603577 {
	private void analyseCorpus(final IStatusDisplayer fStatus) {
		...
		try {
			...
			sSymbols = (!bSymbolsLoadedOK) ? getSymbolsByProbabilities(sgOverallGraph.getDataString(), fStatus)
					: sSymbols;
			...
			while (iSymbol.hasNext()) {
				...
				iMaxSymbolSize = (iMaxSymbolSize < sCurSymbol.length()) ? sCurSymbol.length() : iMaxSymbolSize;
				iMinSymbolSize = (iMinSymbolSize > sCurSymbol.length()) ? sCurSymbol.length() : iMinSymbolSize;
			}
			try {
