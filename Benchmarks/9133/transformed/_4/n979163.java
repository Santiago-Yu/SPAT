	...
	private List loadPluginFromDir(File directory, boolean bSkipAlreadyLoaded, boolean loading_for_startup,
		...
		try {
			...
			if (plugin_class_string == null) {
				...
				plugin_class_string = (plugin_class_string == null) ? "" : plugin_class_string;
			}
			String plugin_name_string = (String) props.get("plugin.name");
			plugin_name_string = (plugin_name_string == null) ? (String) props.get("plugin.names") : plugin_name_string;
			int pos1 = 0;
			int pos2 = 0;
			while (true) {
				...
				if (existing_pi != null) {
					...
					existing_parent = (existing_pi.getInitializerKey() instanceof File)
							? ((File) existing_pi.getInitializerKey()).getParentFile()
							: existing_parent;
					...
					for (int j = 0; j < default_version_details.length; j++) {
						if (plugin_class.equals(default_version_details[j][0])) {
							...
							plugin_name = (plugin_name == null) ? default_version_details[j][2] : plugin_name;
						...
						if (load_failure != null) {
							...
							if (!pid.equals(UpdaterUpdateChecker.getPluginID())) {
								String msg = "Error loading plugin '" + pluginName + "' / '" + plugin_class_string
										+ "'";
								LogAlert la;
								la = (load_failure instanceof UnsupportedClassVersionError)
										? new LogAlert(LogAlert.UNREPEATABLE, LogAlert.AT_ERROR,
												msg + ". "
														+ MessageText.getString("plugin.install.class_version_error"))
										: new LogAlert(LogAlert.UNREPEATABLE, msg, load_failure);
								Logger.log(la);
