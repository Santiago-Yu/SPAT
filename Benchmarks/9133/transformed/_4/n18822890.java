class n18822890 {
	public void actualizar() throws SQLException, ClassNotFoundException, Exception {
		...
		validado = (!validado) ? validar() : validado;
		if (!validado) {
		...
		try {
			...
			entrada1 = (entrada1.equals("")) ? "0" : entrada1;
			entrada2 = (entrada2.equals("")) ? "0" : entrada2;
			int fzanloc = Integer.parseInt(entrada1);
			int fzaaloc = Integer.parseInt(entrada2);
			disquet = (disquet.equals("")) ? "0" : disquet;
			...
			ss = (ss.length() > 2) ? ss.substring(0, 2) : ss;
			...
			if (!motivo.equals("")) {
				...
				if (!entidad1Nuevo.trim().equals("")) {
					entidad2Nuevo = (entidad2Nuevo.equals("")) ? "0" : entidad2Nuevo;
				}
				int fzanentNuevo;
				String fzacentNuevo;
			...
			if ((modificado && !motivo.equals("")) || motivo.equals("")) {
				...
				registroActualizado = (afectados > 0) ? true : false;
				String remitente = "";
