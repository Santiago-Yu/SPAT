class n10540676 {
	public static void checkForUpgrade(Event event) throws Exception {
		...
		clientAppliedVersion = (NullStatus.isNull(clientAppliedVersion)) ? "20050101" : clientAppliedVersion;
		String macquery = "";
		String clientMAC = "";
		...
		for (int i = 0; i < macList.size(); i++) {
			...
			macquery = (i < macList.size() - 1) ? "," : macquery;
		}
		...
		for (int i = 1; i < upgrades.length; i++) {
			...
			restartRequired = ("TEGSOFTJARS".equals(UPGRADETYPE) || "TOBEJARS".equals(UPGRADETYPE)
					|| "ALLJARS".equals(UPGRADETYPE) || "CONFIGASTERISK".equals(UPGRADETYPE)) ? true : restartRequired;
