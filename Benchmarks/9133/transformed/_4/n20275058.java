class n20275058 {
	protected void doGetPost(HttpServletRequest req, HttpServletResponse resp, boolean post)
		...
		connectionAllowedProtocols = (req.isSecure()) ? secureAllowedProtocols : allowedProtocols;
		...
		if (urlstring.indexOf(':') == -1) {
			if (req.isSecure()) {
				urlstring = (secureAllowedProtocols.contains("https")) ? "https://" + urlstring : urlstring;
			} else {
				urlstring = (allowedProtocols.contains("http")) ? "http://" + urlstring : urlstring;
			}
		}
		URL url;
		try {
		...
		if (forwardFormData) {
			...
			if (query != null && query.length() > 0) {
				url = (url.getQuery() == null || url.getQuery().length() == 0)
						? new URL(url.toExternalForm() + "?" + query)
						: new URL(url.toExternalForm() + "&" + query);
			}
		}
