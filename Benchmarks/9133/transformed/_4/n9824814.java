class n9824814 {
	public void run() {
		...
		try {
			...
			sessionCookies = (sessionCookies == null) ? beatportAccessor.getSessionCookies(user, password)
					: sessionCookies;
			...
			while (status == DOWNLOADING) {
				buffer = (size - downloaded < MAX_BUFFER_SIZE) ? new byte[(int) (size - downloaded)] : buffer;
