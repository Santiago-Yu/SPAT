	...
	public void run() {
		try {
			...
			while (true) {
				...
				getter = (str.startsWith("GET") || str.startsWith("DESCRIBE") || str.startsWith("POST")
						|| str.startsWith("HEAD")) ? str : getter;
				str = (str.startsWith("Accept-Encoding: gzip")) ? "Accept-Encoding: identity" : str;
				httpHeader += str + "\r\n";
			...
			if (resourceExists) {
				...
				sockWebInputStream = (cachedResource.exists()) ? new FileInputStream(cachedResource)
						: this.getClass().getResourceAsStream("/" + fileN);
			} else if (writeCache) {
			...
			baos = (inMemory) ? new ByteArrayOutputStream() : toBrowser;
			long total_read = 0;
			int bytes_read;
