class n16518661 {
	public String quebraLink(String link) throws StringIndexOutOfBoundsException {
		...
		if (linkOrig.contains("vinxp")) {
			...
			newlink = (newlink.contains(".vinxp")) ? newlink.replace(".vinxp", "") : newlink;
		...
		if (linkOrig.contains("?")) {
			...
			newlink = "";
			linkOrig = (linkOrig.contains("go!")) ? linkOrig.replace("?go!", "?") : linkOrig;
			if (linkOrig.contains("=")) {
				for (int i = 0; i < linkOrig.length(); i++) {
					letra = linkOrig.charAt(i);
					if (letra == '=') {
		...
		do {
			try {
				...
				if (!tmp.isEmpty() && tmp.length() > 5 && !tmp.contains("") && !tmp.contains("¡ì") && !tmp.contains("?")
					...
					ltmp = (!inverteFrase(tmp2).isEmpty() && inverteFrase(tmp2).length() > 5
							&& !inverteFrase(tmp2).contains("?") && !inverteFrase(tmp2).contains("¡ì")
							&& !inverteFrase(tmp2).contains("?")) ? HexToChar(inverteFrase(ltmp)) : ltmp;
			} catch (NumberFormatException e) {
			}
			tmp = decode64(ltmp);
			...
			if (!tmp.contains("?") && !tmp.contains("?")) {
				...
				ltmp = (!tmp2.contains("?") && !tmp2.contains("?")) ? decode64(inverteFrase(ltmp)) : ltmp;
			try {
				...
				if (!tmp.contains("?") && !tmp.contains("?") && !tmp.contains("¡ì") && !tmp.contains("?")
					...
					ltmp = (!tmp2.contains("?") && !tmp2.contains("?") && !tmp2.contains("¡ì") && !tmp2.contains("?")
							&& !tmp2.contains("*") && !tmp2.contains("\"") && !tmp2.contains("^"))
									? decodeAscii(inverteFrase(ltmp))
									: ltmp;
			} catch (NumberFormatException e) {
			}
			cont++;
