	...
	public static byte[] sendRequestV1(String url, Map<String, Object> params, String secretCode, String method,
		...
		if (method.equalsIgnoreCase("get")) {
			...
			try {
				...
				if (params != null) {
					getData = (secretCode == null) ? new NameValuePair[params.size()]
							: new NameValuePair[params.size() + 1];
					...
					if (secretCode != null) {
						...
						isHMac = (signMethod != null && signMethod.equalsIgnoreCase("hmac")) ? true : isHMac;
			...
			try {
				if (contentType == null) {
					if (files != null && files.size() > 0) {
						Part[] parts;
						parts = (secretCode == null) ? new Part[params.size() + files.size()]
								: new Part[params.size() + 1 + files.size()];
						Iterator<?> iters = params.keySet().iterator();
						int i = 0;
						while (iters.hasNext()) {
							String key = (String) iters.next();
						...
						if (params != null) {
							postData = (secretCode == null) ? new NameValuePair[params.size()]
									: new NameValuePair[params.size() + 1];
							Iterator<?> iters = params.keySet().iterator();
							int i = 0;
							while (iters.hasNext()) {
