	...
	public byte[] getBytesMethod(String url) {
		...
		try {
			...
			if (entity != null) {
				...
				in = (response.getEntity().getContentEncoding().getValue().equals("gzip")) ? new GZIPInputStream(in)
						: in;
			...
			logger.info("Facebook: Get Bytes Method done(" + statusCode + "), response bytes length: "
