class n9647574 {
	private void FindAvail() throws ParserConfigurationException, SQLException {
		...
		try {
			...
			availxml = (rsavail.next()) ? rsavail.getString(1) : availxml;
			...
			for (int i = 0; i < coachlist.getLength(); i++) {
				...
				seatxml = (rsseat.next()) ? rsseat.getString(1) : seatxml;
				...
				if (routesrcstr.equals(sourcenws) && routedeststr.equals(destnnws)) {
					...
					nspavailstr = (useragent) ? routesrc.getAttribute("agent") : routesrc.getAttribute("user");
					...
					navailcoach = Integer.parseInt(navailstr.trim());
					nspavailstr = (useragent) ? e2.getAttribute("agent") : e2.getAttribute("user");
