	...
	public String tranportRemoteUnitToLocalTempFile(String urlStr) throws UnitTransportException {
		...
		try {
			...
			throw new UnitTransportException(
					String.format("Unknown error. Maybe miss the username and password in url [%s].", urlStr), e);
		}
		String unitName = urlStr.substring(urlStr.lastIndexOf('/') + 1);
		try {
			tempUnit = (!StringUtils.isEmpty(unitName)) ? new File(CommonUtil.getTempDir(), unitName)
					: new File(CommonUtil.createTempFile());
