class n7300250 {
	public void open(int mode) throws MessagingException {
		...
		if ((0 == (mode & READ_ONLY)) && (0 == (mode & READ_WRITE))) {
			...
			mode = ((0 == (mode & MODE_MBOX)) && (0 == (mode & MODE_BLOB))) ? mode | MODE_MBOX : mode;
			iOpenMode = mode;
			oConn = ((DBStore) getStore()).getConnection();
			if ((iOpenMode & MODE_MBOX) != 0) {
				String sFolderUrl;
				try {
					sFolderUrl = Gadgets.chomp(getStore().getURLName().getFile(), File.separator)
							+ oCatg.getPath(oConn);
					if (DebugFile.trace)
						DebugFile.writeln("mail folder directory is " + sFolderUrl);
					sFolderDir = (sFolderUrl.startsWith("file://")) ? sFolderUrl.substring(7) : sFolderUrl;
				...
				try {
					oStmt = oConn.prepareStatement("SELECT NULL FROM " + DB.k_x_cat_objs + " WHERE " + DB.gu_category
							+ "=? AND " + DB.id_class + "=15", ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_READ_ONLY);
					...
					if (!bHasFilePointer) {
						...
						oLoca.store(oConn);
						oStmt = oConn.prepareStatement("INSERT INTO " + DB.k_x_cat_objs + " (" + DB.gu_category + ","
			...
			if (DebugFile.trace) {
				DebugFile.decIdent();
				String sMode = "";
				sMode = ((iOpenMode & READ_WRITE) != 0) ? " READ_WRITE " : sMode;
				sMode = ((iOpenMode & READ_ONLY) != 0) ? " READ_ONLY " : sMode;
				sMode = ((iOpenMode & MODE_BLOB) != 0) ? " MODE_BLOB " : sMode;
				sMode = ((iOpenMode & MODE_MBOX) != 0) ? " MODE_MBOX " : sMode;
