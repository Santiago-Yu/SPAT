		...
		try {
			...
			for (int i = 0; i < noMovies - 1; i++) {
				...
				for (int j = i + 1; j < noMovies; j++) {
					...
					pearsonCorr = (pearsonDenominator == 0.0) ? 0
							: new Double(sumXYPearson / pearsonDenominator).floatValue();
					double adjCosineDenominator = Math.sqrt(sumX2ACos) * Math.sqrt(sumY2ACos);
					adjustedCosineCorr = (adjCosineDenominator == 0.0) ? 0
							: new Double(sumXYACos / adjCosineDenominator).floatValue();
					double cosineDenominator = Math.sqrt(sumX2) * Math.sqrt(sumY2);
					cosineCorr = (cosineDenominator == 0.0) ? 0 : new Double(sumXY / cosineDenominator).floatValue();
