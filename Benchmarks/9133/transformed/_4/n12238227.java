	...
	public void doActionxxx() {
		try {
			...
			if (server_name.length() == 0) {
				...
				server_name = (!server_name.startsWith("http://")) ? "http://" + server_name : server_name;
				server_name = (!server_name.endsWith("/")) ? server_name + "/" : server_name;
			}
			...
			while (is.available() > 0) {
				array = (is.available() < 1024) ? new byte[is.available()] : array;
				is.read(array);
				raf.write(array);
