class n17613722 {
	public void go() throws FBConnectionException, FBErrorException, IOException {
		...
		for (int i = 0; i < gals.getLength(); i++) {
			...
			try {
				g = (DOMUtil.getSimpleElementText(curelement, "Name").startsWith("Tag: "))
						? new Tag(rg, DOMUtil.getSimpleElementText(curelement, "Name").substring(5),
								Integer.parseInt(DOMUtil.getSimpleAttributeText(curelement, "id")))
						: rg.createGallery(Integer.parseInt(DOMUtil.getSimpleAttributeText(curelement, "id")),
