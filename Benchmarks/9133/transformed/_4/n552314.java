class n552314 {
	public void refreshFileItem(YahooInfo legroup) throws Exception {
		...
		for (int i = 0; i < UrlList.size(); i++) {
			if (UrlList.get(i).getGroup().equals(legroup.getGroup())
					&& UrlList.get(i).getDir().startsWith(legroup.getDir())) {
				if (UrlList.get(i).isGroup()) {
					...
					if (mat.find()) {
						yi = new YahooInfo(UrlList.get(i).getGroup(), "/", "",
				...
				if (UrlList.get(i).isDir()) {
					...
					do {
						...
						while (mat.find()) {
							...
							ledir += "/";
							yi = new YahooInfo(UrlList.get(i).getGroup(), ledir, "",
									myuri.resolve(HTMLDecoder.decode(mat.group(1))).toURL().toString());
							UrlList.add(yi);
						}
						pat = Pattern.compile("<span class=\"title\">\n<a href=\"(.+?yahoofs.+?)\".*?>(.+?)</a>");
						mat = pat.matcher(lapage);
						while (mat.find()) {
							yi = new YahooInfo(UrlList.get(i).getGroup(), UrlList.get(i).getDir(), mat.group(2),
									myuri.resolve(HTMLDecoder.decode(mat.group(1))).toURL().toString());
							UrlList.add(yi);
						}
						System.out.println("");
						...
						mat = pat.matcher(lapage);
						myurl = null;
						myurl = (mat.find()) ? myuri.resolve(HTMLDecoder.decode(mat.group(1))).toURL() : myurl;
					} while (myurl != null);
				}
			}
		}
