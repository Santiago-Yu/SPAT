class n8015400 {
	protected synchronized InputStream openURLConnection(StreamDataSetDescriptor dsd, Datum start, Datum end,
			StringBuffer additionalFormData) throws DasException {
		...
		try {
			...
			if (dsd.isRestrictedAccess()) {
				key = server.getKey("");
				formData = (key != null) ? "&key=" + URLEncoder.encode(key.toString(), "UTF-8") : formData;
			}
			formData = (redirect) ? "&redirect=1" : formData;
