			...
			try {
				...
				if (response == 503) {
					...
					if (retry_after_date <= -1) {
						...
						last_response_retry_after_secs = (last_response_retry_after_secs < 0) ? -1
				...
				if (con_fail_is_perm_fail && !connected) {
					...
					outcome = (last_response_retry_after_secs >= 0)
							? ", Retry-After: " + last_response_retry_after_secs + " seconds"
							: outcome;
