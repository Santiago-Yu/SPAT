class n14577922 {
	public TwilioRestResponse request(String path, String method, Map<String, String> vars) throws TwilioRestException {
		...
		if (vars != null) {
			...
			encoded = (encoded.length() > 0) ? encoded.substring(1) : encoded;
		}
		String url = this.endpoint + path;
		url = (method.toUpperCase().equals("GET")) ? ((path.indexOf('?') == -1) ? "?" : "&") + encoded : url;
		try {
			...
			try {
				in = (con.getInputStream() != null) ? new BufferedReader(new InputStreamReader(con.getInputStream()))
						: in;
			} catch (IOException e) {
				in = (con.getErrorStream() != null) ? new BufferedReader(new InputStreamReader(con.getErrorStream()))
						: in;
			}
			if (in == null) {
