		...
		try {
			...
			if (objecttype.equalsIgnoreCase("T")) {
				...
				if (!exists && !modo.equals("3")) {
					...
					if (node.getString("SCHEMA").equals("DEF")) {
						...
						vt = (node.getString("SCHEMA").equals("SYS")) ? "SYS_" + vt : vt;
			...
			if (objecttype.equalsIgnoreCase("F")) {
				...
				if (rslt.next()) {
					...
					if (",C,N,".indexOf("," + getNgtFieldTypeFromDDL(rslt.getString(2)) + ",") != -1) {
						fieldsiz = (getNgtFieldTypeFromDDL(rslt.getString(2)).equals("N")) ? rslt.getInt(4) : fieldsiz;
						if (fielddec != 0) {
							fldchg = (!(fieldsiz + "," + fielddec).equals(node.getString("FIELDSIZE"))) ? true : fldchg;
						} else {
							if (!((fieldsiz == 0) && ((node.getString("FIELDSIZE") == null)
									|| (node.getString("FIELDSIZE").length() == 0)))) {
								fldchg = (!("" + fieldsiz).equals(node.getString("FIELDSIZE"))) ? true : fldchg;
							}
						}
					}
					fldexi = true;
				...
				if ((dml != null) && (dml.length() > 0) && !modo.equals("3")) {
					...
					if ((mfield != null) && !(!mfield.equals("TEXTOLIVRE") && !mfield.equals("NUMEROLIVRE")
							&& !mfield.equals("TEXT") && !mfield.equals("BLOB") && !mfield.equals("MDATA"))) {
						String ngtft = "";
						if (mfield.equals("TEXTOLIVRE")) {
							...
							ngtft = (mfield.equals("BLOB")) ? "BL" : ngtft;
						dml += getDDLFieldFromNGT(ngtft, node.getString("FIELDSIZE"));
					} else
						dml = ((mfield != null) && (mfield.length() > 0))
								? getMacrofieldDef(cndef, node.getString("MACROFIELD"))
								: getDDLFieldFromNGT(node.getString("FIELDTYPE"), node.getString("FIELDSIZE"));
				}
			...
			if (objecttype.startsWith("PK") || objecttype.startsWith("UN")) {
				...
				if (!modo.equals("3") && (!exists || diff)) {
					dml = (isunique)
							? "alter table " + node.getString("TABLENAME") + " add constraint "
									+ node.getString("OBJECTNAME") + " unique (" + node.getString("EXPRESSION") + ")"
							: "alter table " + node.getString("TABLENAME") + " add primary key ("
			...
			if (objecttype.startsWith("FK")) {
				...
				if (rslt.next()) {
					exists = true;
					cExpress = (cExpress.length() > 0) ? "," : cExpress;
			...
			if (objecttype.startsWith("IDX")) {
				...
				if (rslt.next()) {
					exists = true;
					drop = ((unflag && !(dbunflag = rslt.getString(1).equals("UNIQUE")))) ? true : drop;
					rslt.close();
					pstm.close();
				...
				String[] nexo;
				nexo = (aux != null) ? node.getString("EXPRESSION").split(",") : new String[0];
				for (byte i = 0; i < nexo.length; i++) {
					newexpression += (((newexpression.length() > 0) ? "," : "") + ((nexo[i]).toUpperCase().trim()));
				}
				drop = (!drop) ? (!newexpression.equals(oldexpression)) && !oldexpression.equals("") : drop;
				if (exists && (drop || modo.equals("3"))) {
					dml = (!dbunflag) ? "DROP INDEX " + node.getString("OBJECTNAME")
							: "ALTER TABLE " + node.getString("TABLENAME") + " DROP CONSTRAINT "
									+ node.getString("OBJECTNAME");
					executeDDL(dml, node.getString("SCHEMA"));
					exists = false;
				}
				if (!exists && !modo.equals("3")) {
					if (!node.getString("OBJECTNAME").equals("") && !newexpression.equals("")) {
						dml = (!unflag)
								? "CREATE INDEX " + node.getString("OBJECTNAME") + " ON " + node.getString("TABLENAME")
										+ "(" + newexpression + ")"
								: "ALTER TABLE " + node.getString("TABLENAME") + " ADD CONSTRAINT "
