class n20856391 {
	public void launchJob(final String workingDir, final AppConfigType appConfig) throws FaultType {
		...
		serviceName = (serviceName == null) ? "Unknown service" : serviceName;
		...
		if (args == null) {
			...
			args = (userArgs != null) ? " " + userArgs : args;
		}
		...
		if (AppServiceImpl.drmaaInUse) {
			...
			if (appConfig.isParallel()) {
				...
				args = (args != null) ? newArgs + " " + args : newArgs;
			...
			String rsl = null;
			rsl = (appConfig.isParallel())
					? "&(directory=" + workingDir + ")" + "(executable=" + appConfig.getBinaryLocation() + ")"
			...
			String cmd = null;
			cmd = (appConfig.isParallel())
					? new String(AppServiceImpl.mpiRun + " " + "-np " + jobIn.getNumProcs() + " "
							+ appConfig.getBinaryLocation())
					: new String(appConfig.getBinaryLocation());
			cmd = (args != null) ? " " + args : cmd;
				...
				if (!AppServiceImpl.dbInUse) {
					...
					for (int i = 0; i < outputFile.length; i++) {
						sqlStmt = "insert into output_file(job_id, name, url) " + "values ('" + jobID + "', " + "'"
								+ outputFile[i].getName() + "', " + "'" + outputFile[i].getUrl().toString() + "');";
						try {
							stmt = conn.createStatement();
							stmt.executeUpdate(sqlStmt);
						} catch (SQLException e) {
