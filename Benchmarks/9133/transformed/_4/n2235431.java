class n2235431 {
	private LinkedList<Datum> processDatum(Datum dataset) {
		...
		theAdapter = (dataset.getDataType().equals(MarkerPhenotype.class))
				? new MarkerPhenotypeAdapter((MarkerPhenotype) dataset.getData())
				: new MarkerPhenotypeAdapter((Phenotype) dataset.getData());
		...
		for (int ph = 0; ph < numberOfPhenotypes; ph++) {
			...
			permutedData = (permute) ? permuteData(phenotypeData, missing, factorList, covariateList, theAdapter)
					: permutedData;
			for (int m = 0; m < numberOfMarkers; m++) {
				...
				if (!filterOutput || p < maxp) {
					...
					traitname = (traitname == null) ? blank : traitname;
					String marker = theAdapter.getMarkerName(m);
					marker = (marker == null) ? blank : marker;
		...
		if (hasAlleleNames) {
			if (hasMarkerNames && hasMap) {
				...
				outputIndex = (hasMap) ? new int[] { 0, 2, 3, 4, 5, 6, 7, 8 } : new int[] { 0, 2, 3, 4, 7, 8 };
		} else {
			if (hasMarkerNames && hasMap) {
				...
				outputIndex = (hasMap) ? new int[] { 0, 2, 3, 4, 5, 6, 8 } : new int[] { 0, 2, 3, 4, 8 };
		}
