class n6403005 {
	...
	protected void processTransformAction(HttpServletRequest request, HttpServletResponse response, String action)
			throws Exception {
		...
		if (StringUtils.isNotBlank(tr)) {
			...
			if (!transformationFile.isFile()) {
				response.sendError(HttpServletResponse.SC_BAD_REQUEST,
						"Parameter \"" + Definitions.REQUEST_PARAMNAME_XSLT + "\" " + "with value \"" + tr
		...
		if (isDevelopmentMode) {
			...
			result = (StringUtils.equals(action, "get"))
					? new StreamResult(new Base64.OutputStream(baos, Base64.DECODE))
					: new StreamResult(baos);
		} else {
			result = (StringUtils.equals(action, "get"))
					? new StreamResult(new Base64.OutputStream(response.getOutputStream(), Base64.DECODE))
		...
		if (method.endsWith("html")) {
			...
			contentType = (method.equals("xml")) ? Definitions.MIMETYPE_XML : Definitions.MIMETYPE_TEXTPLAIN;
