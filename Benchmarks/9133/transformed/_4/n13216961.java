class n13216961 {
	public void actionPerformed(ActionEvent ev) {
		...
		if (fileChooser.showOpenDialog(SwingSelectDevicePanel.this) == JFileChooser.APPROVE_OPTION) {
			...
			manifestDeviceName = (descriptorEntries.size() > 1) ? null : manifestDeviceName;
			...
			try {
				...
				deviceFile = (deviceFile.exists()) ? File.createTempFile("device", ".jar", Config.getConfigPath())
						: deviceFile;
				...
				for (Iterator it = devices.keySet().iterator(); it.hasNext();) {
					...
					entry = (manifestDeviceName != null)
							? new DeviceEntry(manifestDeviceName, deviceFile.getName(), descriptorLocation, false)
							: new DeviceEntry(device.getName(), deviceFile.getName(), descriptorLocation, false);
