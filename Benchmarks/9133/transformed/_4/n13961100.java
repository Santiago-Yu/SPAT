		...
		try {
			...
			if (getTemplateURIPath() != null) {
				...
				if (templateURI.isPlatformResource()) {
					...
					theClassLoader = (bundle != null)
							? AccessController.doPrivileged(new GetURLClassLoaderBundleAction(bundle))
							: theClassLoader;
				}
				if (theClassLoader != null) {
					String className = (packageName.length() == 0 ? "" : packageName + ".")
							+ jetCompiler.getSkeleton().getClassName();
					className = (className.endsWith("_")) ? className.substring(0, className.length() - 1) : className;
					try {
