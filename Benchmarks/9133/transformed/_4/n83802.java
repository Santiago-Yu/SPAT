			...
			if (blGo) {
				...
				try {
					...
					for (int i = 0; i < subFiles.length; i++) {
						cp = (subFiles[i].isFile())
								? File.pathSeparatorChar + "lib" + File.separatorChar + subFiles[i].getName() + ""
								: cp;
					}
				} catch (Exception e) {
				...
				try {
					...
					settings = (settings == null) ? new File("etc" + File.separatorChar + "settings.dbp") : settings;
					if (settings.exists() && settings.canRead()) {
						DBSettingsWriter writ = new DBSettingsWriter();
						info = (DBSettings) writ.loadSettingsDB(settings);
						blExecutePOS = (info != null) ? true : blExecutePOS;
					}
					if (blExecutePOS) {
						log("[OK]");
						String strSSL = "";
						...
						if (info != null) {
							...
							if (info.get(DBSettings.POS_SSLENABLED).compareTo("1") == 0) {
								strSSL = "-Djavax.net.ssl.keyStore=" + info.get(DBSettings.POS_SSLKEYSTORE)
										+ " -Djavax.net.ssl.keyStorePassword=pscafe -Djavax.net.ssl.trustStore="
										+ info.get(DBSettings.POS_SSLTRUSTSTORE)
						...
						String strPOSRun = "java  -cp " + cp + " " + strSSL + " " + strSSLDebug + " POSDriver "
								+ settings.getPath();
						debug(strPOSRun);
						System.out.print("\n  + Running PSCafePOS...                 ");
						...
						while ((line = br.readLine()) != null || (lineError = bre.readLine()) != null) {
							if (line != null)
								System.out.println(" [PSCafePOS]" + line);
							if (lineError != null)
								System.out.println(" [ERR]" + lineError);
						}
						pr.waitFor();
						log(" =================================================");
