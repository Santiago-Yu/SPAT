		...
		try {
			...
			while ((entry = in.getNextJarEntry()) != null) {
				...
				if (entryName.equals("classes.pack")) {
					...
					File dest;
					dest = (DownloadManager.isWindowsVista())
							? new File(lowJavaPath, entryName.replace('/', File.separatorChar))
							: new File(DownloadManager.JAVA_HOME, entryName.replace('/', File.separatorChar));
					dest = (entryName.equals(BUNDLE_JAR_ENTRY_NAME)) ? useJarPath : dest;
