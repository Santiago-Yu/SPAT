class n3287281 {
	private void googleImageSearch() {
		...
		custom = "";
		int r = JOptionPane.showConfirmDialog(this, "Customize google search?", "Google Search",
				JOptionPane.YES_NO_OPTION);
		custom = (r == JOptionPane.YES_OPTION) ? JOptionPane.showInputDialog("Custom Search", "") : artist;
		try {
			...
			while ((lin = readIn.readLine()) != null) {
				while (lin.contains("href=\"/imgres?imgurl=")) {
					...
					String s = lin.substring(lin.indexOf("href=\"/imgres?imgurl="),
							lin.indexOf(">", lin.indexOf("href=\"/imgres?imgurl=")));
					lin = lin.substring(lin.indexOf(">", lin.indexOf("href=\"/imgres?imgurl=")));
					s = (s.contains("&amp;") && s.indexOf("http://") < s.indexOf("&amp;"))
							? s.substring(s.indexOf("http://"), s.indexOf("&amp;"))
							: s.substring(s.indexOf("http://"), s.length());
					googleImages.add(s);
				}
			}
			readIn.close();
