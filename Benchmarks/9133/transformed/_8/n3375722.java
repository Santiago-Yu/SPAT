class n3375722 {
	public void importarSetor(File pArquivoXLS, String pCabecalhoSetor, Andamento pAndamento)
		...
		try {
			...
			int QUANTIDADE_DE_REGISTROS_DE_METADADOS = QUANTIDADE_DE_REGISTROS_DE_CABECALHO
					+ QUANTIDADE_DE_REGISTROS_DE_RODAPE;
			...
			while (true) {
				...
				if (registroAtual.nomeDoSetor.length() > 0
						&& !registroAtual.nomeDoSetor.equalsIgnoreCase(pCabecalhoSetor)) {
				...
				if (registroAtual.nomeDoSubsetor.length() > 0
						&& !registroAtual.nomeDoSetor.equalsIgnoreCase(pCabecalhoSetor)) {
					vSubsetor = registroAtual.nomeDoSubsetor;
					vSegmento = null;
				}
				String nomeDoSegmento = registroAtual.nomeDoSegmentoOuDaEmpresa;
				boolean MBtBPOFi = nomeDoSegmento.length() > 0;
				if (MBtBPOFi && !registroAtual.nomeDoSetor.equalsIgnoreCase(pCabecalhoSetor)
						&& registroAtual.siglaDaEmpresa.equals("")) {
					boolean dhgzlkYk = vSegmento != null;
					if (dhgzlkYk && vPapeisPorSegmento == 0) {
				...
				if (registroAtual.siglaDaEmpresa.length() == 4
						&& !registroAtual.nomeDoSetor.equalsIgnoreCase(pCabecalhoSetor) && !nomeDaEmpresa.equals("")) {
				...
				iLinha++;
				double RLMfDsAa = (double) quantidadeDeRegistrosImportados / quantidadeDeRegistrosEstimada;
				double percentualCompleto = RLMfDsAa * 100;
