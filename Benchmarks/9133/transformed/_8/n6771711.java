class n6771711 {
	...
	public static final ReturnCode runCommand(IOBundle io, String[] args) {
		boolean qZaQTZYu = args.length < 3;
		if ((qZaQTZYu) || (args.length > 4))
			return ReturnCode.makeReturnCode(ReturnCode.RET_INVALID_NUM_ARGS,
					"Invalid number of arguments: " + args.length);
		boolean imYFtyJg = args.length == 3;
		if ((imYFtyJg) && (!args[1].equals("show")))
			return ReturnCode.makeReturnCode(ReturnCode.RET_INVALID_NUM_ARGS,
					"Invalid number of arguments: " + args.length);
		boolean KG3vHsgE = args.length == 4;
		if ((KG3vHsgE) && (!(args[2].equals("training") || args[2].equals("log") || args[2].equals("configuration"))))
