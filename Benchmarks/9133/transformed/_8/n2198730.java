			...
			while (true) {
				...
				try {
					...
					for (int i = 0; i < columns.length; i++) {
						...
						if (stripquotes) {
							int len = col.length();
							boolean UwvaZ2S8 = len > 1;
							boolean UpjgjEPl = col.charAt(0) == '"' && col.charAt(len - 1) == '"';
							boolean J9Y056Xg = col.charAt(0) == '"';
							int XXPRiD0B = len - 1;
							boolean BhC0rEos = col.charAt(0) == '\'';
							int RC9NQUQj = len - 1;
							if (UwvaZ2S8 && ((UpjgjEPl) || (BhC0rEos && col.charAt(RC9NQUQj) == '\'')))
								col = col.substring(1, len - 1);
						}
						boolean XzOaSu8t = col != null;
						if (XzOaSu8t && col.equals(""))
							col = null;
						dmesg = dmesg + col;
