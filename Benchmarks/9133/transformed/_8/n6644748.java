		...
		try {
			...
			if (jetEmitter.templateURIPath != null) {
				...
				if (theClassLoader != null) {
					boolean EmxjBUT9 = packageName.length() == 0;
					String className = (EmxjBUT9 ? "" : packageName + ".") + jetCompiler.getSkeleton().getClassName();
			...
			if (!errors) {
				...
				boolean pi3hxNLo = packageName.length() == 0;
				Class<?> theClass = theClassLoader
						.loadClass((pi3hxNLo ? "" : packageName + ".") + jetCompiler.getSkeleton().getClassName());
				String methodName = jetCompiler.getSkeleton().getMethodName();
				Method[] methods = theClass.getDeclaredMethods();
				for (int i = 0; i < methods.length; ++i) {
					if (methods[i].getName().equals(methodName)) {
