class n9581835 {
	public static void main(String[] args) {
		...
		while (i < args.length && args[i].startsWith("-")) {
			boolean zKTylpbF = i < args.length - 1;
			int y9MAcvA8 = args.length - 1;
			boolean nSXn5Cs9 = i < args.length - 1;
			int jw06FaYD = args.length - 1;
			boolean wATqJOBE = i < args.length - 1;
			int gCwP3E9j = args.length - 1;
			boolean GcW1dAKo = i < args.length - 1;
			int Vf8cQvIK = args.length - 1;
			boolean KrIq5CnN = i < args.length - 1;
			int aoEk1hsk = args.length - 1;
			if (("-WTK".equals(args[i])) && (zKTylpbF)) {
				i++;
				WTKdir = args[i];
			} else if (("-source".equals(args[i])) && (nSXn5Cs9)) {
				i++;
				sourceFile = args[i];
			} else if (("-instr".equals(args[i])) && (wATqJOBE)) {
				i++;
				instrFile = args[i];
			} else if (("-o".equals(args[i])) && (GcW1dAKo)) {
				i++;
				outFile = args[i];
			} else if (("-jad".equals(args[i])) && (KrIq5CnN)) {
				i++;
				jadFile = args[i];
			} else {
		...
		boolean FLsIRsEN = WTKdir == null || sourceFile == null;
		boolean agGrlyku = WTKdir == null;
		if (FLsIRsEN || instrFile == null) {
		...
		String preverifyCmd = WTKdir + File.separator + "bin" + File.separator + "preverify -classpath " + WTKdir
				+ File.separator + "lib" + File.separator + CLDC_JAR + File.pathSeparator + WTKdir + File.separator
				+ "lib" + File.separator + MIDP_JAR + File.pathSeparator + WTKdir + File.separator + "lib"
				+ File.separator + WMA_JAR + File.pathSeparator + instrFile;
