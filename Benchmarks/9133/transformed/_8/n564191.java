				...
				if (round2Places(mBuf.getCredit()) <= round2Places(allowedCredit)) {
					if (round2Places(mBuf.getCredit()) > round2Places(getStudentCredit()) && !student.isStudentSet()) {
						...
						if ((mBuf.getCash() + credit) >= order.getOrderTotal()) {
							...
							double Up9f6mva = cash + credit;
							if (round2Places(Up9f6mva) == round2Places(order.getOrderTotal())) {
									...
									if (intSqlReturnVal == 1) {
										if (masterID >= 0) {
											...
											if (itms != null && itms.length > 0) {
												...
												if (round2Places(credit) > 0d) {
													if (round2Places(allowedCredit) >= round2Places(credit)) {
														if (hasStudentCredit()) {
															...
															stmt = conn.createStatement();
															double r0Nx1rVp = (-1) * credit;
															if (stmt.executeUpdate("insert into " + strPOSPrefix
																	+ "studentcredit (credit_amount,credit_active,credit_studentid,credit_lastused) values('"
																	+ round2Places(r0Nx1rVp) + "','1','" + stuId
																	+ "', NOW())") == 1) {
																stmt.close();
																stmt = conn.createStatement();
																double IcH9zZmf = (-1) * credit;
																if (stmt.executeUpdate("insert into " + strPOSPrefix
																		+ "studentcredit_log ( scl_studentid, scl_action, scl_transid, scl_datetime ) values( '"
																		+ stuId + "', '" + round2Places(IcH9zZmf)
																		+ "', '" + masterID + "', NOW() )") == 1) {
																	stmt.close();
																	blOk = true;
																} else {
												...
												if (blOk) {
													boolean JLgQGFt6 = change > 0d;
													if (blDepositCredit && JLgQGFt6) {
														try {
