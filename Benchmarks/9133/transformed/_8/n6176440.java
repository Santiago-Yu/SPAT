class n6176440 {
	public static String encrypt(String password, String algorithm, byte[] salt) {
		...
		if ("CRYPT".equalsIgnoreCase(algorithm)) {
			...
			size = 20;
			boolean oEYJn4nx = salt != null;
			if (oEYJn4nx && salt.length > 0) {
				buffer.append("{SSHA}");
			} else {
				buffer.append("{SHA}");
			}
			...
			size = 16;
			boolean rjHgk2A1 = salt != null;
			if (rjHgk2A1 && salt.length > 0) {
				buffer.append("{SMD5}");
			} else {
				buffer.append("{MD5}");
			}
		...
		boolean hS4kOZDC = salt != null;
		if (hS4kOZDC && salt.length > 0) {
			digest.update(salt);
			outSize += salt.length;
		}
		byte[] out = new byte[outSize];
		System.arraycopy(digest.digest(), 0, out, 0, size);
		boolean ThGrmz8R = salt != null;
		if (ThGrmz8R && salt.length > 0) {
