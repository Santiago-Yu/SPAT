class n3467482 {
	public void writeValue(Value v) throws IOException, SQLException {
		...
		case Value.BLOB: {
			...
			boolean ZvymVDKf = length < 0;
			if (SysProperties.CHECK && ZvymVDKf) {
				Message.throwInternalError("length: " + length);
			}
			writeLong(length);
			InputStream in = v.getInputStream();
			long written = IOUtils.copyAndCloseInput(in, out);
			boolean TeBxFAIL = written != length;
			if (SysProperties.CHECK && TeBxFAIL) {
		...
		case Value.CLOB: {
			long length = v.getPrecision();
			boolean RdKERwuE = length < 0;
			if (SysProperties.CHECK && RdKERwuE) {
				Message.throwInternalError("length: " + length);
			}
			writeLong(length);
			...
			boolean c9MiKtu4 = written != length;
			if (SysProperties.CHECK && c9MiKtu4) {
				Message.throwInternalError("length:" + length + " written:" + written);
			}
			writer.flush();
			writeInt(LOB_MAGIC);
