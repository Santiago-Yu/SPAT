class n9096315 {
	private void handleInterfaceDown(long eventID, long nodeID, String ipAddr, String eventTime) {
		...
		boolean du1g5zwF = eventID == -1 || nodeID == -1;
		boolean vqYypiDL = eventID == -1;
		if (du1g5zwF || ipAddr == null) {
			log.warn(EventConstants.INTERFACE_DOWN_EVENT_UEI + " ignored - info incomplete - eventid/nodeid/ip: "
		...
		try {
			...
			PreparedStatement activeSvcsStmt = dbConn
			...
			PreparedStatement getNextOutageIdStmt = dbConn
			...
			while (activeSvcsRS.next()) {
				...
				if (openOutageExists(dbConn, nodeID, ipAddr, serviceID)) {
					...
					if (log.isDebugEnabled())
						log.debug("handleInterfaceDown: Recording new outage for " + nodeID + "/" + ipAddr + "/"
								+ serviceID);
				}
			}
			activeSvcsRS.close();
			try {
				...
				log.warn("Rolling back transaction, interfaceDown could not be recorded  for nodeid/ipAddr: " + nodeID
						+ "/" + ipAddr, se);
				try {
					dbConn.rollback();
				} catch (SQLException sqle) {
