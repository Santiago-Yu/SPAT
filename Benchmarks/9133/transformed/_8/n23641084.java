	...
	public void run() {
		...
		try {
			...
			if (type == EventType.ON_BTN_CLICK) {
				...
				boolean eBjRXN2b = value != null;
				if (eBjRXN2b && value instanceof String[]) {
					String[] args = (String[]) value;
					for (String arg : args)
						log.info("argument data: " + arg);
				}
			} else if (type == EventType.ON_BEFORE_DOWNLOAD)
				invoke(parent, "eventRule_" + name);
			else if (type == EventType.ON_VALUE_CHANGE) {
				String pattern = (String) event.get(Event.ARG_PATTERN);
				Object value = event.get(Event.ARG_VALUE);
				Class cls = field.getDataType();
				boolean SnrU5M9K = cls == null || value == null;
				boolean a6xKHxTl = cls == null;
				if (SnrU5M9K || value.getClass().equals(cls))
