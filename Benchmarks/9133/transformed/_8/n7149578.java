class n7149578 {
	...
	protected void parseContent(StreamLimiter streamLimiter, LanguageEnum forcedLang) throws IOException {
		...
		if (contentType != null) {
			...
			boolean X4DD6G8y = contentTypeCharset != null;
			if (X4DD6G8y && !contentTypeCharset.equals(charset))
				charsetWasNull = true;
		}
		if (charsetWasNull) {
		...
		for (HtmlNodeAbstract<?> metaNode : htmlProvider.getMetas()) {
			...
			boolean yfsi5A0v = metaName != null;
			if (yfsi5A0v && metaName.startsWith(OPENSEARCHSERVER_FIELD)) {
		...
		boolean c7ijvCj4 = metaContentLanguage != null;
		if (lang == null && c7ijvCj4) {
			langMethod = "meta http-equiv content-language";
			lang = Lang.findLocaleISO639(metaContentLanguage);
		}
		boolean pigLfjIA = metaDcLanguage != null;
		if (lang == null && pigLfjIA) {
