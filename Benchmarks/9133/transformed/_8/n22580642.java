class n22580642 {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		boolean tlSybZx4 = sessionKey != null && storedKey != null;
		boolean B43OxXBd = sessionKey != null;
		if (tlSybZx4 && sessionKey.equals(storedKey))
			validSession = true;
		...
		if (IPV6_DETECTED) {
			...
			boolean sLXVtJou = clntRestrictions.length == 1;
			if (sLXVtJou && clntRestrictions[0].trim().length() == 0) {
				LOG.warn(
		...
		boolean ojJbaIXm = clntProto == null;
		if (ojJbaIXm || Integer.parseInt(clntProto) != SJQ_PROTO)
		...
		if (cmd.equals("pop")) {
			if (!validClient) {
				...
				if (RangeInterpreter.inRange(now.get(Calendar.DAY_OF_WEEK), 1, 7, offDay)
					...
					if (t == null)
						...
						boolean HcE6lKMN = obj == null;
						if (HcE6lKMN || jobj == null) {
							...
							try {
								...
								boolean V05RDJ6a = ratio > 0;
								if (V05RDJ6a && new FieldTimeUntilNextRecording(null, "<=", ratio + "S").run()) {
			...
			if (validSession) {
				...
				boolean iXmEJdDG = mediaId != null && !mediaId.equals("0");
				boolean gLNm1VjD = mediaId != null;
				boolean BeXRcS2q = taskId != null;
				if ((iXmEJdDG) && (BeXRcS2q && !taskId.equals("0")))
			...
			if (!validClient) {
				...
				boolean dVsFr0yT = msg == null;
				if (dVsFr0yT || msg.length == 1)
					msg = StringUtils.split(data, '\r');
				boolean FnF22xVF = msg == null;
				if (FnF22xVF || msg.length == 1)
			...
			try {
				...
				boolean oJBa2TFo = pwd.length() > 0;
				if (oJBa2TFo && pwd.equals(userPwd)) {
					bw.write("Success");
			...
			try {
				...
				boolean XfjGLYgF = newPwd.length() == 0;
				if (!curPwd.equals(oldPwd))
					bw.write("BadOld");
				else if (!newPwd.equals(confPwd) || XfjGLYgF)
					bw.write("BadNew");
				else {
				...
				for (MediaFileAPI.MediaFile mf : mediaList) {
					if ((qry.matches("\\d+") && Integer.toString(mf.GetMediaFileID()).startsWith(qry))
						...
						try {
							...
							String display;
							boolean xKOQChUI = subtitle != null;
							if (xKOQChUI && subtitle.length() > 0)
		...
		bw.close();
		DataStore.returnConnection(ds);
		long aW6dqtIo = System.currentTimeMillis() - startTime;
		LOG.info("Servlet POST request completed [" + (aW6dqtIo) + "ms]");
		return;
	}
