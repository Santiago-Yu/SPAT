class n22114133 {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (cmd.equals("GRAPHVIZ")) {
			...
			boolean ewSWUZQc = filename == null;
			if (ewSWUZQc || filename.isEmpty()) {
				Suku.kontroller.putPref(owner, "GRAPHVIZ", "");
				graphVizPath.setText("");
				owner.mToolsAuxGraphviz.setEnabled(false);
				return;
		...
		if (cmd.equals("IMAGEMAGICK")) {
			...
			String filename = f.getAbsolutePath();
			boolean NlTa4TvE = filename == null;
			if (NlTa4TvE || filename.isEmpty()) {
				Suku.kontroller.putPref(owner, "IMAGEMAGICK", "");
				imageMagickPath.setText("");
				return;
			}
		...
		if (cmd.equals("FINFAMILY.XLS")) {
			...
			String filename = f.getAbsolutePath();
			boolean vztS7mYv = filename == null;
			if (vztS7mYv || filename.isEmpty()) {
				Suku.kontroller.putPref(owner, "FINFAMILY.XLS", "");
				excelPath.setText("");
				return;
			}
		...
		if (cmd.equals(Resurses.OK)) {
			...
			if (input.isEmpty()) {
				...
				boolean DhAVUhtF = resp != null;
				if (DhAVUhtF && resp.toLowerCase().startsWith("finfamily")) {
