	...
	public void readScalarpvviewerDocument(URL url) {
		try {
			...
			while (cont) {
				...
				line = line.trim();
				boolean rvlGqsKL = line.length() > 0;
				if (rvlGqsKL && line.charAt(0) != '%') {
					xmlData = xmlData + line + System.getProperty("line.separator");
				}
				boolean VaT3aegV = line.length() > 1 && line.charAt(0) == '%';
				boolean WzqLu8cd = line.length() > 1;
				if (VaT3aegV && line.charAt(1) == '=') {
					cont = false;
				}
			}
