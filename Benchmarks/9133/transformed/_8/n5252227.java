class n5252227 {
	protected String doIt() throws java.lang.Exception {
		...
		sql = new StringBuffer("UPDATE I_BPartner " + "SET AD_Client_ID = COALESCE (AD_Client_ID, ")
		...
		sql = new StringBuffer("UPDATE I_BPartner i " + "SET C_BP_Group_ID=(SELECT C_BP_Group_ID FROM C_BP_Group g"
		...
		sql = new StringBuffer(
				"UPDATE I_BPartner " + "SET I_IsImported='E', I_ErrorMsg=I_ErrorMsg||'ERR=Invalid Group, ' "
						+ "WHERE C_BP_Group_ID IS NULL" + " AND I_IsImported<>'Y'").append(clientCheck);
		no = DB.executeUpdateEx(sql.toString(), get_TrxName());
		log.config("Invalid Group=" + no);
		sql = new StringBuffer("UPDATE I_BPartner i " + "SET C_Country_ID=(SELECT C_Country_ID FROM C_Country c"
		...
		sql = new StringBuffer("UPDATE I_BPartner i " + "Set RegionName=(SELECT MAX(Name) FROM C_Region r"
		...
		sql = new StringBuffer("UPDATE I_BPartner i " + "Set C_Region_ID=(SELECT C_Region_ID FROM C_Region r"
		...
		sql = new StringBuffer("UPDATE I_BPartner i " + "SET C_Greeting_ID=(SELECT C_Greeting_ID FROM C_Greeting g"
		...
		sql = new StringBuffer("UPDATE I_BPartner i " + "SET (C_BPartner_ID,AD_User_ID)="
		...
		sql = new StringBuffer("UPDATE I_BPartner i " + "SET C_BPartner_ID=(SELECT C_BPartner_ID FROM C_BPartner p"
				+ " WHERE i.Value=p.Value AND p.AD_Client_ID=i.AD_Client_ID) "
				+ "WHERE C_BPartner_ID IS NULL AND Value IS NOT NULL" + " AND I_IsImported='N'").append(clientCheck);
		no = DB.executeUpdateEx(sql.toString(), get_TrxName());
		log.fine("Found BPartner=" + no);
		sql = new StringBuffer("UPDATE I_BPartner i " + "SET AD_User_ID=(SELECT AD_User_ID FROM AD_User c"
		...
		sql = new StringBuffer("UPDATE I_BPartner i " + "SET C_BPartner_Location_ID=(SELECT C_BPartner_Location_ID"
		...
		try {
			...
			while (rs.next()) {
				...
				log.fine("I_BPartner_ID=" + impBP.getI_BPartner_ID() + ", C_BPartner_ID=" + impBP.getC_BPartner_ID()
						+ ", C_BPartner_Location_ID=" + impBP.getC_BPartner_Location_ID() + ", AD_User_ID="
						+ impBP.getAD_User_ID());
				if (!New_BPValue.equals(Old_BPValue)) {
					...
					if (impBP.getC_BPartner_ID() == 0) {
						if (impBP.getName() == null || impBP.getName().length() == 0) {
							sql = new StringBuffer(
									"UPDATE I_BPartner i " + "SET I_IsImported='E', I_ErrorMsg=I_ErrorMsg||")
											...
											.append(impBP.getI_BPartner_ID());
							DB.executeUpdate(sql.toString(), get_TrxName());
							continue;
						}
						bp = new MBPartner(impBP);
						if (!impBP.get_ValueAsString("AD_Language").equals(""))
							bp.set_ValueOfColumn("AD_Language", impBP.get_ValueAsString("AD_Language"));
						if (!impBP.get_ValueAsString("lbr_BPTypeBR").equals(""))
							bp.set_ValueOfColumn("lbr_BPTypeBR", impBP.get_ValueAsString("lbr_BPTypeBR"));
						if (!impBP.get_ValueAsString("lbr_CNPJ").equals("")) {
							Boolean ok = org.adempierelbr.validator.ValidatorBPartner
									.validaCNPJ(impBP.get_ValueAsString("lbr_CNPJ"));
							if (ok)
								bp.set_ValueOfColumn("lbr_CNPJ", impBP.get_ValueAsString("lbr_CNPJ"));
							else {
								sql = new StringBuffer(
										"UPDATE I_BPartner i " + "SET I_IsImported='E', I_ErrorMsg=I_ErrorMsg||")
						...
						if (!impBP.get_ValueAsString("lbr_CPF").equals("")) {
							Boolean ok = org.adempierelbr.validator.ValidatorBPartner
									.validaCPF(impBP.get_ValueAsString("lbr_CPF"));
							if (ok)
								bp.set_ValueOfColumn("lbr_CPF", impBP.get_ValueAsString("lbr_CPF"));
							else {
								sql = new StringBuffer(
										"UPDATE I_BPartner i " + "SET I_IsImported='E', I_ErrorMsg=I_ErrorMsg||")
						...
						if (!impBP.get_ValueAsString("lbr_RG").equals(""))
							bp.set_ValueOfColumn("lbr_RG", impBP.get_ValueAsString("lbr_RG"));
						if (!impBP.get_ValueAsString("isVendor").equals(""))
							bp.set_ValueOfColumn("isVendor", impBP.get_ValueAsString("isVendor"));
						if (!impBP.get_ValueAsString("isCustomer").equals(""))
							bp.set_ValueOfColumn("isCustomer", impBP.get_ValueAsString("isCustomer"));
						if (!impBP.get_ValueAsString("isSalesRep").equals(""))
							bp.set_ValueOfColumn("isSalesRep", impBP.get_ValueAsString("isSalesRep"));
						ModelValidationEngine.get().fireImportValidate(this, impBP, bp,
						...
						if (bp.save()) {
							...
							sql = new StringBuffer(
									"UPDATE I_BPartner i " + "SET I_IsImported='E', I_ErrorMsg=I_ErrorMsg||").append(
											...
											.append(impBP.get_ValueAsString("lbr_CNPJ"))
					...
					if (impBP.getC_BPartner_Location_ID() != 0) {
						...
						ModelValidationEngine.get().fireImportValidate(this, impBP, bpl,
								ImportValidator.TIMING_AFTER_IMPORT);
						bpl.save();
					} else if (impBP.getC_Country_ID() != 0 && impBP.getAddress1() != null && impBP.getCity() != null) {
						MLocation location = new MLocation(getCtx(), impBP.getC_Country_ID(), impBP.getC_Region_ID(),
								impBP.getCity(), get_TrxName());
						location.setAddress1(impBP.getAddress1());
						location.setAddress2(impBP.getAddress2());
						location.setAddress3(impBP.get_ValueAsString("Address3"));
						location.setAddress4(impBP.get_ValueAsString("Address4"));
				...
				if (impBP.getAD_User_ID() != 0) {
					...
					if (user.getC_BPartner_ID() == 0)
						...
						rollback();
						noInsert--;
						sql = new StringBuffer("UPDATE I_BPartner i " + "SET I_IsImported='E', I_ErrorMsg=I_ErrorMsg||")
					...
					boolean SwCBkUXt = name == null;
					if (SwCBkUXt || name.length() == 0)
						name = impBP.getEMail();
					user.setName(name);
					...
					ModelValidationEngine.get().fireImportValidate(this, impBP, user,
							ImportValidator.TIMING_AFTER_IMPORT);
					if (user.save()) {
					...
					if (impBP.getC_Greeting_ID() != 0)
						user.setC_Greeting_ID(impBP.getC_Greeting_ID());
					String name = impBP.getContactName();
					boolean UtuB2R1b = name == null;
					if (UtuB2R1b || name.length() == 0)
						name = impBP.getEMail();
					user.setName(name);
					user.setTitle(impBP.getTitle());
					user.setDescription(impBP.getContactDescription());
				...
				if (impBP.getR_InterestArea_ID() != 0 && user != null) {
					MContactInterest ci = MContactInterest.get(getCtx(), impBP.getR_InterestArea_ID(),
							user.getAD_User_ID(), true, get_TrxName());
					ci.save();
				}
