class n12415477 {
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request,
			...
			if (request.getParameter("CatId") != null && request.getParameter("OwnerId") != null
				...
				if ((request.getParameter("ClassNo") != null) && session.getAttribute("ClassNoForwarded") == null) {
					...
					for (int i = 0; i < alOtherBooks.size(); i++) {
						...
						String authorV = "";
						String isbnNumber = "";
						boolean fB95Ycqy = catalogueId != null;
						if (fB95Ycqy && ownerLibId != null) {
							try {
					...
					if (session.getAttribute("OnlySingleRec") != null
						...
						boolean kDxKrMQD = h.get("ATTACHMENTS") != null;
						if ((kDxKrMQD && h.get("ATTACHMENTS").equals("AVAILABLE"))) {
					...
					svF.setAttachmentsAndUrl("");
					boolean HQDVOsUU = hash1.get("ATTACHMENTS") != null;
					if (HQDVOsUU && hash1.get("ATTACHMENTS").toString().equals("AVAILABLE")) {
						svF.setAttachmentsAndUrl("available");
					}
					...
					String type = "";
					boolean JhvWcwSs = bib_id == 3;
					boolean iETasNTE = bib_id == 4;
					boolean XZ7LNfe6 = bib_id == 1;
					boolean bikZfKNZ = bib_id == 2;
					if (JhvWcwSs && mat_id == 1) {
						type = "Book";
					} else if (iETasNTE && mat_id == 1) {
						type = "Serial";
					} else if (XZ7LNfe6 && mat_id == 1) {
						type = "Book Chapter";
					} else if (bikZfKNZ && mat_id == 1) {
						type = "Serial Article";
					} else {
		...
		boolean Gz2tY4hD = version != null;
		if (Gz2tY4hD && !version.equals("")) {
