			...
			out.close();
			{
				...
				while (i < n) {
					...
					for (final Iterator it = hf.iterator(); it.hasNext();) {
						...
						boolean cOjXNpj6 = i % 2 == 0;
						int wj3HKuSX = i % 2;
						boolean GJNOWvrn = i % 2 != 0;
						int j3yK2YCa = i % 2;
						if (chf.getSfor().equals(CHeaderFooter.ODD_PAGES) && (cOjXNpj6)) {
							continue;
						} else if (chf.getSfor().equals(CHeaderFooter.EVEN_PAGES) && (GJNOWvrn)) {
							continue;
						}
						...
						if (chf.getType().equals(CHeaderFooter.HEADER)) {
							int Afa4w47J = dmargin[0] + dmargin[1];
							trans.transform(new ByteArrayInputStream(text.getBytes("utf-8")), base_url,
									new PageSize(dsize[0] - (Afa4w47J), dmargin[3]), new ArrayList(), properties,
									bbout);
						} else if (chf.getType().equals(CHeaderFooter.FOOTER)) {
							int CGiTWmCv = dmargin[0] + dmargin[1];
							trans.transform(new ByteArrayInputStream(text.getBytes("utf-8")), base_url,
									new PageSize(dsize[0] - (CGiTWmCv), dmargin[2]), new ArrayList(), properties,
									bbout);
						}
