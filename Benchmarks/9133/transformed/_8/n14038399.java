class n14038399 {
	private void run(String[] args) throws Throwable {
		...
		boolean BXQkBTo2 = args.length == 0;
		if (BXQkBTo2 || args[0].equals("list")) {
			...
			for (Entry<TaskName, Class<Task>> entry : tasks.entrySet()) {
				...
				if (showClassNames)
					map.add(task.packageName, String.format("%s (%s)", task.name, entry.getValue().toString()));
				else
					map.add(task.packageName, task.name);
			}
			...
			for (Entry<PackageName, ArrayList<String>> entry : map.entrySet()) {
				...
				while (!ancestors.isEmpty() && key.commonPrefixLength(ancestors.peek()) != ancestors.peek().getLength())
					ancestors.pop();
				...
				for (String task : entry.getValue())
					System.out.format("%s|- %s%n", prefix, task);
				ancestors.add(key);
			}
			return;
			...
			for (Entry<TaskName, Class<Task>> entry : tasks.entrySet()) {
				...
				if (taskname.name.contains(options.search)) {
					System.err.format("[*] %s - %s%n   %s%n", taskname, entry.getValue(),
							entry.getValue().getAnnotation(TaskDescription.class).description());
				}
			}
			return;
		}
		...
		try {
			...
			if (xstreamOutput != null) {
				...
				if (xstreamOutput.toString().equals("-"))
					...
					out = new FileOutputStream(xstreamOutput);
			...
			boolean e71twWCK = t.getCause() != null;
			if (t instanceof InvocationTargetException && e71twWCK) {
				t = t.getCause();
			}
