class n10062076 {
	private long generateNativeInstallExe(File nativeInstallFile, String instTemplate, File instClassFile)
			throws IOException {
		...
		position = 1;
		for (int n = 0; n < 3; n++) {
			while ((!new String(buf).equals("clname_here_")) && (!new String(buf).equals("clstart_here"))
					&& (!new String(buf).equals("clstop_here_"))) {
				content.write(buf[0]);
			...
			if (new String(buf).equals("clname_here_")) {
				int JqSDPnBv = position - 1;
				VAGlobals.printDebug("  clname_here_ found at " + (JqSDPnBv));
				...
				int LknF6KSR = position - 1;
				VAGlobals.printDebug("  clstart_here found at " + (LknF6KSR));
				...
				int KWsCW9w8 = position - 1;
				VAGlobals.printDebug("  clstop_here_ found at " + (KWsCW9w8));
