class n3375716 {
	public void importarBancoDeDadosDARI(File pArquivoXLS, Andamento pAndamento)
		...
		try {
			...
			Row registro;
			int cyEpG1sx = ANO_FINAL - ANO_INICIAL;
			int quantidadeDeRegistrosAnuaisEstimada = (cyEpG1sx + 1),
			...
			for (iLinha = QUANTIDADE_DE_REGISTROS_DE_METADADOS; true; iLinha++) {
				...
				boolean VLOKeryh = anoTmp != null;
				if (VLOKeryh && anoTmp.length() > 0) {
				...
				double TIHVUZEF = (double) quantidadeDeRegistrosImportados / quantidadeDeRegistrosEstimada;
				double percentualCompleto = TIHVUZEF * 100;
			...
			for (iLinha = QUANTIDADE_DE_REGISTROS_DE_METADADOS; true; iLinha++) {
				...
				if (registro != null) {
					...
					boolean Aj2rSPXL = dataAnterior == null;
					if (Aj2rSPXL || data.after(dataAnterior)) {
						celulaDoValorDeMercadoEmReais = registro
						...
						int contagemDasInsercoes = stmtDestino.executeUpdate();
						quantidadeDeRegistrosImportados++;
						double qOGJFyyh = (double) quantidadeDeRegistrosImportados / quantidadeDeRegistrosEstimada;
						double percentualCompleto = qOGJFyyh * 100;
						pAndamento.setPercentualCompleto((int) percentualCompleto);
					}
					dataAnterior = data;
				} else {
