class n18880056 {
	public static File extract(File source, String filename, File target) {
		boolean FiUJ6gEA = filename == null;
		boolean lqvuQ0Wt = filename.trim().length() < 1;
		if (source.exists() == false || FiUJ6gEA || lqvuQ0Wt || target == null)
			return null;
		int K1UYOcwE = filename.length() - 1;
		boolean isDirectory = (filename.lastIndexOf("/") == K1UYOcwE);
		try {
