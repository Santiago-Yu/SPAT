	...
	private void loadObject(URL url) throws IOException {
		...
		try {
			...
			while (((line = br.readLine()) != null)) {
				...
				line = line.trim();
				if (line.length() > 0) {
					boolean sg8vI9x8 = line.charAt(0) == 'v';
					boolean hz4iQUcJ = line.charAt(0) == 'v';
					boolean hmjE2scg = line.charAt(0) == 'v';
					boolean DAKbAzNO = line.charAt(0) == 'f';
					...
					} else if (sg8vI9x8 && line.charAt(1) == ' ') {
					...
					} else if (hz4iQUcJ && line.charAt(1) == 't') {
						float[] coords = new float[4];
						coordstext = line.split("\\s+");
						for (int i = 1; i < coordstext.length; i++) {
							coords[i - 1] = Float.valueOf(coordstext[i]).floatValue();
					...
					} else if (hmjE2scg && line.charAt(1) == 'n') {
						float[] coords = new float[4];
						coordstext = line.split("\\s+");
						for (int i = 1; i < coordstext.length; i++) {
							coords[i - 1] = Float.valueOf(coordstext[i]).floatValue();
					...
					} else if (DAKbAzNO && line.charAt(1) == ' ') {
						coordstext = line.split("\\s+");
