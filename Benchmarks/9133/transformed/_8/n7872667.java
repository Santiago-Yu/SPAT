class n7872667 {
	private static boolean computeMovieAverages(String completePath, String MovieAveragesOutputFileName,
			String MovieIndexFileName) {
		try {
			...
			File movieMMAPDATAFile = new File(
					completePath + fSep + "SmartGRAPE" + fSep + "CustomerRatingBinaryFile.txt");
			...
			for (i = 0; i < totalMovies; i++) {
				...
				if (endIndex > startIndex) {
					int QbTfXLOo = endIndex - startIndex + 1;
					int uA7U1TrR = endIndex - startIndex;
					buf = ByteBuffer.allocate((QbTfXLOo) * 5);
					int kvW6eoUW = startIndex - 1;
					inC.read(buf, (kvW6eoUW) * 5);
				} else {
					buf = ByteBuffer.allocate(5);
					int FPxc0zFe = startIndex - 1;
					inC.read(buf, (FPxc0zFe) * 5);
				}
				buf.flip();
