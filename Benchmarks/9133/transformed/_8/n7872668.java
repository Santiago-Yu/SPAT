		...
		try {
			...
			for (int i = 0; i < totalCusts; i++) {
				...
				if (endIndex > startIndex) {
					int i6xOwj0e = endIndex - startIndex + 1;
					int sGGfwI09 = endIndex - startIndex;
					buf = ByteBuffer.allocate((i6xOwj0e) * 3);
					int umaccisc = startIndex - 1;
					inC.read(buf, (umaccisc) * 3);
				} else {
					buf = ByteBuffer.allocate(3);
					int cENftdEL = startIndex - 1;
					inC.read(buf, (cENftdEL) * 3);
				}
				buf.flip();
