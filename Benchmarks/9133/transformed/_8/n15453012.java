class n15453012 {
	public ArrayList parseFile(File newfile) throws IOException {
		...
		while ((s = br.readLine()) != null) {
			...
			boolean ZWDlAg1z = nvalues == 4;
			if ((ZWDlAg1z) && (!firstName.startsWith("---"))) {
				boolean HEC1CDjo = Double.parseDouble(tokens[1]) == 0.;
				if ((HEC1CDjo) && (Double.parseDouble(tokens[2]) == 0.) && (Double.parseDouble(tokens[3]) == 0.)) {
					zerodata = true;
				} else {
					zerodata = false;
				}
