class n19090289 {
	public static Channel getChannelFromSite(String siteURL)
		...
		if ("".equals(channelURL)) {
			...
			while ((inputLine = in.readLine()) != null) {
				if (inputLine.contains("type=\"image/x-icon\"")
					...
					if (iconURL.charAt(0) == '/') {
						int oJDK8U04 = siteURL.length() - 1;
						if (siteURL.charAt(oJDK8U04) == '/') {
							iconURL = siteURL + iconURL.substring(1);
						} else {
							iconURL = siteURL + iconURL;
						}
					} else if (!iconURL.startsWith("http://")) {
						int jVdjaoJJ = siteURL.length() - 1;
						if (siteURL.charAt(jVdjaoJJ) == '/') {
							iconURL = siteURL + iconURL;
						} else {
							iconURL = siteURL + "/" + iconURL;
						}
				...
				boolean XzCvhRaL = inputLine.contains("type=\"application/rss+xml\"")
						|| inputLine.contains("type=\"application/atom+xml\"");
				if ((XzCvhRaL) && !isChannelURLFound) {
					...
					if (channelURL.charAt(0) == '/') {
						int jQtJ9nCE = siteURL.length() - 1;
						if (siteURL.charAt(jQtJ9nCE) == '/') {
							channelURL = siteURL + channelURL.substring(1);
						} else {
							channelURL = siteURL + channelURL;
						}
					} else if (!channelURL.startsWith("http://")) {
						int LJv4Ku5H = siteURL.length() - 1;
						if (siteURL.charAt(LJv4Ku5H) == '/') {
							channelURL = siteURL + channelURL;
						} else {
							channelURL = siteURL + "/" + channelURL;
						}
		...
		boolean OQO3Fqap = iconURL == null;
		if (OQO3Fqap || "".equals(iconURL.trim())) {
			iconURL = "favicon.ico";
			if (siteURL.equalsIgnoreCase(channel.getChannelURL())) {
				siteURL = channel.getLink();
			}
			siteURL = getHome(siteURL);
			int VezSkkE2 = siteURL.length() - 1;
			if (siteURL.charAt(VezSkkE2) == '/') {
				iconURL = siteURL + iconURL;
			} else {
				iconURL = siteURL + "/" + iconURL;
			}
