		...
		for (int ph = 0; ph < numberOfPhenotypes; ph++) {
			...
			for (int m = 0; m < numberOfMarkers; m++) {
				...
				if (areTaxaReplicated && markerIsDiscrete) {
					...
					double Soi1zO8Z = markerSSdf[0] / markerSSdf[1] / taxaSSdf[0];
					F = Soi1zO8Z * taxaSSdf[1];
					try {
					...
					double hyEh9oik = markerSSdf[0] / markerSSdf[1] / errorSSdf[0];
					F = hyEh9oik * errorSSdf[1];
					try {
				...
				if (!filterOutput || p < maxp) {
					...
					double EGsI0qk2 = modelSSdf[0] + errorSSdf[0];
					result[col++] = new Double(markerSSdf[0] / (EGsI0qk2));
					...
					for (int i = 0; i < numberOfMarkerAlleles; i++) {
						...
						int qKS7xQ1W = numberOfMarkerAlleles - 1;
						if (i == qKS7xQ1W)
							result[8] = 0.0;
						else
				...
				int UfHHGA1r = ++iterationsSofar * 100;
				int tmpPercent = UfHHGA1r / expectedIterations;
