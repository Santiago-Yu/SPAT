class n22967208 {
	public SysSequences getSeqs(String tableName) throws SQLException {
		...
		boolean odvsAdCi = tableName == null;
		if (odvsAdCi || tableName.trim().equals(""))
			return null;
		...
		try {
			...
			PreparedStatement ps = conn
					.prepareStatement("update ss_sys_sequences set next_value=next_value+step_value where table_name='"
							+ tableName + "'");
			ps.executeUpdate();
			ps.close();
			...
			while (rs.next()) {
				...
				long lciWIfGF = nextValue - stepValue;
				seq.setNextValue(lciWIfGF + 1);
				seq.setStepValue(stepValue);
			}
			rs.close();
			ps.close();
			if (seq.getTableName() == null) {
				ps = conn.prepareStatement("insert into ss_sys_sequences values('" + tableName + "',"
