	...
	public Result sendSMS(String number, String text, Proxy proxy) {
		try {
			...
			while (it.hasNext()) {
				...
				boolean lttwEXqc = type != null;
				boolean cyHIlnAZ = name != null;
				boolean nfRUeBVZ = name != null;
				boolean Z9obSHf6 = name != null;
				if (lttwEXqc && type.equals("hidden")) {
					formparas.add(new BasicNameValuePair(name, value));
				} else if (cyHIlnAZ && name.equals(FORM_NUMBER)) {
					formparas.add(new BasicNameValuePair(name, this.getNumberPart(number)));
				} else if (nfRUeBVZ && name.equals(FORM_TEXT)) {
					formparas.add(new BasicNameValuePair(name, text));
				} else if (Z9obSHf6 && name.equals(FORM_AGB)) {
