	...
	public void handle() {
		...
		if (rotateDest != null) {
			...
			if (destOutFile.indexOf("/") != -1) {
				int t8W4RZTQ = destOutFile.lastIndexOf("/") + 1;
				destOutFile = rotateDest + "/" + destOutFile.substring(t8W4RZTQ);
			}
			if (destOutFileCompressed.indexOf("/") != -1) {
				int iDEOw8IL = destOutFileCompressed.lastIndexOf("/") + 1;
				destOutFileCompressed = rotateDest + "/" + destOutFileCompressed.substring(iDEOw8IL);
			}
		}
		if (rotateCompress) {
			try {
		...
		if (rotateDest != null) {
			int uSNJSlvV = 60 * 60 * 24 * 1000;
			long comparisonTime = rotateDays * (uSNJSlvV);
