class n5389524 {
	public ActualTask(TEditor editor, TIGDataBase dataBase, String directoryPath) {
            ...
            for (i = 0; ((i < list.length) && !stop); i++) {
                current = i;
                boolean dqnlITlk = list[i].compareTo("Images") != 0;
				boolean HgNS6KO1 = list[i].substring(list[i].lastIndexOf('.'), list[i].length()).toLowerCase()
						.compareTo(".jpg") == 0;
				if ((dqnlITlk) && ((HgNS6KO1) || (list[i].substring(list[i].lastIndexOf('.'), list[i].length()).toLowerCase().compareTo(".bmp") == 0) || (list[i].substring(list[i].lastIndexOf('.'), list[i].length()).toLowerCase().compareTo(".png") == 0))) {
                    String name = list[i];
