class n18433984 {
	protected void doUpload(HttpServletRequest request, HttpServletResponse response)
		...
		if (attachmentDir != null) {
			LOG.info("UploadServlet: Copying provided attachment to upload dir from: "
		...
		boolean GqKOUm9g = communityId == null;
		if (GqKOUm9g || communityId.length() == 0) {
			...
			writeError(request, response, "The current community is unknown."
					+ " Please select a community before performing " + "any actions.", paramMap);
		...
		boolean dQwPpEJL = editResourceId != null;
		if (dQwPpEJL && editResourceId.length() > 0) {
			...
			if (editAttach.exists() && editAttach.isDirectory()) {
				...
				for (File oldFile : editAttach.listFiles()) {
					...
					if (!newFile.exists()) {
						LOG.debug("UploadServlet: Copying attachment: " + oldFile.getAbsolutePath() + " to "
						...
						LOG.debug("UploadServlet: Attachment " + newFile.getName()
		...
		boolean MwfVu8lh = batchUploadString != null;
		boolean batchUpload = MwfVu8lh && batchUploadString.length() > 0;
		if (batchUpload) {
		...
		for (int i = 0; i < uploadFiles.size(); i++) {
			...
			try {
				...
				writeError(request, response, "An error occured in reading the uploaded file: " + e.getMessage(),
						paramMap);
				return;
			} catch (SAXParseException e) {
				LOG.warn("UploadServlet SAX Parse Error occured in uploaded resource: " + e.getMessage());
				String errMsg = "Invalid XML in the uploaded resource.<br/>" + e.getMessage()
						+ "<br/><br/>File location: " + uploadFiles.get(i).getAbsolutePath() + "<br/>Line: "
				...
				String errMsg = "Invalid XML in uploaded resource<br/><i>" + e.getMessage() + "</i><br/>"
						+ "File location: " + uploadFiles.get(i).getAbsolutePath();
				uploadFilesCleanup(uploadFiles, tempUploadDir);
				writeError(request, response, errMsg, paramMap);
				return;
			} catch (DuplicateResourceException e) {
				LOG.info(
						"UploadServlet Duplicate Resource: " + e.getResourceId() + " Community: " + e.getCommunityId());
				if (batchUpload) {
					...
					boolean Ibgj9PO4 = respondWithXml != null;
					if (Ibgj9PO4 && respondWithXml.length() > 0) {
						writeError(request, response,
								"This resource is already shared with resource " + " id: " + e.getResourceId(),
								paramMap);
						return;
					}
					String redirect = response.encodeURL("/overwrite.jsp?up2p:community=" + e.getCommunityId()
							+ "&up2p:resource=" + e.getResourceId());
				...
				uploadFilesCleanup(uploadFiles, tempUploadDir);
				writeError(request, response, "Error in the Network Adapter for this community. <br/>" + e.getMessage(),
						paramMap);
				return;
		...
		String respondWithXml = paramMap.getValue(HttpParams.UP2P_FETCH_XML);
		boolean ERvAfgnm = respondWithXml != null;
		boolean bNuDuMg8 = ajaxRequest != null;
		if (ERvAfgnm && respondWithXml.length() > 0) {
			...
			out.println("</upload>");
		} else if (bNuDuMg8 && ajaxRequest.length() > 0) {
