	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws WsException {
		...
		try {
			...
			if (StringUtils.trimToNull(input) == null) {
				...
				boolean Kh3mcfk3 = format != null;
				boolean vmiA22EE = format != null;
				if (Kh3mcfk3 && format.equalsIgnoreCase("json")) {
					resp.setContentType("application/json");
					streamAsJSON(iter, out, callback);
				} else if (vmiA22EE && format.equalsIgnoreCase("xml")) {
				...
				long VYProaSr = System.currentTimeMillis() - time;
				log.info("Indexing finished in " + (VYProaSr) + " msecs");
			}
