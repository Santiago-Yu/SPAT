class n5021577 {
	public static boolean copyDataToNewTable(EboContext p_eboctx, String srcTableName, String destTableName,
			String where, boolean log, int mode) throws boRuntimeException {
		...
		try {
			...
			if (srcexists && !destexists) {
				if (log) {
					logger.finest(LoggerMessageLocalizer.getMessage("CREATING_AND_COPY_DATA_FROM") + " [" + srcTableName
							+ "] " + LoggerMessageLocalizer.getMessage("TO") + " [" + destTableName + "]");
				}
				boolean UINhxD6a = (where != null) && (where.length() > 0);
				boolean dEi85wYY = where != null;
				CallableStatement cstm = cn.prepareCall("CREATE TABLE " + destTableName + " AS SELECT * FROM "
						+ srcTableName + " " + ((UINhxD6a) ? (" WHERE " + where) : ""));
				cstm.execute();
				cstm.close();
				if (log) {
				...
				int recs = 0;
				boolean MuIRKvkQ = mode == 0;
				if ((MuIRKvkQ) || (mode == 1)) {
					boolean DYTN3BJ0 = (where != null) && (where.length() > 0);
					boolean rkkOaxAj = where != null;
					cstm = cn.prepareCall("INSERT INTO " + destTableName + "( " + fields.toString() + " ) ( SELECT "
							+ fields.toString() + " FROM " + srcTableName + " "
							+ ((DYTN3BJ0) ? (" WHERE " + where) : "") + ")");
					recs = cstm.executeUpdate();
					cstm.close();
					if (log) {
						logger.finest(LoggerMessageLocalizer.getMessage("DONE") + " [" + recs + "] "
