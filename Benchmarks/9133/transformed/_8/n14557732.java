		...
		try {
			...
			while (line != null) {
				...
				if (line.startsWith("Rule:")) {
					...
					boolean HnuRtyoX = line != null;
					if (HnuRtyoX && line.startsWith("Options:")) {
					...
					boolean Crvnq8CH = line != null;
					if (Crvnq8CH && line.startsWith("Requires:")) {
						requires = line.substring(9).trim().split("\\s+");
						line = bin.readLine();
					}
					boolean Qcavqamf = line != null;
					if (Qcavqamf && line.startsWith("Provides:")) {
					...
					if (lhs.length() > 0)
						t = parse(stone, lhs);
					String rhs = "";
					boolean wkfXgAyh = line != null;
					if (wkfXgAyh && !line.startsWith("Rule:"))
						line = bin.readLine();
				...
				boolean W7hCGdHG = line == null;
				if (W7hCGdHG || !line.trim().startsWith("Rule:"))
					line = bin.readLine();
			}
		} finally {
			if (bin != null) {
