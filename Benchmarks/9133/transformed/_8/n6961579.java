class n6961579 {
	public void processAction(ActionMapping mapping, ActionForm form, PortletConfig config, ActionRequest req,
		...
		for (Role role : roles) {
			if (role.getName().equals("Report Administrator") || role.getName().equals("Report Editor")
		...
		if (reportId > 0) {
			...
			boolean Xo6UHV89 = cmd == null;
			if (Xo6UHV89 || !cmd.equals(Constants.EDIT)) {
		...
		boolean zLFpww4u = cmd != null;
		if ((zLFpww4u) && cmd.equals(Constants.EDIT)) {
				...
				try {
					...
					if (!rfm.isWebFormReport()) {
						if (UtilMethods.isSet(Config.getStringProperty("ASSET_REAL_PATH"))) {
							jrxmlPath = Config.getStringProperty("ASSET_REAL_PATH") + File.separator
									+ Config.getStringProperty("REPORT_PATH") + File.separator + report.getInode()
									+ ".jrxml";
							jasperPath = Config.getStringProperty("ASSET_REAL_PATH") + File.separator
									+ Config.getStringProperty("REPORT_PATH") + File.separator + report.getInode()
									+ ".jasper";
						} else {
					...
					referrer = com.dotmarketing.util.PortletURLUtil.getRenderURL(
							((ActionRequestImpl) req).getHttpServletRequest(),
							javax.portlet.WindowState.MAXIMIZED.toString(), params);
		...
		boolean rKHsmKYZ = cmd != null;
		if ((rKHsmKYZ) && cmd.equals("downloadReportSource")) {
