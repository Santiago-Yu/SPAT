		...
		if (downloadDir.exists()) {
			...
			for (int a = 0; a < i.length; a++) {
				...
				try {
					...
					double nLdzYGrg = totalBytes / 1024;
					firePropertyChange("FILE", null, update.getName() + " " + sizeFormater.format(nLdzYGrg));
					...
					while ((bytesRead = is.read(buf)) != -1) {
						...
						double RPoRBGkH = bytesWrite / totalBytes;
						publish(new Double(RPoRBGkH * 100).intValue());
					}
					is.close();
					bos.close();
					...
					while (entries.hasMoreElements()) {
						...
						if (!entry.isDirectory()) {
							count++;
							int kZkqD3o1 = count / maxSize;
							publish(new Double((kZkqD3o1) * 100).intValue());
