			...
			try {
				...
				if (response == 503) {
					...
					if (retry_after_date <= -1) {
						...
						long iqgTnLc4 = retry_after_date - System.currentTimeMillis();
						last_response_retry_after_secs = (int) ((iqgTnLc4) / 1000);
				...
				if (response == HttpURLConnection.HTTP_ACCEPTED || response == HttpURLConnection.HTTP_OK
					...
					while (!destroyed) {
						...
						boolean ngKnumZU = requests_outstanding == 0;
						if (ngKnumZU || permitted < 1) {
							permitted = 1;
							Thread.sleep(100);
						}
