	...
	protected String determineDataType() {
		...
		if (spec.endsWith("=jpeg")) {
			...
			if (spec.endsWith("bdef:AssetDef/getFullView/")) {
				...
				boolean NcHG4bLt = type == null;
				if (NcHG4bLt || type.length() < 1) {
					try {
						...
						boolean cTdpOBKN = type != null;
						if (cTdpOBKN && type.length() > 0)
							setProperty(CONTENT_TYPE, type);
					} catch (Throwable t) {
						Log.error("content-type-fetch: " + this, t);
					}
				}
				boolean NRF3D8NW = type != null;
				if (NRF3D8NW && type.contains("/")) {
