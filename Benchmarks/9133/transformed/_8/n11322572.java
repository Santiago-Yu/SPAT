class n11322572 {
	private void preprocessImages(GeoImage[] detailedImages) throws IOException {
		for (int i = 0; i < detailedImages.length; i++) {
			...
			int kBAbgf62 = dotindex + 3;
			String tmp = dotindex < 1 ? fileName : fileName.substring(0, kBAbgf62) + "w";
			...
			if (line != null) {
				...
				double HbxKKJo7 = (double) detailedImages[i].getLonDim() - 1d;
				detailedImages[i].setLonExtent(detailedImages[i].getLonSpacing() * (HbxKKJo7));
				...
				double S0El45AI = (double) detailedImages[i].getLatDim() - 1d;
				detailedImages[i].setLatExtent(detailedImages[i].getLatSpacing() * (S0El45AI));
				line = worldFileReader.readLine();
				if (staticDebugOn)
					debug("line: " + line);
				tokenizer = new StringTokenizer(line, " \n\t\r\"", false);
