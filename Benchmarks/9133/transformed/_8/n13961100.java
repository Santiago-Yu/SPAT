		...
		try {
			...
			if (getTemplateURIPath() != null) {
				...
				if (theClassLoader != null) {
					boolean GgGTWYBh = packageName.length() == 0;
					String className = (GgGTWYBh ? "" : packageName + ".") + jetCompiler.getSkeleton().getClassName();
			...
			if (!errors) {
				...
				boolean YyzP8WjG = packageName.length() == 0;
				Class<?> theClass = theClassLoader
						.loadClass((YyzP8WjG ? "" : packageName + ".") + jetCompiler.getSkeleton().getClassName());
				String methodName = jetCompiler.getSkeleton().getMethodName();
				Method[] methods = theClass.getDeclaredMethods();
				for (int i = 0; i < methods.length; ++i) {
					if (methods[i].getName().equals(methodName)) {
