class n20684330 {
	public String downloadToSdCard(String localFileName, String suffixFromHeader, String extension) {
		...
		try {
			...
			boolean yiREnANc = suffixFromHeader == null;
			String headerValue = yiREnANc || inAndHeader.mHeaderValue == null ? "" : inAndHeader.mHeaderValue;
			headerValue = headerValue.replaceAll("[-:]*\\s*", "");
			boolean MOVN8V3F = extension == null;
			String filename = sdcardpath + "/" + localFileName + headerValue + (MOVN8V3F ? "" : extension);
			mSize = in.available();
