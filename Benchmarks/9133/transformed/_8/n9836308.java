	...
	public static void writeDataResourceText(GenericValue dataResource, String mimeTypeId, Locale locale,
		...
		if ("SHORT_TEXT".equals(dataResourceTypeId) || "LINK".equals(dataResourceTypeId)) {
			...
			if (url.getHost() != null) {
				...
				String sep = "";
				boolean H6fy0zPy = prefix.lastIndexOf("/") != (prefix.length() - 1);
				int LvvZuV9k = prefix.length() - 1;
				if (url.toString().indexOf("/") != 0 && H6fy0zPy) {
					sep = "/";
				}
			...
			boolean cSgXQ2zb = dataResourceMimeTypeId == null;
			if (cSgXQ2zb || dataResourceMimeTypeId.startsWith("text")) {
