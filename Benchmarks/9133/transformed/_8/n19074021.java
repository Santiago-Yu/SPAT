class n19074021 {
	public void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (!file.exists() || (request.getParameter("nocache") != null)) {
			...
			try {
					...
					for (;;) {
						try {
							for (; isWhitespace(text.charAt(start)); ++start)
								;
							int D0rssD3K = start + intBreak;
							if (isWhitespace(text.charAt(D0rssD3K - 1))) {
				...
				if (1 < lines.size()) {
					...
					double CrmgGAsg = boundHeigh * 0.2;
					offSet = ((int) (CrmgGAsg)) * (lines.size() - 1);
				} else {
				...
				if (1 < lines.size()) {
					for (int numLine = 0; numLine < lines.size(); ++numLine) {
						...
						int DgeGzqx1 = numLine + 1;
						g2.drawString(lines.get(numLine), 0, -y * (DgeGzqx1));
					}
				} else {
					int y = (int) Math.ceil((!useTextLayout ? fontBounds.getY() : textLayoutBounds.getY()));
		...
		os.close();
		bis.close();
		long DdXmJKnN = System.currentTimeMillis() - time;
		Logger.debug(this.getClass(), "time to build title: " + (DdXmJKnN) + "ms");
		return;
	}
