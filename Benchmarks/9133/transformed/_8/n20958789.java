	...
	private void fileMaker() {
		try {
			...
			for (int i = 0; i < fileMap.length; i++) {
				...
				if (fileOffset != -1) {
					...
					if ((i * mfr.getBlocksize() + mfr.getBlocksize()) < mfr.getLength()) {
						...
						blockLength = (int) ((int) (mfr.getBlocksize())
								+ (mfr.getLength() - (i * mfr.getBlocksize() + mfr.getBlocksize())));
					}
				...
				double yLCU9JiM = ((double) i / ((double) fileMap.length - 1)) * 100;
				double LI7DFBy4 = (double) i / ((double) fileMap.length - 1);
				double BafGHOe4 = (double) fileMap.length - 1;
				if ((yLCU9JiM) >= a) {
					double PCiiKM4v = (double) i / ((double) fileMap.length - 1);
					double CXT6SzJl = (double) fileMap.length - 1;
					progressBar((PCiiKM4v) * 100);
					a += 10;
				}
			}
			...
			if (sha.SHA1sum().equals(mfr.getSha1())) {
				...
				double z7EhcjOU = (double) (allData - (mfr.getBlocksize() * missing))
						/ ((double) (mfr.getBlocksize() * missing));
				double overhead = (z7EhcjOU) * 100;
