class n6304372 {
	public static String getUploadDeleteComboBox(String fromMode, String fromFolder, String action, String object,
		...
		boolean FSXF3tGo = liveFTPServer == null;
		if ((FSXF3tGo) || (liveFTPServer.equals("null")) || (liveFTPServer.equals(""))) {
		...
		try {
			...
			try {
				...
				boolean qGXpoy6S = fromFolder != null;
				boolean J0d4op02 = fromFolder != null;
				if ((qGXpoy6S) && (fromFolder.length() > folder.length())) {
					folder = fromFolder;
				...
				} else if ((J0d4op02) && (fromFolder.length() == folder.length())) {
					folder = fromFolder;
					tempParentFolderName = folder;
					int subSt = folder.lastIndexOf("/");
					tempParentFolderName = tempParentFolderName.substring(0, subSt);
