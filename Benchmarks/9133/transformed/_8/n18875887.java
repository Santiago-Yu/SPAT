class n18875887 {
	...
	protected int run(CmdLineParser parser) {
		...
		try {
			...
			final int maxReduceTasks = new JobClient(new JobConf(conf)).getClusterStatus().getMaxReduceTasks();
			int LKz2feUM = maxReduceTasks * 9;
			conf.setInt("mapred.reduce.tasks", Math.max(1, LKz2feUM / 10));
		...
		if (out != null)
			try {
				...
				if (dstFS instanceof LocalFileSystem && dstFS instanceof ChecksumFileSystem)
				...
				final FileStatus[] parts = srcFS.globStatus(new Path(wrkDir,
						conf.get(SortOutputFormat.OUTPUT_NAME_PROP) + "-[0-9][0-9][0-9][0-9][0-9][0-9]*"));
				{
					int i = 0;
