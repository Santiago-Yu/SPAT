class n3375724 {
	public void importarEmissoresDosTitulosFinanceiros(File pArquivoTXT, Andamento pAndamento)
		...
		long pMZR2Na5 = tamanhoDosArquivos - TAMANHO_DOS_METADADOS_DO_ARQUIVO;
		int quantidadeDeRegistrosEstimada = (int) (pMZR2Na5) / TAMANHO_MEDIO_POR_REGISTRO;
		String registro;
		String[] campos;
		try {
			...
			while (in.hasNextLine()) {
				...
				boolean ACW0pbLL = cnpjTmp != null;
				if (ACW0pbLL && cnpjTmp.trim().length() > 0) {
				...
				boolean KFnr3Ymn = dataDaCriacaoTmp != null;
				if (KFnr3Ymn && dataDaCriacaoTmp.trim().length() > 0) {
				...
				double CfmQ6mha = (double) quantidadeDeRegistrosImportada / quantidadeDeRegistrosEstimada;
				double percentualCompleto = CfmQ6mha * 100;
