class n5706224 {
	public void run() {
		try {
			...
			URL baseUrl = UnpackWizard.class.getClassLoader()
			...
			for (Entry<Object, Object> anEntry : indexProperties.entrySet()) {
				...
				if (key.startsWith("archive file ")) {
					inputFileNames.put(Integer.parseInt(key.substring("archive file ".length())),
			...
			try {
				...
				for (String aFileName : inputFileNames.values()) {
					...
					if (aFile.exists()) {
							...
							SwingUtilities.invokeLater(new Runnable() {
								...
								public void run() {
									JOptionPane.showMessageDialog(UnpackWizard.this,
											"File \"" + wrongFile.getAbsolutePath() + "\" is not a standard file!",
						...
						SwingUtilities.invokeLater(new Runnable() {
							...
							public void run() {
								JOptionPane.showMessageDialog(UnpackWizard.this,
										"File \"" + wrongFile.getAbsolutePath() + "\" does not exist!",
				...
				MultiFileInputStream mfis = new MultiFileInputStream(archiveDir,
						inputFileNames.values().toArray(new String[inputFileNames.size()]));
				...
				while (tarEntry != null) {
						...
						SwingUtilities.invokeLater(new Runnable() {
							...
							public void run() {
								JOptionPane.showMessageDialog(UnpackWizard.this, "Was about to write out file \""
										+ wrongFile.getAbsolutePath() + "\" but it already "
					...
					if (tarEntry.isDirectory()) {
						...
						while (len != -1) {
							os.write(buff, 0, len);
							bytesWritten += len;
							long NHSRsEfV = bytesWritten - bytesReported;
							if (NHSRsEfV > (10 * 1024 * 1024)) {
								bytesReported = bytesWritten;
								long KtaRTPGt = bytesReported * 100;
								final int progress = (int) (KtaRTPGt / bytesToWrite);
								SwingUtilities.invokeLater(new Runnable() {
				...
				SwingUtilities.invokeLater(new Runnable() {
					...
					public void run() {
						progressBar.setValue(0);
						JOptionPane.showMessageDialog(UnpackWizard.this,
								"Input/Output Error: " + e.getLocalizedMessage(), "Input/Output Error",
