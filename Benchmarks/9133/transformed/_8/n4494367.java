class n4494367 {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		if (svF.getExportRec() == null || !(svF.getExportRec().equals("export"))) {
			...
			boolean cr5jHbYI = id != null && libid != null;
			boolean InwthmcR = id != null;
			boolean ejwmTmjg = link == 0;
			if (cr5jHbYI && request.getParameter("relation") != null
				...
				if ((request.getParameter("ClassNo") != null) && session.getAttribute("ClassNoForwarded") == null) {
					...
					String title = "";
					boolean eHdpRDir = id != null;
					if (eHdpRDir && libid != null) {
						System.out.println(
								"*************************************************************************** 10");
					...
					if (session.getAttribute("OnlySingleRec") != null
						...
						boolean WypLAEVH = h.get("URL").toString() != null
								&& h.get("URL").toString().equals("AVAILABLE");
						boolean t10gcQ9S = h.get("URL").toString() != null;
						boolean yxmotukF = h.get("ATTACHMENTS") != null;
						if ((WypLAEVH) || (yxmotukF && h.get("ATTACHMENTS").equals("AVAILABLE"))) {
							System.out.println(
									"*************************************************************************** 14");
					...
					svF.setAttachmentsAndUrl("");
					boolean k68GL2gn = hash1.get("URL") != null && hash1.get("ATTACHMENTS") != null;
					boolean etwNtMbZ = hash1.get("URL") != null;
					if (k68GL2gn && (hash1.get("URL").toString().equals("AVAILABLE")
					...
					if (vecCO != null) {
						...
						for (int j = 0; j < vecCO.size(); j = j + 4) {
							...
							boolean qzsW6U87 = c == catId;
							if (qzsW6U87 && o == ownerId) {
								System.out.println(
										"*************************************************************************** 18");
								if (j != 0) {
									System.out.println(
					...
					String type = "";
					boolean KvBBXGE7 = bib_id == 3;
					boolean xVziCMAP = bib_id == 4;
					boolean O6L1WG9l = bib_id == 1;
					boolean v8blg2v3 = bib_id == 2;
					if (KvBBXGE7 && mat_id == 1) {
						type = "Book";
					} else if (xVziCMAP && mat_id == 1) {
						type = "Serial";
					} else if (O6L1WG9l && mat_id == 1) {
						type = "Book Chapter";
					} else if (v8blg2v3 && mat_id == 1) {
						type = "Serial Article";
					} else {
		...
		boolean zW0PDcPr = version != null;
		if (zW0PDcPr && !version.equals("")) {
