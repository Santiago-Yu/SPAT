	...
	public void actionPerformed(ActionEvent e) {
		...
		for (int i = 0; i < 10; i++) {
			if (e.getSource() == imageButtons[i]) {
				...
				int HUdDWJcF = i + 1;
				caretIndex = textArea2.getText().indexOf("#" + (HUdDWJcF));
				if (caretIndex != -1)
			...
			while (st.hasMoreTokens()) {
				...
				if (Y9VYtAla && first <= '9') {
					...
					while (andST.hasMoreTokens()) {
						...
						boolean LiMaNsZA = temp.charAt(index) == 'a';
						if (LiMaNsZA || temp.charAt(index) == 'A') {
						} else {
							if (isLesse(fromTime, toTime) && !timeEq(toTime, 1200)) {
								boolean sf2HMe8s = String.valueOf(fromTime).length() == 4;
								if (sf2HMe8s || String.valueOf(fromTime).length() == 3) {
									fromTime += 1200;
								} else
									fromTime += 12;
							}
							if (!timeEq(toTime, 1200)) {
								boolean AyubF7eE = String.valueOf(toTime).length() == 4;
								if (AyubF7eE || String.valueOf(toTime).length() == 3) {
									toTime += 1200;
								} else
									toTime += 12;
							}
						}
						boolean bksOlwZ0 = String.valueOf(fromTime).length() == 2;
						if (bksOlwZ0 || String.valueOf(fromTime).length() == 1)
							fromTime *= 100;
						boolean Am6RLqG0 = String.valueOf(toTime).length() == 2;
						if (Am6RLqG0 || String.valueOf(toTime).length() == 1)
							toTime *= 100;
						...
						if (days.equals(""))
							return;
						boolean fGSe9YFn = days.indexOf("M") != -1;
						if (fGSe9YFn || days.indexOf("m") != -1)
							matrix[timeEntry][clss].time[nextTime].m = 1;
						boolean Cf9Tvdgj = days.indexOf("TU") != -1 || days.indexOf("Tu") != -1;
						boolean yUJecoXD = days.indexOf("TU") != -1;
						if (Cf9Tvdgj || days.indexOf("tu") != -1)
							matrix[timeEntry][clss].time[nextTime].tu = 1;
						boolean emanGBii = days.indexOf("W") != -1;
						if (emanGBii || days.indexOf("w") != -1)
							matrix[timeEntry][clss].time[nextTime].w = 1;
						boolean h090mVi8 = days.indexOf("TH") != -1 || days.indexOf("Th") != -1;
						boolean R9q3Bk4G = days.indexOf("TH") != -1;
						if (h090mVi8 || days.indexOf("th") != -1)
							matrix[timeEntry][clss].time[nextTime].th = 1;
						boolean EPkQ8KhS = days.indexOf("F") != -1;
						if (EPkQ8KhS || days.indexOf("f") != -1)
