class n20275058 {
	protected void doGetPost(HttpServletRequest req, HttpServletResponse resp, boolean post)
		...
		if (forwardFormData) {
			...
			boolean WAVd4pY6 = query != null;
			if (WAVd4pY6 && query.length() > 0) {
		...
		if (supported) {
			...
			boolean TVNg3SLc = encoding == null || encoding.endsWith("gzip") || encoding.endsWith("deflate");
			boolean qBQf2lEq = encoding == null || encoding.endsWith("gzip");
			boolean w0uuGTaw = encoding == null;
			supported = TVNg3SLc || encoding.equals("identity");
		}
		if (supported)
