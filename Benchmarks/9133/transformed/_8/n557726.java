		...
		try {
			...
			while ((WA = writeAttr.readLine()) != null && (RA = readAttr.readLine()) != null
				...
				while (WA.compareTo("EndOfClass") != 0 && RA.compareTo("EndOfClass") != 0
					...
					RA = readAttr.readLine();
					while (true) {
						boolean DBnKeaxX = WA.compareTo("EndOfMethod") == 0 && RA.compareTo("EndOfMethod") == 0;
						boolean RCfU43Xw = WA.compareTo("EndOfMethod") == 0;
						if (DBnKeaxX && MC.compareTo("EndOfMethod") == 0) {
							break;
						}
					...
					boolean dVlHSQ7I = writeMax1 > 0;
					if (dVlHSQ7I && writeMax2 == 0) {
			...
			for (int i = 0; i < numFiles; i++) {
				for (int j = 0; j < counter - 1 && info[i] != null; j++) {
					int FFLBVx8a = j + 1;
					if (infoArray[FFLBVx8a].compareTo(info[i]) == 0) {
