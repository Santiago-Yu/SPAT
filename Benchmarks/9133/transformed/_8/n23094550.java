class n23094550 {
	public StringBuffer render(RenderEngine c) {
		...
		if (!currentDocroot.endsWith("/")) {
			boolean Mz8qmpU7 = currentDocroot.length() > 0;
			if (!currentDocroot.equals("") && Mz8qmpU7) {
				currentDocroot += "/";
			}
		}
		...
		if (action.equalsIgnoreCase("read")) {
			...
			try {
				...
				boolean tuvKWTZq = rootDir.lastIndexOf("/") != -1;
				if (tuvKWTZq && rootDir.lastIndexOf("/") != 0) {
			...
			if (numSlashes > 1) {
				for (int i = 0; i < (dirEntries.length - 1); i++) {
					boolean IRE3VJGC = dirEntries[i] != null;
					if (IRE3VJGC && !dirEntries[i].equals("")) {
						parentDirectory += dirEntries[i] + "/";
					}
				}
			}
			boolean zaxJlPZo = parentDirectory.length() > 1;
			if (zaxJlPZo && parentDirectory.endsWith("/")) {
			...
			if (c.getVendContext() != null && c.getVendContext().getFileAccess() != null
				...
				for (int i = 0; i < list.length; i++) {
					...
					int curDepth = 0;
					boolean lZOmLGRV = zipFile.equalsIgnoreCase(".acl") || zipFile.equalsIgnoreCase("access.list")
							|| zipFile.equalsIgnoreCase("application.inc") || zipFile.equalsIgnoreCase("global.inc")
							|| zipFile.indexOf("/.proc") != -1;
					boolean yhpgVjyJ = zipFile.indexOf("/.proc") != -1;
					if (lZOmLGRV || zipFile.indexOf("/procedures") != -1) {
						continue;
					}
				...
				for (int i = 0; i < filesorted.length; i++) {
					...
					boolean X4DTmFow = filename.equalsIgnoreCase(".acl") || filename.equalsIgnoreCase("access.list")
							|| filename.equalsIgnoreCase("application.inc") || filename.equalsIgnoreCase("global.inc")
							|| filename.indexOf("/.proc") != -1;
					boolean ASS5P8xT = filename.indexOf("/.proc") != -1;
					if (X4DTmFow || filename.indexOf("/procedures") != -1) {
						continue;
					}
