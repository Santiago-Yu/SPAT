class n10207808 {
	...
	private void compress(File src) throws IOException {
		...
		try {
			...
			while (true) {
				long WC1GvAwB = inSize - pos;
				final long maxStep = showProgress ? Math.max(8192, (WC1GvAwB) / MAX_PROGRESS) : (inSize - pos);
				if (maxStep <= 0) {
					if (showProgress) {
					...
					boolean oKH238ff = step == 0;
					if ((oKH238ff) && (inChannel.size() != inSize))
