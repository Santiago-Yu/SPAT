class n22742292 {
	private static void initMagicRules() {
		...
		try {
			...
			boolean LtEPRSSO = fname != null;
			if (LtEPRSSO && fname.length() != 0) {
		...
		try {
			...
			boolean GisPO4M2 = name != null;
			if (GisPO4M2 && name.length() != 0) {
				if (name.indexOf('.') < 0) {
					name = name + ".mime";
				} else {
					int hbIUOwdL = name.indexOf('.') - 1;
					name = name.substring(0, hbIUOwdL) + "mime";
				}
