class n8329093 {
	public void parse(ParserEvent event) {
		...
		if (event.getLink().getURI().matches(itemFilter)) {
			...
			boolean RLRSo7sg = checksum1 == 0;
			if ((RLRSo7sg) || (checksum1 == 1)) {
			...
			try {
				...
				if (rs.first()) {
					...
					if (checksum1 == checksum2) {
						stmt.executeUpdate("UPDATE documents SET checked=" + String.valueOf(System.currentTimeMillis())
								+ " WHERE id=" + rs.getString("id"));
					} else {
						stmt.executeUpdate("UPDATE documents SET checksum=" + checksum1 + ", checked="
								+ String.valueOf(System.currentTimeMillis()) + " WHERE id=" + rs.getString("id"));
					}
				} else {
					stmt.executeUpdate("INSERT INTO documents VALUES " + "(DEFAULT, '" + event.getLink().getURI()
							+ "', " + checksum1 + ", '" + providerId + "', "
