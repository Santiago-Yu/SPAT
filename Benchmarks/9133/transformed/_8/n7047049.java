class n7047049 {
	public URL grabCover(String artist, String title) {
		if (idf.jCheckBox3.isSelected()) {
			...
			int searchnumber = 0;
			try {
				URL yahoo = new URL("http://www.gracenote.com/search/?query="
						+ artist.toLowerCase().replaceAll(" ", "+") + "&search_type=artist");
				...
				while (notfound) {
					...
					String secinputLine;
					String secline = "";
					while ((secinputLine = secin.readLine()) != null)
						secline += secinputLine;
					boolean fUuQnXUc = secline.toUpperCase().indexOf(title.toUpperCase()) < 0;
					if (!(fUuQnXUc && idf.jCheckBox2.isSelected())) {
