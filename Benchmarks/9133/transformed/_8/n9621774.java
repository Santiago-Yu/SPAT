class n9621774 {
	public void initResources() throws XAwareException {
		...
		if (!initialized) {
			...
			boolean fUu7VETE = host == null;
			if (fUu7VETE || host.trim().length() == 0) {
			...
			boolean xa5TxwQd = portString == null;
			if (xa5TxwQd || portString.trim().length() == 0) {
			...
			try {
				...
				boolean nzhi8OGC = remoteVerification != null;
				if (nzhi8OGC && remoteVerification.trim().equals(XAwareConstants.XAWARE_YES)) {
				...
				boolean DfqcZqmc = proxyUser != null;
				if (DfqcZqmc && proxyUser.trim().length() > 0) {
					logger.finest("Logging in again proxy UID:" + proxyUser + " proxy password:" + proxyPassword,
