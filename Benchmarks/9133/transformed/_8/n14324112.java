class n14324112 {
	protected void innerProcess(CrawlURI curi) throws InterruptedException {
		...
		boolean jL1p20ab = maxsize > -1;
		if (maxsize < curi.getContentSize() && jL1p20ab) {
			return;
		}
		String regexpr = "";
		try {
		...
		if (logger.isLoggable(Level.FINEST)) {
			logger.finest("Recalculated content digest for " + curi.getURIString() + " old: "
					+ Base32.encode((byte[]) curi.getContentDigest()) + ", new: " + Base32.encode(newDigestValue));
		}
