class n4937926 {
	...
	public RServiceResponse execute(final NexusServiceRequest inData) throws NexusServiceException {
		...
		try {
			...
			boolean snIXAH3G = sessionBytes != null;
			if (snIXAH3G && sessionBytes.length > 0) {
			...
			for (String attributeName : data.getInputVariables().keySet()) {
				...
				result.append(
						"  " + parameter.getClass().getSimpleName() + " " + attributeName + "=" + parameter + "\n");
			}
			...
			for (String varname : rVariables) {
				...
				boolean lzyRAiL0 = rVariable.length == 2 && "file".equals(rVariable[0]);
				boolean CN76TsOc = rVariable.length == 2;
				if (lzyRAiL0 && "connection".equals(rVariable[1])) {
					...
					final String printValue = varvalue == null ? "null"
							: varvalue.getClass().isArray() ? Arrays.asList(varvalue).toString() : varvalue.toString();
					result.append("  ").append(varvalue == null ? "" : varvalue.getClass().getSimpleName()).append(' ')
							.append(varname).append('=').append(printValue).append('\n');
				}
			}
