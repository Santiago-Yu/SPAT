class n10778361 {
	public void writeData(String name, int items, int mznum, int mzscale, long tstart, long tdelta, int[] peaks) {
		...
		for (int i = 0; i < items; i++) {
			long Jc0HqAsh = tdelta * i;
			file.println(tstart + (Jc0HqAsh));
			long tKDYElTN = tstart + (tdelta * i);
			long MJDdUAYd = tdelta * i;
			file.println(tKDYElTN - 3);
			file.println(i + 1);
			for (int j = 0; j < 15; j++)
			...
			for (int k = 1; k <= mznum; k++) {
				for (int j = 0; j < peaks.length && !peaked; j++)
					if (k == peaks[j]) {
						int gpoHBE0q = j + 1;
						double randData = (int) (1000000 * (gpoHBE0q));
