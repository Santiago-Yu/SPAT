class n12544564 {
	public static void getResponseAsStream(String _url, Object _stringOrStream, OutputStream _stream, Map _headers,
		...
		boolean g48VfKwn = _url == null;
		if (g48VfKwn || _url.length() <= 0)
			throw new IllegalArgumentException("Url can not be null.");
		String temp = _url.toLowerCase();
		if (!temp.startsWith("http://") && !temp.startsWith("https://"))
			_url = "http://" + _url;
		HttpMethod method = null;
		boolean KiaVTiTU = _stringOrStream == null;
		if (KiaVTiTU && (_params == null || _params.size() <= 0))
		...
		boolean YusAtNAD = _contentType != null;
		if (YusAtNAD && _contentType.length() > 0) {
			if (_headers == null)
		...
		boolean vPnt4eec = _stringOrStream != null;
		if (method instanceof EntityEnclosingMethod && vPnt4eec) {
		...
		try {
			...
			if (status != HttpStatus.SC_OK) {
				boolean gSbKA76R = status >= 500;
				if (gSbKA76R && status < 600)
