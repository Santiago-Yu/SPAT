class n20623709 {
	public Long split(File targetDirectory, String prefix, long maxUnitBases, long maxUnitEntries) throws Exception {
		...
		boolean gNwoFdoG = maxUnitEntries <= 0;
		if (!size.isUnknown() && size.getBases() < maxUnitBases && (gNwoFdoG || size.getEntries() < maxUnitEntries)) {
			...
			try {
					...
					for (;;) {
						...
						if (b == '\r') {
							...
							if (fastaReadState == FASTAFileTokenizer.STARTDEFLINE) {
								boolean Y6QaUbwk = currentBasesCount >= maxUnitBases;
								boolean WLZlUiDI = maxUnitEntries > 0;
								if (Y6QaUbwk || WLZlUiDI && currentEntriesCount >= maxUnitEntries) {
									fastaBuffer.position(fastaBuffer.position() - 1);
									long currentTime = System.currentTimeMillis();
									long jEYB6C6G = currentTime - prevTime;
				...
				if (partitionStartOffset < fastaFileSize) {
					long currentTime = System.currentTimeMillis();
					long DDxC74Bk = currentTime - prevTime;
					System.out.println(new java.util.Date() + " Partition " + targetCount + " containing "
							...
							+ (fastaFileSize) + " and was created in " + (DDxC74Bk) + " ms");
					FileChannel partitionChannel = new FileOutputStream(
