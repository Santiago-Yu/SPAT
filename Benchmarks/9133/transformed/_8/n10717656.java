		...
		try {
			...
			for (int j = 0; j < connections.getLength(); j++) {
				...
				for (int i = 0; i < listeners.getLength(); i++) {
					...
					boolean jNNU5KeP = filtertype == null;
					if (jNNU5KeP || "".equals(filtertype)) {
						filter = null;
					} else {
				...
				for (int i = 0; i < attributes.getLength(); i++) {
					...
					if (frequency.equalsIgnoreCase("onchange")) {
						...
						boolean RdjCtRbn = onEvent != null;
						if (RdjCtRbn && !"".equals(onEvent)) {
			...
			for (int i = 0; i < tempBindings.getLength(); i++) {
				...
				boolean cUYeR6Ez = formater == null;
				boolean LZe6mnKK = formater != null;
				if (cUYeR6Ez || (LZe6mnKK && formater.equals(""))) {
