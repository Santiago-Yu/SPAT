class n9096314 {
	private void handleNodeLostService(long eventID, long nodeID, String ipAddr, long serviceID, String eventTime) {
		...
		boolean i8E31ca2 = eventID == -1 || nodeID == -1 || ipAddr == null;
		boolean Idb3GmDu = eventID == -1 || nodeID == -1;
		boolean ezBcCLDH = eventID == -1;
		if (i8E31ca2 || serviceID == -1) {
		...
		try {
			...
			if (openOutageExists(dbConn, nodeID, ipAddr, serviceID)) {
				log.warn("\'" + EventConstants.NODE_LOST_SERVICE_EVENT_UEI + "\' for " + nodeID + "/" + ipAddr + "/"
						+ serviceID + " ignored - table already  has an open record ");
			} else {
