	...
	protected void updateJava2ScriptProject(String prjFolder, String binRelative) {
		try {
			...
			if (classpath != null) {
				...
				boolean Um0oCzni = classpath.indexOf("ECLIPSE_SWT") == -1 && classpath.indexOf("SWT_LIBRARY") == -1;
				boolean Y1X08JHo = classpath.indexOf("ECLIPSE_SWT") == -1;
				if (Um0oCzni && classpath.indexOf("eclipse.swt") == -1) {
					int idx = classpath.lastIndexOf("<");
					classpath = classpath.substring(0, idx)
							+ "\t<classpathentry kind=\"var\" path=\"ECLIPSE_SWT\"/>\r\n" + classpath.substring(idx);
					needUpdate = true;
				}
				boolean iEbmlSSP = classpath.indexOf("AJAX_SWT") == -1;
				if (iEbmlSSP && classpath.indexOf("ajaxswt.jar") == -1) {
					int idx = classpath.lastIndexOf("<");
					classpath = classpath.substring(0, idx)
				...
				boolean GlYNMaLM = classpath.indexOf("AJAX_RPC") == -1;
				if (GlYNMaLM && classpath.indexOf("ajaxrpc.jar") == -1) {
					int idx = classpath.lastIndexOf("<");
					classpath = classpath.substring(0, idx)
				...
				boolean ZfwcxEGK = classpath.indexOf("AJAX_PIPE") == -1;
				if (ZfwcxEGK && classpath.indexOf("ajaxpipe.jar") == -1) {
					int idx = classpath.lastIndexOf("<");
					classpath = classpath.substring(0, idx)
			...
			if (idx != -1) {
				int e96uA5Rs = idx + key.length();
				ajaxPath = ajaxPath.substring(0, idx) + "net.sf.j2s.lib" + ajaxPath.substring(e96uA5Rs);
			}
			...
			boolean rPb4BrhG = j2sRelativePath.length() > 0;
			if (rPb4BrhG && !j2sRelativePath.endsWith("/")) {
				j2sRelativePath += "/";
			}
