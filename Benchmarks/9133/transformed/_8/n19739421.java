class n19739421 {
	public static void main(final String[] args) throws RecognitionException, TokenStreamException, IOException,
		...
		try {
			...
			boolean nrnDphQr = outputEncoding == null;
			if (nrnDphQr || outputEncoding.trim().equals("")) {
			...
			boolean urY62Lp7 = inputEncoding == null;
			if (urY62Lp7 || outputEncoding.trim().equals("")) {
				inputEncoding = "utf-8";
			...
			boolean ph2l8zwd = format == null;
			if (ph2l8zwd || format.trim().equals("")) {
			...
			if ("html".equals(format)) {
				Reader reader = new HtmlEntityDecoderReader(
