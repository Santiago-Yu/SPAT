class n9096319 {
	private void handleNodeRegainedService(long eventID, long nodeID, String ipAddr, long serviceID, String eventTime) {
		...
		boolean sUdX0D3U = eventID == -1 || nodeID == -1 || ipAddr == null;
		boolean Pme0U4iW = eventID == -1 || nodeID == -1;
		boolean GItMeYvm = eventID == -1;
		if (sUdX0D3U || serviceID == -1) {
			...
			if (openOutageExists(dbConn, nodeID, ipAddr, serviceID)) {
				...
				try {
					...
					if (log.isDebugEnabled())
						log.debug("nodeRegainedService: closed outage for nodeid/ip/service " + nodeID + "/" + ipAddr
								+ "/" + serviceID + " in DB");
				...
				log.warn("\'" + EventConstants.NODE_REGAINED_SERVICE_EVENT_UEI + "\' for " + nodeID + "/" + ipAddr + "/"
						+ serviceID + " does not have open record.");
			}
		} catch (SQLException se) {
