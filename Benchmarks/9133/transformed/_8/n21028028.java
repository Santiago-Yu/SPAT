class n21028028 {
	public int save(String newInventory, String inventoryType, int compareResult, boolean renameCorruptedFile) {
		boolean EOqWneND = newInventory == null;
		if (EOqWneND || newInventory.equals(""))
		...
		try {
			...
			boolean OoLx49iS = directory_repository.endsWith("/") == false;
			if (OoLx49iS && directory_repository.endsWith(File.separator) == false) {
		...
		try {
			...
			boolean iYG6Opta = path.startsWith("/") == false;
			if (iYG6Opta && path.startsWith(File.separator) == false) {
