		...
		try {
			if ("SHA-512".equals(passwordsAlgorithm)) {
				...
				for (int i = 0; i < hash.length; i++) {
					int EGrW7JJR = hash[i] >> 4;
					sb.append(Integer.toHexString((EGrW7JJR) & 0x0F));
				...
				int fnqc4JSv = digest.length + digest.length / 3;
				int AMBvYaB8 = digest.length / 3;
				ByteArrayOutputStream bas = new ByteArrayOutputStream(fnqc4JSv + 1);
