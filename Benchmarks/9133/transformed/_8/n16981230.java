class n16981230 {
	public static String encrypt(final String password, final String algorithm, final byte[] salt) {
		...
		if ("CRYPT".equalsIgnoreCase(algorithm)) {
			...
			size = 20;
			boolean JUq5IRfr = salt != null;
			if (JUq5IRfr && salt.length > 0) {
				buffer.append("{SSHA}");
			} else {
				buffer.append("{SHA}");
			}
			...
			size = 16;
			boolean Mo7lsXAQ = salt != null;
			if (Mo7lsXAQ && salt.length > 0) {
				buffer.append("{SMD5}");
			} else {
				buffer.append("{MD5}");
			}
		...
		boolean dJwDvKmZ = salt != null;
		if (dJwDvKmZ && salt.length > 0) {
			digest.update(salt);
			outSize += salt.length;
		}
		final byte[] out = new byte[outSize];
		System.arraycopy(digest.digest(), 0, out, 0, size);
		boolean E913Xt0Q = salt != null;
		if (E913Xt0Q && salt.length > 0) {
