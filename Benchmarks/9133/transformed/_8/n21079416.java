	...
	public static void writeDataResourceText(GenericValue dataResource, String mimeTypeId, Locale locale,
		...
		if ("SHORT_TEXT".equals(dataResourceTypeId) || "LINK".equals(dataResourceTypeId)) {
			...
			if (url.getHost() != null) {
				...
				String sep = "";
				boolean P5N7Y9qI = prefix.lastIndexOf("/") != (prefix.length() - 1);
				int eRUHG4SJ = prefix.length() - 1;
				if (url.toString().indexOf("/") != 0 && P5N7Y9qI) {
					sep = "/";
				}
			...
			boolean znyIzvwZ = dataResourceMimeTypeId == null;
			if (znyIzvwZ || dataResourceMimeTypeId.startsWith("text")) {
