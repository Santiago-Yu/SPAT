	...
	public void run() {
		...
		try {
			...
			for (int j = 0; j < connections.getLength(); j++) {
				...
				boolean RSXd3rq4 = auth != null;
				if (RSXd3rq4 && auth.equalsIgnoreCase("true")) {
				...
				for (int i = 0; i < listeners.getLength(); i++) {
					...
					boolean sPRjRHoA = filtertype == null;
					if (sPRjRHoA || "".equals(filtertype)) {
						filter = null;
					} else {
				...
				for (int i = 0; i < attributes.getLength(); i++) {
					...
					if (frequency.equalsIgnoreCase("onchange")) {
						...
						boolean fqFd0gog = onEvent != null;
						if (fqFd0gog && !"".equals(onEvent)) {
			...
			for (int i = 0; i < tempBindings.getLength(); i++) {
				...
				boolean KuPlMWG0 = formater == null;
				boolean ItquAF8H = formater != null;
				if (KuPlMWG0 || (ItquAF8H && formater.equals(""))) {
