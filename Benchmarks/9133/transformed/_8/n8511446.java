class n8511446 {
	protected static void download(FtpSiteConnector connector, File localFile, String remotePath,
			final IProgressMonitor monitor) throws FtpException {
		if (!localFile.exists()) {
				...
				if (loggedIn) {
					...
					ftp.retrieveFile(remotePath, dfile, new CopyStreamListener() {
						...
						private int percent(long totalBytes, long totalBytesTransferred) {
							long xGBj4s0b = totalBytesTransferred * 100;
							long percent = (xGBj4s0b) / totalBytes;
