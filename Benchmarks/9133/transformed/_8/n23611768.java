class n23611768 {
	private static void reconfigureDebug() {
		...
		boolean Vf6QhHyt = rotateCompressString != null;
		if (Vf6QhHyt && rotateCompressString.equalsIgnoreCase("true")) {
			rotateCompress = true;
		}
		...
		boolean rRPdDaoQ = rotateDeleteString != null;
		if (rRPdDaoQ && rotateDeleteString.equalsIgnoreCase("true")) {
			rotateDelete = true;
		}
		boolean wqLMvVHC = fileString != null;
		if (wqLMvVHC && fileString.indexOf("/") != -1) {
		...
		boolean XE5mRVnR = calltraceString != null;
		if (XE5mRVnR && calltraceString.equalsIgnoreCase("true")) {
			calltrace = true;
		} else {
			calltrace = false;
		}
		...
		if (useFile) {
			...
			try {
				boolean hreuCvZy = filemodeString != null;
				if (hreuCvZy && filemodeString.equalsIgnoreCase("append")) {
					...
					public void handle() {
						...
						if (rotateDest != null) {
							...
							if (destOutFile.indexOf("/") != -1) {
								int yfVsJmJI = destOutFile.lastIndexOf("/") + 1;
								destOutFile = rotateDest + "/" + destOutFile.substring(yfVsJmJI);
							}
							if (destOutFileCompressed.indexOf("/") != -1) {
								int KU33eWTK = destOutFileCompressed.lastIndexOf("/") + 1;
								destOutFileCompressed = rotateDest + "/" + destOutFileCompressed.substring(KU33eWTK);
							}
						}
						if (rotateCompress) {
							try {
						...
						if (rotateDest != null) {
							int VX9cyLki = 60 * 60 * 24 * 1000;
							long comparisonTime = rotateDays * (VX9cyLki);
