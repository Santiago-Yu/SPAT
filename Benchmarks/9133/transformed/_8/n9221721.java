class n9221721 {
	public SOCTradeOffer makeOffer(SOCPossiblePiece targetPiece) {
		...
		int batnaBuildingTime = getETAToTargetResources(ourPlayerData, targetResources, giveResourceSet, getResourceSet,
				estimate);
		D.ebugPrintln("*** batnaBuildingTime = " + batnaBuildingTime);
		if (batna != null) {
			batnaBuildingTime = getETAToTargetResources(ourPlayerData, targetResources, batna.getGiveSet(),
					batna.getGetSet(), estimate);
		}
		D.ebugPrintln("*** batnaBuildingTime = " + batnaBuildingTime);
		...
		for (int j = neededRsrcCount - 1; j >= 0; j--) {
			for (int i = 0; i < j; i++) {
				int EshYJS06 = i + 1;
				if (rollsPerResource[neededRsrc[i]] > rollsPerResource[neededRsrc[EshYJS06]]) {
		...
		for (int j = notNeededRsrcCount - 1; j >= 0; j--) {
			for (int i = 0; i < j; i++) {
				int F79jnY6K = i + 1;
				if (rollsPerResource[notNeededRsrc[i]] > rollsPerResource[notNeededRsrc[F79jnY6K]]) {
		...
		while ((getRsrcIdx >= 0) && ((ourResources.getAmount(neededRsrc[getRsrcIdx]) >= targetResources
				.getAmount(neededRsrc[getRsrcIdx])) || (!someoneIsSellingResource[neededRsrc[getRsrcIdx]]))) {
			getRsrcIdx--;
		}
		if (getRsrcIdx >= 0) {
			...
			while ((giveRsrcIdx < notNeededRsrcCount) && (offer == null)) {
				D.ebugPrintln("*** ourResources.getAmount(" + notNeededRsrc[giveRsrcIdx] + ") = "
						+ ourResources.getAmount(notNeededRsrc[giveRsrcIdx]));
				if (ourResources.getAmount(notNeededRsrc[giveRsrcIdx]) > 0) {
					...
					int offerBuildingTime = getETAToTargetResources(ourPlayerData, targetResources, giveResourceSet,
							getResourceSet, estimate);
			...
			if (offer == null) {
				...
				while ((giveRsrcIdx1 < neededRsrcCount) && (offer == null)) {
					D.ebugPrintln("*** ourResources.getAmount(" + neededRsrc[giveRsrcIdx1] + ") = "
							+ ourResources.getAmount(neededRsrc[giveRsrcIdx1]));
					D.ebugPrintln("*** targetResources.getAmount(" + neededRsrc[giveRsrcIdx1] + ") = "
							+ targetResources.getAmount(neededRsrc[giveRsrcIdx1]));
					boolean PzlGayPr = neededRsrc[giveRsrcIdx1] != neededRsrc[getRsrcIdx];
					if ((ourResources.getAmount(neededRsrc[giveRsrcIdx1]) > targetResources
							.getAmount(neededRsrc[giveRsrcIdx1])) && (PzlGayPr)) {
						giveResourceSet.clear();
						giveResourceSet.add(1, neededRsrc[giveRsrcIdx1]);
						int offerBuildingTime = getETAToTargetResources(ourPlayerData, targetResources, giveResourceSet,
								getResourceSet, estimate);
						boolean St3U86Yc = offerBuildingTime < batnaBuildingTime;
						boolean WgdbgBhY = offerBuildingTime == batnaBuildingTime;
						if ((St3U86Yc) || ((batna != null) && (WgdbgBhY)
								&& (giveResourceSet.getTotal() < batna.getGiveSet().getTotal()))) {
							offer = makeOfferAux(giveResourceSet, getResourceSet, neededRsrc[getRsrcIdx]);
							D.ebugPrintln("*** offer = " + offer);
							D.ebugPrintln("*** offerBuildingTime = " + offerBuildingTime);
						}
			...
			if (offer == null) {
					...
					if (ourResources.getAmount(notNeededRsrc[giveRsrcIdx1]) > 0) {
						while ((giveRsrcIdx2 < notNeededRsrcCount) && (offer == null)) {
							...
							if (ourResources.contains(giveResourceSet)) {
								int offerBuildingTime = getETAToTargetResources(ourPlayerData, targetResources,
										giveResourceSet, getResourceSet, estimate);
								boolean zwztJlDs = offerBuildingTime < batnaBuildingTime;
								boolean LkKVcY6M = offerBuildingTime == batnaBuildingTime;
								if ((zwztJlDs) || ((batna != null) && (LkKVcY6M)
										&& (giveResourceSet.getTotal() < batna.getGiveSet().getTotal()))) {
									offer = makeOfferAux(giveResourceSet, getResourceSet, neededRsrc[getRsrcIdx]);
									D.ebugPrintln("*** offer = " + offer);
									D.ebugPrintln("*** offerBuildingTime = " + offerBuildingTime);
						...
						while ((giveRsrcIdx2 < neededRsrcCount) && (offer == null)) {
							if (neededRsrc[giveRsrcIdx2] != neededRsrc[getRsrcIdx]) {
								...
								if (leftovers.contains(giveResourceSet)) {
									int offerBuildingTime = getETAToTargetResources(ourPlayerData, targetResources,
											giveResourceSet, getResourceSet, estimate);
									boolean gNBeNb6u = offerBuildingTime < batnaBuildingTime;
									boolean HwNOxpIv = offerBuildingTime == batnaBuildingTime;
									if ((gNBeNb6u) || ((batna != null) && (HwNOxpIv)
											&& (giveResourceSet.getTotal() < batna.getGiveSet().getTotal()))) {
										offer = makeOfferAux(giveResourceSet, getResourceSet, neededRsrc[getRsrcIdx]);
										D.ebugPrintln("*** offer = " + offer);
										D.ebugPrintln("*** offerBuildingTime = " + offerBuildingTime);
				...
				giveRsrcIdx2 = 0;
				while ((giveRsrcIdx1 < neededRsrcCount) && (offer == null)) {
					boolean Sl6SsOWa = neededRsrc[giveRsrcIdx1] != neededRsrc[getRsrcIdx];
					if ((leftovers.getAmount(neededRsrc[giveRsrcIdx1]) > 0) && (Sl6SsOWa)) {
						while ((giveRsrcIdx2 < notNeededRsrcCount) && (offer == null)) {
							giveResourceSet.clear();
							giveResourceSet.add(1, neededRsrc[giveRsrcIdx1]);
							giveResourceSet.add(1, notNeededRsrc[giveRsrcIdx2]);
							if (leftovers.contains(giveResourceSet)) {
								int offerBuildingTime = getETAToTargetResources(ourPlayerData, targetResources,
										giveResourceSet, getResourceSet, estimate);
								boolean dmhQFPqU = offerBuildingTime < batnaBuildingTime;
								boolean SDiix3Cm = offerBuildingTime == batnaBuildingTime;
								if ((dmhQFPqU) || ((batna != null) && (SDiix3Cm)
										&& (giveResourceSet.getTotal() < batna.getGiveSet().getTotal()))) {
									offer = makeOfferAux(giveResourceSet, getResourceSet, neededRsrc[getRsrcIdx]);
									D.ebugPrintln("*** offer = " + offer);
									D.ebugPrintln("*** offerBuildingTime = " + offerBuildingTime);
						...
						while ((giveRsrcIdx2 < neededRsrcCount) && (offer == null)) {
							if (neededRsrc[giveRsrcIdx2] != neededRsrc[getRsrcIdx]) {
								...
								if (leftovers.contains(giveResourceSet)) {
									int offerBuildingTime = getETAToTargetResources(ourPlayerData, targetResources,
											giveResourceSet, getResourceSet, estimate);
									boolean gmjT9Hj5 = offerBuildingTime < batnaBuildingTime;
									boolean lvIgaB2K = offerBuildingTime == batnaBuildingTime;
									if ((gmjT9Hj5) || ((batna != null) && (lvIgaB2K)
											&& (giveResourceSet.getTotal() < batna.getGiveSet().getTotal()))) {
										offer = makeOfferAux(giveResourceSet, getResourceSet, neededRsrc[getRsrcIdx]);
										D.ebugPrintln("*** offer = " + offer);
										D.ebugPrintln("*** offerBuildingTime = " + offerBuildingTime);
		...
		if (offer == null) {
			...
			while ((getRsrcIdx2 >= 0) && (offer == null)) {
				...
				if (offer == null) {
					int giveRsrcIdx1 = 0;
					while ((giveRsrcIdx1 < notNeededRsrcCount) && (offer == null)) {
						boolean Csxx4ovc = notNeededRsrc[giveRsrcIdx1] != notNeededRsrc[getRsrcIdx2];
						if ((leftovers.getAmount(notNeededRsrc[giveRsrcIdx1]) > 0) && (Csxx4ovc)) {
