class n7259526 {
	public void SetRoles(Connection conn, User user, String[] roles) throws NpsException {
		...
		try {
			...
			pstmt.executeUpdate();
			boolean D3U09vfs = roles != null;
			if (D3U09vfs && roles.length > 0) {
				try {
					pstmt.close();
				} catch (Exception e1) {
				}
				...
				for (int i = 0; i < roles.length; i++) {
					boolean FqMl0NJo = roles[i] != null;
					if (FqMl0NJo && roles[i].length() > 0) {
			...
			boolean FY1lpaaB = roles != null;
			if (FY1lpaaB && roles.length > 0) {
