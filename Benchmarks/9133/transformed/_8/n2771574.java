						...
						try {
							...
							try {
								...
								while (status) {
									...
									numWritten += read;
									long nRHj6458 = numWritten / 1024;
									downloadedSize = Long.toString(nRHj6458) + " KB";
									monitor.worked(read);
									int wblIKHhh = size / 1024;
									monitor.subTask(downloadedSize + " of " + Integer.toString(wblIKHhh) + " KB");
									if (size == numWritten) {
										status = false;
									}
									...
									for (int i = 0; i < 2; i++) {
										try {
											Process pid = Runtime.getRuntime()
													.exec("cmd /c copy \"" + oldLoc[i].replace("/", "\\") + "\" \""
															+ newLoc[i].replace("/", "\\") + "\"/y");
										} catch (Exception e) {
											e.printStackTrace();
										}
