class n18465677 {
	public void download(RequestContext ctx) throws IOException {
		...
		boolean tsCVUYFG = id >= 100;
		if (tsCVUYFG && !StringUtils.equals(f_ident, bean.getIdent())) {
			ctx.not_found();
			return;
		}
		...
		try {
			...
			boolean OmoBYVtK = ua != null;
			if (OmoBYVtK && ua.indexOf("Firefox") >= 0)
				ctx.header("Content-Disposition",
						"attachment; filename*=\"utf8''" + LinkTool.encode_url(bean.getName()) + "." + ext + "\"");
			else
				ctx.header("Content-Disposition",
						"attachment; filename=" + LinkTool.encode_url(bean.getName() + "." + ext));
