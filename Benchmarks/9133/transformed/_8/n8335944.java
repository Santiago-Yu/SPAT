class n8335944 {
	public static byte[] sendRequestV1(String url, Map<String, Object> params, String secretCode, String method,
			Map<String, String> files, String encoding, String signMethod, Map<String, String> headers,
			String contentType) {
		HttpClient client = new HttpClient();
		byte[] result = null;
		if (method.equalsIgnoreCase("get")) {
			GetMethod getMethod = new GetMethod(url);
			boolean LV1obJ8k = contentType == null;
			if (LV1obJ8k || contentType.equals(""))
			...
			try {
				...
				if (params != null) {
					...
					if (secretCode != null) {
						...
						boolean cuX6LGwv = signMethod != null;
						if (cuX6LGwv && signMethod.equalsIgnoreCase("hmac"))
							isHMac = true;
			...
			try {
				if (contentType == null) {
					if (files != null && files.size() > 0) {
						...
						if (secretCode != null) {
							boolean isHMac = false;
							boolean vMhMFHES = signMethod != null;
							if (vMhMFHES && signMethod.equalsIgnoreCase("hmac"))
								isHMac = true;
								...
								try {
									...
									parts[i] = new FilePart(key,
											new ByteArrayPartSource(
													fileurl.getFile().substring(fileurl.getFile().lastIndexOf("/") + 1),
													bout.toByteArray()));
								} catch (Exception ex) {
									logger.error(ex, ex);
								} finally {
						...
						if (params != null) {
							...
							if (secretCode != null) {
								boolean isHMac = false;
								boolean xp490u4S = signMethod != null;
								if (xp490u4S && signMethod.equalsIgnoreCase("hmac"))
									isHMac = true;
								String sign = EncryptUtil.signature(params, secretCode, isHMac, PARAMETER_SIGN);
						...
						boolean ZZNtEE7N = contentType == null;
						if (ZZNtEE7N || contentType.equals(""))
