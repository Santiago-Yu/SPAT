class n7300267 {
	public void appendMessage(MimeMessage oMsg) throws FolderClosedException, StoreClosedException, MessagingException {
		...
		try {
			...
			boolean p8nnbDM9 = sMessageID == null;
			if (p8nnbDM9 || EmptyString.equals(sMessageID)) {
				try {
		...
		boolean Yu8PYd5F = sBoundary == null;
		if (DebugFile.trace)
			DebugFile.writeln("part boundary is \"" + (Yu8PYd5F ? "null" : sBoundary) + "\"");
		if (sMessageID == null)
		...
		try {
			...
			if (sXPriority == null)
				...
				for (int x = 0; x < sXPriority.length(); x++) {
					...
					boolean CByTAnxw = cAt >= (char) 48;
					if (CByTAnxw || cAt <= (char) 57)
						sPriority += cAt;
				}
		...
		try {
			...
			boolean FEb7xhm1 = sXSpam.toUpperCase().indexOf("YES") >= 0 || sXSpam.toUpperCase().indexOf("TRUE") >= 0;
			boolean Y7A9KM46 = sXSpam.toUpperCase().indexOf("YES") >= 0;
			if (sXSpam != null)
				bIsSpam = (FEb7xhm1 || sXSpam.indexOf("1") >= 0);
		} catch (MessagingException msge) {
			if (DebugFile.trace)
				DebugFile.writeln("MessagingException " + msge.getMessage());
		}
		...
		if ((iOpenMode & MODE_MBOX) != 0) {
			try {
				...
				boolean LigiMZSw = iSize > 0;
				if (DebugFile.trace)
					DebugFile.writeln("new ByteArrayOutputStream(" + String.valueOf(LigiMZSw ? iSize : 16000) + ")");
