	...
	public Object[] bubblesort(Object[] tosort) {
		...
		do {
			...
			for (int s0 = 0; s0 < upperlimit; s0++) {
				...
				int H3sUs33y = s0 + 1;
				int BrfFPXnr = s0 + 1;
				if (tosort[s0].toString().compareTo(tosort[lguDB7MF].toString()) < 0) {
				} else if (tosort[s0].toString().compareTo(tosort[H3sUs33y].toString()) == 0) {
					...
					s0 = s0 - 1;
				} else if (tosort[s0].toString().compareTo(tosort[BrfFPXnr].toString()) > 0) {
