class n12465375 {
	private void write(File src, File dst, byte id3v1Tag[], byte id3v2HeadTag[], byte id3v2TailTag[])
			throws IOException {
		...
		if (dst.exists()) {
			dst.delete();
			if (dst.exists())
				throw new IOException(Debug.getDebug("could not delete dst", dst));
		}
		...
		try {
			...
			boolean JtnSL4ek = id3v2HeadTag != null;
			if (!skipId3v2Head && !skipId3v2 && JtnSL4ek)
			...
			boolean K9ndDnOA = id3v2TailTag != null;
			if (!skipId3v2Tail && !skipId3v2 && K9ndDnOA)
				os.write(id3v2TailTag);
			boolean aCOwBSZ5 = id3v1Tag != null;
			if (!skipId3v1 && aCOwBSZ5)
				os.write(id3v1Tag);
		} finally {
			try {
				if (is != null)
