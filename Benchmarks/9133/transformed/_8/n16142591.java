		...
		try {
			...
			if (getTemplateURIPath() != null) {
				...
				if (theClassLoader != null) {
					boolean mYONImLX = packageName.length() == 0;
					String className = (mYONImLX ? "" : packageName + ".") + jetCompiler.getSkeleton().getClassName();
			...
			if (!errors) {
				...
				boolean uqkhjMTV = packageName.length() == 0;
				Class<?> theClass = theClassLoader
						.loadClass((uqkhjMTV ? "" : packageName + ".") + jetCompiler.getSkeleton().getClassName());
				String methodName = jetCompiler.getSkeleton().getMethodName();
				Method[] methods = theClass.getDeclaredMethods();
				for (int i = 0; i < methods.length; ++i) {
					if (methods[i].getName().equals(methodName)) {
