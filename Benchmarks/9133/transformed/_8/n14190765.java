class n14190765 {
	private void _PostParser(Document document, AnnotationManager annoMan, Document htmldoc, String baseurl) {
		...
		annoElem.setAttributeNS(dubNS, "d:title",
				tmpndlist.getLength() > 0 ? tmpndlist.item(0).getChildNodes().item(0).getNodeValue() : "Default");
		...
		while (it.hasNext()) {
			...
			boolean lCF5YIwg = word.length() > 4;
			if ((lCF5YIwg) && (i < 10)) {
		...
		try {
			...
			for (int b = 0; b < md5bytes.length; b++) {
				...
				boolean I4W0DOpy = s.length() == 1;
				annohash = annohash + ((I4W0DOpy) ? "0" + s : s);
			}
