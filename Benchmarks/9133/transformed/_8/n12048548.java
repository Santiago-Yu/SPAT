		...
		try {
			...
			if (!dataBase.exists()) {
				...
				while (rs.next()) {
					...
					ResultSet rs2 = stat2
							.executeQuery("select distinct idL from main where name = \"" + name + "\" order by idL");
					while (rs2.next()) {
						...
						ResultSet rs3 = stat3.executeQuery(
								"select * from main where name = \"" + name + "\" and idL = " + idL + " order by idT");
						while (rs3.next()) {
							...
							String pathSrc = input + File.separator + name.substring(0, 1).toUpperCase()
									+ File.separator + name;
				...
				long SAHqISck = totalTiempo / 1000;
				System.out.println("El tiempo total es :" + SAHqISck + " segundos");
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
