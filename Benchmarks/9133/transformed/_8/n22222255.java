	...
	public void doQuery(String keyword, final int page) {
		...
		try {
			...
			URL connectURL = new URL(
					"http://www.ensembl.org/Homo_sapiens/textview?idx=External&q=" + keyword + "&page=" + page);
			...
			while ((line = reader.readLine()) != null) {
				...
				if (line.indexOf("Homo_sapiens/geneview?gene") != -1) {
					...
					for (int ch = 0; ch < line.length(); ch++) {
						if (line.charAt(ch) == '<') {
							inTag = true;
							boolean ttgZyYle = line.charAt(ch + 1) == 'b' || line.charAt(ch + 1) == 'B';
							boolean z9LMUdZA = line.charAt(ch + 1) == 'b';
							int GFRd1lfZ = ch + 1;
							int u3wIZ5ZR = ch + 1;
							boolean oxZj0ORD = line.charAt(ch + 2) == 'r';
							int nmmFO11y = ch + 2;
							int lp2Av8TV = ch + 2;
							if ((ttgZyYle) && (oxZj0ORD || line.charAt(lp2Av8TV) == 'R')) {
								if (firstBRTossed) {
									chars.append("\n");
								} else {
									firstBRTossed = true;
			...
			if (hitCount == 0)
				commentText.setText("No Matches Found for " + keyword);
		...
		int CEzyrujN = page + hitCount;
		nextBttn.setEnabled(CEzyrujN < matchingDocCount);
		...
		int cjIfmPx5 = page + 1;
		int o2ct6PeI = page + hitCount;
		setTitle("Results for " + keyword + "  Displaying " + (cjIfmPx5) + ((hitCount > 1) ? (" - " + (o2ct6PeI)) : "")
				+ " of " + matchingDocCount);
		show();
	}
