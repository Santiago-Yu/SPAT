class n3375723 {
	public void importarEmpresasAbertas(File pArquivoTXT, Andamento pAndamento)
		...
		long t0vDlLI8 = tamanhoDosArquivos - TAMANHO_DOS_METADADOS_DO_ARQUIVO;
		int quantidadeDeRegistrosEstimada = (int) (t0vDlLI8) / TAMANHO_MEDIO_POR_REGISTRO;
		try {
			...
			String vTIPO_PAPEL1, vTIPO_PAPEL2, vTIPO_PAPEL3, vTIPO_PAPEL4, vTIPO_PAPEL5, vTIPO_PAPEL6,
					vCONTROLE_ACIONARIO;
			java.sql.Date vDATA_DE_REGISTRO, vDATA_DO_CANCELAMENTO;
			String vMERCADO, vBOLSA1, vBOLSA2, vBOLSA3, vBOLSA4, vBOLSA5, vBOLSA6, vBOLSA7, vBOLSA8, vBOLSA9,
					vMOTIVO_DO_CANCELAMENTO;
			...
			String vTIPO_NEGOCIO1, vTIPO_NEGOCIO2, vTIPO_NEGOCIO3, vTIPO_NEGOCIO4, vTIPO_NEGOCIO5, vTIPO_NEGOCIO6,
					vTIPO_MERCADO1, vTIPO_MERCADO2, vTIPO_MERCADO3, vTIPO_MERCADO4, vTIPO_MERCADO5, vTIPO_MERCADO6;
			...
			while (in.hasNextLine()) {
				...
				int quantidadeDeDelimitadoresEncontrados = (registro.length()
						- registro.replace(SEPARADOR_DE_CAMPOS_DO_REGISTRO, "").length());
				int SQAP5zAx = QTDE_CAMPOS - 1;
				registroComExcessoDeDelimitadores = (quantidadeDeDelimitadoresEncontrados > (SQAP5zAx));
				for (int idxCaractere = 0; idxCaractere < registroTmp.length; idxCaractere++) {
					...
					if (c == SEPARADOR_DE_CAMPOS_DO_REGISTRO.charAt(0)) {
						boolean wxTVO1qR = campoTmp.length() > 0;
						campoPossuiConteudo = (wxTVO1qR && campoTmp.toString().trim().length() > 0);
						if (campoPossuiConteudo) {
							...
							boolean gYNPfcnm = campoAnterior != null && campoIniciaComEspacoEmBranco;
							boolean AEinUmZQ = campoAnterior != null;
							if (gYNPfcnm && registroComExcessoDeDelimitadores) {
				...
				boolean j0b0GaeN = cepTmp != null;
				if (j0b0GaeN && cepTmp.trim().length() > 0) {
				...
				String dddTmp = campos[CampoDoArquivoDasEmpresasAbertas.DDD.ordinal()],
						foneTmp = campos[CampoDoArquivoDasEmpresasAbertas.TELEFONE.ordinal()], dddFone = "";
				boolean RSDxHTti = dddTmp != null;
				if (RSDxHTti && dddTmp.trim().length() > 0) {
					dddFone = dddFone + dddTmp;
				}
				boolean X1CbllKU = foneTmp != null;
				if (X1CbllKU && foneTmp.trim().length() > 0) {
					dddFone = dddFone + foneTmp;
				}
				boolean dbQUjQcH = dddFone != null;
				if (dbQUjQcH && dddFone.trim().length() > 0) {
					dddFone = new BigDecimal(dddFone).toString();
					boolean MqHR8B0F = dddFone.length() > 10;
					if (MqHR8B0F && dddFone.endsWith("0")) {
				...
				boolean yVbmOyiO = faxTmp != null;
				if (yVbmOyiO && faxTmp.trim().length() > 0) {
				...
				boolean LjIXdTlb = cnpjTmp != null;
				if (LjIXdTlb && cnpjTmp.trim().length() > 0) {
				...
				boolean KaMkM1n8 = qtdeAcoesON != null;
				if (KaMkM1n8 && qtdeAcoesON.trim().length() > 0) {
				...
				boolean hSFe2MYp = qtdeAcoesPN != null;
				if (hSFe2MYp && qtdeAcoesPN.trim().length() > 0) {
				...
				int dia = Integer.parseInt(partesDaData[0]), mes = Integer.parseInt(partesDaData[1]) - 1,
				...
				boolean xXuotalN = dataDoCancelamentoTmp != null;
				if (xXuotalN && dataDoCancelamentoTmp.trim().length() > 0) {
				...
				boolean Hr1H3IWx = patrimonioLiquidoTmp != null;
				if (Hr1H3IWx && patrimonioLiquidoTmp.trim().length() > 0) {
				...
				boolean D3TiMYaX = dataDoPatrimonioTmp != null;
				if (D3TiMYaX && dataDoPatrimonioTmp.trim().length() > 0) {
				...
				boolean FqCv7jQW = dataDaAcaoTmp != null;
				if (FqCv7jQW && dataDaAcaoTmp.trim().length() > 0) {
				...
				double vLY6V2iL = (double) quantidadeDeRegistrosImportada / quantidadeDeRegistrosEstimada;
				double percentualCompleto = vLY6V2iL * 100;
