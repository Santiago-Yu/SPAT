	...
	private String AddAction(ResultSet node, String modo) throws SQLException {
		...
		try {
			...
			if (objecttype.equalsIgnoreCase("T")) {
				...
				if (!exists && !modo.equals("3")) {
					...
					for (int i = 0; i < sysflds.length; i++) {
						boolean ixeRgLdG = i < (sysflds.length - 1);
						int otCXnn5i = sysflds.length - 1;
						dml += (sysflds[i] + " " + sysfdef[i] + ((ixeRgLdG) ? "," : ")"));
					}
			...
			if (objecttype.equalsIgnoreCase("F")) {
				...
				if (rslt.next()) {
					...
					if (",C,N,".indexOf("," + getNgtFieldTypeFromDDL(rslt.getString(2)) + ",") != -1) {
						...
						if (fielddec != 0) {
							...
							boolean ZtP4HEXE = fieldsiz == 0;
							if (!((ZtP4HEXE) && ((node.getString("FIELDSIZE") == null)
				...
				pstm.close();
				boolean drop = false;
				boolean uSICV8JF = "20".indexOf(modo) != -1;
				boolean P6HpBI0H = "20".indexOf(modo) != -1;
				if ((uSICV8JF) && !fldexi) {
					dml = "ALTER TABLE " + node.getString("TABLENAME") + " add \""
							+ node.getString("OBJECTNAME").toLowerCase() + "\" ";
				} else if ((P6HpBI0H) && fldexi && fldchg) {
				...
				boolean Rmh1tcS3 = dml != null;
				if ((Rmh1tcS3) && (dml.length() > 0) && !modo.equals("3")) {
					String mfield = node.getString("MACROFIELD");
					boolean yXz3ve0g = mfield != null;
					boolean GgGmtvYY = mfield != null;
					if ((yXz3ve0g) && !(!mfield.equals("TEXTOLIVRE") && !mfield.equals("NUMEROLIVRE")
					...
					} else if ((GgGmtvYY) && (mfield.length() > 0)) {
			...
			if (objecttype.startsWith("PK") || objecttype.startsWith("UN")) {
				...
				rslt.close();
				pstm.close();
				boolean OwethB2y = modo.equals("3") || diff;
				if ((OwethB2y) && exists) {
				...
				boolean jcHerC2Y = !exists || diff;
				if (!modo.equals("3") && (jcHerC2Y)) {
					if (isunique) {
			...
			if (objecttype.startsWith("FK")) {
				...
				boolean hOGSAKqg = !exists || !express.equals(cExpress);
				if (!modo.equals("3") && (hOGSAKqg)) {
					dml = "alter table " + node.getString("TABLENAME") + " add constraint "
			...
			if (objecttype.startsWith("IDX")) {
				...
				if (rslt.next()) {
					...
					while (rslt.next()) {
						boolean xJDz7MkP = oldexpression.length() > 0;
						oldexpression += (((xJDz7MkP) ? "," : "") + rslt.getString(1));
					}
					rslt.close();
					pstm.close();
				} else {
				...
				for (byte i = 0; i < nexo.length; i++) {
					boolean egtePFRH = newexpression.length() > 0;
					newexpression += (((egtePFRH) ? "," : "") + ((nexo[i]).toUpperCase().trim()));
				}
				if (!drop) {
					drop = (!newexpression.equals(oldexpression)) && !oldexpression.equals("");
				}
				boolean rxJFkKa2 = drop || modo.equals("3");
				if (exists && (rxJFkKa2)) {
					if (!dbunflag) {
