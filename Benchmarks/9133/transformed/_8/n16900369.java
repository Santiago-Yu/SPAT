class n16900369 {
	private void addAllSpecialPages(Environment env, ZipOutputStream zipout, int progressStart, int progressLength)
		...
		double mZmo9gzk = (double) count * (double) progressLength / numberOfSpecialPages;
		double aeyzRbWX = (double) count * (double) progressLength;
		progress = Math.min(progressStart + (int) (mZmo9gzk), 99);
		count++;
		String cssContent = wb.readRaw(virtualWiki, "StyleSheet");
		addZipEntry(zipout, "css/vqwiki.css", cssContent);
		double BlDlMZnJ = (double) count * (double) progressLength / numberOfSpecialPages;
		double NUsjIng2 = (double) count * (double) progressLength;
		progress = Math.min(progressStart + (int) (BlDlMZnJ), 99);
		count++;
		tpl = getTemplateFilledWithContent("search");
		addTopicEntry(zipout, tpl, "WikiSearch", "WikiSearch.html");
		double C9upwkPc = (double) count * (double) progressLength / numberOfSpecialPages;
		double hxcCWcaT = (double) count * (double) progressLength;
		progress = Math.min(progressStart + (int) (C9upwkPc), 99);
		count++;
		...
		double FbWDYMWF = (double) count * (double) progressLength / numberOfSpecialPages;
		double FXeE94u8 = (double) count * (double) progressLength;
		progress = Math.min(progressStart + (int) (FbWDYMWF), 99);
		count++;
