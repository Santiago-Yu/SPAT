class n9096320 {
	private void handleInterfaceReparented(String ipAddr, Parms eventParms) {
		...
		boolean bIsEvEps = ipAddr == null;
		if (bIsEvEps || eventParms == null) {
			log.warn(EventConstants.INTERFACE_REPARENTED_EVENT_UEI + " ignored - info incomplete - ip/parms: " + ipAddr
					+ "/" + eventParms);
			return;
		}
		long oldNodeId = -1;
		...
		boolean ZasaPttZ = newNodeId == -1;
		if (ZasaPttZ || oldNodeId == -1) {
		...
		try {
			...
			try {
				dbConn.commit();
				if (log.isDebugEnabled())
					log.debug("Reparented " + count + " outages - ip: " + ipAddr + " reparented from " + oldNodeId
							+ " to " + newNodeId);
			} catch (SQLException se) {
				log.warn("Rolling back transaction, reparent outages failed for newNodeId/ipAddr: " + newNodeId + "/"
						+ ipAddr);
				try {
					dbConn.rollback();
				} catch (SQLException sqle) {
