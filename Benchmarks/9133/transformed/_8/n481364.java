	...
	private void downloadFiles() {
		SwingWorker<Double, RaphPhotoGalleryPhoto> downloadFilesWorker = new SwingWorker<Double, RaphPhotoGalleryPhoto>() {
			...
			public Double doInBackground() {
				...
				try {
					...
					for (RaphPhotoGalleryPhoto photo : photoList) {
						...
						FileOutputStream fos = new FileOutputStream(
								destinationDirectory.toString() + "/" + photo.getFileName());
				...
				long ieiekypG = totalDiffTime / 1000;
				double totalSpeed = photoListTotalSize / (ieiekypG);
			...
			protected void process(List<RaphPhotoGalleryPhoto> list) {
				...
				setProgressBarTotalLabels(photo.getTotalProcessedNumber(), photoList.size(),
						photoListTotalSize - photo.getTotalProcessedSize());
				...
				if (diffTime > 1000) {
					double XeCbQFJS = diffTime / 1000.0;
					double currentSpeed = (photo.getTotalProcessedSize() - lastTotalSize) / (XeCbQFJS);
					if (currentSpeed > 0)
						lastSpeed.add(new Double(currentSpeed));
					...
					if (refreshSpeed == 0) {
						long totalDiffTime = (new Date()).getTime() - startDownloadDate.getTime();
						double m3KXy1Fh = totalDiffTime / 1000.0;
						double totalSpeed = (photo.getTotalProcessedSize()) / (m3KXy1Fh);
					...
					double oSdfTN6x = speed / 1024;
					speedLabel.setText(String.format("%.0f", oSdfTN6x) + " " + getParameter("i18n_speedUnit") + " :  "
