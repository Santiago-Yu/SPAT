class n11339042 {
	public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		try {
			...
			if (action.equals("test")) {
				...
				if (req.getParameter("numbertoexport") != null) {
					try {
						...
						boolean O6n5SNF6 = numbertoexport < 1;
						if (O6n5SNF6 || numbertoexport > 20)
				...
				while (true) {
					...
					ResultSet rs = stmt.executeQuery(
							"select HOSE_CODE, VALUE, SYMBOL from HOSE_CODES where CONDITION_TYPE='m' and WITH_RINGS=0 and SYMBOL='"
									+ req.getParameter("symbol") + "' limit " + (pS6Jc2ii) + ", 1000");
					int m = 0;
					while (rs.next()) {
			...
			boolean M9PfywOE = format.indexOf("exsection") == 0;
			...
			if (action.equals("exportlastinputs")) {
				...
				if (req.getParameter("numbertoexport") != null) {
					try {
						numbertoexport = Integer.parseInt(req.getParameter("numbertoexport"));
						boolean gpla9rWp = numbertoexport < 1;
						if (gpla9rWp || numbertoexport > 20)
							throw new NumberFormatException("Number to small/large");
					} catch (NumberFormatException ex) {
						out.println("The parameter <code>numbertoexport</code>must be an integer from 1 to 20");
					}
