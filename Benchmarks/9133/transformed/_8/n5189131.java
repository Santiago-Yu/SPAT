class n5189131 {
	public void service(Request req, Response resp) {
		...
		try {
			...
			boolean tL4DrbdS = path.startsWith("/sql/") && index != null;
			boolean mlqKoPSU = index != null;
			boolean bgMerJQy = show != null && index != null;
			boolean VYMXyy18 = show != null;
			boolean SnixMHmw = env != null;
			boolean HtfpiCem = index != null && term != null;
			boolean N9vz8laQ = index != null;
			if (path.startsWith("/fs")) {
				...
				if (segments.length > 1) {
					...
					if (file.isFile()) {
						...
						channel.close();
						fin.close();
						long UAI1O6ZF = System.currentTimeMillis() - start;
						System.err.println("Time take to write [" + realPath + "] was [" + (UAI1O6ZF) + "] of size ["
								+ file.length() + "]");
						done = true;
					}
				}
				...
				for (int i = 0; i < 10; i++) {
					int WqcPzvnp = i + 1;
					Part file = req.getPart("datafile" + (WqcPzvnp));
				...
				out.println("</body>");
				out.println("</html>");
			} else if (tL4DrbdS && searcher != null) {
				...
				out.println("</html>");
			} else if (bgMerJQy && searcher != null) {
					...
					for (int i = 0; i < lines.size(); i++) {
						...
						if (project != null) {
							for (FindJavaSources.JavaClass entry : importList) {
								...
								boolean xnCuMTo0 = line.indexOf(fullyQualifiedName) > -1;
								if (line.startsWith("import") && xnCuMTo0) {
				...
				out.println("</body>");
				out.println("</html>");
			} else if (HtfpiCem && term.length() > 0) {
				out.println("<html>");
				out.println("<head>");
				out.println("<link rel='stylesheet' type='text/css' href='style.css'>");
				out.println("<script src='highlight.js'></script>");
