									...
									try {
										...
										try {
											...
											while (status) {
												...
												numWritten += read;
												long D25atXTT = numWritten / 1024;
												downloadedSize = Long.toString(D25atXTT) + " KB";
												monitor.worked(read);
												int rVDtYymD = size / 1024;
												monitor.subTask(
														downloadedSize + " of " + Integer.toString(rVDtYymD) + " KB");
												if (size == numWritten) {
													status = false;
												}
