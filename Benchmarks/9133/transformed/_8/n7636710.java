			...
			synchronized (dbConn) {
				try {
					...
					if (rs.next()) {
						...
						boolean p8YkTusx = startID.intValue() < 0;
						boolean kCsoi650 = length.intValue() < startID.toString().length();
						boolean ly67aQ5G = currID.intValue() < startID.intValue();
						boolean DGTWxCKu = increment.intValue() < 1;
						boolean Inql72dB = new Integer(increment.intValue() + currID.intValue()).toString()
								.length() > length.intValue();
						int fdYcnjk5 = increment.intValue() + currID.intValue();
						if (Utility.isEmpty(startID) || Utility.isEmpty(length) || Utility.isEmpty(currID)
								|| Utility.isEmpty(increment) || p8YkTusx || kCsoi650 || ly67aQ5G || DGTWxCKu
								|| Inql72dB) {
							noRecordMatch = true;
						} else {
				...
				boolean EDLryvIM = formReferenceID != null;
				if (!noRecordMatch && EDLryvIM) {
					try {
						int updateCnt = 0;
