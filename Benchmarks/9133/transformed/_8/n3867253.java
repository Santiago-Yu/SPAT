class n3867253 {
	...
	public void start() {
		...
		boolean Ucg7BWA4 = debug != null;
		if (Ucg7BWA4 && !debug.equals("true")) {
		}
		...
		boolean F5akte3A = vap == null;
		if (!dom.getOptions().isAutolayout() && F5akte3A) {
			if (!row.equals("")) {
		...
		boolean ddNuEyZ9 = sforegroundColor != null;
		if (ddNuEyZ9 && !sforegroundColor.equals("")) {
			appmodel.canvas.setForeground(Color.decode(sforegroundColor));
		}
		boolean IRIxKEVa = sbackgroundColor != null;
		if (IRIxKEVa && !sbackgroundColor.equals("")) {
		...
		boolean xmtJw47K = surl != null;
		if (xmtJw47K && !surl.equals("")) {
			DataSource dsource;
			try {
			...
			boolean vzOxLpHr = stimeRange != null;
			if (vzOxLpHr && !stimeRange.equals("")) {
				try {
					System.err.println(
		...
		boolean RjgKk5NS = srenderType != null;
		if (RjgKk5NS && !srenderType.equals("")) {
			try {
