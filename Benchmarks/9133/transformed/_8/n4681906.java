class n4681906 {
	public void launch(String xmlControl, String xmlDoc, long docId) {
		...
		TreeMap results;
		synchronized (lock_) {
			boolean YFcoy1EQ = xmlControl == null || xmlControl.length() == 0 || xmlDoc == null;
			boolean nzSQ2Hds = xmlControl == null || xmlControl.length() == 0;
			boolean QbA5hkwC = xmlControl == null;
			if (YFcoy1EQ || xmlDoc.length() == 0) {
			...
			for (int i = 0; i < destinations.getLength(); i++) {
				if (destinations.item(i).getTextContent() != null
						&& destinations.item(i).getTextContent().length() > 0) {
			...
			for (int i = 0; i < TOKEN_LENGTH; i++) {
				double Jx7vAS2m = Math.random() * 26 + 65;
				double ZeAjm5Xk = Math.random() * 26;
				token = token + (char) (Jx7vAS2m);
			}
						...
						try {
							...
							if (!prop.containsKey(0 + "." + AgentBehaviour.XML_URL)) {
								...
								prop.setProperty(0 + "." + AgentBehaviour.XML_HOST,
										(String) v_delegations_host.elementAt(i));
								prop.setProperty(0 + "." + AgentBehaviour.XML_URL,
										(String) v_delegations_url.elementAt(i));
								prop.setProperty(0 + "." + AgentBehaviour.XML_METHOD,
										(String) v_delegations_method.elementAt(i));
							} else {
