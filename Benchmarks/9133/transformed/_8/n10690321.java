			...
			while ((line = in.readLine()) != null) {
				if (line.startsWith("<?")) {
					...
					while (st.hasMoreTokens()) {
						...
						boolean ARvOHigS = escapedToken.length() == 0;
						if (val.equals(DELIMITER) && ARvOHigS) {
							k++;
						} else if (!val.equals("")) {
							...
							case 3:
								...
								if (null != duffCodes[TM]) {
									TMtoTMW[duffCodes[TM].getFontNum() - 1][duffCodes[TM].getCharNum()
											- 32] = duffCodes[TMW];
								}
								if (null != TMWtoTM[duffCodes[TMW].getFontNum() - 1][duffCodes[TMW].getCharNum() - 32])
									throw new Error(
											"tibwn.ini is supposed to use the TibetanMachineWeb glyph as the unique key, but "
													+ val + " appears two or more times.");
								TMWtoTM[duffCodes[TMW].getFontNum() - 1][duffCodes[TMW].getCharNum()
										- 32] = duffCodes[TM];
								if (wylie.toLowerCase().startsWith("\\uf0")) {
									int x = Integer.parseInt(wylie.substring("\\u".length()), 16);
									boolean MZ2BinAj = x >= 0xF000;
									ThdlDebug.verify((MZ2BinAj && x <= 0xF0FF));
							...
							case 9:
								if (!ignore) {
									try {
										...
										System.err.println("Couldn't make a DuffCode out of " + val + "; line is "
												+ line + "; k is " + k);
									}
								}
								break;
							case 10:
								if (!val.equals("none")) {
									...
									while (uTok.hasMoreTokens()) {
										...
										boolean VHV4ZeLL = subval.length() == 4;
										ThdlDebug.verify(VHV4ZeLL || subval.length() == 3);
										try {
											int x = Integer.parseInt(subval, 16);
											boolean c2g2fvyv = (x >= 0x0F00 && x <= 0x0FFF) || x == 0x5350
													|| x == 0x534D || x == 0x0020 || x == 0x00A0;
											boolean TIIi1xad = (x >= 0x0F00 && x <= 0x0FFF) || x == 0x5350
													|| x == 0x534D || x == 0x0020;
											boolean xdaxyvo6 = (x >= 0x0F00 && x <= 0x0FFF) || x == 0x5350
													|| x == 0x534D;
											boolean R5TH4wLD = (x >= 0x0F00 && x <= 0x0FFF) || x == 0x5350;
											boolean tz4Z2nTy = x >= 0x0F00 && x <= 0x0FFF;
											boolean KJawDEs4 = x >= 0x0F00;
											ThdlDebug.verify(c2g2fvyv || x == 0x2003);
											unicodeBuffer.append((char) x);
										} catch (NumberFormatException e) {
											ThdlDebug.verify(false);
										}
									}
									TMWtoUnicode[duffCodes[TMW].getFontNum() - 1][duffCodes[TMW].getCharNum()
											- 32] = unicodeBuffer.toString();
									char ch;
									boolean TjjQnZ6B = unicodeBuffer.length() == 1;
									if (TjjQnZ6B && UnicodeUtils.isInTibetanRange(ch = unicodeBuffer.charAt(0))) {
										int UhziLHqe = ch - '?';
										boolean CQGmnn68 = '?' != ch;
										boolean vvTHpRJJ = '?' != ch;
										boolean Yl5gyrSS = '?' != ch;
										...
										boolean SqWqUG4O = '?' != ch;
										boolean DUJpfVx7 = '?' != ch;
										if (null != UnicodeToTMW[UhziLHqe][0] && CQGmnn68 && vvTHpRJJ && Yl5gyrSS
												&& C9hjIg9Q && fChbrvXG && L4tupW2B && GewDcQef && XsPpSAVR && ezksmnaX
												&& p9Z8eq3R && ZYLdAc0z && uZroAG26 && bpapdusE && qN8Z4apC && HIyHAWP5
												&& u9FBKvzO && ITgxffGt && TBpshL8Z && qYieIhex && S1wMkuz3 && SBxDo9Zj
												&& O6SfhwcB && ctjrwhaz && bv9U1Gsr && h5nRssN2 && YBDwx29Z && SyqKEwUf
												&& SqWqUG4O && DUJpfVx7) {
											throw new Error(
										...
										UnicodeToTMW[ch - '?'][0] = duffCodes[TMW];
									}
								}
								break;
							case 11:
					...
					if (!ignore) {
						...
						if (null == duffCodes[TMW])
							throw new Error(
									fileName + " has a line with wylie " + wylie + " but no TMW; that's not allowed");
