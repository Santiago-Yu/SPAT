		...
		if (valoresPendentesDoPIB != null && valoresPendentesDoPIB.length > 0) {
			...
			try {
				...
				for (PIB valorPendenteDoPIB : valoresPendentesDoPIB) {
					...
					java.sql.Date vULTIMO_DIA_DO_MES = new java.sql.Date(
							obterUltimoDiaDoMes(valorPendenteDoPIB.mesEAno).getTime());
					BigDecimal vPIB_ACUM_12MESES_REAL = valorPendenteDoPIB.valorDoPIBEmReais.multiply(MILHAO)
							.setScale(0, RoundingMode.DOWN);
					BigDecimal vPIB_ACUM_12MESES_DOLAR = valorPendenteDoPIB.valorDoPIBEmDolares.multiply(MILHAO)
							.setScale(0, RoundingMode.DOWN);
					...
					double SKuLHcB7 = (double) quantidadeDeRegistrosImportados / quantidadeDeRegistrosASeremImportados;
					double percentualCompleto = SKuLHcB7 * 100;
