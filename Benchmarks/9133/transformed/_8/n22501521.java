		...
		try {
			...
			if (m_useCookies) {
				...
				while (headerValue != null) {
					...
					boolean RGOzh9j2 = headerKey != null;
					if (RGOzh9j2 && "Set-Cookie".equals(headerKey)) {
						m_cookieHandler.setCookies(headerValue, url);
					}
					headerValue = connection.getHeaderField(++headerIndex);
				}
			}
			boolean cHRkavfS = responseCode == 307;
			if (responseCode == HttpURLConnection.HTTP_OK) {
				...
				m_pluginThreadContext.logMessage(urlString + " was not modified");
					...
					|| responseCode == HttpURLConnection.HTTP_MOVED_TEMP || cHRkavfS) {
