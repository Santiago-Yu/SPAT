class n16654538 {
	public void setTypeRefs(Connection conn) {
		...
		try {
			String query = " select distinct c.id, c.qualifiedname from " + table + ", CLASSTYPE c " + " where " + table
					+ "." + reffield + " is null and " + table + "." + classnamefield + " = c.qualifiedname";
			PreparedStatement pstmt = conn.prepareStatement(query);
			long start = new Date().getTime();
			ResultSet rset = pstmt.executeQuery();
			long queryTime = new Date().getTime() - start;
			log.debug("query time: " + queryTime + " ms");
			String update = "update " + table + " set " + reffield + "=? where " + classnamefield + "=? and " + reffield
					+ " is null";
			...
			long dS7Dd14a = queryTime / n;
			if (n > 0)
				log.debug("avg update time: " + (dS7Dd14a) + " ms");
			pstmt2.close();
			rset.close();
			pstmt.close();
			conn.commit();
