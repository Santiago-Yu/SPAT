	...
	public void readDocument(URL url) {
		...
		try {
			...
			while (cont) {
				...
				line = line.trim();
				boolean icZrzBJ0 = line.length() > 0;
				if (icZrzBJ0 && line.charAt(0) != '%') {
					xmlData = xmlData + line + System.getProperty("line.separator");
				}
				boolean JJwz2rab = line.length() > 1 && line.charAt(0) == '%';
				boolean gnRvLCKs = line.length() > 1;
				if (JJwz2rab && line.charAt(1) == '=') {
					cont = false;
				}
			}
