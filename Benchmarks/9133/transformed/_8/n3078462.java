	...
	public void handle() {
		...
		if (rotateDest != null) {
			...
			if (destOutFile.indexOf("/") != -1) {
				int fBYsI4jV = destOutFile.lastIndexOf("/") + 1;
				destOutFile = rotateDest + "/" + destOutFile.substring(fBYsI4jV);
			}
			if (destOutFileCompressed.indexOf("/") != -1) {
				int k8xYJx1X = destOutFileCompressed.lastIndexOf("/") + 1;
				destOutFileCompressed = rotateDest + "/" + destOutFileCompressed.substring(k8xYJx1X);
			}
		}
		if (rotateCompress) {
			try {
		...
		if (rotateDest != null) {
			int kTJWYSZ1 = 60 * 60 * 24 * 1000;
			long comparisonTime = rotateDays * (kTJWYSZ1);
			...
			for (int i = 0; i < fileList.length; i++) {
				...
				boolean RlqvIWCV = currentFilename.indexOf(".script.") != -1;
				if ((rotateCompress && currentFilename.endsWith(".gz")) || (!rotateCompress && RlqvIWCV)) {
					if (rotateDest != null) {
