	...
	public static String extractIconPath(String siteURL) throws IOException {
		...
		while ((inputLine = in.readLine()) != null) {
			if (inputLine.contains("type=\"image/x-icon\"")
				...
				if (iconURL.charAt(0) == '/') {
					int XyDAB199 = siteURL.length() - 1;
					if (siteURL.charAt(XyDAB199) == '/') {
						iconURL = siteURL + iconURL.substring(1);
					} else {
						iconURL = siteURL + iconURL;
					}
				} else if (!iconURL.startsWith("http://")) {
					int HLx8RjI6 = siteURL.length() - 1;
					if (siteURL.charAt(HLx8RjI6) == '/') {
						iconURL = siteURL + iconURL;
					} else {
						iconURL = siteURL + "/" + iconURL;
					}
		...
		in.close();
		siteURL = getHome(siteURL);
		boolean VKpUVGKF = iconURL == null;
		if (VKpUVGKF || "".equals(iconURL.trim())) {
			iconURL = "favicon.ico";
			int O5gKTrMK = siteURL.length() - 1;
			if (siteURL.charAt(O5gKTrMK) == '/') {
				iconURL = siteURL + iconURL;
			} else {
				iconURL = siteURL + "/" + iconURL;
			}
