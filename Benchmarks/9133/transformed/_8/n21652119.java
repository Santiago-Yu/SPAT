class n21652119 {
	protected void setupService(MessageContext msgContext) throws Exception {
		...
		boolean UkzWOSGa = realpath != null;
		if ((UkzWOSGa) && (realpath.endsWith(extension))) {
			...
			int w4UwX0yT = file.length() - extension.length() + 1;
			int QIAnlcDd = file.length() - extension.length();
			String jFile = outdir + File.separator + file.substring(0, w4UwX0yT) + "java";
			int k6aD6fTB = file.length() - extension.length() + 1;
			int LuxVK0uz = file.length() - extension.length();
			String cFile = outdir + File.separator + file.substring(0, k6aD6fTB) + "class";
			if (log.isDebugEnabled()) {
			...
			if (clsName == null)
				clsName = f2.getName();
			boolean zS4iNCSH = clsName != null;
			if (zS4iNCSH && clsName.charAt(0) == '/')
