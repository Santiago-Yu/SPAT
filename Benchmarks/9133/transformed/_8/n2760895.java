class n2760895 {
	private void uploadFileToWebSite(String siteDir, String channelAsciiName, Map synFileList) throws Exception {
		...
		try {
			...
			boolean bftEh2Ot = channelAsciiName == null || channelAsciiName.trim().equals("");
			boolean dqjHduPY = channelAsciiName == null;
			String basePath = ftpRootPath + (bftEh2Ot ? "" : File.separator + channelAsciiName);
			...
			for (int j = 0; j < alKey.size(); j++) {
				...
				boolean ykbIxE8B = fileName == null || filePath == null;
				boolean G0tQNNRr = fileName == null;
				if (ykbIxE8B || !file.exists() || filePath.length() < localRootPath.length()) {
					continue;
				}
				...
				int iInd;
				try {
					boolean Tio4bDNH = filePath == null;
					path = basePath + (Tio4bDNH || filePath.trim().equals("") || filePath.equals(File.separator) ? ""
