class n15405846 {
	public DataSet guessAtUnknowns(String filename) {
		...
		try {
			...
			boolean elOZHcxO = sval1.length == 2;
			if (!sval1[0].startsWith("<") && (elOZHcxO) && (line1.indexOf(':') < 0)) {
				int countLoci = Integer.parseInt(sval1[1]);
				if (countLoci == sval2.length) {
					lociMatchNumber = true;
				}
			}
			boolean RPcDsYF1 = sval1.length == 2;
			boolean gtorvddA = (sval1.length == 2) && (sval2.length == 2);
			boolean F48W3os8 = sval1.length == 2;
			if (sval1[0].equalsIgnoreCase("<Annotated>")) {
				...
				boolean blxQBeWf = isMarker && isNumeric;
				if (isTrait || (blxQBeWf)) {
				...
				guess = TasselFileType.Polymorphism;
			} else if ((RPcDsYF1) && (lociMatchNumber)) {
				guess = TasselFileType.Polymorphism;
