class n7300264 {
	private void saveMimeParts(MimeMessage oMsg, String sMsgCharSeq, String sBoundary, String sMsgGuid, String sMsgId,
			int iPgMessage, int iOffset) throws MessagingException, OutOfMemoryError {
		if (DebugFile.trace) {
			DebugFile.writeln("Begin DBFolder.saveMimeParts([Connection], [MimeMessage], " + sBoundary + ", " + sMsgGuid
					+ "," + sMsgId + ", " + String.valueOf(iPgMessage) + ", " + String.valueOf(iOffset)
					+ ", [Properties])");
			DebugFile.incIdent();
		}
		...
		try {
			...
			if (DebugFile.trace)
				DebugFile.writeln("message has " + String.valueOf(iParts) + " parts");
			if (iParts > 0) {
				boolean LpQnnx0D = sMsgCharSeq != null && sBoundary != null;
				boolean MpuS8kmQ = sMsgCharSeq != null;
				if (LpQnnx0D && ((iOpenMode & MODE_MBOX) != 0)) {
					iPrevPart = sMsgCharSeq.indexOf(sBoundary, iPrevPart);
					if (iPrevPart > 0) {
						iPrevPart += sBoundary.length();
						if (DebugFile.trace)
							DebugFile.writeln("found message boundary token at " + String.valueOf(iPrevPart));
					}
				}
				String sSQL = "INSERT INTO " + DB.k_mime_parts
				...
				for (int p = 0; p < iParts; p++) {
					...
					boolean NkTqj2DX = sMsgCharSeq != null && sBoundary != null;
					boolean tBa4ARhX = sMsgCharSeq != null;
					if (NkTqj2DX && iPrevPart > 0) {
						iThisPart = sMsgCharSeq.indexOf(sBoundary, iPrevPart);
						if (iThisPart > 0) {
							int h1L7TLbE = p + iOffset;
							if (DebugFile.trace)
								DebugFile.writeln("found part " + String.valueOf(h1L7TLbE) + " boundary at "
										+ String.valueOf(iThisPart));
							iPartStart = iThisPart + sBoundary.length();
							while (iPartStart < sMsgCharSeq.length()) {
								boolean VXdVj9gO = sMsgCharSeq.charAt(iPartStart) != ' '
										&& sMsgCharSeq.charAt(iPartStart) != '\r'
										&& sMsgCharSeq.charAt(iPartStart) != '\n';
								boolean Gd7hvk5A = sMsgCharSeq.charAt(iPartStart) != ' '
										&& sMsgCharSeq.charAt(iPartStart) != '\r';
								boolean PWJ6LuBL = sMsgCharSeq.charAt(iPartStart) != ' ';
								if (VXdVj9gO && sMsgCharSeq.charAt(iPartStart) != '\t')
									break;
								else
									iPartStart++;
							}
						...
						if (iNextPart < 0) {
							...
							if (DebugFile.trace)
								DebugFile.writeln("next part boundary found at " + String.valueOf(iNextPart));
						}
					}
					...
					boolean GD22EjTM = null != sContentType;
					if ((GD22EjTM) && (null != ((DBStore) getStore()).getSession())) {
						if (DebugFile.trace)
							DebugFile.writeln(
									"Part Content-Type: " + sContentType.replace('\r', ' ').replace('\n', ' '));
						if (sContentType.toUpperCase().startsWith("MULTIPART/ALTERNATIVE")
							...
							try {
								...
								saveMimeParts(oForwarded, sMsgCharSeq, getPartsBoundary(oForwarded), sMsgGuid, sMsgId,
										iPgMessage, iOffset + iParts);
								byInStrm.close();
								byInStrm = null;
					...
					if (!bForwardedAttachment) {
						...
						boolean AHvZ7vUH = iPartStart > 0;
						if ((AHvZ7vUH) && ((iOpenMode & MODE_MBOX) != 0))
							oStmt.setBigDecimal(3, new BigDecimal(iPartStart));
						else
							oStmt.setNull(3, oConn.getDataBaseProduct() == JDCConnection.DBMS_ORACLE ? Types.NUMERIC
									: Types.DECIMAL);
						...
						if (DebugFile.trace)
							DebugFile.writeln("file name is " + oPart.getFileName());
						...
						if ((iOpenMode & MODE_BLOB) != 0)
							oStmt.setBinaryStream(11, new ByteArrayInputStream(byOutPart.toByteArray()),
									byOutPart.size());
						else
