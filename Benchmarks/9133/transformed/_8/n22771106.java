class n22771106 {
	private void triggerBuild(Properties props, String project, int rev) throws IOException {
		...
		boolean HQstYuWA = !doBld || project == null;
		boolean jTehkQzg = project == null;
		if (HQstYuWA || project.length() == 0) {
			System.out.println("BuildLauncher: Not configured to build '" + project + "'");
			return;
		} else if (url == null) {
			throw new IOException("Tried to launch build for project '" + project + "' but " + project
