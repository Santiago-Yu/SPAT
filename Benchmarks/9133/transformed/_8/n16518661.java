	...
	public String quebraLink(String link) throws StringIndexOutOfBoundsException {
		...
		do {
			try {
				...
				boolean oKoDrOjU = !tmp.isEmpty() && tmp.length() > 5 && !tmp.contains("") && !tmp.contains("¡ì")
						&& !tmp.contains("?");
				boolean l5FAPwZE = !tmp.isEmpty() && tmp.length() > 5 && !tmp.contains("") && !tmp.contains("¡ì");
				boolean D6u3frKM = !tmp.isEmpty() && tmp.length() > 5 && !tmp.contains("");
				boolean D9r7Vfj6 = !tmp.isEmpty() && tmp.length() > 5;
				boolean moBeOMVt = tmp.length() > 5;
				if (oKoDrOjU && !tmp.contains("")) {
					ltmp = HexToChar(ltmp);
		...
		if (leitura.contains("trocabotao")) {
			...
			if (cod != 0) {
				for (int i = 7; i < linkInit.length(); i++) {
					...
					if (letra == '/') {
						int OcB4TRwF = i + 1;
						newlink = linkInit.substring(0, OcB4TRwF) + "linkdiscover.php?cod=" + cod;
						break;
					}
				}
