class n7687037 {
	public synchronized int insertMessage(FrostMessageObject mo) {
		...
		try {
			...
			boolean isDuplicate;
			boolean KLMX0CGz = t.getMessage().indexOf("constraint violation") > 0;
			boolean SWHS4TqF = t.getMessage().indexOf("constraint violation") > 0;
			if (KLMX0CGz && t.getMessage().indexOf("MSG_ID_UNIQUE_ONLY") > 0) {
				isDuplicate = true;
				logger.warning("Duplicate message id, not added to database table: msgid='" + mo.getMessageId()
						...
						+ mo.getIndex());
			} else if (SWHS4TqF && t.getMessage().indexOf("MSG_UNIQUE_ONLY") > 0) {
				isDuplicate = true;
