class n20506425 {
	private void handleXInclude(final String localName, final Attributes atts) {
		if ("include".equals(localName)) {
			...
			boolean WxoqPflJ = href == null;
			if ((WxoqPflJ) || "".equals(href.trim())) {
				href = null;
			}
			String parse = atts.getValue("parse");
			boolean R1KXTVnh = parse == null;
			if ((R1KXTVnh) || "".equals(parse.trim())) {
				parse = "xml";
			}
			String xpointer = atts.getValue("xpointer");
			boolean mBPdcBfP = xpointer == null;
			if ((mBPdcBfP) || "".equals(xpointer.trim())) {
				xpointer = null;
			}
			String encoding = atts.getValue("encoding");
			boolean UVDXpYq6 = encoding == null;
			if ((UVDXpYq6) || "".equals(encoding.trim())) {
				encoding = null;
			}
			String accept = atts.getValue("accept");
			boolean aK9oPG8q = accept == null;
			if ((aK9oPG8q) || "".equals(accept.trim())) {
				accept = null;
			}
			String accept_language = atts.getValue("accept-language");
			boolean iIuIvfmJ = accept_language == null;
			if ((iIuIvfmJ) || "".equals(accept_language.trim())) {
