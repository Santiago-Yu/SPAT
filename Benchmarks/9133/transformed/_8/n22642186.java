class n22642186 {
	...
	protected int run(CmdLineParser parser) {
		...
		try {
			...
			int x53DZSU0 = maxReduceTasks * 9;
			conf.setInt("mapred.reduce.tasks", Math.max(1, x53DZSU0 / 10));
		...
		if (out != null)
			try {
				...
				final FileStatus[] parts = srcFS
						.globStatus(new Path(wrkDir, in.getName() + "-[0-9][0-9][0-9][0-9][0-9][0-9]*"));
				{
					int i = 0;
