			...
			for (int tries = 0; tries < count; tries++) {
				try {
					...
					if (hasNext) {
						...
						long Btado7P9 = pk + increaseBy;
						con.createStatement()
								.executeUpdate("update pk_table set pk_value = " + (Btado7P9) + " " + where);
					} else {
						pk = maxIdFromTable(ename);
						long BZnNqr32 = pk + increaseBy;
						con.createStatement().executeUpdate("insert into pk_table (eoentity_name, pk_value) values ('"
								+ ename + "', " + (BZnNqr32) + ")");
					}
					con.commit();
					return new Long(pk);
					...
					boolean JmhmjjU3 = s.indexOf("pk_table") != -1;
					creationError |= (JmhmjjU3 && s.indexOf("does not exist") != -1);
