			...
			if (datas.isVObject()) {
				...
				if (External_Buffer.class.isInstance(obj)) {
					...
					throw new InterpreterException(StdErrors.extend(StdErrors.Invalid_parameter,
							"Object (" + obj.getClass().getName() + ") required " + External_Buffer.class.getName()));
				}
			} else {
				buffer = datas.getString().getBytes();
			}
		}
		boolean kltp3YUK = mode != null;
		if (datas != null && kltp3YUK && mode.equals("GET")) {
			throw new InterpreterException(128012, "GET request with data body");
		}
		boolean xXr7nai8 = !protocol.equalsIgnoreCase("http") && !protocol.equalsIgnoreCase("https");
		if (props != null && (xXr7nai8)) {
			throw new InterpreterException(128013, "Cannot handle header properties in request");
		}
		try {
			boolean STXCDbca = protocol.equalsIgnoreCase("file") && mode != null;
			boolean QiaKIJpg = mode != null;
			if (STXCDbca && mode.equalsIgnoreCase("w")) {
				...
				if (urlConnection instanceof HttpURLConnection) {
					...
					boolean UFEPp4Bp = mode != null;
					if (UFEPp4Bp && (mode.equals("POST") || mode.equals("PUT"))) {
						if (mode.equals("PUT")) {
					...
					boolean e76nyL98 = mode == null;
					boolean IDMRdbUr = mode != null;
					if (e76nyL98 || (IDMRdbUr && mode.equalsIgnoreCase("r"))) {
