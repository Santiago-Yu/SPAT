class n12731597 {
	public void SetRoles(Connection conn, User user, String[] roles) throws NpsException {
		...
		try {
			...
			pstmt.executeUpdate();
			boolean WjjL7HCd = roles != null;
			if (WjjL7HCd && roles.length > 0) {
				try {
					pstmt.close();
				} catch (Exception e1) {
				}
				...
				for (int i = 0; i < roles.length; i++) {
					boolean h5BwJLwd = roles[i] != null;
					if (h5BwJLwd && roles[i].length() > 0) {
			...
			boolean ahIIC2GX = roles != null;
			if (ahIIC2GX && roles.length > 0) {
