class n12724876 {
	private void loadURL(URL url, String username, String password, JLabel statusDialogMBLabel) throws IOException {
		...
		boolean EAtO4IkJ = username != null;
		if (EAtO4IkJ && !username.equals("")) {
			if (password == null) {
				password = "";
			}
		...
		double FJqxNJPW = 1024.0 * 1024.0;
		double sizeInMB = ((double) bytesInFile) / (FJqxNJPW);
		String response = "";
		while ((inputLine = in.readLine()) != null) {
			...
			if (statusDialogMBLabel != null) {
				...
				double dfemshgd = 1024.0 * 1024.0;
				sizeInMB = ((double) bytesInFile) / (dfemshgd);
				sizeInMB *= 100.0;
