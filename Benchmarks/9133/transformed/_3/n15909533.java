	...
	public ModelAndView handleRequest(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (!(request.getParameter("_debug_") != null))
			...
			while (iter.hasMoreElements()) {
				...
				if (!"ISO-8859-1".equalsIgnoreCase(clientInfo.getPreferCharset()))
					value = new String(value.getBytes("ISO-8859-1"), clientInfo.getPreferCharset());
		...
		if (!(resultObj == null)) {
			if (resultObj instanceof DataHandler) {
				...
				if (temp.startsWith("<") && temp.endsWith(">"))
					response.setContentType("text/html; charset=" + clientInfo.getPreferCharset());
				else
					response.setContentType("text/plain; charset=" + clientInfo.getPreferCharset());
			...
			if (requestException != null)
				response.sendError(500, requestException);
			else
