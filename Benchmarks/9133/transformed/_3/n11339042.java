		...
		try {
			...
			if (!(action.equals("test"))) {
				if (action.equals("rss")) {
					...
					JasperReport jasperReport = (JasperReport) JRLoader.loadObject(relativepath + "/reports/"
							+ (yearly ? "yearly" : "monthly") + "_report_" + type + ".jasper");
					...
					if (type == 1) {
						rs = stmt.executeQuery("select YEAR(DATE) as YEAR, " + (yearly ? "" : " MONTH(DATE) as MONTH, ")
								...
								+ " and LOGIN_NAME<>'testuser' group by YEAR, " + (yearly ? "" : "MONTH, ")
						...
						if (i % 500 == 0) {
							...
							zipout.write(new String(
									"<html><body><h1>This is a list of strcutures in <a href=\"http://www.nmrshiftdb.org\">NMRShiftDB</a>, starting at "
											+ i + ", Its main purpose is to be found by search engines</h1>")
													.getBytes());
						}
