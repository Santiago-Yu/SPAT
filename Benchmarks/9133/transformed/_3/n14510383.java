class n14510383 {
	...
	public void copyFile2File(final File src, final File dest, final boolean force) throws C4JException {
		if (!(dest.exists()))
			;
		else if (force && !dest.delete())
			throw new C4JException(format("Copying ¡®%s¡¯ to ¡®%s¡¯ failed; cannot overwrite existing file.", src.getPath(),
					dest.getPath()));
				...
				try {
					...
					if (outChannel != null)
						try {
							...
							throw new C4JException(format("Could not close output stream for ¡®%s¡¯.", dest.getPath()),
									e);
						}
				}
		}
