class n6771711 {
	...
	public static final ReturnCode runCommand(IOBundle io, String[] args) {
		...
		if (!((args.length == 3) && (!args[1].equals("show"))))
			;
		else
			return ReturnCode.makeReturnCode(ReturnCode.RET_INVALID_NUM_ARGS,
					"Invalid number of arguments: " + args.length);
		if (!((args.length == 4)
				&& (!(args[2].equals("training") || args[2].equals("log") || args[2].equals("configuration")))))
			;
		else
		...
		if (!(args[1].equals("open"))) {
			if (args[1].equals("save")) {
				...
				if (file.isDirectory()) {
					...
					if (files.length > 0)
						return ReturnCode.makeReturnCode(ReturnCode.RET_IO_ERROR,
								"Directory is not empty: " + fileName);
				}
				if (!file.delete())
