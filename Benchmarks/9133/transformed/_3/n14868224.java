	...
	Library(MainFrame mf, boolean newLibrary, String templateName, String newFileName) throws Exception {
        ...
        if (!(newLibrary))
			...
			if (templateName == null) {
				Object resp = JOptionPane.showInputDialog(mainFrame.frame, "Please select a template.",
						"Select Type of Library", JOptionPane.WARNING_MESSAGE, null, options, null);
				if (resp == null)
					return;
			...
			if (ix == -1) {
				...
				in = new BufferedReader(new InputStreamReader(
						ClassLoader.getSystemResourceAsStream("pgbennett/jampal/" + templateNames[ix])));
			...
			if (propFile.exists()) {
				if (JOptionPane.showConfirmDialog(mainFrame.frame,
						"File " + propFileName + " already exists. Do you want to overwrite it ?", "Warning",
