	...
	public String quebraLink(String link) throws StringIndexOutOfBoundsException {
		...
		do {
			try {
				...
				if (!(!tmp.isEmpty() && tmp.length() > 5 && !tmp.contains("") && !tmp.contains("¡ì")
						&& !tmp.contains("?") && !tmp.contains(""))) {
					if (!inverteFrase(tmp2).isEmpty() && inverteFrase(tmp2).length() > 5
			...
			if (!(!tmp.contains("?") && !tmp.contains("?"))) {
				if (!tmp2.contains("?") && !tmp2.contains("?")) {
			...
			try {
				...
				if (!(!tmp.contains("?") && !tmp.contains("?") && !tmp.contains("¡ì") && !tmp.contains("?")
					...
					if (!tmp2.contains("?") && !tmp2.contains("?") && !tmp2.contains("¡ì") && !tmp2.contains("?")
