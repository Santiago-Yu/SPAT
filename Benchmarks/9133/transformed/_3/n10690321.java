					...
					if (line.startsWith("//")) {
						...
						while (st.hasMoreTokens()) {
							...
							if (val.equals(DELIMITER) && escapedToken.length() == 0) {
								...
								case 3:
									...
									if (null != TMWtoTM[duffCodes[TMW].getFontNum() - 1][duffCodes[TMW].getCharNum()
											- 32])
										throw new Error(
									...
									if (!val.equals("none")) {
										...
										while (uTok.hasMoreTokens()) {
											...
											try {
												...
												ThdlDebug.verify((x >= 0x0F00 && x <= 0x0FFF) || x == 0x5350
														|| x == 0x534D || x == 0x0020 || x == 0x00A0 || x == 0x2003);
						...
						if (!ignore) {
							...
							if (null == duffCodes[TMW])
								throw new Error(fileName + " has a line with wylie " + wylie
