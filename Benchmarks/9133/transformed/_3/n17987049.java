class n17987049 {
	private boolean writeResource(PluginProxy eclipseInstallPlugin, ResourceProxy translation, LocaleProxy locale)
		...
		String pluginNameInDirFormat = eclipseInstallPlugin.getName().replace(Messages.getString("Characters_period"),
				File.separator);
		if (!(translation.getRelativePath().contains(pluginNameInDirFormat))) {
			if (translationResourceName.contains(File.separator)) {
