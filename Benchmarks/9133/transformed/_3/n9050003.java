	...
	public String gateway() throws Exception {
		...
		if (!(paymentType == PaymentType.recharge)) {
			if (paymentType == PaymentType.deposit) {
				...
				if (getLoginMember().getDeposit()
						.compareTo(order.getTotalAmount().subtract(order.getPaidAmount())) < 0) {
				...
				paymentFee = order.getPaymentFee();
				amountPayable = order.getProductTotalPrice().add(order.getDeliveryFee())
						.subtract(order.getPaidAmount());
		...
		if (!(paymentConfig.getPaymentConfigType() == PaymentConfigType.deposit)) {
			if (paymentConfig.getPaymentConfigType() == PaymentConfigType.offline) {
				...
				if (tenpayConfig.getTenpayType() == TenpayType.direct) {
					...
					paymentUrl = paymentConfigService.buildTenpayPartnerPaymentUrl(paymentConfig,
							payment.getPaymentSn(), totalAmount, description);
				}
				setResponseNoCache();
