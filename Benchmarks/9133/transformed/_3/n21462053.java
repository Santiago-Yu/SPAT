	...
	public int deleteRecord(Publisher publisher, MmdQueryCriteria criteria) throws Exception {
		...
		try {
			...
			if (!(database.contains("mysql"))) {
				...
				sbData.append(" DELETE ").append(getResourceDataTableName()).append(" FROM ")
						.append(getResourceDataTableName());
				sbData.append(" LEFT JOIN ").append(getResourceTableName());
				sbData.append(" ON ").append(getResourceDataTableName()).append(".ID=").append(getResourceTableName())
