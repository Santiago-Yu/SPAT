			...
			if (action.equals("edit_event")) {
				...
				if (event.deleteEvent(id)) {
					...
					RequestDispatcher dispatcher = getServletContext()
							.getRequestDispatcher("/admin?action=edit_events");
					dispatcher.forward(request, response);
					return;
				} else {
				...
				Hospital new_hospital = new Hospital(fname, lname, address1, address2, city, state, zip, email,
						password, is_admin, name, phone, url);
				...
				String zip = request.getParameter("zip");
				String email = request.getParameter("email");
				String start_date = request.getParameter("start_year") + "-" + request.getParameter("start_month")
						+ "-01";
				String graduation_date = "";
				if (!request.getParameter("grad_year").equals("") && !request.getParameter("grad_month").equals("")) {
					graduation_date = request.getParameter("grad_year") + "-" + request.getParameter("grad_month")
							+ "-01";
				}
				String password = "";
				if (request.getParameter("password") != null && !request.getParameter("password").equals("")) {
				...
				if (dbResultSet != null) {
					...
					request.setAttribute("result", dbResultSet);
					RequestDispatcher dispatcher = getServletContext()
							.getRequestDispatcher("/admin/edit_residency.jsp");
					dispatcher.forward(request, response);
					return;
				} else {
				...
				if (person.isAdmin()) {
					...
					String deadline_date = deadline_year + deadline_month + deadline_day;
					int hId = Integer.parseInt(request.getParameter("hId"));
					Residency residency = new Residency(job_id, title, start_date, end_date, deadline_date,
							description);
				...
				if (person.isAdmin()) {
					...
					RequestDispatcher dispatcher = getServletContext()
							.getRequestDispatcher("/admin?action=new_residency");
					dispatcher.forward(request, response);
					return;
				} else {
				...
				if (mentor.delMentorship(mentorship_id)) {
					...
					request.setAttribute("notice", notice);
					RequestDispatcher dispatcher = getServletContext()
							.getRequestDispatcher("/admin?action=manage_mship");
					dispatcher.forward(request, response);
					return;
				} else {
