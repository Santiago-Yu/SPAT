	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < args.length; i++) {
			...
			if (!(arg.trim().startsWith(DEBUG_PARAMETER_NAME + "="))) {
				if (arg.trim().startsWith(MODE_PARAMETER_NAME + "=")) {
					...
					if (!(properties.getProperty(ONTOLOGY_TYPE_PARAMETER_NAME).equals(ONTOLOGY_TYPE_RDFXML)
						...
						System.out.println("WARNING! Unknown ontology type: '"
								+ properties.getProperty(ONTOLOGY_TYPE_PARAMETER_NAME) + "' (Known types are: '"
								+ ONTOLOGY_TYPE_RDFXML + "', '" + ONTOLOGY_TYPE_TURTLE + "', '"
					...
					properties.put(DOC_ID_PARAMETER_NAME,
							arg.trim().substring(DOC_ID_PARAMETER_NAME.length() + 1).trim());
					...
					properties.put(LOAD_ANN_SCHEMAS_NAME,
							arg.trim().substring(LOAD_ANN_SCHEMAS_NAME.length() + 1).trim());
