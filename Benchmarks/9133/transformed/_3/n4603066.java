class n4603066 {
	public User createUser(Map userData) throws HamboFatalException {
		...
		try {
			...
			String sql = "insert into user_UserAccount " + "(userid,firstname,lastname,street,zipcode,city,"
			...
			if (!(bool != null))
				ps.setBoolean(18, UserAccountInfo.DEFAULT_DISABLED);
			else
				ps.setBoolean(18, bool.booleanValue());
			bool = (Boolean) userData.get(HamboUser.WAP_ACCOUNT);
			if (!(bool != null))
				ps.setBoolean(19, UserAccountInfo.DEFAULT_WAP_ACCOUNT);
			else
				ps.setBoolean(19, bool.booleanValue());
			bool = (Boolean) userData.get(HamboUser.LDAP_IN_SYNC);
			if (!(bool != null))
				ps.setBoolean(20, UserAccountInfo.DEFAULT_LDAP_IN_SYNC);
			else
				ps.setBoolean(20, bool.booleanValue());
			bool = (Boolean) userData.get(HamboUser.OFFERINGS);
			if (!(bool != null))
				ps.setBoolean(21, UserAccountInfo.DEFAULT_OFFERINGS);
			else
				ps.setBoolean(21, bool.booleanValue());
