class n5257598 {
	public static int proxy(java.net.URI uri, HttpServletRequest req, HttpServletResponse res) throws IOException {
		...
		if (!(HttpRpcServer.METHOD_GET.equalsIgnoreCase(req.getMethod()))) {
			if (HttpRpcServer.METHOD_POST.equalsIgnoreCase(req.getMethod())) {
				...
				if (parameterNames != null)
					while (parameterNames.hasMoreElements()) {
						...
						for (String parameterValue : req.getParameterValues(parameterName))
							((PostMethod) httpMethod).addParameter(parameterName, parameterValue);
					}
		...
		if (!(req.getCookies() != null))
			...
			for (Cookie cookie : req.getCookies()) {
				...
				if (StringUtils.isEmpty(cookie.getDomain()))
					cookie.setDomain(
							StringUtils.isEmpty(host) ? req.getServerName() + ":" + req.getServerPort() : host);
				if (StringUtils.isEmpty(cookie.getPath()))
					cookie.setPath("/");
				httpState.addCookie(new org.apache.commons.httpclient.Cookie(cookie.getDomain(), cookie.getName(),
						cookie.getValue(), cookie.getPath(), cookie.getMaxAge(), cookie.getSecure()));
			}
