			...
			if (!(path.startsWith("/fs"))) {
				if (path.startsWith("/files/")) {
					...
					if (authentication == null) {
						...
						Repository repository = Subversion.login(Scheme.HTTP, principal.getName(),
							...
							if (project != null) {
								for (FindJavaSources.JavaClass entry : importList) {
									...
									if (line.startsWith("import") && line.indexOf(fullyQualifiedName) > -1) {
										...
										File classFile = entry.getSourceFile();
										String url = getRelativeURL(root, classFile);
										escaped = escaped.replaceAll("\\s" + className + ",",
												" <a href='/?show=" + url + "&index=" + index + "&searcher=" + searcher
														+ "'>" + className + "</a>,");
										escaped = escaped.replaceAll("\\s" + className + "\\{",
												" <a href='/?show=" + url + "&index=" + index + "&searcher=" + searcher
										...
										escaped = escaped.replaceAll("," + className + "\\{",
												",<a href='/?show=" + url + "&index=" + index + "&searcher=" + searcher
														+ "'>" + className + "</a>{");
										escaped = escaped.replaceAll("\\s" + className + "\\s",
												" <a href='/?show=" + url + "&index=" + index + "&searcher=" + searcher
														+ "'>" + className + "</a> ");
										...
										escaped = escaped.replaceAll("\\(" + className + "\\(",
												"(<a href='/?show=" + url + "&index=" + index + "&searcher=" + searcher
														+ "'>" + className + "</a>(");
										escaped = escaped.replaceAll("&gt;" + className + ",",
												"&gt;<a href='/?show=" + url + "&index=" + index + "&searcher="
														+ searcher + "'>" + className + "</a>,");
										escaped = escaped.replaceAll("&gt;" + className + "\\s",
												"&gt;<a href='/?show=" + url + "&index=" + index + "&searcher="
														+ searcher + "'>" + className + "</a> ");
										escaped = escaped.replaceAll("&gt;" + className + "&lt;",
						...
						if (project != null) {
							...
							for (FindJavaSources.JavaClass entry : importList) {
								...
								out.println("import <a href='/?show=" + url + "&index=" + index + "&searcher="
										+ searcher + "'>" + entry.getFullyQualifiedName() + "</a> as "
										+ entry.getClassName());
							}
							out.println("</pre>");
						}
						if (storedProc != null) {
							...
							for (String procName : storedProc.getReferences()) {
								...
								if (proc != null) {
									...
									out.println("using <a href='/?show=" + url + "&index=" + index + "&searcher="
											+ searcher + "'>" + proc.getName() + "</a>");
								}
							}
							out.println("</pre>");
						}
					...
					for (Searchable value : list) {
						...
						for (String token : tokens) {
							text = text.replaceAll(token,
									"<font style='BACKGROUND-COLOR: yellow'>" + token + "</font>");
						}
						out.println(text);
