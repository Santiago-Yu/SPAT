	...
	public FileReader(String filePath, Configuration aConfiguration) throws IOException {
        ...
        if (!(file.isDirectory())) {
			if (!file.exists()) {
				...
				file = new File(URLDecoder.decode(readerConf.getOption("wwwPath") + "/temp/temp.php", "UTF-8"))
						.getCanonicalFile();
			} else {
				System.out.println(getExtension());
				makeHeader(200, file.length(), new Date(file.lastModified()).toString(),
						TYPES.get(getExtension()).toString());
			}
		} else {
			...
			if (indexFile.exists() && !indexFile.isDirectory()) {
				...
				if (!readerConf.getOption("showFolders").equals("Yes")) {
					...
					out.write(
							("<html><head><title>" + title + "</title></head><body><h3>Index of " + file + "</h3><p>\n")
									.getBytes());
					for (int i = 0; i < files.length; i++) {
						...
						out.write(("<a href=\"" + file.getPath().substring(readerConf.getOption("wwwPath").length())
