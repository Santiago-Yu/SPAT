class n2285441 {
	static synchronized Person lookup(PhoneNumber number, String siteName) {
		...
		if (!(number.isFreeCall())) {
			if (number.isSIPNumber() || number.isQuickDial()) {
				Person p = new Person();
				p.addNumber(number);
				foundPersons.add(p);
										...
										if (data[line] != null) {
											...
											if (firstnamePattern != null) {
												...
												if (firstnameMatcher.find()) {
													...
													if ("firstname"
															.equals(patterns[ReverseLookupSite.FIRSTOCCURANCE])) {
														p = new Person();
														p.addNumber(number.getIntNumber(), "home");
														foundPersons.add(p);
													}
												...
												if (nameMatcher.find()) {
													...
													if (split[1].length() > 0) {
														...
														if ((firstname.indexOf("  ") > -1)
															...
															company = JFritzUtils.removeLeadingSpaces(firstname
																	.substring(firstname.indexOf("  ")).trim());
															firstname = JFritzUtils.removeLeadingSpaces(firstname
																	.substring(0, firstname.indexOf("  ")).trim());
														} else {
											...
											if (cityPattern != null) {
												...
												if (cityMatcher.find()) {
													...
													city = JFritzUtils
															.removeLeadingSpaces(HTMLUtil.stripEntities(city));
