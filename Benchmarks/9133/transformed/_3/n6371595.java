	...
	public void setUserPassword(final List<NewUser> users) {
		try {
			...
			while (iter.hasNext()) {
				...
				encrypted.add(new JESRealmUser(null, jesRealmUser.userId, jesRealmUser.realmId, jesRealmUser.domainId,
						PasswordFactory.getInstance().getPasswordHasher().hashRealmPassword(
								jesRealmUser.username.toLowerCase(locale),
								realm.getFullRealmName().equals("null") ? "" : realm.getFullRealmName(),
