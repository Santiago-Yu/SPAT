class n13514558 {
	public void load(String filename) throws VisbardException {
		...
		try {
			...
			sLogger.info("Unable to load custom settings from user's settings directory (" + fnf.getMessage()
			...
			try {
				...
				int c;
				while ((c = settingsInStreamFromJar.read()) != -1)
					settingsOutStream.write(c);
				settingsInStreamFromJar.close();
				settingsOutStream.close();
				settingsInStreamFromFile = new FileInputStream(defaultFile);
			} catch (IOException ioe) {
				sLogger.warn("Unable to copy default settings to user's settings directory (" + ioe.getMessage()
