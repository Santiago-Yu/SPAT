		...
		try {
			...
			for (int i = 0; i < this.getCompileTarget().length; i++) {
				...
				for (Iterator it = this.getProject().getCompileSourceRoots().iterator(); it.hasNext()
						&& moduleFile == null;) {
					File check = new File(
							it.next().toString() + "/" + this.getCompileTarget()[i].replace('.', '/') + ".gwt.xml");
				...
				for (Iterator it = this.getProject().getResources().iterator(); it.hasNext();) {
					...
					File check = new File(
							r.getDirectory() + "/" + this.getCompileTarget()[i].replace('.', '/') + ".gwt.xml");
					getLog().debug("Looking for file: " + check.getAbsolutePath());
					if (check.exists()) {
						moduleFile = check;
					}
				...
				try {
					if (moduleFile != null) {
						...
						processor = new GwtWebInfProcessor(this.getCompileTarget()[i], moduleFile,
