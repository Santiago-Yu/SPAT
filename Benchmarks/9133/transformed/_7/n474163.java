	...
	public static XMLShowInfo NzbSearch(TVRageShowInfo tvrage, XMLShowInfo xmldata, int latestOrNext) {
		...
		if (latestOrNext == 0) {
			if (xmldata.searchBy.equals("ShowName Season x Episode"))
				newzbin_query = newzbin_query + (tvrage.latestSeasonNum + "x" + tvrage.latestEpisodeNum);
			else if (xmldata.searchBy.equals("Showname SeriesNum"))
				newzbin_query = newzbin_query + (tvrage.latestSeriesNum);
			else if (xmldata.searchBy.equals("Showname YYYY-MM-DD")) {
				try {
					Date airTime = tvRageDateFormat.parse(tvrage.latestAirDate);
					newzbin_query = newzbin_query + (tvRageDateFormatFix.format(airTime));
				} catch (ParseException e) {
					e.printStackTrace();
				}
			} else if (xmldata.searchBy.equals("Showname EpisodeTitle"))
				newzbin_query = newzbin_query + (tvrage.latestTitle);
		} else {
			if (xmldata.searchBy.equals("ShowName Season x Episode"))
				newzbin_query = newzbin_query + (tvrage.nextSeasonNum + "x" + tvrage.nextEpisodeNum);
			else if (xmldata.searchBy.equals("Showname SeriesNum"))
				newzbin_query = newzbin_query + (tvrage.nextSeriesNum);
			else if (xmldata.searchBy.equals("Showname YYYY-MM-DD")) {
				try {
				...
				newzbin_query = newzbin_query + (tvrage.nextTitle);
		}
		newzbin_query = newzbin_query + ("&searchaction=Search");
		newzbin_query = newzbin_query + ("&fpn=p");
		newzbin_query = newzbin_query + ("&category=8category=11");
		newzbin_query = newzbin_query + ("&area=-1");
		newzbin_query = newzbin_query + ("&u_nfo_posts_only=0");
		newzbin_query = newzbin_query + ("&u_url_posts_only=0");
		newzbin_query = newzbin_query + ("&u_comment_posts_only=0");
		newzbin_query = newzbin_query + ("&u_v3_retention=1209600");
		newzbin_query = newzbin_query + ("&ps_rb_language=" + xmldata.language);
		newzbin_query = newzbin_query + ("&sort=ps_edit_date");
		newzbin_query = newzbin_query + ("&order=desc");
		newzbin_query = newzbin_query + ("&areadone=-1");
		newzbin_query = newzbin_query + ("&feed=csv");
		newzbin_query = newzbin_query + ("&ps_rb_video_format=" + xmldata.format);
		...
		try {
			...
			if (csvData != null) {
				...
				String content = "username=" + JavaNZB.newzbinUsr + "&password=" + JavaNZB.newzbinPass + "&reportid="
						+ tmp[2];
