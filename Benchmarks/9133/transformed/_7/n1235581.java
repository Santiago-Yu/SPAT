	...
	public Object process(Atom oAtm) throws IOException {
		...
		if (DebugFile.trace) {
			DebugFile.writeln("Begin FileDumper.process([Job:" + getStringNull(DB.gu_job, "") + ", Atom:"
					+ String.valueOf(oAtm.getInt(DB.pg_atom)) + "])");
			DebugFile.incIdent();
		}
		if (bHasReplacements) {
			sPathHTML = getProperty("workareasput");
			if (!sPathHTML.endsWith(sSep))
				sPathHTML = sPathHTML + (sSep);
			sPathHTML = sPathHTML + (getParameter("gu_workarea") + sSep + "apps" + sSep + "Mailwire" + sSep + "html"
					+ sSep + getParameter("gu_pageset") + sSep);
			sPathHTML = sPathHTML + (getParameter("nm_pageset").replace(' ', '_') + ".html");
			if (DebugFile.trace)
			...
			if (null == oReplaced) {
				...
				if (!sPathHTML.endsWith(sSep))
					sPathHTML = sPathHTML + (sSep);
				sPathHTML = sPathHTML + (getParameter("gu_workarea") + sSep + "apps" + sSep + "Mailwire" + sSep + "html"
						+ sSep + getParameter("gu_pageset") + sSep + getParameter("nm_pageset").replace(' ', '_')
						+ ".html");
				if (DebugFile.trace)
					DebugFile.writeln("PathHTML = " + sPathHTML);
		...
		if (!sPathJobDir.endsWith(sSep))
			sPathJobDir = sPathJobDir + (sSep);
		sPathJobDir = sPathJobDir + ("jobs" + sSep + getParameter("gu_workarea") + sSep + getString(DB.gu_job) + sSep);
		FileWriter oFileWrite = new FileWriter(
				sPathJobDir + getString(DB.gu_job) + "_" + String.valueOf(oAtm.getInt(DB.pg_atom)) + ".html", true);
