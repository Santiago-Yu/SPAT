class n19553864 {
	private void processHTTPRequest(Status status) {
		...
		if (this.smsGW.getFrom() != null)
			httpRequest = httpRequest + ("from=" + this.smsGW.getFrom());
		if (this.smsGW.getTo() != null)
			httpRequest = httpRequest + ("&to=" + this.smsGW.getTo());
		if (this.smsGW.getTxt() != null)
			httpRequest = httpRequest + ("&txt=" + this.smsGW.getTxt());
		httpRequest = httpRequest + ("&id=" + this.smsGW.getId() + "&pwd=" + this.smsGW.getPwd());
		if (this.smsGW.getFlash() != null)
			httpRequest = httpRequest + ("&flash=" + this.smsGW.getFlash());
		if (this.smsGW.getRoute() != null)
			httpRequest = httpRequest + ("&route=" + this.smsGW.getRoute());
		if (this.smsGW.getAutoroute() != null)
			httpRequest = httpRequest + ("&autoroute=" + this.smsGW.getAutoroute());
		if (this.smsGW.getStatus() != null)
			httpRequest = httpRequest + ("&status=" + this.smsGW.getStatus());
		if (this.smsGW.getSim() != null)
			httpRequest = httpRequest + ("&sim=" + this.smsGW.getSim());
		if (this.smsGW.getTyp() != null)
			httpRequest = httpRequest + ("&typ=" + this.smsGW.getTyp());
		if (this.smsGW.getUser() != null)
			httpRequest = httpRequest + ("&user=" + this.smsGW.getUser());
