class n20568568 {
	private String createHTML(PAGE_TYPE requestPage) {
		...
		switch (requestPage) {
		case PAGE_LOGO:
			result = result + ("</head><body class=\"header\">" + "<table width=\"100%\"><tr><td>"
			...
			try {
				...
				while (rs.next()) {
					...
					if (appendToHome != null) {
						...
						result = result + (stripHtml(appendToHome, false) + "<br>\n");
					}
					if (rs.getInt("AD_MENU_ID") > 0) {
						result = result + ("<a class=\"hrefNode\" href=\"http:///window/node#" + String.valueOf(
								rs.getInt("AD_WINDOW_ID") + "\">" + rs.getString("DESCRIPTION") + "</a><br>\n"));
					}
					result = result + ("<br>\n");
					if (rs.getInt("PA_GOAL_ID") > 0)
						result = result + (goalsDetail(rs.getInt("PA_GOAL_ID")));
				}
