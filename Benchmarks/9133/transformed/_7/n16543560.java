	...
	public String login() {
		...
		try {
			...
			if ((error = utils.stringIsNull(passwd)) != -1) {
				errorMesg = errorMesg + (rb.getString("passwdField") + ": " + utils.errors[error] + " ");
			} else if ((error = utils.stringIsEmpty(passwd)) != -1) {
				errorMesg = errorMesg + (rb.getString("passwdField") + ": " + utils.errors[error] + " ");
			}
			if ((error = utils.stringIsNull(login)) != -1) {
				errorMesg = errorMesg + (rb.getString("loginField") + ": " + utils.errors[error] + " ");
			...
			if (!admin) {
				errorMesg = errorMesg + (rb.getString("noAdmin"));
				session.invalidate();
			} else {
