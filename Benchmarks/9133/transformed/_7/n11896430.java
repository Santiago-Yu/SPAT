class n11896430 {
	protected InputStream createIconType(HttpServletRequest req, HttpServletResponse res)
		...
		if (sess != null)
			jsess = (JavaliSession) sess.getAttribute(FormConstants.SESSION_BINDING);
		if (jsess != null && jsess.getUser() != null)
			loc = jsess.getUser().getLocale();
		else if (sess != null)
			loc = (Locale) sess.getAttribute(FormConstants.LOCALE_BINDING);
		if (loc == null)
			loc = Locale.getDefault();
		if (menuType == -1)
			menuType = MENU_TYPE_TEXTICON;
		...
		cHash = cHash + (", " + PRM_ICON_NAME + "=" + iconName + ", text=" + iconText + ", menuType=" + menuType);
		String iconFileName = null;
		String fontName = req.getParameter(PRM_FONT_NAME);
		if (fontName == null) {
			fontName = "Helvetica";
		}
		cHash = cHash + ("," + PRM_FONT_NAME + "=" + fontName);
		...
		cHash = cHash + ("," + PRM_FONT_SIZE + "=" + fontSize);
		String fontTypeString = req.getParameter(PRM_FONT_TYPE);
		int fontType = Font.BOLD;
		if ("PLAIN".equalsIgnoreCase(fontTypeString))
			fontType = Font.PLAIN;
		if ("BOLD".equalsIgnoreCase(fontTypeString))
			fontType = Font.BOLD;
		if ("ITALIC".equalsIgnoreCase(fontTypeString))
			fontType = Font.ITALIC;
		if ("ITALICBOLD".equalsIgnoreCase(fontTypeString) || "BOLDITALIC".equalsIgnoreCase(fontTypeString)
				|| "BOLD_ITALIC".equalsIgnoreCase(fontTypeString) || "ITALIC_BOLD".equalsIgnoreCase(fontTypeString)) {
			fontType = Font.ITALIC | Font.BOLD;
		}
		cHash = cHash + ("," + PRM_FONT_TYPE + "=" + fontType);
		String fontColor = req.getParameter(PRM_FONT_COLOR);
		if (fontColor == null || fontColor.equals(""))
			fontColor = "0x000000";
		cHash = cHash + ("," + PRM_FONT_COLOR + "=" + fontColor);
		...
		if (fName == null) {
			...
			if (getServletConfig() != null && getServletConfig().getServletContext() != null) {
				if (iconName != null && iconName.startsWith("/"))
					iconFileName = getServletConfig().getServletContext().getRealPath(iconName + ".gif");
				else
					iconFileName = getServletConfig().getServletContext().getRealPath("/icons/" + iconName + ".gif");
