			...
			if (users != null) {
				for (FTPFile user : users) {
					...
					if (alias.equals(userName)) {
						outputRecord = outputRecord + ("Found account " + userName + ".\n");
						...
						if (experiments != null && userNode != null) {
							for (FTPFile experiment : experiments) {
								...
								outputRecord = outputRecord + ("Exploring " + userName + "/" + experimentName + ".\n");
								...
								if (datasets != null) {
									for (FTPFile dataset : datasets) {
										...
										client.changeWorkingDirectory(
												"/" + userName + "/" + experimentName + "/" + datasetName + "/");
											...
											if (images != null) {
												for (FTPFile image : images) {
													outputRecord = outputRecord
															+ (processImage(userName, experimentName, datasetName,
																	collectionDate, image, client, userNode, session));
												}
											}
										}
									}
