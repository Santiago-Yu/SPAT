class n21808207 {
	public static void test() {
		...
		try {
			...
			urlstr = urlstr + (cal.get(Calendar.HOUR_OF_DAY));
			urlstr = urlstr + ("&m=");
			urlstr = urlstr + (cal.get(Calendar.MINUTE));
			urlstr = urlstr + ("&s=");
			urlstr = urlstr + (cal.get(Calendar.SECOND));
			urlstr = urlstr
					+ ("&fla=1&dir=1&qt=1&realp=1&pdf=1&wma=1&java=1&cookie=0&title=JAVAACCESS&urlref=http%3a%2f%2flyricscatcher.sourceforge.net%2fcomputeraccespage.html");
			...
			jep.setText(
					"<html><img src=\"http://apps.sourceforge.net/piwik/lyricscatcher/piwik.php?idsite=1\" alt=\"there's a problem...\"/><img src=\""
							+ urlstr + "\" alt=\"Another problem\" style=\"border:0\" /></html>");
