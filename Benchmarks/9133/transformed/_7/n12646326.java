	...
	private void weightAndPlaceClasses() {
		...
		for (int curRow = _maxPackageRank; curRow < rows; curRow++) {
				...
				if (curRow == _maxPackageRank) {
					...
					if (nUplinks > 0) {
						...
						for (int j = 0; j < uplinks.size(); j++) {
							average_col = average_col + (((BOTLRuleDiagramNode) (uplinks.elementAt(j))).getColumn());
						}
						average_col = average_col / (nUplinks);
			...
			for (int i = 0; i < pos.length; i++) {
				...
				if ((i > _vMax) && (rowObject[pos[i]].getUplinks().size() == 0)
					...
					xPos = xPos + (rowObject[pos[i]].getSize().getWidth() + getHGap());
				}
			}
			yPos = yPos + (getRowHeight(curRow) + getVGap());
		}
	}
