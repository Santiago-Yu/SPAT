	...
	public void process(@NotNull Template tpl, @NotNull Document model, @NotNull String packageName,
		...
		if (tpl.path == null) {
			if (tpl.destination == Destination.redora)
				destinationPath = destinationPath + (separator + artifact);
			else
				destinationPath = destinationPath + (separator + packageName.replace('.', separatorChar));
		} else {
			destinationPath = destinationPath
					+ (separator + tpl.path.replace('/', separatorChar).replace('\\', separatorChar));
		}
