class n9647574 {
	private void FindAvail() throws ParserConfigurationException, SQLException {
		...
		availsql = availsql + ("passing hp_administrator.availability.AVAIL as \"c\") ");
		availsql = availsql + (" from hp_administrator.availability ");
		availsql = availsql + (" where date = '" + booking_details.getDate() + "' and train_no like '"
		...
		try {
			...
			for (int i = 0; i < coachlist.getLength(); i++) {
				...
				seatsql = seatsql
						+ (" passing hp_administrator.book_tickets.SEAT as \"c\") from hp_administrator.book_tickets ");
				seatsql = seatsql + (" where  date = '" + booking_details.getDate() + "' and train_no like '"
						+ booking_details.getTrain_no() + "' ");
				...
				if (routesrcstr.equals(sourcenws) && routedeststr.equals(destnnws)) {
					...
					seatstart = seatstart + (alloccnt);
					seatcnt = seatcnt - (alloccnt);
					Element seat, stn;
					...
					if (booking_details.getNoOfPersons() <= navailcoach
						...
						navailcoach = navailcoach - (booking_details.getNoOfPersons());
						nspavailcoach = nspavailcoach - (booking_details.getNoOfPersons());
						if (!firsttime)
							conn.rollback(sp1);
						String availupdstr = "update hp_administrator.availability set AVAIL = xmlquery( 'transform copy $new := $AVAIL  modify do ";
						availupdstr = availupdstr + (" replace value of $new/coach_status/class[@name=\""
								+ booking_details.getTclass() + "\"]/coach[@number=\"" + currentcoachstr + "\"]/"
								+ sourcenws + " with \"" + navailcoach + "\"");
						availupdstr = availupdstr + (" return  $new')  where train_no like '"
						...
						seatupdstr = seatupdstr
								+ (" modify do replace $new/status/class[@type=\"" + booking_details.getTclass()
										+ "\"]/coach[@number=\"" + currentcoachstr + "\"] with " + sw.toString());
						seatupdstr = seatupdstr + (" return $new') where train_no like '"
								+ booking_details.getTrain_no() + "' and date = '" + booking_details.getDate() + "'  ");
					...
					if (booking_details.getNoOfPersons() <= desttillcnt
						...
						number_of_tickets_rem = 0;
						desttillcnt = desttillcnt - (booking_details.getNoOfPersons());
						spdesttillcnt = spdesttillcnt - (booking_details.getNoOfPersons());
						if (!firsttime)
							conn.rollback(sp1);
						...
						availupd = availupd + ("$new/coach_status/class[@name=\"" + booking_details.getTclass()
								+ "\"]/coach[@number=\"" + currentcoachstr + "\"]/" + destnnws + "/@" + destnnws
								+ "TILL" + " with \"" + desttillcnt + "\" ");
						availupd = availupd + (" return  $new')  where train_no like '" + booking_details.getTrain_no()
						...
						excesssrc = excesssrc - (diff);
						spexcesssrc = spexcesssrc - (diff);
						number_of_tickets_rem = 0;
						if (!firsttime)
							conn.rollback(sp1);
						String availupd = "update hp_administrator.availability set AVAIL = xmlquery( 'transform copy $new := $AVAIL modify do replace value of ";
						...
						availupd = availupd + (" replace value of $new/coach_status/class[@name=\""
								+ booking_details.getTclass() + "\"]/coach[@number=\"" + currentcoachstr + "\"]/"
						...
						excesssrc = excesssrc - (booking_details.getNoOfPersons());
						spexcesssrc = spexcesssrc - (booking_details.getNoOfPersons());
						;
						number_of_tickets_rem = 0;
						if (!firsttime)
							conn.rollback(sp1);
					...
					while (spexcesssrc + spdesttillcnt > 0 && number_of_tickets_rem > 0) {
						...
						if (spdesttillcnt > 0) {
							...
							availupd = availupd + ("$new/coach_status/class[@name=\"" + booking_details.getTclass()
									...
									+ "TILL" + " with \"" + desttillcnt + "\" ");
							availupd = availupd
									+ (" return  $new')  where train_no like '" + booking_details.getTrain_no()
											+ "' and date = '" + booking_details.getDate() + "' ");
							stavailupd = conn.createStatement();
