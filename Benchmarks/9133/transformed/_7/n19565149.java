class n19565149 {
	private long generateUnixInstallShell(File unixShellFile, String instTemplate, File instClassFile)
		...
		content = content + ("InstallClassStart=" + installClassStartStr + "\n");
		...
		content = content + (new String("InstallClassSize=" + instClassFile.length() + "\n"));
		line = reader.readLine();
		while ((line != null) && (!line.startsWith("# InstallClassName"))) {
			content = content + (line + "\n");
			line = reader.readLine();
		}
		content = content + (new String("InstallClassName=" + instClassName_ + "\n"));
		line = reader.readLine();
