class n9103010 {
	private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {
		...
		try {
			...
			data = data + ("&" + URLEncoder.encode("pwd", "UTF-8") + "="
					+ URLEncoder.encode(new String(this.txtPwd.getPassword()), "UTF-8"));
			data = data + ("&" + URLEncoder.encode("id_stile", "UTF-8") + "="
					+ URLEncoder.encode(node.getIdStile(), "UTF-8"));
			data = data + ("&" + URLEncoder.encode("nome_ricetta", "UTF-8") + "="
					+ URLEncoder.encode(this.nomeRicetta, "UTF-8"));
			data = data
					+ ("&" + URLEncoder.encode("xml_ricetta", "UTF-8") + "=" + URLEncoder.encode(this.xml, "UTF-8"));
