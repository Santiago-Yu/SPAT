				...
				for (int j = i + 1; j < noMovies; j++) {
					...
					if ((intersectSet.size() == 0) || (intersectSet == null)) {
						...
						for (int l = 0; l < count; l++) {
							...
							sumY = sumY + (ratingY);
							sumX2 = sumX2 + (ratingX * ratingX);
							sumY2 = sumY2 + (ratingY * ratingY);
							sumXY = sumXY + (ratingX * ratingY);
							diffRating = diffRating + (ratingX - ratingY);
							sumXYPearson = sumXYPearson
									+ ((ratingX - movieAverages.get(movie1)) * (ratingY - movieAverages.get(movie2)));
							sumX2Pearson = sumX2Pearson + (Math.pow((ratingX - movieAverages.get(movie1)), 2));
							sumY2Pearson = sumY2Pearson + (Math.pow((ratingY - movieAverages.get(movie2)), 2));
							float custAverage = custAverages.get(commonCust);
							sumXYACos = sumXYACos + ((ratingX - custAverage) * (ratingY - custAverage));
							sumX2ACos = sumX2ACos + (Math.pow((ratingX - custAverage), 2));
							sumY2ACos = sumY2ACos + (Math.pow((ratingY - custAverage), 2));
						}
					}
