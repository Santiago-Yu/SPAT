class n17761372 {
	public void run() {
		try {
			...
			data = data
					+ ("&" + URLEncoder.encode("author", "UTF-8") + "=" + URLEncoder.encode(name.getText(), "UTF-8"));
			data = data + ("&" + URLEncoder.encode("location", "UTF-8") + "="
					+ URLEncoder.encode(System.getProperty("user.language"), "UTF-8"));
			data = data
					+ ("&" + URLEncoder.encode("contact", "UTF-8") + "=" + URLEncoder.encode(email.getText(), "UTF-8"));
			data = data + ("&" + URLEncoder.encode("content", "UTF-8") + "="
					+ URLEncoder.encode(comment.getText(), "UTF-8"));
			data = data + ("&" + URLEncoder.encode("rate", "UTF-8") + "="
