class n7088306 {
	private void modifyEntry(ModifyInterceptorChain chain, DistinguishedName dn, ArrayList<LDAPModification> mods,
			Connection con) throws LDAPException {
		try {
			con.setAutoCommit(false);
			HashMap<String, String> ldap2db = (HashMap<String, String>) chain.getRequest()
			...
			while (it.hasNext()) {
				...
				sql = sql + (ldap2db.get(mod.getAttribute().getName()) + "=? ");
			}
			sql = sql + (" WHERE " + this.rdnField + "=?");
