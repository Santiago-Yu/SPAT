class n8351729 {
	public void submitReport() {
		...
		try {
			...
			data = data + ("&" + URLEncoder.encode("from", "UTF-8") + "=" + URLEncoder.encode(email, "UTF-8"));
			data = data + ("&" + URLEncoder.encode("subject", "UTF-8") + "=" + URLEncoder.encode(subject, "UTF-8"));
			data = data + ("&" + URLEncoder.encode("body", "UTF-8") + "=" + URLEncoder.encode(description, "UTF-8"));
			data = data + ("&" + URLEncoder.encode("jvm", "UTF-8") + "="
					+ URLEncoder.encode(System.getProperty("java.version"), "UTF-8"));
			data = data + ("&" + URLEncoder.encode("ocdsver", "UTF-8") + "="
					+ URLEncoder.encode(Constants.OPENCDS_VERSION, "UTF-8"));
			data = data + ("&" + URLEncoder.encode("os", "UTF-8") + "=" + URLEncoder.encode(
