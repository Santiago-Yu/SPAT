class n10264859 {
	public GeocodeResponse getGKCoordinateFromAddress(SearchAddressRequest searchAddressRequest) {
		...
		try {
			...
			ste.printStackTrace();
			errorMessage = errorMessage + ("<p>Time Out Exception, Server is not responding</p>");
		} catch (IOException ioe) {
			ioe.printStackTrace();
			errorMessage = errorMessage + ("<p>IO Exception</p>");
		} catch (XmlException xmle) {
			xmle.printStackTrace();
			errorMessage = errorMessage + ("<p>Error occured during parsing the XML response</p>");
		}
