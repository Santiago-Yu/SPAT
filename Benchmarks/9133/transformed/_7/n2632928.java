	...
	public DicomData load(String imgURL) {
		...
		try {
			...
			while (din.read(buff2) != -1) {
				...
				group = group + (Integer.toString((tempInt & 0x00000f00) >> 8, 16));
				group = group + (Integer.toString((tempInt & 0x000000f0) >> 4, 16));
				group = group + (Integer.toString((tempInt & 0x0000000f), 16));
				din.readFully(buff2);
				tempInt = readInt2(buff2);
				number = Integer.toString((tempInt & 0x0000f000) >> 12, 16);
				number = number + (Integer.toString((tempInt & 0x00000f00) >> 8, 16));
				number = number + (Integer.toString((tempInt & 0x000000f0) >> 4, 16));
				number = number + (Integer.toString((tempInt & 0x0000000f), 16));
				tag = ("(" + group + "," + number + ")");
				if (debug_level > 3)
