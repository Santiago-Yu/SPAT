class n17454682 {
	...
	public void run() {
		try {
			...
			int itemCount = 0;
			try {
				itemCount = itemCount + (mfDAO.getCount());
				itemCount = itemCount + (aDAO.getCount());
				itemCount = itemCount + (cdDAO.getCount());
				itemCount = itemCount + (ciDAO.getCount());
			...
			_messenger.fireMessageEvent(new MessageEvent(this, "ERROR", MessageEventTypeEnum.ERROR,
					GuiStrings.getInstance().getString("error.backup"), e));
		}
	}
