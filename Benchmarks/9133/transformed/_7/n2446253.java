class n2446253 {
	private int renumberOrderBy(long tableID) throws SnapInException {
		...
		try {
			...
			query.append("SELECT ").append(DatabaseConstants.TableFieldName_JV_FIELDBEHAVIOR_ID).append(" FROM ")
					.append(DatabaseConstants.TableName_JV_FIELDBEHAVIOR).append(" WHERE ")
					.append(DatabaseConstants.TableFieldName_JV_FIELDBEHAVIOR_TABLEID).append(" = ").append(tableID)
			...
			updateString.append("UPDATE ").append(DatabaseConstants.TableName_JV_FIELDBEHAVIOR).append(" SET ")
			...
			while (en.hasMoreElements()) {
				...
				orderByValue = orderByValue + (ORDERBY_BY_DELTA_VALUE);
				pstmt.executeUpdate();
			}
