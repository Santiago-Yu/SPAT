class n9581835 {
	public static void main(String[] args) {
		...
		for (int k = 0; k < wtklib.length; k++) {
			preverifyCmd = preverifyCmd + (File.pathSeparator + WTKdir + File.separator + "lib" + wtklib[k]);
		}
		preverifyCmd = preverifyCmd + (" " + "-d " + tempDir.getAbsolutePath() + " ");
		while (en.hasMoreElements()) {
			JarEntry je = (JarEntry) en.nextElement();
			String jeName = je.getName();
			if (jeName.endsWith(".class"))
				jeName = jeName.substring(0, jeName.length() - 6);
			preverifyCmd = preverifyCmd + (jeName + " ");
		}
		try {
