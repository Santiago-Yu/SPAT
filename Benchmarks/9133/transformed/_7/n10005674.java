class n10005674 {
	public String post() {
		...
		try {
			...
			data = data + ("&" + URLEncoder.encode("content", "UTF-8") + "=" + URLEncoder.encode(content, "UTF-8"));
			data = data + ("&" + URLEncoder.encode("description", "UTF-8") + "="
					+ URLEncoder.encode(descriptionTextArea.getText() + description_suffix, "UTF-8"));
			data = data + ("&" + URLEncoder.encode("expiry", "UTF-8") + "="
			...
			data = data + ("&" + URLEncoder.encode("name", "UTF-8") + "="
			...
			String line;
			line = rd.readLine();
			if (line == null || line.length() < 8 || !line.substring(0, 8).equals("SUCCESS:"))
				return "Unknown error in publishing the post";
			else
				return "*Publish successful.! The link to your post is.: " + "http://pastebin.ca/" + line.substring(8);
