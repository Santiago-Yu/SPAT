				...
				try {
					...
					for (RaphPhotoGalleryPhoto photo : photoList) {
						...
						while ((sizeRead = dis.read(b)) > -1) {
							...
							totalSizeRead = totalSizeRead + (sizeRead);
			...
			protected void process(List<RaphPhotoGalleryPhoto> list) {
				...
				if (diffTime > 1000) {
					...
					if (refreshSpeed == 0) {
						...
						refreshSpeed--;
						remainingTime = remainingTime - (diffTime / 1000.0);
					}
					speedLabel.setText(String.format("%.0f", speed / 1024) + " " + getParameter("i18n_speedUnit")
