			...
			if (objecttype.equalsIgnoreCase("T")) {
				...
				if (!exists && !modo.equals("3")) {
					...
					for (int i = 0; i < sysflds.length; i++) {
						dml = dml + ((sysflds[i] + " " + sysfdef[i] + ((i < (sysflds.length - 1)) ? "," : ")")));
					}
			...
			if (objecttype.equalsIgnoreCase("F")) {
				...
				if ((dml != null) && (dml.length() > 0) && !modo.equals("3")) {
					...
					if ((mfield != null) && !(!mfield.equals("TEXTOLIVRE") && !mfield.equals("NUMEROLIVRE")
						...
						dml = dml + (getDDLFieldFromNGT(ngtft, node.getString("FIELDSIZE")));
					} else if ((mfield != null) && (mfield.length() > 0)) {
						dml = dml + (getMacrofieldDef(cndef, node.getString("MACROFIELD")));
					} else {
						dml = dml + (getDDLFieldFromNGT(node.getString("FIELDTYPE"), node.getString("FIELDSIZE")));
					}
				}
			...
			if (objecttype.startsWith("FK")) {
				...
				if (rslt.next()) {
					...
					if (cExpress.length() > 0)
						cExpress = cExpress + (",");
					cExpress = cExpress + (rslt.getString(1));
				}
				rslt.close();
				pstm.close();
			...
			if (objecttype.startsWith("IDX")) {
				...
				if (rslt.next()) {
					...
					while (rslt.next()) {
						oldexpression = oldexpression
								+ ((((oldexpression.length() > 0) ? "," : "") + rslt.getString(1)));
					}
					rslt.close();
					pstm.close();
				} else {
				...
				for (byte i = 0; i < nexo.length; i++) {
					newexpression = newexpression
							+ ((((newexpression.length() > 0) ? "," : "") + ((nexo[i]).toUpperCase().trim())));
				}
				if (!drop) {
