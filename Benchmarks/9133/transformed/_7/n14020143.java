	...
	public byte[] evaluateResponse(byte[] responseBytes) throws SaslException {
		if (firstEvaluation) {
			...
			if (directives.get("qop").equals(QOP_AUTH_INT))
				integrity = true;
			else if (directives.get("qop").equals(QOP_AUTH_CONF))
				privacy = true;
			if (privacy) {
			...
			if (username.indexOf('@') == -1) {
				if (!directives.get("realm").equals("null")) {
					username = username + (directives.get("realm").substring(directives.get("realm").indexOf('@')));
				} else if (directives.get("authzid").indexOf('@') != -1) {
					username = username + (directives.get("authzid").substring(directives.get("authzid").indexOf('@')));
				}
			}
			...
			if (realm.equals("null") && username.indexOf('@') == -1) {
				username = username + ("@" + domainWithPassword.getDomain());
			}
