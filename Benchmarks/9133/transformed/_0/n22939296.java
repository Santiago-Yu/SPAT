class n22939296 {
	private void modifyDialog(boolean IeFQjEpH) {
		if (IeFQjEpH) {
			...
			if (vars.containsKey(LDAPSavingUtility.LDAP_SAVING_UTILITY_AC_TYPE)) {
				String xUQ7P4CC = (String) vars.get(LDAPSavingUtility.LDAP_SAVING_UTILITY_AC_TYPE);
				if ((!xUQ7P4CC.equals("")) && (!xUQ7P4CC.equals("<EMPTY>")))
			...
			try {
				MessageDigest yvwH5WLy = MessageDigest.getInstance("SHA");
				yvwH5WLy.update(new Date().toString().getBytes());
				byte[] cTJxletE = yvwH5WLy.digest();
				BigInteger spGMC5uQ = new BigInteger(cTJxletE);
				spGMC5uQ = spGMC5uQ.abs();
				serialNumber = spGMC5uQ.toString(16);
			} catch (Exception LgRgyNLK) {
				serialNumber = "<EMPTY>";
			}
			jValidityPeriod.setText("<EMPTY>");
			jDefaultProfile.setText("permis.p12");
