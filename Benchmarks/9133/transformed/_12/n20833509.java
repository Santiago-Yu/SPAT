class n20833509 {
	protected String doIt() throws java.lang.Exception {
		...
		sql = new StringBuffer("UPDATE I_BPartner " + "SET I_IsImported='E', I_ErrorMsg=I_ErrorMsg||'"
		...
		sql = new StringBuffer("UPDATE I_BPartner i " + "Set RegionName=(SELECT Name FROM C_Region r"
				+ " WHERE r.IsDefault='Y' AND r.C_Country_ID=i.C_Country_ID"
				+ " AND r.AD_Client_ID IN (0, i.AD_Client_ID) AND ROWNUM=1) "
				+ "WHERE RegionName IS NULL AND C_Region_ID IS NULL" + " AND I_IsImported<>'Y'").append(clientCheck);
		no = DB.executeUpdate(sql.toString());
		...
		sql = new StringBuffer("UPDATE I_BPartner i " + "SET C_Categoria_IVA_Codigo=" + MCategoriaIva.CONSUMIDOR_FINAL
				+ " WHERE (C_Categoria_IVA_Codigo IS NULL OR C_Categoria_IVA_Codigo = 0) " + "  AND I_IsImported='N'")
						.append(clientCheck);
		no = DB.executeUpdate(sql.toString());
		sql = new StringBuffer("UPDATE I_BPartner i " + "SET C_Categoria_IVA_ID= " + " (SELECT C_Categoria_IVA_ID "
				+ "  FROM C_Categoria_IVA c "
				+ "  WHERE i.C_Categoria_IVA_Codigo=c.Codigo AND c.AD_Client_ID=i.AD_Client_ID " + " ) "
				+ "WHERE C_Categoria_IVA_ID IS NULL " + "  AND I_IsImported='N'").append(clientCheck);
		no = DB.executeUpdate(sql.toString());
		sql = new StringBuffer("UPDATE I_BPartner i " + "SET SalesRep_ID=(SELECT AD_User_ID " + "FROM AD_User u "
		...
		sql = new StringBuffer("SELECT I_BPartner_ID, C_BPartner_ID,"
				...
				+ "AD_User_ID,ContactName " + "FROM I_BPartner " + "WHERE I_IsImported='N'").append(clientCheck);
		...
		try {
			...
			PreparedStatement pstmt_insertLocation = conn.prepareStatement("INSERT INTO C_Location (C_Location_ID,"
					...
					+ "WHERE I_BPartner_ID=?");
			PreparedStatement pstmt_insertBPLocation = conn.prepareStatement("INSERT INTO C_BPartner_Location ( "
					...
					+ "WHERE I_BPartner_ID=?");
			PreparedStatement pstmt_insertBPContact = conn.prepareStatement("INSERT INTO AD_User (AD_User_ID,"
					+ "AD_Client_ID,AD_Org_ID,IsActive,Created,CreatedBy,Updated,UpdatedBy,"
			...
			PreparedStatement pstmt_updateBPContact = conn.prepareStatement("UPDATE AD_User "
					...
					+ " WHERE AD_User_ID=?");
			PreparedStatement pstmt_setImported = conn.prepareStatement("UPDATE I_BPartner SET I_IsImported='Y',"
			...
			while (rs.next()) {
				int I_BPartner_ID = rs.getInt(1), C_BPartner_ID = rs.getInt(2);
				...
				log.fine("I_BPartner_ID=" + I_BPartner_ID + ", C_BPartner_ID=" + C_BPartner_ID
				...
				if (newBPartner) {
					...
					if (bp.save()) {
						...
						sql = new StringBuffer("UPDATE I_BPartner i " + "SET I_IsImported='E', I_ErrorMsg=I_ErrorMsg||")
								.append(DB.TO_STRING("Insert BPartner failed: " + CLogger.retrieveErrorAsString()))
					...
					try {
						...
						sql = new StringBuffer("UPDATE I_BPartner i " + "SET I_IsImported='E', I_ErrorMsg=I_ErrorMsg||")
								.append(DB.TO_STRING("Update BPartner: " + ex.toString()))
								.append(" WHERE I_BPartner_ID=").append(I_BPartner_ID);
						DB.executeUpdate(sql.toString());
						continue;
					}
				...
				if (C_BPartner_Location_ID != 0) {
					...
					try {
						...
						noInsert--;
						sql = new StringBuffer("UPDATE I_BPartner i " + "SET I_IsImported='E', I_ErrorMsg=I_ErrorMsg||")
								.append(DB.TO_STRING("Insert Location: " + ex.toString()))
								.append(" WHERE I_BPartner_ID=").append(I_BPartner_ID);
						DB.executeUpdate(sql.toString());
						continue;
					}
					try {
						...
						noInsert--;
						sql = new StringBuffer("UPDATE I_BPartner i " + "SET I_IsImported='E', I_ErrorMsg=I_ErrorMsg||")
								.append(DB.TO_STRING("Insert BPLocation: " + ex.toString()))
								.append(" WHERE I_BPartner_ID=").append(I_BPartner_ID);
						DB.executeUpdate(sql.toString());
						continue;
					}
				...
				if (AD_User_ID != 0) {
					...
					try {
						...
						noInsert--;
						sql = new StringBuffer("UPDATE I_BPartner i " + "SET I_IsImported='E', I_ErrorMsg=I_ErrorMsg||")
								.append(DB.TO_STRING("Update BP Contact: " + ex.toString()))
								.append(" WHERE I_BPartner_ID=").append(I_BPartner_ID);
						DB.executeUpdate(sql.toString());
						continue;
					}
					...
					try {
						...
						noInsert--;
						sql = new StringBuffer("UPDATE I_BPartner i " + "SET I_IsImported='E', I_ErrorMsg=I_ErrorMsg||")
								.append(DB.TO_STRING("Insert BPContact: " + ex.toString()))
								.append(" WHERE I_BPartner_ID=").append(I_BPartner_ID);
						DB.executeUpdate(sql.toString());
						continue;
					}
