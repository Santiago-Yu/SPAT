class n12109713 {
	protected long loadFromSource(long afterThisTime) {
		...
		int a = calendar.get(Calendar.MONTH), b = calendar.get(Calendar.DAY_OF_MONTH);
		int c = calendar.get(Calendar.YEAR);
		calendar.setTime(toDate);
		int d = calendar.get(Calendar.MONTH), e = calendar.get(Calendar.DAY_OF_MONTH);
		...
		try {
			...
			int iDateTime = 0, iOpen = 1;
			int iHigh = 2, iLow = 3;
			int iClose = 4, iVolume = 5;
			int iAdjClose = 6;
			count = 0;
			calendar.clear();
