class n8641070 {
	public static void main(String[] args) {
		String email = "josh888@byu.net", username = "josh8573";
		String password = "josh8573", IDnumber = "3030";
		double[] apogee = { 1000 };
		double[] perigee = apogee, inclination = { 58.0 };
		int[] trp_solmax = { 0, 1, 2 };
		double[] init_long_ascend = { 0 }, init_displ_ascend = { 0 };
		double[] displ_perigee_ascend = { 0 }, orbit_sect = null;
		boolean[] gtrn_weather = { false, true };
		boolean print_altitude = true, print_inclination = false;
		boolean print_gtrn_weather = true, print_ita = false;
		boolean print_ida = false, print_dpa = false;
		ORBIT[] orbit_array;
		orbit_array = ORBIT.CreateOrbits(apogee, perigee, inclination, gtrn_weather, trp_solmax, init_long_ascend,
				init_displ_ascend, displ_perigee_ascend, orbit_sect, print_altitude, print_inclination,
				print_gtrn_weather, print_ita, print_ida, print_dpa);
		TRP[] trp_array = {};
		GTRN[] gtrn_array = {};
		if (orbit_array != null) {
		...
		int[] flux_min_element = { 1 }, flux_max_element = { 92 };
		int[] weather_flux = { 00, 01, 11, 12, 13 };
		boolean print_weather = true, print_min_elem = false;
		...
		flux_array = FLUX.CreateFLUX_URF(flux_min_element, flux_max_element, weather_flux, orbit_array_into_flux,
				print_weather, print_min_elem, print_max_elem);
		...
		int[] letspec_min_element = { 2 }, letspec_max_element = { 0 };
		double[] min_energy_value = { .1 };
		boolean[] diff_spect = { false };
		boolean print_min_energy = false;
		LETSPEC[] letspec_array;
		letspec_array = LETSPEC.CreateLETSPEC_URF(input_files_for_letspec, letspec_min_element, letspec_max_element,
				min_energy_value, diff_spect, print_min_energy);
		...
		boolean print_bits_in_device_pup = false, print_weibull_onset_pup = false;
		boolean print_weibull_width_pup = false, print_weibull_exponent_pup = false;
		boolean print_weibull_cross_sect_pup = false;
		PUP[] pup_array;
		pup_array = PUP.CreatePUP_URF(input_files_for_pup, pup_device_array, print_bits_in_device_pup,
				print_weibull_onset_pup, print_weibull_width_pup, print_weibull_exponent_pup,
				print_weibull_cross_sect_pup);
		LETSPEC[] let_objects_into_hup = letspec_array;
		double[][] weib_params = { { 9.74, 30.25, 2.5, 22600 }, { 9.74, 30.25, 2.5, 2260 }, { 9.74, 30.25, 2.5, 226 },
		...
		for (int i = 0; i < 7; i++) {
			hup_device_array[i][0] = new HUP_Device("sample", null, null, 0, 0, (Math.sqrt(weib_params[i][3]) / 100), 0,
					(int) Math.pow(10, i), 4, weib_params[i]);
			z_depth += .01;
		}
		boolean print_label = false, print_commenta = false;
		boolean print_commentb = false, print_RPP_x = false;
		boolean print_RPP_y = false, print_RPP_z = false;
		boolean print_funnel_length = false, print_bits_in_device_hup = true;
		boolean print_weibull_onset_hup = false, print_weibull_width_hup = false;
		boolean print_weibull_exponent_hup = false, print_weibull_cross_sect_hup = false;
		HUP[] hup_array;
		hup_array = HUP.CreateHUP_URF(let_objects_into_hup, hup_device_array, print_label, print_commenta,
				print_commentb, print_RPP_x, print_RPP_y, print_RPP_z, print_funnel_length, print_bits_in_device_hup,
				print_weibull_onset_hup, print_weibull_width_hup, print_weibull_exponent_hup,
				print_weibull_cross_sect_hup);
		...
		int num_of_files = trp_array.length + gtrn_array.length + flux_array.length + trans_array.length
				+ letspec_array.length + pup_array.length + hup_array.length, index = 0;
		...
		String host = "creme96.nrl.navy.mil", user = "anonymous";
