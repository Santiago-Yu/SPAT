class n12082137 {
	public static Set<Municipality> getMunicipios(String pURL) {
		...
		String iniCuerr = "<cuerr>", finCuerr = "</cuerr>";
		String iniDesErr = "<des>", finDesErr = "</des>";
		String iniMun = "<muni>", finMun = "</muni>";
		String iniNomMun = "<nm>", finNomMun = "</nm>";
		String iniCarto = "<carto>", iniCodDelMEH = "<cd>";
		String finCodDelMEH = "</cd>", iniCodMunMEH = "<cmc>";
		String finCodMunMEH = "</cmc>", iniCodProvINE = "<cp>";
		String finCodProvINE = "</cp>", iniCodMunINE = "<cm>";
					...
					if (str.contains(iniMun)) {
						...
						while ((str = br.readLine()) != null && !str.contains(finMun)) {
							...
							if (str.contains(iniCodDelMEH)) {
								...
								municipio.setCodemunicipalitydgc(municipio.getCodemunicipalitydgc()
										+ Integer.parseInt(str.substring(ini, fin)) * 1000);
							}
							if (str.contains(iniCodMunMEH)) {
								ini = str.indexOf(iniCodMunMEH) + iniCodMunMEH.length();
								fin = str.indexOf(finCodMunMEH);
								municipio.setCodemunicipalitydgc(
										municipio.getCodemunicipalitydgc() + Integer.parseInt(str.substring(ini, fin)));
							}
							if (str.contains(iniCodProvINE)) {
								ini = str.indexOf(iniCodProvINE) + iniCodProvINE.length();
								fin = str.indexOf(finCodProvINE);
								municipio.setCodemunicipalityine(municipio.getCodemunicipalityine()
										+ Integer.parseInt(str.substring(ini, fin)) * 1000);
							}
							if (str.contains(iniCodMunINE)) {
								ini = str.indexOf(iniCodMunINE) + iniCodMunINE.length();
								fin = str.indexOf(finCodMunINE);
								municipio.setCodemunicipalityine(
										municipio.getCodemunicipalityine() + Integer.parseInt(str.substring(ini, fin)));
							}
