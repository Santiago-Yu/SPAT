class n9120775 {
	public void addFile(File fileobj, boolean delete) {
		String oldFileName = fileobj.getPath(), currFileName = setUpFile(fileobj);
		if (currFileName != null) {
			...
			if (oldFileName.equals(currFileName)) {
			...
			} else if (!f.exists() || JOptionPane.OK_OPTION == (deleteFiles = JOptionPane.showConfirmDialog(this, "File"
					+ f.getName() + " Already Exists! Do You Wish To Overwrite That File?"
				...
				currFileName = currFileName.substring(
						currFileName.lastIndexOf(openProject.getName()) + openProject.getName().length() + 1);
				openProject.addFile(currFileName);
				if (f.getName().toLowerCase().endsWith(".exp"))
					addExpFile(f.getPath());
			}
			...
			if (fileobj.getName().endsWith(".gprj")) {
				...
				message += "File Extension Unknown. Please Check The File To Ensure It Has The Correct Extension";
			if (!message.equals(""))
