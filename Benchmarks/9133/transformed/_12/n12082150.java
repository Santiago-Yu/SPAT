class n12082150 {
	public static Set<Address> getDatosCatastrales(String pURL) {
		...
		String iniInm1 = "<rcdnp>", finInm1 = "</rcdnp>";
		String iniInm2 = "<bi>", finInm2 = "</bi>";
		String iniPC1 = "<pc1>", iniPC2 = "<pc2>";
		String finPC1 = "</pc1>", finPC2 = "</pc2>";
		String iniCar = "<car>", finCar = "</car>";
		String iniCC1 = "<cc1>", finCC1 = "</cc1>";
		String iniCC2 = "<cc2>", finCC2 = "</cc2>";
		String iniLDT = "<ldt>", iniBq = "<bq>";
		String finBq = "</bq>", iniEs = "<es>";
		String finEs = "</es>", iniPt = "<pt>";
		String finPt = "</pt>", iniPu = "<pu>";
		String finPu = "</pu>";
		boolean error = false;
		int ini, fin, postal = 0;
		try {
					...
					if (str.contains(iniInm1) || str.contains(iniInm2)) {
						...
						while ((str = br.readLine()) != null && !str.contains(finInm1) && !str.contains(finInm2)) {
							...
							if (str.contains(iniLDT) && str.contains("-")) {
								postal = Integer
										.parseInt(str.substring(str.lastIndexOf("-") - 5, str.lastIndexOf("-")));
							}
			...
			if (result.size() == 1) {
				...
				for (Address inm : result)
					inm.setCoodinate(coord);
			}
		} catch (Exception e) {
