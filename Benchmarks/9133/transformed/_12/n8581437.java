class n8581437 {
	protected String doIt() throws java.lang.Exception {
		...
		for (int i = 0; i < strFields.length; i++) {
			sql = new StringBuffer("UPDATE I_PRODUCT i " + "SET ").append(strFields[i]).append(" = (SELECT ")
					.append(strFields[i])
		...
		for (int i = 0; i < numFields.length; i++) {
			sql = new StringBuffer("UPDATE I_PRODUCT i " + "SET ").append(numFields[i]).append(" = (SELECT ")
					.append(numFields[i])
					.append(" FROM M_Product p"
							+ " WHERE i.M_Product_ID=p.M_Product_ID AND i.AD_Client_ID=p.AD_Client_ID) "
							+ "WHERE M_Product_ID IS NOT NULL" + " AND EXISTS (SELECT * FROM M_Product p WHERE "
		...
		for (int i = 0; i < strFieldsPO.length; i++) {
			sql = new StringBuffer("UPDATE I_PRODUCT i " + "SET ").append(strFieldsPO[i]).append(" = (SELECT ")
					...
					.append(" FROM M_Product_PO p"
							...
							+ " AND EXISTS (SELECT * FROM M_Product_PO p WHERE " + strFieldsPO[i]
							+ " IS NOT NULL AND p.M_Product_ID = i.M_Product_ID AND i.AD_Client_ID=p.AD_Client_ID)"
							+ " AND I_IsImported='N'")
					.append(clientCheck);
			no = DB.executeUpdate(sql.toString());
		...
		for (int i = 0; i < numFieldsPO.length; i++) {
			sql = new StringBuffer("UPDATE I_PRODUCT i " + "SET ").append(numFieldsPO[i]).append(" = (SELECT ")
					.append(numFieldsPO[i])
					.append(" FROM M_Product_PO p"
							+ " WHERE i.M_Product_ID=p.M_Product_ID AND i.C_BPartner_ID=p.C_BPartner_ID AND i.AD_Client_ID=p.AD_Client_ID) "
							+ "WHERE M_Product_ID IS NOT NULL AND C_BPartner_ID IS NOT NULL"
							+ " AND EXISTS (SELECT * FROM M_Product_PO p WHERE " + numFieldsPO[i]
							+ " IS NOT NULL AND p.M_Product_ID = i.M_Product_ID AND i.AD_Client_ID=p.AD_Client_ID)"
							+ " AND I_IsImported='N'")
					.append(clientCheck);
			no = DB.executeUpdate(sql.toString());
		...
		for (int i = 0; i < numFieldsPO.length; i++) {
			sql = new StringBuffer("UPDATE I_PRODUCT i " + "SET ").append(numFieldsPO[i]).append(" = (SELECT ")
					...
					.append(numFieldsPO[i]).append(" IS NULL OR ").append(numFieldsPO[i])
		...
		int noInsert = 0, noUpdate = 0;
		int noInsertPO = 0, noUpdatePO = 0;
		...
		try {
			...
			ResultSet rs = pstmt.executeQuery();
			while (rs.next()) {
				int I_Product_ID = rs.getInt(1), M_Product_ID = rs.getInt(2);
