class n22328849 {
	protected void doRestoreOrganize() throws Exception {
		...
		String strDelQuery = "DELETE FROM " + Common.ORGANIZE_TABLE,
				strSelQuery = "SELECT organize_id,organize_type_id,organize_name,organize_manager,"
						+ "organize_describe,work_type,show_order,position_x,position_y " + "FROM "
						+ Common.ORGANIZE_B_TABLE + " " + "WHERE version_no = ?";
		String strInsQuery = "INSERT INTO " + Common.ORGANIZE_TABLE + " "
				...
				while (result.next()) {
					...
					if (resultCount != 1) {
						...
						throw new CesSystemException("Organize_backup.doRestoreOrganize(): ERROR Inserting data "
								+ "in T_SYS_ORGANIZE INSERT !! resultCount = " + resultCount);
					}
				}
				con.commit();
