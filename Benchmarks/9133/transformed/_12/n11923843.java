	...
	public String getLastVersion() {
		try {
			...
			Boolean useProxy = new Boolean(icescrum2Properties.get("proxy.active").toString()),
					authProxy = new Boolean(icescrum2Properties.get("proxy.auth.active").toString());
			URL url = new URL(server);
			if (useProxy) {
				String proxy = icescrum2Properties.get("proxy.url").toString(),
						port = icescrum2Properties.get("proxy.port").toString();
			...
			if (authProxy) {
				String username = icescrum2Properties.get("proxy.auth.username").toString(),
						password = icescrum2Properties.get("proxy.auth.password").toString();
