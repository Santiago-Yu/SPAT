class n21028028 {
	public int save(String newInventory, String inventoryType, int compareResult, boolean renameCorruptedFile) {
		...
		String directory_repository = iConfig.getFileRepository(), path = (String) parameters.get("path");
		if (path == null) {
		...
		String nodeDirectory_repository = "", file_repository = "";
		String oldPathToFile = "", newPathToFile = "";
		...
		try {
			...
			if (renameCorruptedFile) {
				...
				if (renameFile) {
					File f = new File(oldPathToFile), dest = new File(newPathToFile);
					dest.createNewFile();
					f.renameTo(dest);
				} else {
					FileWriter fileout = new FileWriter(newPathToFile);
