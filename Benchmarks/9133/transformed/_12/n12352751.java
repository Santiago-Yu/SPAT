	...
	public void startElement(String uri, String tag, String qName, org.xml.sax.Attributes attributes)
		...
		try {
			if (tag.equals("window")) {
				...
				int x = prop.getValue("x", 0), y = prop.getValue("y", 0);
				int width = prop.getValue("width", 0), height = prop.getValue("height", 0);
				...
				String id = prop.getValue("id", ""), label = prop.getValue("label", "");
				...
				JMenuItem item;
				String action = prop.getValue("action", ""), label = prop.getValue("label", "");
				...
				boolean editable = prop.getValue("editable", false), visible = prop.getValue("visible", true);
				...
				if (layout == 0)
					...
					sheet.getContentPane().add(combo, constraints);
				...
				if (layout == 0)
					sheet.getContentPane().add(sp);
				else
					sheet.getContentPane().add(sp, constraints);
			} else if (tag.equals("option")) {
				if (prop == null) {
					System.err.println("choice.option without properties");
					return;
				}
				String value = prop.getValue("value", ""), text = prop.getValue("text", "");
				if (list != null)
					listdata.addElement(new ComboOption(text, value));
				if (combo != null) {
					ComboOption co = new ComboOption(text, value);
					combo.addItem(co);
					if (combo_text.equals(text.trim()))
						combo.setSelectedItem(co);
				}
				...
				if (layout == 0)
					sheet.getContentPane().add(label);
				else
					sheet.getContentPane().add(label, constraints);
				...
				int x = prop.getValue("x", 0), y = prop.getValue("y", 0);
				int width = prop.getValue("width", 0), height = prop.getValue("height", 0);
				String id = prop.getValue("id", ""), text = prop.getValue("text", "");
				...
				if (layout == 0)
					sheet.getContentPane().add(btn);
				else
					sheet.getContentPane().add(btn, constraints);
				...
				if (layout == 0)
					sheet.getContentPane().add(rb);
				else
					sheet.getContentPane().add(rb, constraints);
				...
				if (layout == 0)
					sheet.getContentPane().add(cb);
				else
					sheet.getContentPane().add(cb, constraints);
				...
				JTextField sle;
				if (pwd.equalsIgnoreCase("true"))
					sle = new JPasswordField();
				else
					sle = new JTextField();
				int x = prop.getValue("x", 0), y = prop.getValue("y", 0);
				int width = prop.getValue("width", 0), height = prop.getValue("height", 0);
				...
				if (layout == 0)
					sheet.getContentPane().add(sle);
				else
					sheet.getContentPane().add(sle, constraints);
				...
				String text = prop.getValue("text", ""), value = prop.getValue("value", "");
				...
				System.err.println("[sheet] unparsed tag: " + tag);
		} catch (Exception e) {
