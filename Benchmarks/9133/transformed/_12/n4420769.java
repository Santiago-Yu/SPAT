class n4420769 {
	public void _saveWebAsset(ActionRequest req, ActionResponse res, PortletConfig config, ActionForm form, User user,
			String subcmd) throws WebAssetException, Exception {
		long maxsize = 50, maxwidth = 3000;
		long maxheight = 3000, minheight = 10;
		ActionRequestImpl reqImpl = (ActionRequestImpl) req;
		HttpServletRequest httpReq = reqImpl.getHttpServletRequest();
		try {
			UploadPortletRequest uploadReq = PortalUtil.getUploadPortletRequest(req);
			String parent = ParamUtil.getString(req, "parent");
			int countFiles = ParamUtil.getInteger(req, "countFiles"), fileCounter = 0;
			Folder folder = (Folder) InodeFactory.getInode(parent, Folder.class);
			_checkUserPermissions(folder, user, PERMISSION_WRITE);
			String userId = user.getUserId(),
					customMessage = "Some file does not match the filters specified by the folder: ";
			boolean filterError = false;
			for (int k = 0; k < countFiles; k++) {
				File file = new File();
				String title = ParamUtil.getString(req, "title" + k),
						friendlyName = ParamUtil.getString(req, "friendlyName" + k);
				...
				if (fileName.length() > 0) {
					String mimeType = FileFactory.getMimeType(fileName), URI = folder.getPath() + fileName;
					...
					if ((FileFactory.existsFileName(folder, fileName))) {
						...
						fileInodePath = fileInodePath.substring(0, 1) + java.io.File.separator
								+ fileInodePath.substring(1, 2);
						...
						java.io.File f = new java.io.File(filePath + java.io.File.separator + fileInodePath
								+ java.io.File.separator + file.getInode() + "." + suffix);
						...
						outputChannel.close();
						inputChannel.close();
						Logger.debug(this, "SaveFileAction New File in =" + filePath + java.io.File.separator
								+ fileInodePath + java.io.File.separator + file.getInode() + "." + suffix);
						if (suffix.equals("jpg") || suffix.equals("gif")) {
							com.dotmarketing.util.Thumbnail.resizeImage(
									filePath + java.io.File.separator + fileInodePath + java.io.File.separator,
									String.valueOf(file.getInode()), suffix);
