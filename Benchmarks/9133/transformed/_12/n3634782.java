	...
	public static void main(String[] args) {
		...
		try {
			...
			while (fr.ready()) {
				...
				String chr = tokens[0];
				int start = Integer.parseInt(tokens[1]), stop = Integer.parseInt(tokens[2]);
		...
		try {
			...
			for (Iterator intronIterator = intronReadCntMap.keySet().iterator(); intronIterator.hasNext();) {
				...
				Object ksInfoArray[] = distributionAccepter((TreeMap) splicingMap.clone(), readCnt, hdCDF, ksTable1,
						ksTable2);
				boolean ksAccepted = (Boolean) ksInfoArray[0];
				double testK = (Double) ksInfoArray[1], standardK1 = (Double) ksInfoArray[2];
				...
				int modelAgreedSiteCnt = (Integer) modelInfoArray[0],
						maxAgreedTransSiteCnt = (Integer) modelInfoArray[1];
				boolean containedBySomeGene = (Boolean) modelInfoArray[2];
				int numIntersectingGenes = (Integer) modelInfoArray[3], distance = intron.getStop() - intron.getStart();
				fw.write(intron.getChr() + ":" + intron.getStart() + ".." + intron.getStop() + "\t" + distance + "\t"
						+ readCnt + "\t" + splicingMap + "\t"
						+ probabilityEvaluation(readLength, distance, readCnt, splicingMap, positionCnt) + "\t"
						+ ksAccepted + "\t" + testK + "\t" + standardK1 + "\t" + standardK2 + "\t" + positionCnt + "\t"
						+ modelAgreedSiteCnt + "\t" + maxAgreedTransSiteCnt + "\t" + containedBySomeGene + "\t"
						+ numIntersectingGenes + "\n");
			}
			fw.close();
		} catch (IOException ex) {
			ex.printStackTrace();
