class n22031237 {
	...
	public ReturnValue do_run() {
		...
		if (((String) options.valueOf("input-file")).startsWith("s3://")) {
			...
			result = m.find();
			if (result) {
				String bucket = m.group(1), key = m.group(2);
			...
			Matcher m = p.matcher((String) options.valueOf("input-file"));
			boolean result = m.find();
			String protocol = null, user = null;
			String pass = null, URL = (String) options.valueOf("input-file");
			if (result) {
				protocol = m.group(1);
				user = m.group(2);
				pass = m.group(3);
			...
			try {
				...
				if (urlObj != null) {
					...
					result = m.find();
					if (result) {
						String host = m.group(1), path = m.group(2);
