class n2305051 {
	public CandleSeries fetchSeries(final String symbol) throws Exception {
		...
		cal.setTime(begin);
		String beginYear = String.valueOf(cal.get(Calendar.YEAR)), beginMonth = String.valueOf(cal.get(Calendar.MONTH));
		...
		cal.setTime(end);
		String day = String.valueOf(cal.get(Calendar.DAY_OF_MONTH)), month = String.valueOf(cal.get(Calendar.MONTH));
		String year = String.valueOf(cal.get(Calendar.YEAR)), resolution = "d";
		...
		for (String currentLine : lineList) {
			...
			double open = round(Double.parseDouble(str.nextToken()), 2), high = Double.parseDouble(str.nextToken());
			double low = Double.parseDouble(str.nextToken()), close = Double.parseDouble(str.nextToken());
			long volume = 0;
			double adjclose = 0;
