class n20588811 {
	public void run() {
		...
		String hostname = properties.getProperty("mercuriushost"),
				protocol = properties.getProperty("mercuriusprotocol");
		String port = properties.getProperty("mercuriusport"), path = properties.getProperty("mercuriuspath");
		...
		if (timexfrmk.getSessionCount() > 0) {
			try {
				...
				if (isAdminFeedKeySet()) {
					data += "&" + URLEncoder.encode("adminfeedkey", "UTF-8") + "="
