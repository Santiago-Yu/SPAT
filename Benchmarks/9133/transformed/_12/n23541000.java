class n23541000 {
	public static boolean matchPassword(String prevPassStr, String newPassword)
		...
		byte[] seed = new byte[12], prevPass = new sun.misc.BASE64Decoder().decodeBuffer(prevPassStr);
		System.arraycopy(prevPass, 0, seed, 0, 12);
		md.update(seed);
		md.update(newPassword.getBytes("UTF8"));
		byte[] digestNewPassword = md.digest(), choppedPrevPassword = new byte[prevPass.length - 12];
