	...
	private String AddAction(ResultSet node, String modo) throws SQLException {
		...
		try {
			...
			cndef = this.getRepositoryConnection(p_ctx.getApplication(), p_ctx.getBoSession().getRepository().getName(),
					2);
			String dml = null, objecttype = node.getString("OBJECTTYPE");
			if (objecttype.equalsIgnoreCase("T")) {
				boolean exists = existsTable(p_ctx, schema, node.getString("OBJECTNAME").toLowerCase());
				String[] sysflds = { "SYS_USER", "SYS_ICN", "SYS_DTCREATE", "SYS_DTSAVE", "SYS_ORIGIN" }, sysfdef = {
						"VARCHAR(25)", "NUMERIC(7)", "TIMESTAMP DEFAULT now()", "TIMESTAMP", "VARCHAR(30)" };
				String[] sysftyp = { "C", "N", "D", "D", "C" }, sysfsiz = { "25", "7", "", "", "30" };
				String[] sysfndef = { "", "", "", "", "" }, sysfdes = { "", "", "", "", "" };
			...
			if (objecttype.equalsIgnoreCase("F")) {
				boolean fldchg = false, fldexi = false;
				...
				if (rslt.next()) {
					int fieldsiz = rslt.getInt(3), fielddec = rslt.getInt(5);
				...
				if (("20".indexOf(modo) != -1) && !fldexi) {
					...
					while (rsltrelc.next()) {
						String constname = rsltrelc.getString(1), consttype = rsltrelc.getString(2);
			...
			if (objecttype.startsWith("PCK")) {
				String templatestr = node.getString("EXPRESSION"), bstr = "/*begin_package*/";
			...
			if (objecttype.startsWith("FK")) {
				...
				ResultSet rslt = pstm.executeQuery();
				boolean exists = false;
				String cExpress = "", express = node.getString("EXPRESSION");
				if (rslt.next()) {
					exists = true;
			...
			if (objecttype.startsWith("IDX")) {
				...
				ResultSet rslt = pstm.executeQuery();
				boolean drop = false, exists = false;
