class n17829989 {
	...
	public void run() {
		...
		if (dataV.size() > 0) {
			int[] dataPending = new int[dataV.size()], dataOccupied = new int[dataV.size()];
			long[] dataDropped = new long[dataV.size()], dataException = new long[dataV.size()];
			int[] dataWeight = new int[dataV.size()];
			long[] dataExecution = new long[dataV.size()], dataWait = new long[dataV.size()];
