class n14800928 {
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if (-1 == bytes.length || 23 > bytes.length)
				throw new Exception();
			MsgPrint.showMsg("========byte length" + bytes.length);
			String userTag = request.getParameter("userTag"), isEncrypt = request.getParameter("isEncrypt");
			...
			byte[] resultBytes = new byte[httpConn.getContentLength()], tempByte = new byte[1024];
		...
		if (null != result) {
			...
			sos.write(new byte[] { 48, 48, 55, -23, 3, 56, 49, 54, 57, 55, 49, 51, 54, 72, 71, 52, 48, 1, 3, 3, 48, 48,
					48, 48, 48, 48 });
		}
		sos.flush();
		sos.close();
