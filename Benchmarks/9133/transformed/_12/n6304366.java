class n6304366 {
	public static String uploadArticleMedia(String localPath, String articleImageName, String year, String month,
			String day, DataStore db, HttpSession session) {
		...
		String liveFTPLogin = (String) user.workingPubConfigElementsHash.get("LIVEFTPLOGIN"),
				liveFTPPassword = (String) user.workingPubConfigElementsHash.get("LIVEFTPPASSWORD");
		String liveImagesServer = (String) user.workingPubConfigElementsHash.get("LIVEFTPSERVER"),
				liveImagesFolder = (String) user.workingPubConfigElementsHash.get("LIVEIMAGESFOLDER");
		if (!liveImagesFolder.endsWith("/")) {
			liveImagesFolder = liveImagesFolder + "/";
		}
		String liveImagesYearFolder = "", liveImagesMonthFolder = "";
		String fileLocation = "";
		...
		for (int count = 0; count < servers.size(); count++) {
			...
			try {
				...
				try {
					...
					if (changed == false) {
						CofaxToolsUtil.log("CofaxToolsFTP: uploadArticleMedia attempting to create directory :"
								+ liveImagesFolder);
						boolean newDirYear = ftp.makeDirectory(liveImagesYearFolder),
								newDirMonth = ftp.makeDirectory(liveImagesMonthFolder);
						boolean newDir = ftp.makeDirectory(liveImagesFolder);
						CofaxToolsUtil.log("CofaxToolsFTP: uploadArticleMedia results: YearDir: " + newDirYear
								+ " MonthDir: " + newDirMonth + " finalDir: " + newDir);
