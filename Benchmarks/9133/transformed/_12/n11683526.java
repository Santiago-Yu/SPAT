class n11683526 {
	private static boolean moveFiles(String sourceDir, String targetDir) {
		boolean isFinished = false, fileMoved = false;
		...
		if (stagingDir.listFiles() == null) {
			System.out.println(getTimeStamp()
					+ "ERROR - Empty file list. Possible permission error on source directory " + sourceDir);
			return true;
		}
		...
		for (int x = 0; x < fileList.length; x++) {
			...
			String targetFileName = targetDir + File.separator + f.getName(), operation = "move";
			...
			if (success) {
				...
				try {
					Process process = Runtime.getRuntime()
							.exec(new String[] { "mv", f.getCanonicalPath(), targetFileName });
					process.waitFor();
					process.destroy();
			...
			if (success) {
				...
				System.out.println(getTimeStamp() + "ERROR - " + operation + " " + f.getName() + " to " + targetFileName
						+ " failed.");
				isFinished = true;
			}
		}
		if (fileMoved && !isFinished) {
			try {
				...
				System.out.println(
						getTimeStamp() + "ERROR: IOException when trying to update lastActivity. " + e.toString());
				isFinished = true;
			}
		}
		return isFinished;
