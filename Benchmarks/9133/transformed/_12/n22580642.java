class n22580642 {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		boolean validClient = true, validSession = false;
		String sessionKey = req.getParameter("sid"), storedKey = CLIENT_SESSION_KEYS.get(req.getRemoteAddr());
		if (sessionKey != null && storedKey != null && sessionKey.equals(storedKey))
			validSession = true;
		...
		if (cmd.equals("pop")) {
			if (!validClient) {
				...
				String offDay = clnt.getGlobalOption("OFFDAY"), offHour = clnt.getGlobalOption("OFFHOUR");
			...
			if (validSession) {
				String mediaId = req.getParameter("m"), taskId = req.getParameter("t");
				String objType = req.getParameter("o");
				if ((mediaId != null && !mediaId.equals("0")) && (taskId != null && !taskId.equals("0")))
			...
			if (!validClient) {
				...
				String mediaId = req.getParameter("m"), taskId;
				if (!mediaId.equals("-1"))
			...
			try {
				MessageDigest msg = MessageDigest.getInstance("MD5");
				String curPwd = ds.getSetting("password", ""), oldPwd = req.getParameter("old");
				msg.update(oldPwd.getBytes());
				oldPwd = new String(msg.digest());
				msg.reset();
				String newPwd = req.getParameter("new"), confPwd = req.getParameter("conf");
				if (!curPwd.equals(oldPwd))
					bw.write("BadOld");
				else if (!newPwd.equals(confPwd) || newPwd.length() == 0)
					bw.write("BadNew");
				else {
			...
			if (validSession) {
				String mid = req.getParameter("m"), tid = req.getParameter("t");
