class n22222255 {
	public void doQuery(String keyword, final int page) {
		...
		int matchingDocCount = 0, hitCount = 0;
		...
		try {
			...
			Box tabBox = null;
			String linkstr, comment = "";
			int EnsExtCount, EnsGeneCount;
			while ((line = reader.readLine()) != null) {
				...
				if (line.indexOf("Homo_sapiens/geneview?gene") != -1) {
					...
					boolean inTag = false, inEntity = false;
					boolean firstBRTossed = false;
					line = line.substring(line.indexOf("<"));
					for (int ch = 0; ch < line.length(); ch++) {
						if (line.charAt(ch) == '<') {
							inTag = true;
							if ((line.charAt(ch + 1) == 'b' || line.charAt(ch + 1) == 'B')
									&& (line.charAt(ch + 2) == 'r' || line.charAt(ch + 2) == 'R')) {
								if (firstBRTossed) {
									chars.append("\n");
								} else {
									firstBRTossed = true;
		...
		setTitle("Results for " + keyword + "  Displaying " + (page + 1)
				+ ((hitCount > 1) ? (" - " + (page + hitCount)) : "") + " of " + matchingDocCount);
		show();
	}
