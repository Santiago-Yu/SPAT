class n17116122 {
	public void addAuthors() throws Exception {
		...
		String[] pids = proposalIds.split(","), uids = usrIds.split(",");
		int pnum = pids.length, unum = uids.length;
		if (pnum == 0 || unum == 0)
			throw new Exception("No proposal or author selected.");
		int i, j;
		...
		String dt = String.valueOf(year) + "-" + String.valueOf(month + 1) + "-" + String.valueOf(day), pStr = "";
		...
		try {
			...
			pStr = "insert into event (summary,document1,document2,document3,publicComments,privateComments,ACTION_ID,eventDate,ROLE_ID,reviewText,USR_ID,PROPOSAL_ID,SUBJECTUSR_ID) values "
					+ "('','','','','','','member added','" + dt + "',2,'add member'," + userId + ",?,?)";
