	...
	public static IProject createEMFProject(IPath javaSource, URI projectLocationURI, List<IProject> referencedProjects,
		...
		try {
			...
			boolean isInitiallyEmpty = classpathEntries.isEmpty();
			{
				...
				boolean hasManifestBuilder = false, hasSchemaBuilder = false;
				...
				if (isInitiallyEmpty) {
					...
					String jreContainer = JavaRuntime.JRE_CONTAINER,
							complianceLevel = CodeGenUtil.EclipseUtil.getJavaComplianceLevel(project);
						...
						if (pluginVariables != null) {
							for (Iterator<?> i = pluginVariables.iterator(); i.hasNext();) {
								...
								if (variable instanceof IClasspathEntry) {
									...
									String pluginVariable = (String) variable, name;
									String id;
