class n15723802 {
	void createAgentProgram(PrintStream ps, String agentName, String agentID) throws IOException, ConfigException {
		...
		String ii = "\"", agentScript = "agent-" + agentName + ".py";
		...
		}
		{
			...
			as.println(tab[i] + "    " + ii + "create a namespace for injection " + "into a task script and returns it."
					+ ii);
			...
			for (VarAttributes va : attributesMap.values()) {
				...
				if (Constants.VAR_STDOUT.equals(varname))
					continue;
			...
			for (String taskName : taskNames) {
				...
				as.println(tab[i] + "def task_" + taskName + "():");
				{
					...
					if (background) {
						...
						as.println(tab[i] + "    port_task=ZSIWrapper(URL, agentID, "
					...
					as.println(
							tab[i] + "    port0.setTaskStatus('" + taskName + "', " + Status.FAILED.ordinal() + " )");
			...
			as.println(tab[i] + "while action != '" + Agent.STOP + "' and action != '" + Agent.KILL
					+ "' and action != '" + Agent.FINISH + "' :");
			{
				...
				for (String taskName : taskNames) {
					...
					if (background) {
						...
						as.println(
								tab[i] + "    t = threading.Thread(target=" + function + ", name='" + function + "')");
