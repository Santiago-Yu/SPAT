		...
		if (file.canRead()) {
			...
			try {
				...
				while (lineReader.ready()) {
					...
					int idx0 = 0, idx1 = data.indexOf(SERVICE_FIELD_SEPERATOR);
					String name = StringUtilities.decodeASCII(data.substring(idx0, idx1)),
							uri = StringUtilities.decodeASCII(data.substring(idx1 + 1));
					...
					try {
						...
						try {
							...
							bindName = ServiceDigester.parseService(inputSource, IsqlToolkit.getSharedEntityResolver())
									.getName();
