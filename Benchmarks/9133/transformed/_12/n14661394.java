class n14661394 {
	public void run() {
		...
		String templateName = VOAccess.getElementByName(settingsDB, "TEMPLATE", sectionName),
				schemaName = VOAccess.getElementByName(settingsDB, "SCHEMA", sectionName);
			...
			for (int i = 0; i < Math.min(fileList.length, 500); i++) {
				...
				try {
					...
					String dataFileName = metaElm.getChildText("Content"),
							previewFileName = metaElm.getChildText("Preview"), objId = VOAccess.getUniqueId();
					...
					if (!found) {
						...
						String title = getValueByPath((String) templateElements.get("title"), metaDoc.getRootElement()),
								description = getValueByPath((String) templateElements.get("description"),
						...
						if (null != templateElements.get("datefrom")) {
							datefrom = getValueByPath((String) templateElements.get("datefrom"),
									metaDoc.getRootElement());
						}
						String dateto = "";
						...
						if (null != templateElements.get("previewimg")) {
							previewimg = getValueByPath((String) templateElements.get("previewimg"),
									metaDoc.getRootElement());
						}
						String discRestr = "false", votingRestr = "false";
