class n19382420 {
	protected void processRequest(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			String mdPrefix = "", mdPrefixValue = "", from = "", fromValue = "";
			String until = "", untilValue = "", set = "", setValue = "";
			String oai_pmhRequest = request.getParameter("verb"), oai_pmhQuery = request.getQueryString();
			String urlStr = "";
			urlStr = "http://" + request.getServerName() + ":" + request.getServerPort()
			...
			if (vector.size() > 0) {
				for (int k = 0; k < vector.size(); k = k + 3) {
					...
					String harvesturl = vector.elementAt(k + 1).toString(), status = vector.elementAt(k + 2).toString();
					if (status.equals("A")) {
						String oai_pmhRequest1 = request.getParameter("verb"), oai_pmhQuery1 = request.getQueryString();
						urlStr = "http://" + harvesturl
								+ ":8080/newgenlibctxt/oai2.0?verb=ListRecords&metadataPrefix=marc21&from=" + fromValue
								+ "&until=" + untilValue + "";
						...
						for (int n = 0; resExists == true; n++) {
							...
							String resT = doc.getRootElement().getChildText("resumptionToken",
									doc.getRootElement().getNamespace());
							if (!(resT == null)) {
								urlStr = "http://" + harvesturl
										+ ":8080/newgenlibctxt/oai2.0?verb=ListRecords&resumptionToken=" + resT;
							} else {
								resExists = false;
							}
						}
						...
						for (int b = 0; b < file1.length; b++) {
							...
							java.util.Date d1 = c1.getTime(), d2 = c2.getTime();
