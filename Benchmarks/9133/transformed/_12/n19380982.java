class n19380982 {
	...
	private boolean copyAvecProgressNIO(File sRC2, File dEST2, JProgressBar progressEnCours) {
		...
		java.nio.channels.FileChannel channelSrc = fis.getChannel(), channelDest = fos.getChannel();
		...
		try {
			...
			try {
				Historique.ecrire("Erreur ? la copie du fichier " + sRC2 + " vers la destination " + dEST2
		...
		try {
			...
			try {
				Historique.ecrire("Impossible de fermer le flux ? la copie du fichier " + sRC2
						+ " pour la raison suivante : " + e);
			} catch (IOException e1) {
				e1.printStackTrace();
			}
		...
		try {
			...
			try {
				Historique.ecrire("Impossible de fermer le flux ? la copie du fichier " + dEST2
						+ " pour la raison suivante : " + e);
			} catch (IOException e1) {
				e1.printStackTrace();
			}
