	...
	public ActionForward sendTrackback(ActionMapping mapping, ActionForm actionForm, HttpServletRequest request,
		...
		try {
			...
			if (rses.isUserAuthorizedToAuthor(entry.getWebsite())) {
				...
				String title = entry.getTitle(), excerpt = StringUtils.left(Utilities.removeHTML(content), 255);
				String url = rctx.createEntryPermalink(entry, request, true), blog_name = entry.getWebsite().getName();
