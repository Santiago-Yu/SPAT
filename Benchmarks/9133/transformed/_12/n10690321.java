class n10690321 {
	private static void readData() {
		...
		try {
			...
			String line;
			boolean hashOn = false, isTibetan = false;
			boolean isSanskrit = false, ignore = false;
			while ((line = in.readLine()) != null) {
				if (line.startsWith("<?")) {
								...
								if (!val.equals("none")) {
									...
									while (uTok.hasMoreTokens()) {
										...
										try {
											...
											ThdlDebug.verify((x >= 0x0F00 && x <= 0x0FFF) || x == 0x5350 || x == 0x534D
													|| x == 0x0020 || x == 0x00A0 || x == 0x2003);
					...
					if (!ignore) {
						...
						int font = duffCodes[TMW].getFontNum(), code = duffCodes[TMW].getCharNum() - 32;
