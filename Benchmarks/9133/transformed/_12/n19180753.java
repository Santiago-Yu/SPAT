	...
	public final void loadAllData(final String ticker, final File output, final CSVFormat outputFormat, final Date from,
		...
		try {
			...
			while (csv.next() && !shouldStop()) {
				...
				final double adjClose = csv.getDouble("adj close"), open = csv.getDouble("open");
				final double close = csv.getDouble("close"), high = csv.getDouble("high");
				final double low = csv.getDouble("low"), volume = csv.getDouble("volume");
