class n20856618 {
	public void downloadClicked() {
		String s_url;
		try {
			double minlat = Double.parseDouble(minLat.text()), maxlat = Double.parseDouble(maxLat.text());
			double minlong = Double.parseDouble(minLong.text()), maxlong = Double.parseDouble(maxLong.text());
			s_url = "http://www.openstreetmap.org/api/0.5/map?bbox=" + minlong + "," + minlat + "," + maxlong + ","
					+ maxlat;
		} catch (Exception e) {
