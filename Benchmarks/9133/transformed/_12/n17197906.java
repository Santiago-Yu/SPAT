	...
	public void run() {
		...
		if (IsAlreadyDownloaded) {
			...
			for (URL url : ListFastest) {
				...
				long timestart = System.currentTimeMillis(), bytecount = 0;
				try {
				...
				if (OnError) {
					...
					BigDecimal speed = new BigDecimal(bytecount).multiply(new BigDecimal(1000))
							.divide(new BigDecimal(timetook), MathContext.DECIMAL32);
