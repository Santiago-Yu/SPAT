class n21652119 {
	protected void setupService(MessageContext msgContext) throws Exception {
		String realpath = msgContext.getStrProp(Constants.MC_REALPATH),
				extension = (String) getOption(OPTION_JWS_FILE_EXTENSION);
		if (extension == null)
			extension = DEFAULT_JWS_FILE_EXTENSION;
		if ((realpath != null) && (realpath.endsWith(extension))) {
			String jwsFile = realpath, rel = msgContext.getStrProp(Constants.MC_RELATIVE_PATH);
			...
			String file = rel.substring(lastSlash + 1), outdir = msgContext.getStrProp(Constants.MC_JWS_CLASSDIR);
			if (outdir == null)
				outdir = ".";
			if (dir != null) {
			...
			String jFile = outdir + File.separator + file.substring(0, file.length() - extension.length() + 1) + "java",
					cFile = outdir + File.separator + file.substring(0, file.length() - extension.length() + 1)
							+ "class";
			if (log.isDebugEnabled()) {
