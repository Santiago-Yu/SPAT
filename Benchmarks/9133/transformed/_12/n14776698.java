			...
			try {
				...
				if (file.isDirectory()) {
					...
					if (existed = outFile.exists()) {
						int res = askOnFileExist(c.getString(R.string.file_exist, outFile.getAbsolutePath()),
								commander);
					...
					long size = in.size(), max_chunk = 524288;
					long pos = 0, chunk = size > max_chunk ? max_chunk : size;
					long t_chunk = 0, start_time = 0;
					int speed = 0, so_far = (int) (totalBytes * conv);
					String sz_s = Utils.getHumanSize(size), rep_s = c.getString(R.string.copying, fn);
					for (pos = 0; pos < size;) {
						if (t_chunk == 0)
					...
					out = null;
					if (i >= list.length - 1)
						sendProgress(c.getString(R.string.copied_f, fn) + sizeOfsize(pos, sz_s),
								(int) (totalBytes * conv));
					counter++;
				}
				if (move)
