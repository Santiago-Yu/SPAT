class n5189131 {
	public void service(Request req, Response resp) {
		...
		try {
			...
			String env = req.getParameter("env"), regex = req.getParameter("regex");
			String deep = req.getParameter("deep"), term = req.getParameter("term");
			String index = req.getParameter("index"), refresh = req.getParameter("refresh");
			String searcher = req.getParameter("searcher"), grep = req.getParameter("grep");
			String fiServerDetails = req.getParameter("fi_server_details"),
					serverDetails = req.getParameter("server_details");
			String hostDetails = req.getParameter("host_details"), name = req.getParameter("name");
			String show = req.getParameter("show"), path = req.getPath().getPath();
			...
			if (path.startsWith("/fs")) {
				...
				if (segments.length > 1) {
					...
					if (file.isFile()) {
						...
						channel.close();
						fin.close();
						System.err.println("Time take to write [" + realPath + "] was ["
								+ (System.currentTimeMillis() - start) + "] of size [" + file.length() + "]");
						done = true;
					}
				}
				if (!done) {
				...
				for (int i = 0; i < 10; i++) {
					...
					if (file != null && file.isFile()) {
						String partName = file.getName(), partFileName = file.getFileName();
						...
						if (partFileName.toLowerCase().endsWith(".xml")) {
							...
							String formatted = format(xml), fileFormatName = fileName + ".formatted";
				...
				File indexFile = getStoredProcIndexFile(engine.getRoot(), index), search = new File(root, "cpsql");
				File source = new File(root, file.replace('/', File.separatorChar));
				FindStoredProcs.StoredProcProject storedProcProj = FindStoredProcs.getStoredProcProject(search,
						indexFile);
				...
				for (String procName : proc.getReferences()) {
					...
					if (theProc != null) {
						...
						out.println("<a href='/?show=" + url + "&index=" + index + "&searcher=" + searcher + "'><b>"
								+ theProc.getName() + "</b>");
					}
				}
				...
				if (authentication == null) {
					...
					File javaIndexFile = getJavaIndexFile(root, index),
							storedProcIndexFile = getStoredProcIndexFile(root, index);
					File sql = new File(root, "cpsql"), source = new File(root, file.replace('/', File.separatorChar));
					File javaSource = new File(root, file.replace('/', File.separatorChar)),
							canonical = source.getCanonicalFile();
					...
					if (info != null) {
						out.println("<table border='1'>");
						out.printf("<tr><td bgcolor=\"#C4C4C4\"><tt>Author</tt></td><td><tt>" + info.author
								+ "</tt></td></tr>");
						out.printf("<tr><td bgcolor=\"#C4C4C4\"><tt>Version</tt></td><td><tt>" + info.version
								+ "</tt></td></tr>");
						out.printf("<tr><td bgcolor=\"#C4C4C4\"><tt>URL</tt></td><td><tt>" + info.location
								+ "</tt></td></tr>");
						out.printf("<tr><td bgcolor=\"#C4C4C4\"><tt>Path</tt></td><td><tt>" + canonical
								+ "</tt></td></tr>");
						out.println("</table>");
					}
					out.println("<table border='1''>");
					...
					FindStoredProcs.StoredProcProject storedProcProj = FindStoredProcs.getStoredProcProject(sql,
							storedProcIndexFile);
						...
						if (project != null) {
							for (FindJavaSources.JavaClass entry : importList) {
								String className = entry.getClassName(),
										fullyQualifiedName = entry.getFullyQualifiedName();
								if (line.startsWith("import") && line.indexOf(fullyQualifiedName) > -1) {
									...
									escaped = escaped.replaceAll(fullyQualifiedName + ";",
											"<a href='/?show=" + url + "&index=" + index + "&searcher=" + searcher
													+ "'>" + fullyQualifiedName + "</a>;");
								} else if (line.indexOf(className) > -1) {
									File classFile = entry.getSourceFile();
									String url = getRelativeURL(root, classFile);
									escaped = escaped.replaceAll("\\s" + className + ",", " <a href='/?show=" + url
											+ "&index=" + index + "&searcher=" + searcher + "'>" + className + "</a>,");
									escaped = escaped.replaceAll("\\s" + className + "\\{", " <a href='/?show=" + url
											+ "&index=" + index + "&searcher=" + searcher + "'>" + className + "</a>{");
									escaped = escaped.replaceAll("," + className + ",", ",<a href='/?show=" + url
											+ "&index=" + index + "&searcher=" + searcher + "'>" + className + "</a>,");
									escaped = escaped.replaceAll("," + className + "\\{", ",<a href='/?show=" + url
											+ "&index=" + index + "&searcher=" + searcher + "'>" + className + "</a>{");
									escaped = escaped.replaceAll("\\s" + className + "\\s", " <a href='/?show=" + url
											+ "&index=" + index + "&searcher=" + searcher + "'>" + className + "</a> ");
									escaped = escaped.replaceAll("\\(" + className + "\\s", "(<a href='/?show=" + url
											+ "&index=" + index + "&searcher=" + searcher + "'>" + className + "</a> ");
									escaped = escaped.replaceAll("\\s" + className + "\\.", " <a href='/?show=" + url
											+ "&index=" + index + "&searcher=" + searcher + "'>" + className + "</a>.");
									escaped = escaped.replaceAll("\\(" + className + "\\.", "(<a href='/?show=" + url
									...
									escaped = escaped.replaceAll("&gt;" + className + ",", "&gt;<a href='/?show=" + url
											+ "&index=" + index + "&searcher=" + searcher + "'>" + className + "</a>,");
									escaped = escaped.replaceAll("&gt;" + className + "\\s",
											"&gt;<a href='/?show=" + url + "&index=" + index + "&searcher=" + searcher
													+ "'>" + className + "</a> ");
									escaped = escaped.replaceAll("&gt;" + className + "&lt;",
											"&gt;<a href='/?show=" + url + "&index=" + index + "&searcher=" + searcher
													+ "'>" + className + "</a>&lt;");
									escaped = escaped.replaceAll("\\(" + className + "\\);", "(<a href='/?show=" + url
											+ "&index=" + index + "&searcher=" + searcher + "'>" + className + "</a>)");
							...
							for (String procFound : sortedProcs) {
								if (escaped.indexOf(procFound) != -1) {
									...
									if (nameFile != null) {
										...
										escaped = escaped.replaceAll("\\s" + procFound + "\\s",
												" <a href='/?show=" + url + "&index=" + index + "&searcher=" + searcher
														+ "'>" + procFound + "</a> ");
										escaped = escaped.replaceAll("\\s" + procFound + ",",
												" <a href='/?show=" + url + "&index=" + index + "&searcher=" + searcher
														+ "'>" + procFound + "</a>,");
										escaped = escaped.replaceAll("\\s" + procFound + ";",
												" <a href='/?show=" + url + "&index=" + index + "&searcher=" + searcher
														+ "'>" + procFound + "</a>;");
										escaped = escaped.replaceAll("," + procFound + "\\s",
												",<a href='/?show=" + url + "&index=" + index + "&searcher=" + searcher
														+ "'>" + procFound + "</a> ");
										escaped = escaped.replaceAll("," + procFound + ",",
												",<a href='/?show=" + url + "&index=" + index + "&searcher=" + searcher
														+ "'>" + procFound + "</a>,");
										escaped = escaped.replaceAll("," + procFound + ";",
												",<a href='/?show=" + url + "&index=" + index + "&searcher=" + searcher
					...
					for (Change message : logMessages) {
						out.printf(
								...
								message.version, format.format(message.date).replaceAll("\\s", "&nbsp;"),
					...
					if (project != null) {
						...
						for (FindJavaSources.JavaClass entry : importList) {
							...
							out.println("import <a href='/?show=" + url + "&index=" + index + "&searcher=" + searcher
									+ "'>" + entry.getFullyQualifiedName() + "</a> as " + entry.getClassName());
						}
						out.println("</pre>");
					}
					if (storedProc != null) {
						...
						for (String procName : storedProc.getReferences()) {
							...
							if (proc != null) {
								...
								out.println("using <a href='/?show=" + url + "&index=" + index + "&searcher=" + searcher
										+ "'>" + proc.getName() + "</a>");
							}
						}
						out.println("</pre>");
				...
				ServerDetails details = config.getEnvironment(env).load(persister, serverDetails != null,
						fiServerDetails != null, hostDetails != null);
				...
				for (Searchable value : list) {
					out.println("    <tr><td>" + countIndex++ + "&nbsp;<a href='" + value.getSource() + "'><b>"
							+ value.getSource() + "</b></a></td></tr>");
				...
				boolean isRefresh = refresh != null, isGrep = grep != null;
