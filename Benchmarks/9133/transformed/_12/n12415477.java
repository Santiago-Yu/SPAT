class n12415477 {
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		String limitsSet = "",
				synstatus = SystemFilesLoader.getInstance().getNewgenlibProperties().getProperty("SYNDETICS", "OFF");
		...
		if (svF.getExportRec() == null || !(svF.getExportRec().equals("export"))) {
			if (request.getParameter("CatId") != null && request.getParameter("OwnerId") != null
				...
				String catId1 = request.getParameter("CatId"), ownId1 = request.getParameter("OwnerId");
				...
				if ((request.getParameter("ClassNo") != null) && session.getAttribute("ClassNoForwarded") == null) {
					...
					String record = request.getParameter("record"), recNo = request.getParameter("recNo");
					...
					for (int i = 0; i < alOtherBooks.size(); i++) {
						...
						String catalogueId = scData[0], ownerLibId = scData[1];
						System.out.println("catId is +++=" + catalogueId);
						System.out.println("OwnerId is +++=" + ownerLibId);
						String xmlWholeRecord = "", titleD = "";
						String titleV = "", authorV = "";
						String isbnNumber = "";
					...
					if (vecCO != null) {
						for (int j = 0; j < vecCO.size(); j = j + 4) {
							int c = new Integer(vecCO.elementAt(j).toString()).intValue(),
									o = new Integer(vecCO.elementAt(j + 1).toString()).intValue();
							if (c == catId && o == ownerId) {
								if (j != 0) {
									int catPrev = new Integer(vecCO.elementAt(j - 4).toString()).intValue(),
											ownerPrev = new Integer(vecCO.elementAt(j - 3).toString()).intValue();
								...
								if (j < vecCO.size() - 4) {
									int catNext = new Integer(vecCO.elementAt(j + 4).toString()).intValue(),
											ownerNext = new Integer(vecCO.elementAt(j + 5).toString()).intValue();
					...
					int bib_id = new Integer(str[0]).intValue(), mat_id = new Integer(str[1]).intValue();
