class n22075089 {
	public static void executa(String arquivo, String filial, String ip) {
		...
		String servidor = p.getProperty("servidor"), impressora = p.getProperty("fila");
		String arqRel = new String(drive + p.getProperty("arquivo")), copias = p.getProperty("copias");
		if (filial.equalsIgnoreCase(servidor)) {
			...
			while (impressora.length() < 30)
				impressora += " ";
			byte aux[] = new byte[30], cop[] = new byte[2];
			try {
