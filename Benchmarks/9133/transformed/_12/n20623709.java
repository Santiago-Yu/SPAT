class n20623709 {
	public Long split(File targetDirectory, String prefix, long maxUnitBases, long maxUnitEntries) throws Exception {
		...
		if (!size.isUnknown() && size.getBases() < maxUnitBases
			...
			long currentBasesCount = 0, currentEntriesCount = 0;
			int targetCount = 0;
			FileChannel fastaChannel = new FileInputStream(this).getChannel();
			int totalSeqCount = 0;
			long totalResiduesCount = 0;
			try {
				long prevTime = System.currentTimeMillis(), fastaFileSize = this.length();
				long fastaFileReadOffset = 0L, partitionStartOffset = 0L;
