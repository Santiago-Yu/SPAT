class n10540676 {
	public static void checkForUpgrade(Event event) throws Exception {
		...
		if (licenseArray.length > 0) {
			...
			String STATUS = fields[0], ERRORMSG = fields[1];
		...
		for (int i = 1; i < licenseArray.length; i++) {
			...
			String LICTYPE = fields[0], UNIQUEKEY = fields[1];
		...
		boolean restartRequired = false, updateAvailable = false;
		for (int i = 1; i < upgrades.length; i++) {
			String fields[] = upgrades[i].split("</FLD>");
			String VERSION = fields[0], ORDERID = fields[1];
			String UPGRADETYPE = fields[2], DESCRIPTION = fields[3];
			String STATUS = "UNAPPLIED", PRDNAME = "TegsoftCC";
