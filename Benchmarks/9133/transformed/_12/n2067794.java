class n2067794 {
	public SingularValueDecomposition(Matrix Arg) {
        ...
        int nct = Math.min(m - 1, n), nrt = Math.max(0, Math.min(n - 2, m));
        ...
        int pp = p - 1, iter = 0;
        double eps = Math.pow(2.0, -52.0), tiny = Math.pow(2.0, -966.0);
        while (p > 0) {
            int k, kase;
            for (k = p - 2; k >= -1; k--) {
                if (k == -1) {
                ...
                case 3:
                    {
                        ...
                        double sp = s[p - 1] / scale, spm1 = s[p - 2] / scale;
                        double epm1 = e[p - 2] / scale, sk = s[k] / scale;
                        double ek = e[k] / scale, b = ((spm1 + sp) * (spm1 - sp) + epm1 * epm1) / 2.0;
                        double c = (sp * epm1) * (sp * epm1), shift = 0.0;
                        if ((b != 0.0) | (c != 0.0)) {
