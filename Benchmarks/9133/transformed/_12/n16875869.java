	...
	public static ArrayList<Quote> fetchAllQuotes(String symbol, Date from, Date to) {
		try {
			...
			String monthFrom = (new Integer(calendar.get(GregorianCalendar.MONTH))).toString(),
					dayFrom = (new Integer(calendar.get(GregorianCalendar.DAY_OF_MONTH))).toString();
			...
			String monthTo = (new Integer(calendar.get(GregorianCalendar.MONTH))).toString(),
					dayTo = (new Integer(calendar.get(GregorianCalendar.DAY_OF_MONTH))).toString();
			String yearTo = (new Integer(calendar.get(GregorianCalendar.YEAR))).toString();
			URL url = new URL(
					"http://ichart.finance.yahoo.com/table.csv?s=" + symbol + "&a=" + monthFrom + "&b=" + dayFrom
							+ "&c=" + yearFrom + "&d=" + monthTo + "&e=" + dayTo + "&f=" + yearTo + "&g=d&ignore=.csv");
			...
			while ((line = reader.readLine()) != null) {
				...
				double open = Double.valueOf(values[1]), high = Double.valueOf(values[2]);
				double low = Double.valueOf(values[3]), close = Double.valueOf(values[4]);
