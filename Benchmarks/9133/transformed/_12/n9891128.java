	...
	private static void readIzvestiyaArticles() throws IOException {
		...
		try {
			while (reader.readRecord()) {
				String id = reader.get(0), filename = reader.get(1);
				StringTokenizer st = new StringTokenizer(filename, "-.");
				String name = st.nextToken(), volume = st.nextToken();
				String year = st.nextToken(), extension = st.nextToken();
				String filepath = String.format("%s/%s/%s-%s.%s", year, volume.length() == 1 ? "0" + volume : volume,
		...
		for (Map.Entry<String, String> entry : id2filename.entrySet()) {
			...
			if (new File(filepath).exists()) {
				...
				FileOutputStream out = new FileOutputStream(String.format("%s/%s.tex", OUTPUT_DIR, entry.getKey()),
						false);
				try {
