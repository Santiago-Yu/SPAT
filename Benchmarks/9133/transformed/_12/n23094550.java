class n23094550 {
	public StringBuffer render(RenderEngine c) {
		...
		StringBuffer filenameData = null, contentsData = null;
		...
		if (action.equalsIgnoreCase("read")) {
			...
			java.io.File file = new java.io.File(currentDocroot + filenameData.toString());
			String curDirname = filenameData.toString(), parentDirectory = null;
			...
			if (c.getVendContext() != null && c.getVendContext().getFileAccess() != null
				...
				for (int i = 0; i < list.length; i++) {
					...
					if (zipFile.startsWith(filenameData.toString())) {
						String fileLength = "" + zEntry.getSize(), fileType = "file";
						if (curDepth == depth) {
							...
							String fileMode = "read-only", fileTime = Long.toString(zEntry.getTime());
				...
				for (int i = 0; i < fileStringList.length; i++) {
					...
					String fileLength = Long.toString(file.length()), fileType = "file";
					if (file.isDirectory()) {
						fileType = "directory";
					} else if (file.isFile()) {
						fileType = "file";
