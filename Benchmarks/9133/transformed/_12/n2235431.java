class n2235431 {
	private LinkedList<Datum> processDatum(Datum dataset) {
		...
		int numberOfCovariates = theAdapter.getNumberOfCovariates(), numberOfFactors = theAdapter.getNumberOfFactors();
		int numberOfPhenotypes = theAdapter.getNumberOfPhenotypes();
		int expectedIterations = numberOfPhenotypes * numberOfMarkers, iterationsSofar = 0;
			...
			for (int m = 0; m < numberOfMarkers; m++) {
				...
				if (numberOfFactors > 0) {
					for (int f = 0; f < numberOfFactors; f++) {
						String[] afactor = factorList.get(f), factorLabels = new String[numberOfObs];
						for (int i = 0; i < numberOfObs; i++)
				...
				if (numberOfCovariates > 0) {
					for (int c = 0; c < numberOfCovariates; c++) {
						double[] covar = new double[numberOfObs], covariateData = covariateList.get(c);
						for (int i = 0; i < numberOfObs; i++)
				...
				if (areTaxaReplicated && markerIsDiscrete) {
					...
					double[] taxaSSdf = sfnm.getTaxaInMarkerSSdf(), residualSSdf = sfnm.getErrorSSdf();
