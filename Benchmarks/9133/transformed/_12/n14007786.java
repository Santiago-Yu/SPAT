class n14007786 {
	...
	protected IStatus run(IProgressMonitor monitor) {
		final int BUFFER_SIZE = 1024, DISPLAY_BUFFER_SIZE = 8196;
		...
		if (sourceFile.exists()) {
			try {
				...
				monitor.beginTask(Messages.getString("FileCopierJob.MainTask") + " " + _sourceFile,
						(int) ((sourceFile.length() / DISPLAY_BUFFER_SIZE) + 4));
				monitor.worked(1);
