class n564199 {
	private boolean doStudentCreditUpdate(Double dblCAmnt, String stuID) throws Exception {
		...
		try {
			...
			String building = settings.get(DBSettings.MAIN_BUILDING), cashier = dbMan.getPOSUser();
			if (hasStudentCredit()) {
				stmt = conn.createStatement();
				if (stmt.executeUpdate("UPDATE " + strPOSPrefix + "studentcredit set credit_amount = credit_amount + "
						+ round2Places(dblCAmnt) + " WHERE credit_active = '1' and credit_studentid = '" + stuId
						+ "'") == 1) {
					stmt.close();
					stmt = conn.createStatement();
					if (stmt.executeUpdate("UPDATE " + strPOSPrefix
						...
						if (stmt.executeUpdate("INSERT into " + strPOSPrefix
								+ "studentcredit_log ( scl_studentid, scl_action, scl_datetime ) values( '" + stuId
								+ "', '" + round2Places(dblCAmnt) + "', NOW() )") == 1) {
							stmt.close();
							blOk = true;
						} else {
				...
				if (stmt.executeUpdate("insert into " + strPOSPrefix
						...
						+ round2Places(dblCAmnt) + "','1','" + stuId + "', NOW())") == 1) {
					stmt.close();
					stmt = conn.createStatement();
					if (stmt.executeUpdate("insert into " + strPOSPrefix
							+ "studentcredit_log ( scl_studentid, scl_action, scl_datetime ) values( '" + stuId + "', '"
			...
			if (blOk) {
				...
				if (stmt.executeUpdate("insert into " + strPOSPrefix
						...
						+ stuId + "', '" + round2Places(dblCAmnt) + "', '" + building + "', '" + host + "', '" + cashier
						+ "', NOW() )") == 1) {
					stmt.close();
					blOk = true;
				} else
