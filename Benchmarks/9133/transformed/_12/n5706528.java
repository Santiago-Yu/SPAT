	...
	public void run() {
		try {
			...
			while ((line = reader.readLine()) != null) {
				...
				if (matcher.matches()) {
					String dirUrl = myListURL + matcher.group(1) + "/", name = matcher.group(2);
				...
				if (matcher.matches()) {
					String dirUrl = myListURL.substring(0, myListURL.indexOf(".com/") + ".com/".length())
							+ matcher.group(1), name = matcher.group(2);
					DownloadMenu.this
							.add(new DownloadMenu(DownloadMenu.this.myMainFrame, dirUrl, name, myLoadChildren));
					continue;
				}
				...
				if (matcher.matches()) {
					String fileUrl = myListURL.substring(0, myListURL.indexOf(".com") + ".com".length())
							+ matcher.group(1) + ".bz2";
					final int typeAt = 3, nameAt = 2;
					String type = matcher.group(typeAt), name = matcher.group(nameAt);
					if (type.length() > 0) {
						if (type.startsWith(".")) {
