	...
	public ActualTask(TEditor editor, TIGDataBase dataBase, String directoryPath) {
            ...
            for (i = 0; ((i < list.length) && !stop); i++) {
                ...
                if ((list[i].compareTo("Images") != 0) && ((list[i].substring(list[i].lastIndexOf('.'), list[i].length()).toLowerCase().compareTo(".jpg") == 0) || (list[i].substring(list[i].lastIndexOf('.'), list[i].length()).toLowerCase().compareTo(".bmp") == 0) || (list[i].substring(list[i].lastIndexOf('.'), list[i].length()).toLowerCase().compareTo(".png") == 0))) {
                    String name = list[i], pathSrc = directoryPath.concat(list[i]);
                    name = name.replace(' ', '_').replace(',', '-').replace('ив', 'a').replace('иж', 'e').replace('ик', 'i').replace('ио', 'o').replace('и▓', 'u').replace('?', 'A').replace('?', 'E').replace('?', 'I').replace('?', 'O').replace('?', 'U');
                    ...
                    String nameSrc = '.' + src.separator + "Images" + src.separator + name.substring(0, 1).toUpperCase() + src.separator + pathDst.substring(pathDst.lastIndexOf(src.separator) + 1, pathDst.length()), newDirectory = '.' + src.separator + "Images" + src.separator
                    ...
                    if (src != null) {
                        ...
                        if (image.getImageLoadStatus() == MediaTracker.ERRORED) {
                            ...
                            try {
                                int thumbWidth = PREVIEW_WIDTH, thumbHeight = PREVIEW_HEIGHT;
                                double thumbRatio = (double) thumbWidth / (double) thumbHeight;
                                int imageWidth = image.getIconWidth(), imageHeight = image.getIconHeight();
