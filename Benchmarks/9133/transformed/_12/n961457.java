class n961457 {
	public void actualizar() throws SQLException, ClassNotFoundException, Exception {
		...
		try {
			...
			int fzanume = Integer.parseInt(numeroEntrada), fzacagc = Integer.parseInt(oficina);
			int off_codi = 0;
			try {
			...
			int ceros = 0, fzacorg = Integer.parseInt(destinatari);
			int fzanent;
			String fzacent;
			if (altres.equals("")) {
				altres = "";
			...
			int fzanloc = Integer.parseInt(salida1), fzaaloc = Integer.parseInt(salida2);
			if (disquet.equals("")) {
				disquet = "0";
			}
