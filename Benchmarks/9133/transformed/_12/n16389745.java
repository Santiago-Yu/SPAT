			...
			if (srcDir.isDirectory() && srcDir.exists()) {
				final String[] fileList = srcDir.list(new XFilenameFilter() {
					...
					public boolean accept(XFile dir, String file) {
						...
						return (fNArr.length > 1 && (fNArr[fNArr.length - 1].equalsIgnoreCase("map")
								|| fNArr[fNArr.length - 1].equalsIgnoreCase("plt")
								|| fNArr[fNArr.length - 1].equalsIgnoreCase("wpt")));
					}
				});
				int pointsCounter = 0, tracksCounter = 0;
				int mapsCounter = 0;
				for (final String fName : fileList) {
					...
					try {
						...
						if (isOziWPT) {
							...
							if (projection != null && projection.getPoints() != null
									&& projection.getPoints().size() >= 2) {
								...
								if (!srcFile.exists() || !srcFile.isFile())
									throw new IllegalArgumentException("file: " + srcFile.getPath() + " not found");
								final XFile mapStorage = new XFile(
						...
						for (MapTrackPointsScaleRequest track : tracks) {
							if (track != null) {
								try {
									...
									pool.put(track, new StatesStack(new byte[] { 0x00, 0x11 }),
