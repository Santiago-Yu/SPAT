class n15338765 {
	public void upload()
		...
		final String META_XML_SUFFIX = "_meta.xml", FILES_XML_SUFFIX = "_files.xml";
		final String username = getUsername(), password = getPassword();
		...
		final String metaXmlString = serializeDocument(getMetaDocument()),
				filesXmlString = serializeDocument(getFilesDocument());
		final byte[] metaXmlBytes = metaXmlString.getBytes(), filesXmlBytes = filesXmlString.getBytes();
		final int metaXmlLength = metaXmlBytes.length, filesXmlLength = filesXmlBytes.length;
