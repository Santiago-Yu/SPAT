class n14282702 {
	...
	public void runTask(HashMap jobStack) throws Exception {
		String lstrFilter = (String) getResources().get("filter"), lstrTarget = (String) getResources().get("target");
		...
		if (lstrFilesFound != null) {
			for (int i = 0; i < lstrFilesFound.length; i++) {
				...
				File lfileSource = new File(lstrSource + File.separator + lstrFilesFound[i]),
						lfileTarget = new File(lstrTarget + File.separator + lstrFilesFound[i]);
				...
				try {
					...
					long size = lfisInput.size(), position = 0;
					while (position < size) {
