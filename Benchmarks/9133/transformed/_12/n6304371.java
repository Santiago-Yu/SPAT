class n6304371 {
	public static String getImportFileBody(String fileName, HttpSession session) {
		...
		String fileTransferFolder = CofaxToolsServlet.fileTransferFolder,
				importFTPServer = (String) user.workingPubConfigElementsHash.get("IMPORTFTPSERVER");
		String importFTPLogin = (String) user.workingPubConfigElementsHash.get("IMPORTFTPLOGIN"),
				importFTPPassword = (String) user.workingPubConfigElementsHash.get("IMPORTFTPPASSWORD");
		String importFTPFilePath = (String) user.workingPubConfigElementsHash.get("IMPORTFTPFILEPATH"), body = ("");
		try {
			int reply;
			...
			try {
				...
				CofaxToolsUtil
						.log("CofaxToolsFTP getImportFileBody deleting remote file: " + importFTPFilePath + fileName);
