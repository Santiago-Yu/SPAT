class n3791822 {
	public Void doInBackground() {
		...
		for (int i = 0; i < uploadFiles.size(); i++) {
			String filePath = uploadFiles.elementAt(i).getFilePath(), fileName = uploadFiles.elementAt(i).getFileName();
			...
			try {
				...
				int count = 1, maxCount = (int) (inFile.length() / chunkSize);
				...
				while (readCount > 0) {
					...
					float thisProgress = (count * 100) / (maxCount),
							completeProgress = (i * (100 / uploadFiles.size()));
