class n3673682 {
	private void DrawModel(Graphics offg, int obj_num, boolean object, float h, float s, int vt_num, int fc_num) {
		...
		int count = 0, tmp[] = new int[fc_num];
		double tmp_depth[] = new double[fc_num];
		rotate(vt_num);
		offg.setColor(Color.black);
		for (int i = 0; i < fc_num; i++) {
			double a1 = fc[i].vt1.x - fc[i].vt0.x, a2 = fc[i].vt1.y - fc[i].vt0.y;
			double a3 = fc[i].vt1.z - fc[i].vt0.z, b1 = fc[i].vt2.x - fc[i].vt1.x;
			double b2 = fc[i].vt2.y - fc[i].vt1.y, b3 = fc[i].vt2.z - fc[i].vt1.z;
			fc[i].nx = a2 * b3 - a3 * b2;
			fc[i].ny = a3 * b1 - a1 * b3;
			fc[i].nz = a1 * b2 - a2 * b1;
			if (fc[i].nz < 0) {
