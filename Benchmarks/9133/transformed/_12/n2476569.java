		...
		try {
			...
			out.close();
			{
				...
				while (i < n) {
					...
					final int[] dsize = size[indexSize].getSize(), dmargin = size[indexSize].getMargin();
			...
			if (signed) {
				final String keypassword = (String) properties
						...
						password = (String) properties.get(IHtmlToPdfTransformer.PDF_ENCRYPTION_PASSWORD);
				final String keyStorepassword = (String) properties
						.get(IHtmlToPdfTransformer.PDF_SIGNING_KEYSTORE_PASSWORD),
						privateKeyFile = (String) properties.get(IHtmlToPdfTransformer.PDF_SIGNING_PRIVATE_KEY_FILE);
				final String reason = (String) properties.get(IHtmlToPdfTransformer.PDF_SIGNING_REASON),
						location = (String) properties.get(IHtmlToPdfTransformer.PDF_SIGNING_LOCATION);
