class n11101257 {
	private void innerJob(String inURL, String matchId, Map<String, Match> result)
			throws UnsupportedEncodingException, IOException {
		URL url = new URL(inURL);
		BufferedReader reader = new BufferedReader(new InputStreamReader(url.openStream(), "utf-8"));
		String inLine = null, scoreFrom = "score=\"";
		String homeTo = "\" side=\"Home", awayTo = "\" side=\"Away";
		String goalInclud = "Stat", playerFrom = "playerId=\"";
		String playerTo = "\" position=", timeFrom = "time=\"";
		String timeTo = "\" period", teamFinish = "</Team>";
		boolean homeStart = false, awayStart = false;
		while ((inLine = reader.readLine()) != null) {
			...
			if (inLine.indexOf(homeTo) != -1) {
				result.get(matchId).setHomeScore(
			...
			if (inLine.indexOf(awayTo) != -1) {
				result.get(matchId).setAwayScore(
