class n13644040 {
	public static void main(String[] args) throws Exception {
		String layerName = args[0], layerDescription = args[1];
		String units = args[2], rawDataDirPath = args[3];
		String processDirPath = args[4], divaDirPath = args[5];
		String legendDirPath = args[6], geotiffDirPath = args[7];
		String dbJdbcUrl = args[8], dbUsername = args[9];
		String dbPassword = args[10], geoserverUsername = args[11];
		...
		try {
			...
			float minValue = Float.parseFloat(matchPattern(strDivaGrd, "^MinValue=(.+)$")),
					maxValue = Float.parseFloat(matchPattern(strDivaGrd, "^MaxValue=(.+)$"));
			float minLatitude = Float.parseFloat(matchPattern(strDivaGrd, "^MinY=(.+)$")),
					maxLatitude = Float.parseFloat(matchPattern(strDivaGrd, "^MaxY=(.+)$"));
			float minLongitude = Float.parseFloat(matchPattern(strDivaGrd, "^MinX=(.+)$")),
					maxLongitude = Float.parseFloat(matchPattern(strDivaGrd, "^MaxX=(.+)$"));
