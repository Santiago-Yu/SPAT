class n19597469 {
	protected Element downloadAndVerify(Element gElem) throws CacheException {
		try {
			String url = XMLHelper.GetChildText(gElem, "originalLocation"), id = XMLHelper.GetChildText(gElem, "id");
			...
			gElemStr = gElemStr.substring(0, index) + "<location>" + cachedFileName + "</location>"
					+ gElemStr.substring(index);
			index = gElemStr.indexOf("</generator>");
			gElemStr = gElemStr.substring(0, index) + "<downloadTime>" + System.currentTimeMillis() + "</downloadTime>"
					+ gElemStr.substring(index);
