class n98309 {
	public void readMESHDescriptorFileIntoFiles(String outfiledir) {
		String inputLine, ins, filename = getMESHdescriptorfilename();
		String uid = "", name = "";
		String description = "", element_of = "";
		...
		try {
			...
			while (rs.next()) {
				String db_treenr = rs.getString("hierarchy_complete"), db_uid = rs.getString("uid");
			...
			while ((in_for_mapping != null) && (inputLine != null)) {
				if (inputLine.startsWith("<DescriptorRecord DescriptorClass")) {
					...
					if (mesh_uid.compareTo("D041441") == 0)
						leave = true;
					inputLine = getNextLine(in_for_mapping);
					inputLine = getNextLine(in_for_mapping);
					cut1 = "<String>";
			...
			BufferedReader in_ec_numbers = new BufferedReader(
					new FileReader("e:\\projects\\ondex\\ec_concept_acc.txt"));
			...
			while (inputLine != null) {
				if (inputLine.startsWith("<DescriptorRecord DescriptorClass")) {
					...
					actions.clear();
					chemicals.clear();
					boolean id_ready = false, line_read = false;
					while ((inputLine != null) && (!inputLine.startsWith("</DescriptorRecord>"))) {
						...
						if (inputLine.startsWith("<Concept PreferredConceptYN")) {
							...
							if (inputLine.compareTo("<Concept PreferredConceptYN=\"Y\">") == 0)
								prefConcept = true;
							while ((inputLine != null) && (inputLine.indexOf("</Concept>") == -1)) {
								if (inputLine.startsWith("<CASN1Name>") && prefConcept) {
									...
									String chem_name = casn1, chem_description = "";
									if (casn1.length() > chem_name.length() + 2)
										chem_description = casn1.substring(chem_name.length() + 2, casn1.length());
									String reg_number = "";
									inputLine = getNextLine(in);
								...
								if (inputLine.startsWith("<TermUI>")) {
									...
									if (syn.indexOf("&amp;") != -1) {
										String syn1 = syn.substring(0, syn.indexOf("&amp;")),
												syn2 = syn.substring(syn.indexOf("amp;") + 4, syn.length());
										syn = syn1 + " & " + syn2;
									}
									if (name.compareTo(syn) != 0)
										synonyms.add(syn);
								}
					...
					String name_stemmed = "", name_tagged = "";
					element_of = "MESHD";
					String is_unique = "0";
					int is_preferred = 1;
					String original_name = name, is_not_substring = "0";
					...
					for (int i = 0; i < treenr.size(); i++) {
						...
						if (tnr.length() > 3)
							tnr = tnr.substring(0, tnr.lastIndexOf("."));
						String rel_uid = (String) treenr2uid.get(tnr);
						if (rel_uid != null)
							related.add(rel_uid);
						else
							System.out.println(uid + ": No DUI found for " + tnr);
					}
					for (int i = 0; i < related.size(); i++) {
						String to_uid = (String) related.get(i);
						String to_name = (String) uid2name.get(to_uid);
					...
					if (related.size() == 0)
						System.out.println(uid + ": No is_a relations");
					rel_type = "act";
					element_of = "MESHD";
					...
					String method = "IMPM", score = "1.0";
					for (int i = 0; i < chemicals.size(); i++) {
						Vector chemical = (Vector) chemicals.get(i);
						String type = (String) chemical.get(0), chem = (String) chemical.get(1);
						if (!ec_numbers.contains(chem) && (type.compareTo("EC") == 0)) {
							if (chem.compareTo("1.14.-") == 0)
								chem = "1.14.-.-";
							else
								System.out.println("MISSING EC: " + chem);
						}
						String id = type + ":" + chem;
						String entry = uid + "\t" + id + "\t" + method + "\t" + score + "\n";
						if (type.compareTo("CAS") == 0)
							cas_mapping.write(entry);
						else
							ec_mapping.write(entry);
					}
				} else
					inputLine = getNextLine(in);
			}
			...
			for (int i = 0; i < allCASchemicals.size(); i++) {
				...
				String cas_id = "CAS:" + (String) chemical.get(1), cas_name = (String) chemical.get(2);
				String cas_pos_tag = "", cas_description = (String) chemical.get(3);
				String cas_element_of = "CAS", cas_is_primary = "0";
				out_concept.write(cas_id + "\t" + cas_pos_tag + "\t" + cas_description + "\t");
				out_concept.write(cas_element_of + "\t" + cas_is_primary + "\n");
				String cas_name_stemmed = "", cas_name_tagged = "";
				String cas_is_unique = "0", cas_is_preferred = "0";
				String cas_original_name = cas_name, cas_is_not_substring = "0";
