class n18822890 {
	public void actualizar() throws SQLException, ClassNotFoundException, Exception {
		...
		try {
			...
			int fzanume = Integer.parseInt(numeroSalida), fzacagc = Integer.parseInt(oficina);
			int off_codi = 0;
			try {
			...
			int ceros = 0, fzacorg = Integer.parseInt(remitent);
			int fzanent;
			String fzacent;
			if (altres.equals("")) {
				altres = "";
			...
			int fzanloc = Integer.parseInt(entrada1), fzaaloc = Integer.parseInt(entrada2);
			if (disquet.equals("")) {
				disquet = "0";
			}
			...
			if (correo != null) {
				String insertBZNCORR = "INSERT INTO BZNCORR (FZPCENSA, FZPCAGCO, FZPANOEN, FZPNUMEN, FZPNCORR)"
						+ "VALUES (?,?,?,?,?)",
						updateBZNCORR = "UPDATE BZNCORR SET FZPNCORR=? WHERE FZPCENSA=? AND FZPCAGCO=? AND FZPANOEN=? AND FZPNUMEN=?";
			...
			ms.setString(20, (usuario.toUpperCase().length() > 10) ? usuario.toUpperCase().substring(0, 10)
					: usuario.toUpperCase());
			...
			if ((modificado && !motivo.equals("")) || motivo.equals("")) {
				...
				logLopdBZSALIDA("UPDATE", (usuario.toUpperCase().length() > 10) ? usuario.toUpperCase().substring(0, 10)
						: usuario.toUpperCase(), fzahsis, horamili, fzanume, fzaanoe, fzacagc);
				conn.commit();
			} else {
				registroActualizado = false;
