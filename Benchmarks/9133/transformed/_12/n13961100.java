		...
		try {
			...
			if (getTemplateURIPath() != null) {
				...
				if (theClassLoader != null) {
					...
					try {
						Class<?> theClass = theClassLoader.loadClass(className), theOtherClass = null;
						try {
			...
			IClasspathEntry classpathEntry = JavaCore.newSourceEntry(new Path("/" + project.getName() + "/src")),
					jreClasspathEntry = JavaCore.newContainerEntry(new Path("org.eclipse.jdt.launching.JRE_CONTAINER"));
