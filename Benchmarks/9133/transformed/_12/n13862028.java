class n13862028 {
	public static void main(String[] args) {
		try {
			String completePath = null, predictionFileName = null;
			if (args.length == 2) {
				completePath = args[0];
			...
			int ignoreProcessedRows = 0, movieViewershipSize = 0;
			while (probemappedfile.hasRemaining()) {
				...
				TShortByteHashMap testCustMovieAndRatingsMap = (TShortByteHashMap) MoviesAndRatingsPerCustomer
						.get(testCustomer);
				short[] testCustMovies = testCustMovieAndRatingsMap.keys();
				float finalPrediction = 0;
				finalPrediction = predictRating(testCustomer, testmovie, custsandratings, custAndRatingSize,
						testCustMovies, testCustMovieAndRatingsMap);
				System.out.println("prediction for movie: " + testmovie + " for customer " + testCustomer + " is "
						+ finalPrediction);
