class n4254750 {
	protected static byte[] httpConnection(Context context, long token, String url, byte[] pdu, int method,
		...
		try {
			...
			{
				String xWapProfileTagName = MmsConfig.getUaProfTagName(), xWapProfileUrl = MmsConfig.getUaProfUrl();
			...
			if (extraHttpParams != null) {
				String line1Number = ((TelephonyManager) context.getSystemService(Context.TELEPHONY_SERVICE))
						.getLine1Number(), line1Key = MmsConfig.getHttpParamsLine1Key();
				String paramList[] = extraHttpParams.split("\\|");
				for (String paramPair : paramList) {
					String splitPair[] = paramPair.split(":", 2);
					if (splitPair.length == 2) {
						String name = splitPair[0].trim(), value = splitPair[1].trim();
						if (line1Key != null) {
