class n9647576 {
	private void Reserve() throws SQLException {
		...
		if (useragent)
			...
			sp = "user";
		String userbooksql = "", agentbooksql = "";
		String bookingid = String.valueOf(System.currentTimeMillis()), currentcoach;
		String currentseat;
		try {
			if (useragent) {
				agentbooksql = "update hp_administrator.agent_bookings set BOOKINGS = xmlquery('copy $new := $BOOKINGS modify do insert ";
				agentbooksql += " <detail booking_id=\"" + booking_details.getTicketno()
				...
				userbooksql += " <detail booking_id=\"" + booking_details.getTicketno()
						+ "\" status=\"open\" train_no=\"" + booking_details.getTrain_no() + "\" source=\""
						+ booking_details.getSource() + "\" dest=\"" + booking_details.getDestination()
						+ "\" dep_date=\"" + booking_details.getDate() + "\" > ";
			}
			for (int tickpos = 0; tickpos < booking_details.getNoOfPersons(); tickpos++) {
				...
				if (!currentcoach.equals("WL")) {
					...
					chartavailupdsql += "<seat number=\"" + currentseat + "\"><details user_id=\""
					...
					chartavailupdsql += " into $new/status/class[@name=\"" + booking_details.getTclass()
							+ "\"]/coach[@number=\"" + currentcoach + "\"] ";
					chartavailupdsql += " return  $new' ) where train_no like '" + booking_details.getTrain_no()
					...
					chartwlupdsql += "<details user_id=\"" + booking_details.getUserId() + "\" usertype=\"" + sp
					...
					chartwlupdsql += " return  $new' ) where train_no like '" + booking_details.getTrain_no()
							+ "' and date = '" + booking_details.getDate() + "' ";
				...
				if (useragent)
					agentbooksql += "<person><coach>" + currentcoach + "</coach><seat>" + currentseat
							+ "</seat></person>";
				else
					userbooksql += "<person><coach>" + currentcoach + "</coach><seat>" + currentseat
							+ "</seat></person>";
			}
			if (useragent) {
				agentbooksql += "</detail>   as first into $new/book return  $new' ) where agent_id like '"
				...
				userbooksql += "</detail>   as first into $new/book return  $new' ) where user_id like '"
						+ booking_details.getUserId() + "'";
