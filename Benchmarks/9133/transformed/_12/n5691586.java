class n5691586 {
	public static void main(String[] args) throws IOException {
		...
		String cands_fileName = (inFile_params.readLine().split("\\s+"))[0],
				alignSrcCand_phrasal_fileName = (inFile_params.readLine().split("\\s+"))[0];
		String alignSrcCand_word_fileName = (inFile_params.readLine().split("\\s+"))[0],
				source_fileName = (inFile_params.readLine().split("\\s+"))[0];
		String trainSrc_fileName = (inFile_params.readLine().split("\\s+"))[0],
				trainTgt_fileName = (inFile_params.readLine().split("\\s+"))[0];
		String trainAlign_fileName = (inFile_params.readLine().split("\\s+"))[0],
				alignCache_fileName = (inFile_params.readLine().split("\\s+"))[0];
		...
		int numSourceWords = sourceWordsSentences[0], numSourceSentences = sourceWordsSentences[1];
		println("Reading src corpus @ " + (new Date()));
		srcCorpusArray = SuffixArrayFactory.createCorpusArray(trainSrc_fileName, srcVocab, numSourceWords,
				numSourceSentences);
		...
		int numTargetWords = targetWordsSentences[0], numTargetSentences = targetWordsSentences[1];
		println("Reading tgt corpus @ " + (new Date()));
		tgtCorpusArray = SuffixArrayFactory.createCorpusArray(trainTgt_fileName, tgtVocab, numTargetWords,
				numTargetSentences);
		...
		if ("AlignmentArray".equals(alignmentsType)) {
			...
			alignments = new AlignmentGrids(new Scanner(new File(trainAlign_fileName)), srcCorpusArray, tgtCorpusArray,
					trainingSize, true);
		...
		int countSatisfied = 0, countAll = 0;
		int countSatisfied_sizeOne = 0, countAll_sizeOne = 0;
		int prev_i = -1;
		String srcSent = "";
		String[] srcWords = null;
		int candsRead = 0, C50count = 0;
		while (line != null) {
			++candsRead;
